...
import {
  buildPublicDouyinAnalysisMediaUrl,
  createDouyinAnalysisMediaSession,
  deleteDouyinAnalysisMediaSession,
} from './douyin-analysis-media.service.js'
import {
  cleanupDouyinMediaFile,
  createDouyinMediaClips,
  prepareDouyinMediaFile,
...
function assertDouyinAnalysisDuration(durationSeconds: number | undefined): number {
  if (!durationSeconds) {
    throw new AppError('未能识别视频时长，请重新提取后再分析', 422)
  }

  if (durationSeconds > maxAnalysisDurationSeconds) {
    throw new AppError('当前仅支持分析 10 分钟以内的抖音视频，建议选择 30 秒到 2 分钟的视频', 422)
  }

  return durationSeconds
}

function assertDouyinAnalysisMediaPublicUrlAvailable(): void {
  buildPublicDouyinAnalysisMediaUrl('douyin-analysis-media-preflight')
}
...
function formatSegmentLabel(clip: Pick<DouyinMediaClip, 'clipIndex' | 'startSeconds' | 'endSeconds'>): string {
...
function shouldDropRedundantTrailingSegment(
  clip: Pick<DouyinMediaClip, 'startSeconds' | 'endSeconds'>,
  trimmedLines: string[],
  isLastClip: boolean,
): boolean {
  if (!isLastClip) {
...
function mergeSegmentedScriptField(
  clips: Array<Pick<DouyinMediaClip, 'clipIndex' | 'startSeconds' | 'endSeconds'>>,
...
function mergeSegmentedAnalysisResults(clips: DouyinMediaClip[], results: VideoAnalysisResult[]): VideoAnalysisResult {
...
async function cleanupDouyinMediaClips(clips: DouyinMediaClip[]): Promise<void> {
  await Promise.all(clips.map(async (clip) => {
    await cleanupDouyinMediaFile(clip.filePath)
  }))
}

async function analyzeDouyinSegmentedMedia(input: {
  mediaFile: {
    filePath: string
    fileSize: number
    filename: string
    ...
    for (const clip of clips) {
      ...
      try {
        ...
        try {
          ...
          }, 'Failed to clean up Douyin analysis media session after segmented analysis')
        }
      }
    }
...
export async function analyzeDouyinVideoByProxyUrl(
  ...
  if (durationSeconds <= segmentedAnalysisClipDurationSeconds) {
    return analyzeVideoContent(buildPublicDouyinProxyUrl(token), options)
  }
