...
- [ ] 1.1 为六维边界、缺样本、总分与稳定并列排序写失败单测。
- [ ] 1.2 实现 `DeterministicMatchScorer`，常量化 `deterministic-v1`、权重和分段。
- [ ] 1.3 仓储一次查询返回全站历史候选及目标平台已形成履约次数，同时排除当前任务报名者/owner。
- [ ] 1.4 service 批量调用 `ReputationService.snapshots`，过滤最低等级，评分、排序后截断 1-100。
...
- [ ] 2.1 写 V30：唯一邀请、JSON 对象约束、`applied_at`、候选/任务索引。
- [ ] 2.2 写失败 IT：manager 列表、非 manager 403、六维响应、最低等级/已报名排除、重复邀请幂等、outbox payload。
- [ ] 2.3 实现 GET/POST；POST 在同事务内复算候选、冻结快照、append `TaskRecommenderInvited`。
- [ ] 2.4 报名成功事务中调用 `markApplied(taskId, accountId)`；IT 断言 `applied_at`。
- [ ] 2.5 跑 marketplace 定向测试、全量 test 与 bootJar。
...
- [ ] 3.1 先写模板/路由失败测试：`TaskRecommenderInvited` → engagement + `/me/task-invitations` + task payload。
- [ ] 3.2 收件人只取 `recommenderAccountId`，不反查 marketplace。
- [ ] 3.3 确认 inbox 幂等和既有邮件白名单语义不变；跑 identity notification 定向与全量测试。
...
- [ ] 4.1 失败测试覆盖 GET/POST 路径、总分/六维/理由渲染、邀请中/已邀请/错误态。
- [ ] 4.2 选中 published 任务时并行加载推荐列表；展示紧凑表格与可展开六维证据。
- [ ] 4.3 邀请成功就地更新状态，不重载整页；已报名后刷新列表自然移除。
...
- [ ] 5.1 `resolveLinkTarget` 接收 payload，为专用 path 生成 `taskId` + recommender side。
- [ ] 5.2 layout provide 结构化 grassland 导航目标；旧 anchor 跳转继续兼容。
- [ ] 5.3 workbench 收到目标后激活推荐官身份、GET 指定任务、放入 feed/选中、滚到任务大厅。
- [ ] 5.4 点击报名仍调用原端点；成功后刷新 applications 和推荐列表状态。

### Task 6: 验证、文档与提交

- [ ] 6.1 `:services:marketplace-service:test :bootJar`。
- [ ] 6.2 `:services:identity-service:test :bootJar`。
- [ ] 6.3 前端定向 Vitest，再跑 `npm run test && npm run typecheck && npm run build`。
- [ ] 6.4 更新 `CLAUDE.md`、`项目速览.md`、`docs/草场开发进度与续接指南.md`，只调整本功能状态。
- [ ] 6.5 检查 `git diff`、`git status`、提交；不 push。
