...
本文不提前冻结尚未确认的商业或风控规则。未决内容统一使用以下标记：
...
- 三种资金模式是否允许组合以及组合优先级。
- 商家确认超时、拒绝或失联时的自动处理规则。
- 商家三级权限所需材料、审核时效、额度和行业限制。
- 全量数据库字段、索引、事件 Payload 和错误码。
- AI/视频/图片供应商、模型路由算法、计费数值和内容审核规则。
...
**DECISION REQUIRED：**争议开启时资金已付款、已分账或处于供应商不可逆状态下的补偿与追偿规则。
...
- **TBD**：数据保留、删除、导出、合规归档、地域和高风险行业规则。
...
|通知|`NotificationAdapter`|邮件、短信、推送和验证码|**TBD**：供应商、模板和送达 SLA。|
...
|商家确认超时规则缺失|履约和资金长期悬置|在 Marketplace/Finance LLD 前完成产品决策。|
|资金模式任意组合|状态和退款规则爆炸|首期限制合法组合，逐一形成版本化 Policy。|
...
|编号|决策|阻塞范围|
|---|---|---|
|D-01|支付、托管、分账、退款、付款和对账供应商及合规模式|真实支付、退款、结算、核销分账|
|D-02|三种任务资金模式是否组合及合法组合规则|Finance Product Policy|
|D-03|商家确认超时、拒绝和失联规则|Marketplace、Trust、Finance、Temporal|
|D-04|各发布平台 P0/P1 核实方法和合法性|Verification LLD|
|D-05|商家三级权限材料、审核、额度、行业和申诉规则|Identity、Marketplace、Finance|
|D-06|争议对已付款、已分账或已退款资金的处置|Trust、Finance|
|D-07|商品/套餐、定价、库存、有效期和订单快照归属|Marketplace/Commerce|
|D-08|活动身份在 Session、多标签页和多设备中的规则|BFF、Identity、客户端|
|D-09|`/api/v2` 首批客户端、OAuth/微信绑定和支付回跳|BFF、Identity、APP/小程序|
|D-10|数据保留、删除、导出、审计、证据脱敏和地域要求|所有服务|
|D-11|AI 用量单位、预留/退回、平台模型和 BYOK 计费边界|Intelligence、Finance|
|D-12|Legacy Node 各能力退出优先级和验收阈值|BFF、Intelligence、部署|
...
> 本文档为 v0.1 草案。标记为 `DECISION REQUIRED` 的事项未完成前，不得将相关金融、核实、争议或消费者交易模块视为可直接进入生产实现的最终设计。
