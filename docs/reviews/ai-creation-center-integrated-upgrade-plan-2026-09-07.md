...
版本：1.0。日期：2026-09-07。源码基线：`851f0691` 所在的当前工作区。
...
1. 给当前草稿和版本快照表同时增加工作区结构字段，确保历史版本恢复包含新数据。
2. 当前 intelligence 迁移最大版本为 V67，#92 计划占用后续迁移。执行时重新检查最大版本并分配，避免多个任务争用同一编号；本方案不预占迁移文件名。
...
第 7 项属于 W04 的后续可靠性扩展，应复用现有执行与运行查询能力；操作请求记录属于生成生命周期，不是另一套最近项目存储。进程在供应商受理后中断且供应商不支持查询/幂等时，状态保持待确认，不自动认定失败并二次计费。
...
建议在 `/Users/LXH/claude/y-1/src/views/ai-center/components/` 放置简报编辑、最近项目、交付预览等跨工作流区块；只被文章使用的排版/卡片区块放在文章 `components` 目录。

拟新增 `useCreationWorkspace` 作为已有 `useCreationDraft` 的工作流适配层，负责序列化/恢复与来源映射，不再创建第二套 HTTP 或自动保存引擎。公共类型放入 `src/types`，规则检查和字段适配放入已有领域目录。
...
所有任务当前均为待实施。W00-W08 构成近期改造主线；W09-W11 根据样稿和接入条件推进。
...
| 批次 | 能给用户带来的变化 | 上线前条件 |
| --- | --- | --- |
| A | 点评/知乎表达更可信，用户要求和平台策略进入生成 | W01/W02 对应测试与样稿；W00 已明确后续契约 |
| B | 草稿可恢复，回答不会丢模式，图卡重试和成品关联正确 | 草稿迁移、版本与权限测试，主流程恢复 E2E |
| C | 已有文章/媒体可直接加工，九平台交付字段完整 | 各内容形式有成品样例，导出/素材/声明检查通过 |
| D | 高精度图卡、部分平台草稿/发布及复盘 | 样稿证明制作效果，目标渠道接入经过实际验证 |
