...
开发规格入口：[任务书 #101](../任务书/草场任务书-101-AI创作工作流与图文交付升级.md)。编写任务书时进一步核对到 #55 独立信息图品类已明确撤销；本研究中信息图／SVG 的后续建议仅作为能力评估，不构成恢复该品类的决定。#101 保留撤销边界，并将排版实现确定为 Java 原生服务，沿用现有基础设施。
