...
> 归档日期：2026-09-15。状态：**已合并的历史来源，不再作为当前架构或待办清单**。
> 替代文档：[系统技术总体设计 HLD](../架构/草场系统技术总体设计（HLD-v0.1）.md)（§2.4 技术选型、§15 迁移与合并对照）、[项目架构详解](../架构/项目架构详解.md)（当前实现）。原文的候选技术、90 天排期和 Express 回退阶段仅供追溯。

> 原阅读边界：本文保留迁移时的目标方案和阶段取舍，不是当前依赖清单。当前后端已统一到 Java 25；数据库访问采用 WebFlux/R2DBC，服务内 Outbox 发布器轮询发事件，移动端使用已实现的 token 协议。下文的 jOOQ/HikariCP、Debezium、OAuth/OIDC 等目标项应结合[项目架构详解](../架构/项目架构详解.md)、[移动 token 方案](../架构/移动端刷新token认证方案设计.md)和实际模块核对，不据此假定已部署。
