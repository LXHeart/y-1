...
check(JSON.stringify(openIds) === JSON.stringify([40]), 'only backlog #40 may be open')

const overview = read('项目速览.md')
check(overview.includes('`docs/status.yaml`'), '项目速览.md must link to docs/status.yaml')
check(overview.includes('#22–#39、#41–#45 已完成'), '项目速览.md completion summary is stale')
check(!overview.includes('#22–#41 已全部完成'), '项目速览.md must not mark #40 completed')
