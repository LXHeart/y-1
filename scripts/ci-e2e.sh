...
AI_FRONTEND_PORT="${AI_FRONTEND_PORT:-18082}"
...
export EDGE_BFF_PORT="${EDGE_BFF_PORT:-18085}"
...
# C11 争议窗口下限在 E2E 关闭（否则 Lv5 T+1 也被抬到 48h，结算等待全部超时）。
...
# 任务书 #76：跨应用 origin 运行时配置（nginx envsubst → /app-config.js）+ e2e 的 AI 应用锚
...
for key in \
  ...
  IDENTITY_ASSERTION_KEY_MARKETPLACE_SERVICE_IDENTITY \
  IDENTITY_ASSERTION_KEY_IDENTITY_SERVICE_FINANCE \
  IDENTITY_ASSERTION_KEY_IDENTITY_SERVICE_MARKETPLACE \
  IDENTITY_ASSERTION_KEY_IDENTITY_SERVICE_TRUST \
  ...
  IDENTITY_ASSERTION_KEY_INTELLIGENCE_SERVICE_FINANCE \
