...
/**
 * 原四服务各自一份的 OutboxPublisherTest 收敛为单份（diff 仅 topic 字面量）。 覆盖：ack 门禁、5 字段 wire
 * 信封、重叠轮询守卫、并发上限、指数退避封顶与错误码口径。
 */
...
class OutboxPublisherTest {
	...
	void setUp() {
		...
		OutboxSettings settings = new OutboxSettings(TOPIC, true, 2_000, 10, 1, 5_000, 60_000, 100, 250);
		publisher = new OutboxPublisher(repository, kafka, new ObjectMapper().findAndRegisterModules(), meterRegistry,
				settings, "test");
	...
	void publishPending_waitsForAck_preservesFiveFieldEnvelope_andGuardsOverlap() throws Exception {
		...
		when(repository.claimBatch(eq(10), any(UUID.class), eq(Duration.ofSeconds(60)))).thenReturn(Flux.just(row),
				Flux.empty());
		...
		verify(repository, timeout(1_000).times(1)).claimBatch(eq(10), any(UUID.class), eq(Duration.ofSeconds(60)));
		...
		publisher.publishPending();
		verify(repository, timeout(1_000).times(2)).claimBatch(eq(10), any(UUID.class), eq(Duration.ofSeconds(60)));
	}
	...
	void publishPending_schedulesBoundedExponentialRetry_withErrorCodeOnly() throws Exception {
		...
		when(repository.markFailure(row.id(), row.claimToken(), Duration.ofMillis(200), "IllegalStateException"))
				.thenReturn(Mono.just(true));

		publisher.publishPending();

		verify(repository, timeout(1_000)).markFailure(row.id(), row.claimToken(), Duration.ofMillis(200),
	...
	void publishPending_capsRetryDelay_withoutDeadLettering_andCountsMarkMiss() throws Exception {
		...
		when(repository.claimBatch(eq(10), any(UUID.class), eq(Duration.ofSeconds(60)))).thenReturn(Flux.just(row));
		when(kafka.send(eq(TOPIC), eq(row.aggregateId()), anyString())).thenReturn(nack);
		when(repository.markFailure(row.id(), row.claimToken(), Duration.ofMillis(250), "IllegalArgumentException"))
				.thenReturn(Mono.just(false));

		publisher.publishPending();

		verify(repository, timeout(1_000)).markFailure(row.id(), row.claimToken(), Duration.ofMillis(250),
	...
	private static OutboxRepository.OutboxRow row(String suffix, UUID claimToken, int attemptCount) {
		return new OutboxRepository.OutboxRow("d7440f7c-7d40-4e77-9f25-6cc9c58b758" + suffix, "evt-" + suffix,
				"DomainChanged", "Domain", "aggregate-" + suffix, "{\"text\":\"quote \\\" and 中文\"}", claimToken,
				attemptCount);
	}
