 ...
 *
 * <p>原 identity/finance/trust/intelligence 四份逐字相同（仅表名与 trust 的 id/payload 方言不同），
...
public class OutboxRepository {
    ...
    public Mono<Void> append(EventEnvelope event) {
        try {
            ...
            return db.sql("""
                    ...
                    """.formatted(schema.table(), schema.payloadCast()))
    ...
    public Mono<Long> pendingCount() {
        return db.sql(("SELECT COUNT(*)::bigint AS pending_count FROM %s WHERE published_at IS NULL")
                        .formatted(schema.table()))
