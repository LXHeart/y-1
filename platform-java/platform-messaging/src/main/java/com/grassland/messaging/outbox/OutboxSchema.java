...
/**
 * outbox 表的物理方言。五服务共库同 schema，表名必须互不相同；
 * 列契约（event_id 唯一、published_at/claim 租约、attempt_count 退避）由各服务 Flyway 保证。
 *
 * <p>{@code trust_outbox} 是唯一历史分叉：bigserial 主键 + jsonb payload
 * （其余四表为 uuid + json）——不改线上表结构，以方言参数吸收差异。
 */
