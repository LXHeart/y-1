 ...
 * {@link IdentityItSupport}。
 *
 * <p>
 * 覆盖：owner 申请升级（带合规 materials）+PermissionRequested、缺料 400、同级 409、非 owner 403、
 * admin 列 pending、批准→tier 升级、拒绝→tier 不变、终态再审 409、非 admin 403、无 cookie 401；
 * Slice 2L：申诉（rejected→201 引用原申请 / 非
 * rejected→409）、额度查询、slaStatus（within/overdue/completed）、行业快照。
 */
...
class PermissionRequestControllerIT extends IdentityItSupport {
	...
	void ownerRequestsUpgradePendingAndEvent() {
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				...
				.bodyValue(bodyFor("basic_publish")).exchange().expectStatus().isCreated().expectBody()
				.jsonPath("$.data.requestedTier").isEqualTo("basic_publish").jsonPath("$.data.status")
				.isEqualTo("pending").jsonPath("$.data.industry").isEqualTo("other").jsonPath("$.data.slaStatus")
				.isEqualTo("within").jsonPath("$.data.reviewDeadline").isNotEmpty();
	...
	void missingRequiredMaterialReturns400() {
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + owner.cookie())
				.bodyValue(
						"{\"requestedTier\":\"finance_transaction\",\"materials\":{\"business_license\":\"BL\",\"contact_info\":\"13800138000\"}}")
				.exchange().expectStatus().isBadRequest().expectBody().jsonPath("$.error")
				.value(m -> assertThat((String) m).contains("financial_qualification"));
	}

	@Test
	void invalidContactInfoAndOverlongMaterialReturn400() {
		var owner = seedAccount("pr-contact@example.com");
		String orgId = createOrg(owner.cookie(), "联系方式非法主体");
		// 任务书 #78 卡 G：contact_info 只收 11 位手机号——座机/字母/邮箱/位数不对均 400。
		for (String bad : new String[]{"010-12345678", "abcdefghijk", "kyb@example.com", "12800138000", "1380013800"}) {
			client().post().uri("/api/organizations/" + orgId + "/permission-requests")
					.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + owner.cookie())
					.bodyValue(
							"{\"requestedTier\":\"basic_publish\",\"materials\":{\"business_license\":\"BL\",\"contact_info\":\""
									+ bad + "\"}}")
					.exchange().expectStatus().isBadRequest().expectBody().jsonPath("$.error")
					.value(m -> assertThat((String) m).contains("手机号"));
		}
		// 全部材料值统一 128 长度帽。
		String overlong = "B".repeat(129);
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + owner.cookie())
				.bodyValue("{\"requestedTier\":\"basic_publish\",\"materials\":{\"business_license\":\"" + overlong
						+ "\",\"contact_info\":\"13800138000\"}}")
				.exchange().expectStatus().isBadRequest().expectBody().jsonPath("$.error")
				.value(m -> assertThat((String) m).contains("过长"));
	}
	...
	void duplicateOpenRequestReturns409() {
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + owner.cookie())
				.bodyValue(bodyFor("basic_publish")).exchange().expectStatus().isEqualTo(409);
	}
	...
	void nonOwnerRequestForbidden() {
		...
		db.sql("INSERT INTO organization_membership(id, organization_id, account_id, role)"
				+ " VALUES (gen_random_uuid(), CAST(:org AS uuid), CAST(:acct AS uuid), 'member')").bind("org", orgId)
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				...
				.bodyValue(bodyFor("basic_publish")).exchange().expectStatus().isForbidden();
	}
	...
	void adminListsPending() {
		...
		client().get().uri("/api/admin/permission-requests").header("Cookie", "y1.sid=" + admin.cookie()).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.length()")
				.value(l -> assertThat((Integer) l).isGreaterThan(0));
	}
	...
	void adminApproveUpgradesTierAndEvent() {
		...
		client().post().uri("/api/admin/permission-requests/" + requestId + "/review")
				...
				.bodyValue("{\"decision\":\"approve\",\"note\":\"材料齐全\"}").exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.status").isEqualTo("approved").jsonPath("$.data.slaStatus").isEqualTo("completed")
				.jsonPath("$.data.reviewerAccountId").isEqualTo(admin.accountId());

		client().get().uri("/api/organizations/" + orgId).header("Cookie", "y1.sid=" + owner.cookie()).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.permissionTier").isEqualTo("basic_publish");
	...
	void adminRejectKeepsTier() {
		...
		client().post().uri("/api/admin/permission-requests/" + requestId + "/review")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + admin.cookie())
				.bodyValue("{\"decision\":\"reject\",\"note\":\"材料不足\"}").exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.status").isEqualTo("rejected");

		client().get().uri("/api/organizations/" + orgId).header("Cookie", "y1.sid=" + owner.cookie()).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.permissionTier").isEqualTo("draft");
	}
	...
	void rejectsRequestsWithoutSessionCookie() {
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				.contentType(MediaType.APPLICATION_JSON).bodyValue(bodyFor("basic_publish")).exchange().expectStatus()
				.isUnauthorized();
	...
	void appealRejectedCreatesPendingReferencingOriginal() {
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests/" + originalId + "/appeal")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + owner.cookie())
				.bodyValue(
						"{\"materials\":{\"business_license\":\"BL-new\",\"contact_info\":\"13800138000\"},\"note\":\"已更新证照\"}")
				.exchange().expectStatus().isCreated().expectBody().jsonPath("$.data.status").isEqualTo("pending")
				.jsonPath("$.data.originalRequestId").isEqualTo(originalId).jsonPath("$.data.appealNote")
				.isEqualTo("已更新证照");
	}
	...
	void getQuotaReturnsTierPolicy() {
		...
		client().get().uri("/api/organizations/" + orgId + "/quota").header("Cookie", "y1.sid=" + owner.cookie())
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.tier").isEqualTo("draft")
				.jsonPath("$.data.quota.maxActiveTasks").isEqualTo(0).jsonPath("$.data.quota.maxTxAmountCents")
				.isEqualTo(0);
	}
	...
	void slaStatusOverdueWhenDeadlinePassed() {
		...
		client().get().uri("/api/admin/permission-requests/" + requestId).header("Cookie", "y1.sid=" + admin.cookie())
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.slaStatus").isEqualTo("overdue");
	}
	...
	void prohibitedIndustryCannotEnterAdmissionFlow() {
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				...
				.exchange().expectStatus().isBadRequest().expectBody().jsonPath("$.error").isEqualTo("该行业暂不开放商家准入");
	}
	...
	void passedBusinessLicenseOcrProducesAutomaticRecommendation() {
		...
		db.sql("""
				...
				""").bind("id", attachmentId).bind("pendingId", unrelatedPendingId).bind("org", orgId)
				.bind("owner", owner.accountId()).then().block();
		...
		client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				...
				.exchange().expectStatus().isCreated().expectBody().jsonPath("$.data.autoReviewStatus")
				.isEqualTo("passed").jsonPath("$.data.reviewMode").isEqualTo("auto_recommendation")
	...
	void pendingOcrIsReconciledAfterDocumentAnalysisCompletes() {
		...
		client().get().uri("/api/admin/permission-requests/" + requestId).header("Cookie", "y1.sid=" + admin.cookie())
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.autoReviewStatus").isEqualTo("passed")
	...
	void claimUsesUnderReviewStateAndWritesAudit() {
		...
		client().post().uri("/api/admin/permission-requests/" + requestId + "/claim")
				...
				.jsonPath("$.data.status").isEqualTo("under_review").jsonPath("$.data.version").isEqualTo(1)
				.jsonPath("$.data.reviewerAccountId").isEqualTo(admin.accountId());
		...
		client().get().uri("/api/admin/permission-requests/" + requestId + "/audit")
				.header("Cookie", "y1.sid=" + admin.cookie()).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data[0].action").isEqualTo("claimed").jsonPath("$.data[1].action").isEqualTo("submitted");
	}
	...
	void explicitAutomaticVerificationFailureRequiresRecentReauthentication() {
		...
		client().post().uri("/api/admin/permission-requests/" + requestId + "/review")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + admin.cookie())
				.bodyValue("{\"decision\":\"approve\"}").exchange().expectStatus().isOk().expectBody()
	...
	void staleLowerTierApprovalCompletesWithoutDowngradingOrganization() {
		...
		client().get().uri("/api/organizations/" + orgId).header("Cookie", "y1.sid=" + owner.cookie()).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.permissionTier").isEqualTo("finance_transaction");

		Integer grants = db
				.sql("SELECT COUNT(*)::int AS count FROM outbox" + " WHERE event_type = 'MerchantPermissionGranted'"
	...
	void appealCannotCrossOrganizationBoundary() {
		...
		client().post().uri("/api/admin/permission-requests/" + requestId + "/review")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + admin.cookie())
				.bodyValue("{\"decision\":\"reject\",\"note\":\"材料不符\"}").exchange().expectStatus().isOk();
	...
	private String submitRequest(String orgId, String cookie, String tier) {
		Map<String, Object> body = client().post().uri("/api/organizations/" + orgId + "/permission-requests")
				.contentType(MediaType.APPLICATION_JSON).header("Cookie", "y1.sid=" + cookie).bodyValue(bodyFor(tier))
				.exchange().expectStatus().isCreated().expectBody(Map.class).returnResult().getResponseBody();
	...
	private static String bodyFor(String tier) {
		String materials = switch (tier) {
			case "finance_transaction" -> "\"materials\":{\"business_license\":\"BL\",\"legal_representative\":\"LR\","
					+ "\"financial_qualification\":\"FQ\",\"contact_info\":\"13800138000\"}";
			default -> "\"materials\":{\"business_license\":\"BL\",\"contact_info\":\"13800138000\"}";
		};
