...
class PasswordUpgradeIT extends IdentityItSupport {
	...
	void registerStoresArgon2Hash() {
		...
		String code = "654321";
		// 表名/列名对齐 EmailVerificationService 实际读写的 legacy 表：email_verification_codes(明文
		// code, used)。
		...
		boolean created = client().post().uri("/api/auth/register").contentType(MediaType.APPLICATION_JSON)
		...
		if (created) {
			String hash = db.sql("SELECT password_hash FROM app_users WHERE email = :email").bind("email", email)
	...
	private String seedWithHash(String email, String hash) {
		...
		db.sql("INSERT INTO app_users(id, email, password_hash, display_name, role, status) "
				+ "VALUES (CAST(:id AS uuid), :email, :hash, 'Upgrade User', 'user', 'active')").bind("id", id)
				.bind("email", email).bind("hash", hash).then().block();
		return id;
	}

	private String storedHash(String id) {
		return db.sql("SELECT password_hash FROM app_users WHERE id = CAST(:id AS uuid)").bind("id", id)
				.map(row -> row.get(0, String.class)).one().block();
	}
}
