 ...
 *
 * <p>
 * TC98-011 新自报数据带采集时间（source=self_reported + collectedAt=保存时点，客户端伪造值不采信）；
 * TC98-012 存量回填幂等（V50 SQL 重复执行两次结果一致，已有值不覆盖）；TC98-013 展示兼容缺省 （存储缺
 * source/collectedAt 的旧形态读出按自报解释）；TC98-014 枚举不虚标（verified/platform_fact
 * 任何路径不得产出——客户端提交即 400）。
 */
class SocialAccountSourceIT extends IdentityItSupport {
	...
	void tc98_011NewSelfReportedDataCarriesCollectionTime() {
		...
		// 客户端尝试伪造 source/collectedAt：collectedAt 不采信（保存时点覆盖）；source 非 self_reported 见
		// TC98-014。
		client().put().uri("/api/me/recommender-profile").contentType(MediaType.APPLICATION_JSON)
				.header("Cookie", "y1.sid=" + me.cookie()).bodyValue("""
						...
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.data.socialAccounts[0].source")
				.isEqualTo("self_reported").jsonPath("$.data.socialAccounts[0].collectedAt").isNotEmpty()
		...
		client().put().uri("/api/me/recommender-profile").contentType(MediaType.APPLICATION_JSON)
				.header("Cookie", "y1.sid=" + me.cookie()).bodyValue("""
						...
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.data.socialAccounts[0].platform")
				.isEqualTo("douyin").jsonPath("$.data.socialAccounts[0].source").isEqualTo("self_reported")
		...
		for (String forged : new String[]{"verified", "platform_fact"}) {
			client().put().uri("/api/me/recommender-profile").contentType(MediaType.APPLICATION_JSON)
					...
					.bodyValue("{\"displayName\":\"x\",\"contentTags\":[],\"domainTags\":[],"
							+ "\"socialAccounts\":[{\"platform\":\"xiaohongshu\",\"followers\":1,\"source\":\"" + forged
							+ "\"}]}")
					.exchange().expectStatus().isBadRequest();
		}
		// 全库无 verified/platform_fact 值（读路径 only self_reported 的直接证据）。
		Long forged = db.sql("SELECT count(*) AS c FROM recommender_profile"
	...
	void tc98_012BackfillIsIdempotentAndTc98_013ReadToleratesLegacyShape() {
		...
		db.sql("INSERT INTO recommender_profile(account_id, display_name, bio, content_tags, domain_tags,"
				...
				+ " CAST(:social AS jsonb), '', ARRAY[]::text[], '', '[]'::jsonb)").bind("acct", legacyAccount)
				.bind("social", "[{\"platform\":\"xiaohongshu\",\"handle\":\"@legacy\",\"followers\":300}]").then()
				.block();
		...
		client().get().uri("/api/recommenders/" + legacyAccount + "/profile")
				...
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.socialAccounts[0].source")
				.isEqualTo("self_reported").jsonPath("$.data.socialAccounts[0].collectedAt")
	...
	private String socialAccountsOf(String accountId) {
		return db
				.sql("SELECT social_accounts::text AS s FROM recommender_profile"
						+ " WHERE account_id = CAST(:acct AS uuid)")
				.bind("acct", accountId).map(row -> row.get("s", String.class)).one().block();
	}

	/** 从 classpath 读迁移原文（与生产同一份 SQL，防测试与迁移漂移）。 */
	private static String migrationSql(String fileName) {
		try (var input = SocialAccountSourceIT.class.getClassLoader().getResourceAsStream("db/migration/" + fileName)) {
			if (input == null) {
