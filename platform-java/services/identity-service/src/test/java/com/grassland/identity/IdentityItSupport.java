 ...
 *
 * <p>
 * {@code from-database-url=true} + {@code DATABASE_URL} 激活生产同款
 * {@code DataSourceConfig} → Flyway 跑 V1+V2 建
 * organization/store/organization_membership（含 permission_tier）与
 * outbox；{@code @BeforeAll} 手建 app_users/session。 提供
 * {@link #seedAccount(String)} 生成登录态 + {@link #createOrg(String, String)} 建组织。
 *
 * <p>
 * <b>容器生命周期</b>：采用 testcontainers 单例容器模式——{@link #POSTGRES} 在 static
 * 块启动一次、全程不重启， 端口稳定；多个子类 + Spring 缓存的 ApplicationContext 共享同一容器/同一端口（避免
 * {@code @Testcontainers} 按类重启 导致缓存上下文持有的旧端口失效、连接被拒）。
 *
 * <p>
 * 注意：单例容器在同类内跨 {@code @Test} 共享，数据累积；outbox 计数请按 orgId 限定以解耦顺序。
 */
...
public abstract class IdentityItSupport {
	...
	static void props(DynamicPropertyRegistry r) {
		...
		r.add("DATABASE_URL", () -> "postgresql://" + POSTGRES.getUsername() + ":" + POSTGRES.getPassword() + "@" + host
				+ ":" + p + "/" + name);
		...
		r.add("identity.external-delivery.challenge-secret", () -> "test-sms-challenge-secret-at-least-32-characters");
	...
	static void schema() throws Exception {
		// organization/store/organization_membership/outbox 由 Flyway 建；这里补 Java
		// bootstrap 管理的共享身份表。
		...
		try (var c = java.sql.DriverManager.getConnection(POSTGRES.getJdbcUrl(), POSTGRES.getUsername(),
			...
			s.execute(
					"CREATE TABLE IF NOT EXISTS email_verification_codes (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), "
	...
	void stubKybMediaValidation() {
		when(financeCreditsAdminClient.award(anyString(), anyInt(), anyString(), anyString())).thenReturn(Mono.empty());
		...
		when(kybMediaClient.acquireLease(any(), any(), any(), any(), any(Long.class))).thenAnswer(invocation -> Mono
				.just(new KybMediaRetentionReceipt(invocation.getArgument(0), invocation.getArgument(2),
						invocation.getArgument(3), Instant.now().plusSeconds(invocation.getArgument(4)), null)));
		when(kybMediaClient.seal(any(), any(), any(), any(), any()))
				.thenAnswer(invocation -> Mono.just(new KybMediaRetentionReceipt(invocation.getArgument(0),
						invocation.getArgument(2), invocation.getArgument(3), null, invocation.getArgument(4))));
		when(kybMediaClient.requireUsable(any(), any(), any())).thenAnswer(invocation -> {
			...
			return Mono.just(new KybMediaMetadata(mediaId, accountId, organizationId, "merchant_kyb", "merchant_kyb",
					organizationId, "active", "image/png", 4096L, null));
		});
	}
	...
	void stubAvatarMedia() {
		when(avatarMediaClient.requireUsable(any(), any())).thenAnswer(invocation -> {
			...
			return Mono.just(new AvatarMediaMetadata(mediaId, accountId, "avatar", "active", "image/png", 4096L, null));
		});
		when(avatarMediaClient.issueDownloadUrl(any())).thenAnswer(invocation -> {
			UUID mediaId = invocation.getArgument(0);
			return Mono.just(new AvatarMediaDownload(URI.create("https://cdn.example.com/avatar/" + mediaId), null));
		});
	}
	...
	/**
	 * 门店媒体默认替身（#42 D2/D5）：开票放行 + 批量换 URL 全量命中，避免打真 intelligence。 默认全命中；测
	 * fail-closed/503 分支必须在用例内覆盖本 stub。
	 */
	@BeforeEach
	void stubStoreMedia() {
		when(storeMediaClient.createTicket(anyString(), anyString(), anyString(), anyString(), anyLong()))
				.thenAnswer(invocation -> {
					...
					return Mono.just(new StoreMediaUploadTicket(ticketId, "media/store_media/" + ticketId,
		...
		when(storeMediaClient.downloadUrls(anyString(), anyString(), any())).thenAnswer(invocation -> {
			...
			for (String mediaId : mediaIds) {
				resolved.put(mediaId, new ResolvedMedia("image/png", 1024L,
						"https://cdn.example.com/store-media/" + mediaId, Instant.now().plusSeconds(3600)));
			}
	...
	protected Seeded seedAccount(String email) {
		...
		db.sql("INSERT INTO app_users(id, email, password_hash, display_name, role, status) "
				+ "VALUES (CAST(:id AS uuid), :email, 'x', 'Test Account', 'user', 'active')").bind("id", accountId)
				.bind("email", email).then().block();
	...
	protected String createOrg(String cookie, String name) {
		Map<String, Object> body = client().post().uri("/api/organizations").contentType(MediaType.APPLICATION_JSON)
				.header("Cookie", "y1.sid=" + cookie).bodyValue("{\"name\":\"" + name + "\"}").exchange().expectStatus()
	...
	protected String createStore(String orgId, String cookie, String name) {
		Map<String, Object> body = client().post().uri("/api/organizations/" + orgId + "/stores")
				...
				.bodyValue("{\"name\":\"" + name + "\"}").exchange().expectStatus().isCreated().expectBody(Map.class)
