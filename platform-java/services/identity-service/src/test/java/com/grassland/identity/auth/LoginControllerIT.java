...
class LoginControllerIT {
	...
	static void schema() throws Exception {
		try (var c = java.sql.DriverManager.getConnection(postgres.getJdbcUrl(), postgres.getUsername(),
			...
			// C04-A: login and /auth/me both read the authoritative multi-role table.
			s.execute(
	...
	private WebTestClient client() {
		// responseTimeout 30s：默认 5s 在 CI 慢 runner 负载下偶发超时（与
		// IdentityItSupport/FinanceItSupport 同口径）
	...
	void loginSuccessReturnsUserAndSetCookie() {
		...
		client().post().uri("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"email\":\"active@example.com\",\"password\":\"correct-pass\"}").exchange().expectStatus()
				.isOk().expectBody().jsonPath("$.success").isEqualTo(true).jsonPath("$.data.user.id").isEqualTo(id)
	...
	void loginWithWrongPasswordReturns401() {
		...
		client().post().uri("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"email\":\"wrong@example.com\",\"password\":\"bad-pass\"}").exchange().expectStatus()
	...
	void loginWithUnknownEmailReturns401SameMessage() {
		client().post().uri("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"email\":\"nobody@example.com\",\"password\":\"anything\"}").exchange().expectStatus()
				.isUnauthorized().expectBody().jsonPath("$.error")
				.isEqualTo("\u8d26\u53f7\u6216\u5bc6\u7801\u9519\u8bef");
	}
	...
	void suspendedLoginReturns403WithDedicatedMessage() {
		...
		client().post().uri("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"email\":\"inactive@example.com\",\"password\":\"correct-pass\"}").exchange()
				.expectStatus().isForbidden().expectBody().jsonPath("$.error").isEqualTo("账号已停用，请联系商家管理员");
	}

	@Test
	void loginReturnsAuthoritativeRolesWithoutRefresh() {
		String id = seedUser("finance@example.com", "correct-pass", "active");
		db.sql("INSERT INTO backend_role(account_id, role) VALUES (CAST(:id AS uuid), 'finance'), (CAST(:id AS uuid), 'risk')")
				.bind("id", id).then().block();
		client().post().uri("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"email\":\"finance@example.com\",\"password\":\"correct-pass\"}").exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.user.roles")
				.isEqualTo(java.util.List.of("finance", "risk"));
	}
