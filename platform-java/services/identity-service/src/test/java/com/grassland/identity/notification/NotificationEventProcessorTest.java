 ...
 *
 * <p>
 * 事务用 pass-through mock（{@code transactional} 直接返回原 Mono），隔离事务语义—— 真实事务回滚由
 * {@code NotificationInboxIT} 验证。覆盖类型路由、IGNORED 不写 inbox、坏 envelope、
...
class NotificationEventProcessorTest {
	...
	@Mock
	private MailOutboxEnqueuer mailOutbox;
	@Mock
	private ExternalDeliveryEnqueuer externalDelivery;
	@Mock
	...
	void ignoredEventDoesNotTouchInbox() {
		...
		StepVerifier.create(processor.process(record)).expectNext(NotificationProcessingResult.IGNORED)
				.verifyComplete();
	...
	void subAccountCreatedFansOutToOneRecipient() {
		...
		when(notifications.insertIfAbsent(eq("acct-1"), any(), anyString(), anyString(), any(), any(), eq("evt-1"),
				any())).thenReturn(Mono.just(notification()));

		StepVerifier.create(processor.process(record)).expectNext(NotificationProcessingResult.PROCESSED)
				.verifyComplete();
		verify(notifications, times(1)).insertIfAbsent(eq("acct-1"), any(), anyString(), anyString(), any(), any(),
				eq("evt-1"), any());
	}
	...
	void resolverExcludesActorFromManagers() {
		...
		StepVerifier.create(processor.process(record)).expectNext(NotificationProcessingResult.PROCESSED)
				.verifyComplete();
		verify(notifications).insertIfAbsent(eq("admin2"), any(), anyString(), anyString(), any(), any(), eq("evt-1"),
				any());
	}
	...
	void externalConsumerNameIsUsedAsInboxKey() {
		...
		when(notifications.insertIfAbsent(eq("owner-1"), any(), anyString(), anyString(), any(), any(), eq("evt-1"),
				any())).thenReturn(Mono.just(notification()));

		StepVerifier.create(processor.process(record, "identity-notification-marketplace-consumer"))
				.expectNext(NotificationProcessingResult.PROCESSED).verifyComplete();
	...
	private ConsumerRecord<String, String> record(String eventType, String aggregateId, Map<String, Object> payload) {
		String json;
		try {
			json = mapper.writeValueAsString(Map.of("eventId", "evt-1", "eventType", eventType, "aggregateType",
