...
class OutboxRepositoryIT extends IdentityItSupport {
    ...
    void claimBatch_claimsOnlyDueRows_andReclaimsOnlyExpiredLeases() {
        ...
        db.sql("UPDATE outbox SET next_attempt_at = now() + interval '60 seconds' WHERE event_id = 'later'")
                .then().block();
        ...
        db.sql("UPDATE outbox SET claimed_until = now() - interval '1 second' WHERE event_id = 'due'")
                .then().block();
    ...
    void conditionalTransitions_rejectStaleOwners_andTrackPendingAge() {
        ...
        OutboxRepository.OutboxRow claim = repository.claimBatch(1, token, Duration.ofSeconds(30))
                .single().block();

        assertThat(repository.markPublished(claim.id(), UUID.randomUUID()).block()).isFalse();
        assertThat(repository.markFailure(
                claim.id(), claim.claimToken(), Duration.ofSeconds(10), "TimeoutException").block()).isTrue();
        ...
        assertThat(repository.claimBatch(1, UUID.randomUUID(), Duration.ofSeconds(30))
                .collectList().block()).isEmpty();

        db.sql("UPDATE outbox SET next_attempt_at = now() - interval '1 second' WHERE event_id = 'old'")
                .then().block();
        OutboxRepository.OutboxRow retry = repository.claimBatch(
                1, UUID.randomUUID(), Duration.ofSeconds(30)).single().block();
    ...
    void markPublished_isConditional_andRemovesRowFromPendingBacklog() {
        ...
        OutboxRepository.OutboxRow claim = repository.claimBatch(
                1, UUID.randomUUID(), Duration.ofSeconds(30)).single().block();
        ...
        try (var connection = DriverManager.getConnection(
            ...
            try (var result = statement.executeQuery()) {
                ...
                assertThat(columns).contains(
                        "attempt_count", "next_attempt_at", "claimed_until", "claim_token", "last_error_code");
                assertThat(columns).doesNotContain(
                        "aggregate_version", "occurred_at", "correlation_id", "claimed_at", "last_error", "failed_at");
            }
        }
    }
