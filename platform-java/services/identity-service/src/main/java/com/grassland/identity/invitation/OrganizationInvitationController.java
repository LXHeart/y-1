...
public class OrganizationInvitationController {
    ...
    /**
     * 邀请门禁：组织级 → org OWNER（与直接加成员同档）；门店级 staff → 门店 MANAGER+，
     * 门店级 manager → org ADMIN+（镜像 StoreMembershipController.add 的分档；跨 org 门店 404）。
     */
    private reactor.core.publisher.Mono<com.grassland.identity.user.AuthUser> invitationGate(
            String orgId, String storeId, String role, ServerHttpRequest request) {
        if (storeId == null) {
                ...
                .flatMap(inviter -> invitations.findPending(orgId, body.email())
                        ...
                        .switchIfEmpty(Mono.defer(() ->
                                transactions.transactional(
                                        invitations.create(orgId, body.storeId(), body.email(), body.role(), inviter.id(), ttl)
                                                .flatMap(invitation -> outbox.append(new EventEnvelope(
                                                        ...
                                                        invitedPayload(invitation, inviter.id())))
