...
public class LoginController {
	...
	private final PasswordVerificationExecutor passwordExecutor;

	public LoginController(UserLookup userLookup, UserRepository userRepository, PasswordVerifier passwordVerifier,
			Argon2PasswordHasher argon2Hasher, SessionWriter sessionWriter, LoginRateLimiter rateLimiter,
			RefreshTokenService refreshTokens, AccountFlagRepository accountFlags,
			AuthUserResponseBuilder userResponses, PasswordVerificationExecutor passwordExecutor) {
		...
		this.passwordExecutor = passwordExecutor;
	}

	@PostMapping(value = "/api/auth/login", consumes = MediaType.APPLICATION_JSON_VALUE)
	public Mono<ResponseEntity<Map<String, Object>>> login(@RequestBody Mono<LoginRequest> bodyMono,
		...
		return bodyMono.flatMap(body -> {
			...
			return userLookup.findByIdentifier(email == null ? null : email.trim())
					.flatMap(user -> attemptLogin(user, password, ip, email, request)).switchIfEmpty(Mono.defer(() -> {
	...
	private Mono<ResponseEntity<Map<String, Object>>> attemptLogin(LoginUser user, String password, String ip,
			String email, ServerHttpRequest request) {
		// 密码错误 → 统一 401（不暴露账号存在性）。密码正确但状态非 active（任务书 #48：子账号
		// pending_review/rejected/suspended）→ 明确区分的 403，让用户知道找谁解封。
		return passwordExecutor.verify(passwordVerifier, password, user.passwordHash()).flatMap(passwordOk -> {
			if (!passwordOk) {
				rateLimiter.recordOutcome(ip, email, true);
				return Mono.just(build401());
			}
			...
			if (needsRehash) {
				loginOps = loginOps.then(
						Mono.fromCallable(() -> argon2Hasher.hash(password)).subscribeOn(Schedulers.boundedElastic())
			...
			if (deviceInfo != null && !deviceInfo.isBlank()) {
				...
				return loginOps.then(Mono.zip(mustChange, username)).flatMap(pair -> refreshTokens
						.issue(authUser, DeviceFingerprint.from(request), resolveDeviceName(request), deviceInfo)
						.flatMap(issued -> userResponses
								.build(authUser, Boolean.TRUE.equals(pair.getT1()), pair.getT2()).map(userInfo -> {
									rateLimiter.recordOutcome(ip, email, false);
									return buildToken200(issued, userInfo);
								})));
			}
			return loginOps.then(Mono.zip(mustChange, username))
					.flatMap(pair -> sessionWriter.createSession(authUser, request).flatMap(created -> userResponses
							.build(authUser, Boolean.TRUE.equals(pair.getT1()), pair.getT2()).map(userInfo -> {
								rateLimiter.recordOutcome(ip, email, false);
								return build200(created.setCookieHeader(), userInfo);
							})));
		});
	}
	...
	private ResponseEntity<Map<String, Object>> build200(String setCookie, Map<String, Object> user) {
		return ResponseEntity.ok().header("Set-Cookie", setCookie)
				.body(Map.of("success", true, "data", Map.of("user", user)));
	}

	/**
	 * 移动端 token 模式响应：与 Web 共用 user 字段，额外带 tokens，且刻意不发 Set-Cookie。
	 */
	private ResponseEntity<Map<String, Object>> buildToken200(RefreshTokenService.IssuedTokens issued,
	...
	private ResponseEntity<Map<String, Object>> build503() {
		return ResponseEntity.status(503).body(Map.of("success", false, "error", "移动端登录暂未启用"));
	}
