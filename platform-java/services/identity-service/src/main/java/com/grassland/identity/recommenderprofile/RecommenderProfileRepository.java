 ...
 *
 * <p>
 * 两处类型细节：{@code text[]} 用 {@code String[]} 绑定/读取；{@code jsonb} 只能绑字符串， 故 SQL
 * 里显式 {@code CAST(:social AS jsonb)}，读出来再用 Jackson 解回对象列表—— <b>JSON 字符串只存在于 DB
 * 与本类之间</b>，不外泄到 API（本项目被「响应回字符串」坑过）。
 */
@Component
public class RecommenderProfileRepository {

	private static final String SELECT_COLS = "account_id::text, display_name, bio, content_tags, domain_tags,"
	...
	public Mono<RecommenderProfile> findByAccount(String accountId) {
		return db.sql("SELECT " + SELECT_COLS + " FROM recommender_profile WHERE account_id = CAST(:acct AS uuid)")
				.bind("acct", accountId).map(this::map).one();
	}
	...
	public Mono<RecommenderProfile> upsert(String accountId, UpdateRecommenderProfileRequest body) {
		...
		List<SocialAccount> stamped = body
				.socialAccounts() == null
						? List.of()
						: body.socialAccounts().stream().map(account -> new SocialAccount(account.platform(),
								account.handle(), account.followers(), SocialAccount.SOURCE_SELF_REPORTED, collectedAt))
								.toList();
		...
		var spec = db.sql("""
				...
				ON CONFLICT (account_id) DO UPDATE
				  SET display_name = :name, bio = :bio, content_tags = :content,
				      ...
				      updated_at = now()
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("acct", accountId)
				...
				.bind("domain", body.domainTags().toArray(String[]::new)).bind("social", socialJson)
	...
	private RecommenderProfile map(Readable row) {
		return new RecommenderProfile(row.get("account_id", String.class),
				emptyToNull(row.get("display_name", String.class)), emptyToNull(row.get("bio", String.class)),
				toList(row.get("content_tags", String[].class)), toList(row.get("domain_tags", String[].class)),
				readList(row.get("social_accounts", String.class), SOCIAL_LIST),
				emptyToNull(row.get("resident_city", String.class)), toList(row.get("service_regions", String[].class)),
