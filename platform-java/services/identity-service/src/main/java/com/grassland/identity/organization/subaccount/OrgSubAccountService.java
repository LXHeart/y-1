 ...
 *
 * <p>
 * <b>建号</b>：owner/admin 直建（唯一建号路径——#52 决策 A：店长代建与审核流已退役）。 一次事务完成「插
 * app_users（占位邮箱）+ 写 account_username 登录名 + <b>入主体池</b>
 * （organization_membership.role=member）+ 可选挂店 + 置首登改密标记 + outbox」。登录名 =
 * {@code 主体前缀-登录名}（各段 ^[a-z0-9]{3,24}$）；挂店角色 manager/staff 须带 storeId， 且
 * manager 过一店一店长闸（#52 决策 B）。
 *
 * <p>
 * <b>状态真相源 = app_users.status</b>（D4）：{@code active / suspended / deleted}。
 * 停用即时生效依赖平台两道既有闸（edge 每请求 {@code lower(status)='active'} 过滤、 identity resolver
 * 403），本服务只负责改值与守卫。恢复只接受 suspended → active； deleted 是终态，restore 无法复活。
 *
 * <p>
 * <b>三守卫</b>：①org OWNER 账号不可被停用/删除；②不可操作自己；③重复成员关系 409。 #48 D8①「每店最后一个 active
 * MANAGER 不可停/删」已随 #52 池模型废除——店可无店长 （主体代管兜底，新建分店即此态），停/移/调三种路径语义一致（#52 决策 E）。
 */
@Service
public class OrgSubAccountService {
	...
	private final com.grassland.identity.store.StoreAssignmentLock assignmentLock;
	...
	public OrgSubAccountService(DatabaseClient db, TransactionalOperator transactions,
			...
			MembershipRepository orgMemberships, StoreMembershipRepository storeMemberships, StoreRepository stores,
			OrgAuthorization orgAuthz, AccountFlagRepository flags, UserLookup users,
			com.grassland.identity.store.StoreAssignmentLock assignmentLock) {
		this.db = db;
	...
	public Mono<CreatedSubAccount> createByOrg(String operatorAccountId, String organizationId,
		...
		return prepareTarget(organizationId, req).flatMap(target -> ensureStoreInOrg(organizationId, target)
	...
	private Mono<RoleTarget> prepareTarget(String organizationId, CreateSubAccountRequest req) {
		...
		return organizations.selectAccountPrefix(organizationId)
				...
				.map(prefix -> new RoleTarget(req.role(), prefix + "-" + loginName, req.displayName(), req.storeId()));
	}
	...
	/**
	 * #49 单条建号路径（挂靠关联分支已删）：占位邮箱插 app_users + 登录名旁表 + 成员行 + 首登改密标记 +
	 * outbox，全程一个事务。登录名撞（同主体或跨主体）→ 409 换名。
	 */
	private Mono<CreatedSubAccount> doCreate(String operatorAccountId, String organizationId, RoleTarget target,
		...
		return Mono.fromCallable(() -> argon2Hasher.hash(initialPassword)).subscribeOn(Schedulers.boundedElastic())
				.flatMap(hash -> transactions.transactional(
						// 任务书 #103 C103-17：建号链首语句取组织行锁（建号含挂店店长时的一店一店长
						// count 守卫必须与分配/调度在同一组织锁下，防并发建号双店长）。
						assignmentLock.acquireInCurrentTransaction(organizationId).then(db.sql("""
								...
								.flatMap(uid -> db.sql("""
										...
										""").bind("id", uid).bind("username", target.loginName()).fetch().rowsUpdated()
										// username UNIQUE 冲突（与 app_users 占位冲突同源，事务序不同到达）→ 409 换名
										.onErrorMap(DataIntegrityViolationException.class,
												e -> new IdentityException(409, "该登录名已被使用，请换一个"))
										.then(grantMembership(organizationId, uid, target).onErrorMap(
	 ...
	 * store_membership。挂店为<b>店长</b>须过一店一店长闸（决策 B；店员不设限）——建号时 该账号尚无门店行，排除位传空。
	 */
	private Mono<String> grantMembership(String organizationId, String accountId, RoleTarget target) {
		...
		if (target.requiresStore()) {
			Mono<String> gated = "manager".equals(target.role())
					...
					: poolRow;
			return gated.then(storeMemberships.create(target.storeId(), accountId, target.role())).map(m -> m.id());
		}
		return poolRow;
	}
	...
	private Mono<Void> guardUniqueManager(String storeId, String excludeAccountId) {
		return storeMemberships.countManagerRows(storeId, excludeAccountId).flatMap(
				count -> count > 0 ? Mono.error(new IdentityException(409, "该门店已有店长，请先移除或调度原店长")) : Mono.<Void>empty());
	}
	...
	private Mono<Void> changeSuspension(String operatorAccountId, String organizationId, String targetAccountId,
		...
		return organizations.findById(organizationId).switchIfEmpty(Mono.error(new IdentityException(404, "组织不存在")))
				.flatMap(org -> {
					...
					return rejectDeletedTerminal(targetAccountId, "账号已删除，不可恢复")
							...
							.then(transactions.transactional(executeSuspension(organizationId, targetAccountId, action)
	...
	private Mono<Void> rejectDeletedTerminal(String targetAccountId, String message) {
		return db.sql("SELECT status FROM app_users WHERE id = CAST(:id AS uuid)").bind("id", targetAccountId)
	...
	/**
	 * 操作者权限（D7）：org ADMIN+/owner 可动任意成员（含经理）；其余「在本组织有门店身份」者按 纯门店 MANAGER
	 * 规则——只能动在本组织仅有 staff 角色的目标。目标与本组织无关 → 404 （跨主体隔离，对齐 StoreAuthorization 风格）。
	 */
	private Mono<Void> requireOperatorAuthority(String operatorAccountId, String organizationId,
		...
		Mono<Boolean> adminPlus = orgAuthz.roleOfAccount(operatorAccountId, organizationId)
				.filter(role -> role.isAtLeast(MembershipRole.ADMIN)).hasElement();
		return ensureTargetInOrg(targetAccountId, organizationId).then(adminPlus.flatMap(isAdminPlus -> isAdminPlus
				? Mono.<Void>empty()
				: managerOnlyAuthority(operatorAccountId, organizationId, targetAccountId)));
	}

	private Mono<Void> managerOnlyAuthority(String operatorAccountId, String organizationId, String targetAccountId) {
		return storeMemberships.findDistinctRolesByAccountInOrg(operatorAccountId, organizationId).collectList()
	...
	private Mono<Void> guardedStatusUpdate(String targetAccountId, String to, String from, String conflictMessage) {
		return db
				.sql("UPDATE app_users SET status = :to, updated_at = NOW()"
						+ " WHERE id = CAST(:id AS uuid) AND status = :from")
				.bind("to", to).bind("id", targetAccountId).bind("from", from).fetch().rowsUpdated()
				.flatMap(rows -> rows > 0
						? Mono.<Void>empty()
						: db.sql("SELECT status FROM app_users WHERE id = CAST(:id AS uuid)")
								.bind("id", targetAccountId).map(row -> row.get("status", String.class)).one()
								.switchIfEmpty(Mono.error(new IdentityException(404, "账号不存在")))
								.<Void>flatMap(current -> Mono.error(new IdentityException(409,
										current.equals(to)
												? "已是该状态"
												: "deleted".equals(current) ? "账号已删除，不可恢复" : conflictMessage))));
	}

	private Mono<Void> notifySuspensionChanged(String organizationId, String targetAccountId, String operatorAccountId,
			String action) {
		Map<String, Object> payload = new LinkedHashMap<>();
		payload.put("organizationId", organizationId);
	 ...
	 * 留痕，不物理删行），同事务写 outbox {@code OrgSubAccountDeleted}。不可恢复——restore 对 deleted 一律
	 * 409。守卫：自己/owner/权限分档（#52 决策 E：最后店长保护已废除）。
	 */
		...
		return organizations.findById(organizationId).switchIfEmpty(Mono.error(new IdentityException(404, "组织不存在")))
				.flatMap(org -> {
					...
					return rejectDeletedTerminal(targetAccountId, "账号已删除，不可重复删除")
							.then(requireOperatorAuthority(operatorAccountId, organizationId, targetAccountId))
							.then(transactions.transactional(guardedDelete(organizationId, targetAccountId)
									.then(appendDeletedEvent(organizationId, targetAccountId, operatorAccountId))));
				});
	}
	...
	private Mono<Void> guardedDelete(String organizationId, String targetAccountId) {
		return db.sql("""
				...
				""").bind("id", targetAccountId).fetch().rowsUpdated().flatMap(rows -> rows > 0
				? Mono.<Void>empty()
				: db.sql("SELECT status FROM app_users WHERE id = CAST(:id AS uuid)").bind("id", targetAccountId)
						.map(row -> row.get("status", String.class)).one()
						.switchIfEmpty(Mono.error(new IdentityException(404, "账号不存在")))
						.<Void>flatMap(current -> Mono.error(
								new IdentityException(409, "deleted".equals(current) ? "账号已删除，不可重复删除" : "当前状态不可删除"))))
	...
	/**
	 * 账号显式版 ADMIN+ 门禁（内部调用无 request 可解析，与 {@code OrgAuthorization.requireRole}
	 * 同判据）。
	 */
	private Mono<Void> requireAdminPlus(String operatorAccountId, String organizationId) {
		return orgAuthz.roleOfAccount(operatorAccountId, organizationId)
				...
				.switchIfEmpty(Mono.error(new IdentityException(403, "权限不足"))).then();
	}
	...
	public Mono<CreatedSubAccount> resetPassword(String operatorAccountId, String organizationId,
		...
		return ensureTargetInOrg(targetAccountId, organizationId)
				...
				.then(users.findById(targetAccountId).switchIfEmpty(Mono.error(new IdentityException(404, "账号不存在")))
						// 展示名取登录名（子账号），非子账号回退 email（存量挂靠清理前的过渡态）
						.flatMap(account -> users.findUsernameById(targetAccountId).defaultIfEmpty(account.email())
								.flatMap(displayName -> {
									...
									return Mono.fromCallable(() -> argon2Hasher.hash(freshPassword))
											...
											.flatMap(hash -> transactions.transactional(db.sql(
													...
													.bind("hash", hash).bind("id", targetAccountId).fetch()
													.rowsUpdated().then(flags.markMustChangePassword(targetAccountId)))
	...
	private Mono<Void> ensureTargetInOrg(String targetAccountId, String organizationId) {
		return orgAuthz.roleOfAccount(targetAccountId, organizationId).hasElement()
				.flatMap(inOrgAsMemberOrOwner -> inOrgAsMemberOrOwner
						? Mono.<Void>empty()
	...
	/**
	 * 建号结果；{@code initialPassword} 只在本次响应出现一次（D2）。 {@code username}
	 * 是完整账号名（前缀-登录名），重置密码路径回填目标账号的用户名。
	 */
	public record CreatedSubAccount(String accountId, String username, String displayName, String role, String status,
