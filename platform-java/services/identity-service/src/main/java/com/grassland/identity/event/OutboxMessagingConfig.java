 ...
 * identity.outbox.poll-interval-ms 是服务私有配置面，@Scheduled 注解必须留在服务侧。
 */
@Configuration
public class OutboxMessagingConfig {
	...
	public OutboxRepository outboxRepository(DatabaseClient db) {
		return new OutboxRepository(db, OutboxSchema.standard("outbox"));
	}
	...
	public OutboxPublisher outboxPublisher(OutboxRepository repository,
		...
		return new OutboxPublisher(repository, kafka.getIfAvailable(), properties.settings(), "identity",
				meterRegistry);
	}
