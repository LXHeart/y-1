 ...
 *
 * <ul>
 * <li>{@link PermissionTier#BASIC_PUBLISH}：business_license +
 * contact_info。</li>
 * <li>{@link PermissionTier#FINANCE_TRANSACTION}：business_license +
 * legal_representative + financial_qualification + contact_info。</li>
 * <li>受监管行业（{@link Industry#requiresIndustryLicense()}）追加
 * industry_license。</li>
 * </ul>
 *
 * <p>
 * {@link #validate} 校验 materials（{@code Map<dbValue, 文本>}）含全部必填且非空，缺失抛
 * {@link IllegalArgumentException}（消息列出缺失类型）；contact_info 只收 11 位手机号、 全部材料值统一
 * 128 长度帽（任务书 #78 卡 G）。
 */
public final class PermissionMaterialPolicy {

	private static final int MATERIAL_VALUE_MAX_LENGTH = 128;
	...
	public static void validate(PermissionTier tier, Industry industry, Map<String, String> materials) {
		...
		for (Map.Entry<String, String> entry : materials.entrySet()) {
			if (entry.getValue() != null && entry.getValue().length() > MATERIAL_VALUE_MAX_LENGTH) {
				throw new IllegalArgumentException("材料内容过长（上限 " + MATERIAL_VALUE_MAX_LENGTH + " 字）: " + entry.getKey());
			}
		}
		String contactInfo = materials.get(MaterialType.CONTACT_INFO.dbValue());
		if (contactInfo != null && !contactInfo.isBlank()
				&& !MerchantProfileFields.isValidMobilePhone(contactInfo.strip())) {
			throw new IllegalArgumentException("联系方式仅支持 11 位手机号");
		}
	}
}
