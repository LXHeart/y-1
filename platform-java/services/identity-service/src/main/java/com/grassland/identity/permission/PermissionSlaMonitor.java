...
/**
 * Marks SLA breaches durably and emits one event/audit entry per request. Never
 * auto-grants access.
 */
@Component
@ConditionalOnProperty(prefix = "identity.permission.sla-monitor", name = "enabled", havingValue = "true", matchIfMissing = true)
public class PermissionSlaMonitor {
	...
	public PermissionSlaMonitor(MerchantPermissionRequestRepository requests, PermissionRequestAuditRepository audits,
	...
	public void poll() {
		if (!polling.compareAndSet(false, true))
			return;
		processBatch(100).doOnError(error -> log.error("permission SLA monitor failed", error))
	...
	Flux<Void> processBatch(int limit) {
		return transactions.transactional(requests.markOverdue(limit)
				.concatMap(req -> audits
						.append(req.id(), req.organizationId(), null, "system", "sla_breached", req.status(),
								req.status(), null)
						.then(outbox.append(new EventEnvelope(UUID.randomUUID().toString(),
								"PermissionReviewSlaBreached", "MerchantPermissionRequest", req.id(), 1, Instant.now(),
								null, Map.of("organizationId", req.organizationId(), "requestId", req.id(),
