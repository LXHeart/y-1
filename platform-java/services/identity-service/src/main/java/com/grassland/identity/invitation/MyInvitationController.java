 ...
 *
 * <p>
 * <b>邀请与账号的绑定靠邮箱</b>：邀请只写邮箱（发起时不查、也不回答该邮箱是否注册）， 接受时才校验「当前登录账号的邮箱 == 邀请邮箱」。因此邀请
 * id 即便泄露也无法被他人冒领； 且邮箱不匹配一律返回 <b>404 而非 403</b>——403 会变相确认「这个 id 是个真邀请」，成为探测口。
 *
 * <p>
 * <b>顺序</b>：先做 guarded UPDATE（pending→accepted，并发只有一个赢家），再建成员关系。
 * 对方已经是成员时，{@code createIfAbsent}（{@code ON CONFLICT DO NOTHING}）返回空——视为幂等成功
 * （邀请照样消费掉），用 {@code alreadyMember=true} 如实告知前端，不报错。
 *
 * <p>
 * <b>Slice 7C-2</b>：accept 的「邀请状态迁移 + 落成员关系 + 两个 outbox 事件」绑进同一 R2DBC 事务 （镜像
 * {@link #decline}）。成员幂等改用 {@code ON CONFLICT} 预判而非捕获
...
public class MyInvitationController {
	...
	public MyInvitationController(CurrentAccountResolver accounts, InvitationRepository invitations,
			MembershipRepository memberships, StoreMembershipRepository storeMemberships, OutboxRepository outbox,
	...
	public Mono<ResponseEntity<Map<String, Object>>> list(ServerHttpRequest request) {
		return accounts.resolve(request).flatMap(account -> invitations.findPendingForEmail(normalize(account.email()))
				.collectList().map(list -> ResponseEntity.ok(
						Map.of("success", true, "data", list.stream().map(MyInvitationController::toBody).toList()))));
	}

	@PostMapping("/{invitationId}/accept")
	public Mono<ResponseEntity<Map<String, Object>>> accept(@PathVariable String invitationId,
			ServerHttpRequest request) {
		return accounts.resolve(request).flatMap(account -> requireMine(invitationId, account)
				.flatMap(invitation -> transactions.transactional(invitations.accept(invitationId, account.id())
						.flatMap(rows -> rows == 0
								? Mono.<ResponseEntity<Map<String, Object>>>error(new IdentityException(409, "邀请已处理"))
	...
	public Mono<ResponseEntity<Map<String, Object>>> decline(@PathVariable String invitationId,
			ServerHttpRequest request) {
		return accounts.resolve(request).flatMap(account -> requireMine(invitationId, account)
				.flatMap(invitation -> transactions.transactional(invitations
						.transitionFromPending(invitationId, InvitationStatus.DECLINED)
						.flatMap(rows -> rows == 0
								? Mono.<ResponseEntity<Map<String, Object>>>error(new IdentityException(409, "邀请已处理"))
								: outbox.append(event("MembershipInvitationDeclined", invitation,
										Map.of("organizationId", invitation.organizationId(), "accountId",
												account.id())))
										.thenReturn(ResponseEntity.ok(Map.<String, Object>of("success", true)))))));
	}

	/**
	 * 取出确属当前账号的待接受邀请。不存在 / 邮箱不匹配 → 一律 404（不区分，避免成为「这个 id 是真邀请吗」的探测口）； 已过期 →
	 * 410（对本人可以如实说明，便于 UI 提示「请让对方重新邀请」）。
	 */
	private Mono<Invitation> requireMine(String invitationId, AuthUser account) {
		return invitations.findById(invitationId)
				...
				.switchIfEmpty(Mono.error(new IdentityException(404, "邀请不存在"))).flatMap(invitation -> {
	...
	/**
	 * 落成员关系 + 事件。组织级 → organization_membership；门店级 → store_membership（已是成员视为 幂等成功，用
	 * alreadyMember 如实告知前端）。已经是本店店员又被邀为店长时邀请照样消费、 既有角色不降级不升级（与组织级语义一致，改档走成员管理端点）。
	 */
	private Mono<ResponseEntity<Map<String, Object>>> joinOrganization(Invitation invitation, AuthUser account) {
		...
		if (invitation.storeId() != null) {
			return storeMemberships.createIfAbsent(invitation.storeId(), account.id(), invitation.role())
					.flatMap(membership -> outbox
							.append(new EventEnvelope(UUID.randomUUID().toString(), "StoreMembershipGranted",
									"StoreMembership", membership.id(), 1, Instant.now(), null, payload))
							.thenReturn(false))
					.switchIfEmpty(Mono.just(true))
					.flatMap(alreadyMember -> outbox.append(event("MembershipInvitationAccepted", invitation, payload))
							.thenReturn(ResponseEntity.ok(Map.<String, Object>of("success", true, "data",
									Map.of("organizationId", invitation.organizationId(), "storeId",
											invitation.storeId(), "role", invitation.role(), "alreadyMember",
											alreadyMember)))));
		}
		return memberships.createIfAbsent(invitation.organizationId(), account.id(), invitation.role())
				.flatMap(
						membership -> outbox
								.append(new EventEnvelope(UUID.randomUUID().toString(), "MembershipGranted",
										"Membership", membership.id(), 1, Instant.now(), null, payload))
				...
				.flatMap(alreadyMember -> outbox.append(event("MembershipInvitationAccepted", invitation, payload))
						.thenReturn(ResponseEntity.ok(Map.<String, Object>of("success", true, "data",
								Map.of("organizationId", invitation.organizationId(), "role", invitation.role(),
										"alreadyMember", alreadyMember)))));
	}

	private static EventEnvelope event(String type, Invitation invitation, Map<String, Object> payload) {
		return new EventEnvelope(UUID.randomUUID().toString(), type, "OrganizationInvitation", invitation.id(), 2,
