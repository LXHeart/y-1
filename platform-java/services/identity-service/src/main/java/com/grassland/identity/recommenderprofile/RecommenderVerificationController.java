 ...
 * 两类端点：
 * <ul>
 * <li><b>推荐官自助</b>：{@code POST /api/me/recommender-verification}（提交认证申请，须已开通
 * recommender 身份）； {@code GET /api/me/recommender-verification}（查我的认证状态）。</li>
 * <li><b>平台 admin</b>：{@code GET /api/admin/recommender-requests}（待审队列）；
 * {@code POST /api/admin/recommender-requests/{id}/{approve|reject}}（审核）。 鉴权用
 * {@link CurrentAccountResolver#requireRole(BackendRole...)}（MERCHANT_REVIEWER
 * 或 CONTENT_REVIEWER 或 PLATFORM_ADMIN 超集）。</li>
 * </ul>
 *
 * <p>
 * 克隆 KYB 范式：submit 同事务落申请行 + outbox；review 同事务改 status + outbox。
...
public class RecommenderVerificationController {
	...
	public RecommenderVerificationController(CurrentAccountResolver accounts,
			RecommenderVerificationRepository requests, IdentityProfileRepository identities, OutboxRepository outbox,
	...
	public Mono<ResponseEntity<Map<String, Object>>> myStatus(ServerHttpRequest request) {
		return accounts.resolve(request)
				.flatMap(user -> requests.findLatestByAccount(user.id()).map(this::toBody)
	...
	public Mono<ResponseEntity<Map<String, Object>>> submit(@RequestBody SubmitVerificationRequest body,
			ServerHttpRequest request) {
		return accounts.resolve(request).flatMap(user -> identities
				.findByAccountAndType(user.id(), IdentityType.RECOMMENDER.dbValue())
				...
				.flatMap(profile -> requests.findLatestByAccount(user.id())
						...
						.flatMap(existing -> Mono
								.<RecommenderVerificationRequest>error(new IdentityException(409, "已有待审核的认证申请")))
	...
	private Mono<RecommenderVerificationRequest> createRequest(String accountId, SubmitVerificationRequest body) {
		...
		return transactions.transactional(requests.create(accountId, materials, deadline).flatMap(
				req -> outbox.append(reviewEvent(req, "RecommenderVerificationRequested", null)).thenReturn(req)));
	}

	private Mono<ResponseEntity<Map<String, Object>>> review(String id, ReviewRequest body, ServerHttpRequest request,
			String decision, boolean requireNote) {
		return accounts.requireRole(request, BackendRole.MERCHANT_REVIEWER, BackendRole.CONTENT_REVIEWER)
				.flatMap(admin -> {
					UUID requestId = parseUuid(id);
					String note = requireNote ? requireReviewNote(body.note()) : trimNote(body.note());
					return requests.findById(requestId).switchIfEmpty(Mono.error(new IdentityException(404, "认证申请不存在")))
							.flatMap(req -> {
								...
								return transactions
										.transactional(requests.updateStatus(requestId, decision, admin.id(), note)
	...
	private EventEnvelope reviewEvent(RecommenderVerificationRequest req, String eventType, String note) {
		...
		return new EventEnvelope(UUID.randomUUID().toString(), eventType, AGGREGATE, req.id().toString(), 1,
