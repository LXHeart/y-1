...
/**
 * merchant_permission_request 数据访问（R2DBC {@link DatabaseClient} 手写
 * SQL，与组织/成员域风格一致）。 Slice 2L：create 带 industry + review_deadline；新增
 * createAppeal（申诉引用原 rejected 申请）。
 */
@Component
public class MerchantPermissionRequestRepository {

	private static final String SELECT_COLS = "id::text, organization_id::text, requester_account_id::text, requested_tier, materials::text, status,"
	...
	public Mono<MerchantPermissionRequest> create(String organizationId, String requesterAccountId,
			String requestedTier, String materials, String industry, Instant reviewDeadline, String attachmentIds,
		...
		var spec = db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id).bind("org", organizationId).bind("req", requesterAccountId)
				.bind("tier", requestedTier).bind("industry", industry).bind("attachments", attachmentIds)
				.bind("autoStatus", autoReview.status()).bind("autoResult", autoReview.resultJson())
				.bind("reviewMode", autoReview.mode()).bind("riskLevel", autoReview.riskLevel());
	...
	public Mono<MerchantPermissionRequest> createAppeal(String organizationId, String requesterAccountId,
			String requestedTier, String materials, String industry, Instant reviewDeadline, String originalRequestId,
			String appealNote, int appealCount, String attachmentIds, PermissionAutoReview autoReview) {
		String id = UUID.randomUUID().toString();
		var spec = db.sql("""
				INSERT INTO merchant_permission_request(id, organization_id, requester_account_id, requested_tier,
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id).bind("org", organizationId).bind("req", requesterAccountId)
				.bind("tier", requestedTier).bind("industry", industry).bind("original", originalRequestId)
				.bind("appealCount", appealCount).bind("attachments", attachmentIds)
				.bind("autoStatus", autoReview.status()).bind("autoResult", autoReview.resultJson())
				.bind("reviewMode", autoReview.mode()).bind("riskLevel", autoReview.riskLevel());
		spec = bindNullableJson(spec, "materials", materials);
		spec = bindNullableDeadline(spec, "deadline", reviewDeadline);
	...
	public Mono<MerchantPermissionRequest> findById(String id) {
		return db.sql("SELECT " + SELECT_COLS + " FROM merchant_permission_request WHERE id = CAST(:id AS uuid)")
				.bind("id", id).map(MerchantPermissionRequestRepository::map).one();
	}
	...
	public Flux<MerchantPermissionRequest> findByOrganization(String organizationId) {
		return db.sql("SELECT " + SELECT_COLS
				...
				.bind("org", organizationId).map(MerchantPermissionRequestRepository::map).all();
	}

	/**
	 * 审核队列筛选口径（admin 列表 status 参数）：
	 * pending=pending+under_review（待审，逾期优先）；reviewed=approved/rejected（按最近审结时间倒序——
	 * 刚审结的在最前）；all=全部（按提交时间倒序）。WHERE/ORDER 片段由白名单常量拼出，不接用户输入。
	 */
	private record QueueFilter(String where, String order) {
		static QueueFilter of(String filter) {
			return switch (filter) {
				case "pending" -> new QueueFilter(" WHERE status IN ('pending','under_review')",
						"(review_deadline < now()) DESC, review_deadline, created_at");
				case "reviewed" -> new QueueFilter(" WHERE status IN ('approved','rejected')",
						"decision_at DESC NULLS LAST, created_at DESC");
				case "all" -> new QueueFilter("", "created_at DESC");
				default -> throw new IllegalArgumentException("unknown permission queue filter: " + filter);
			};
		}
	}

	/** 按筛选查询审核队列（不分页——权限申请量级可控，保持既有数组契约）。 */
	public Flux<MerchantPermissionRequest> findQueue(String filter) {
		QueueFilter queueFilter = QueueFilter.of(filter);
		return db.sql("SELECT " + SELECT_COLS + " FROM merchant_permission_request" + queueFilter.where() + " ORDER BY "
				+ queueFilter.order()).map(MerchantPermissionRequestRepository::map).all();
	}

	public Flux<MerchantPermissionRequest> findAwaitingAutomaticReview(int limit) {
		return db
				.sql("SELECT " + SELECT_COLS + " FROM merchant_permission_request"
						+ " WHERE status IN ('pending','under_review')"
						+ " AND auto_review_status IN ('not_run','pending')" + " ORDER BY created_at, id LIMIT :limit")
				.bind("limit", Math.max(1, Math.min(limit, 500))).map(MerchantPermissionRequestRepository::map).all();
	}
	...
	public Mono<MerchantPermissionRequest> updateAutomaticReview(String id, int expectedVersion,
		...
		return db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id).bind("expectedVersion", expectedVersion)
	...
	public Mono<MerchantPermissionRequest> claim(String id, String reviewerAccountId) {
		return db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id).bind("reviewer", reviewerAccountId)
				.map(MerchantPermissionRequestRepository::map).one();
	}

	/** CAS 审核；pending 兼容旧客户端直接审核，under_review 只允许领取人完成。 */
	public Mono<MerchantPermissionRequest> review(String id, String status, String reviewerAccountId, String reviewNote,
			int expectedVersion) {
		var spec = db.sql("""
				UPDATE merchant_permission_request
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id).bind("status", status).bind("reviewer", reviewerAccountId)
	...
	public Mono<Integer> countAppeals(String originalRequestId) {
		return db
				.sql("SELECT COUNT(*)::int AS count FROM merchant_permission_request"
						+ " WHERE original_request_id = CAST(:id AS uuid)")
				.bind("id", originalRequestId).map(row -> row.get("count", Integer.class)).one().defaultIfEmpty(0);
	}
	...
	public Flux<MerchantPermissionRequest> markOverdue(int limit) {
		return db.sql("""
				...
				""".formatted(SELECT_COLS)).bind("limit", Math.max(1, Math.min(limit, 500)))
				.map(MerchantPermissionRequestRepository::map).all();
	}

	private static MerchantPermissionRequest map(Readable row) {
		return new MerchantPermissionRequest(row.get("id", String.class), row.get("organization_id", String.class),
				row.get("requester_account_id", String.class), row.get("requested_tier", String.class),
				row.get("materials", String.class), row.get("status", String.class),
				row.get("reviewer_account_id", String.class), row.get("review_note", String.class),
				row.get("industry", String.class), toInstant(row.get("review_deadline", OffsetDateTime.class)),
				row.get("original_request_id", String.class), row.get("appeal_note", String.class),
				...
				row.get("auto_review_status", String.class), row.get("auto_review_result", String.class),
				row.get("review_mode", String.class), row.get("risk_level", String.class),
				row.get("attachment_ids", String.class), toInstant(row.get("decision_at", OffsetDateTime.class)),
