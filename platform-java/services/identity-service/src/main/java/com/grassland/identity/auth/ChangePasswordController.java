 ...
 *
 * <p>
 * 两种形态：<b>带 currentPassword</b> 的常规改密；<b>不带</b>——仅当
...
public class ChangePasswordController {
	...
	public ChangePasswordController(CurrentAccountResolver accounts, UserLookup users, UserRepository userRepository,
			AccountFlagRepository flags, PasswordVerifier passwordVerifier, Argon2PasswordHasher argon2Hasher,
			PasswordVerificationExecutor passwordExecutor) {
	...
	public Mono<ResponseEntity<Map<String, Object>>> change(@RequestBody ChangePasswordRequest body,
		...
		return accounts.resolve(request).flatMap(principal -> {
			...
			return flags.mustChangePassword(accountId).defaultIfEmpty(Boolean.FALSE)
					...
					.thenReturn(ResponseEntity.ok().body(Map.<String, Object>of("success", true)));
		});
	}
	...
	private Mono<Void> requireCurrentWhenNeeded(String accountId, String currentPassword, boolean mustChange) {
		...
		return users.findLoginUserById(accountId).flatMap(login -> verifyCurrent(currentPassword, login));
	}

	private Mono<Void> verifyCurrent(String currentPassword, LoginUser login) {
		return passwordExecutor.verify(passwordVerifier, currentPassword, login.passwordHash())
				.flatMap(passwordOk -> passwordOk ? Mono.empty() : Mono.error(new IdentityException(400, "当前密码不正确")));
	}
