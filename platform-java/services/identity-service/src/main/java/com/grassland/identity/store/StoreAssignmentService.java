 ...
 *
 * <p>
 * 与 #49 下线的外部挂靠（任意 accountId 挂入）不同：这里只操作<b>本主体内</b>的账号 （有组织关系即可；2026-08-28 拍板
 * owner/admin 也可挂店——管理层亲自运营某店时领名分， 权限本就由 orgSuperUserAsManager 覆盖，分配只是如实呈现），门禁在
 * controller 层收归主体 ADMIN+（决策 C）。
 *
 * <ul>
 * <li>{@link #assign} = 分配或调度（assign-or-move）：已挂他店则同事务先解除再挂目标店 （决策
 * D：成员至多挂一店）；同店重复调用 = 改角色。role=manager 过一店一店长闸（决策 B， 排除自身故同店 staff→manager
 * 改角色不被自己挡住）。</li>
 * <li>{@link #remove} = 移除回池：只解除挂靠，账号保留为池内成员；删号走主体区既有端点。</li>
 * </ul>
 *
 * <p>
 * 两动作均与 outbox 审计事件同事务（StoreMembershipAssigned / Removed）。
 */
@Service
public class StoreAssignmentService {
	...
	private final StoreAssignmentLock assignmentLock;

	public StoreAssignmentService(StoreMembershipRepository storeMemberships, MembershipRepository orgMemberships,
			StoreRepository stores, OutboxRepository outbox, StoreAssignmentLock assignmentLock) {
	...
	public Mono<StoreMembership> assign(String operatorAccountId, String organizationId, String storeId,
		...
		// 任务书 #103 C103-17（R08）：全链进组织锁事务——一店一店长的 count 守卫在锁内重读才可信
		// （此前守卫在事务外，两次并发 count=0 产生双店长）；内层 transactional 参与同一事务。
		return assignmentLock.withOrganizationLock(organizationId,
				ensureStoreInOrg(organizationId, storeId).then(ensurePoolMember(accountId, organizationId))
						.then(guardUniqueManagerIfNeeded(storeId, accountId, role))
						// 删旧挂靠与建新挂靠同事务：中途失败不能留下「两边都挂/两边都没挂」
						.then(storeMemberships.deleteAllByAccountInOrg(accountId, organizationId)
								.then(storeMemberships.create(storeId, accountId, role)).flatMap(
										m -> appendAssignedEvent(operatorAccountId, organizationId, m).thenReturn(m))));
	}

	public Mono<Void> remove(String operatorAccountId, String organizationId, String storeId, String accountId) {
		// C103-17：移除同进组织锁事务（与分配/改角色同锁序，避免移除与并发分配交错产生双店长）。
		return assignmentLock.withOrganizationLock(organizationId,
				ensureStoreInOrg(organizationId, storeId)
						.then(storeMemberships.findRole(storeId, accountId)
								.switchIfEmpty(Mono.error(new IdentityException(404, "该成员不在此门店"))).then())
						.then(storeMemberships.deleteByStoreAndAccount(storeId, accountId))
						.then(appendRemovedEvent(operatorAccountId, organizationId, storeId, accountId)));
	}
	...
	/**
	 * 分配对象须为本 org 成员（owner/admin/member 皆可——2026-08-28 拍板放开管理层； 非本 org 账号 404
	 * 跨主体隔离）。挂店为店长即占位（一店一店长闸照常生效）。
	 */
	private Mono<Void> ensurePoolMember(String accountId, String organizationId) {
		return orgMemberships.findRole(organizationId, accountId)
				.switchIfEmpty(Mono.error(new IdentityException(404, "该账号不是本主体成员"))).then();
	}
	...
	private Mono<Void> guardUniqueManagerIfNeeded(String storeId, String accountId, String role) {
		...
		return storeMemberships.countManagerRows(storeId, accountId).flatMap(
