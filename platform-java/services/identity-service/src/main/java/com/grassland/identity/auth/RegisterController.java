...
public class RegisterController {
	...
	public RegisterController(EmailVerificationService codeService, DatabaseClient db, SessionWriter sessionWriter,
			...
			OutboxRepository outbox, TransactionalOperator transactions, Argon2PasswordHasher argon2Hasher) {
	...
	public Mono<ResponseEntity<Map<String, Object>>> register(@RequestBody RegisterRequest body,
		...
		return codeService.verifyAndConsume(normalizedEmail, code.trim()).flatMap(valid -> {
			...
			return Mono.fromCallable(() -> argon2Hasher.hash(password)).subscribeOn(Schedulers.boundedElastic())
					.flatMap(hash -> transactions.transactional(db
							...
							.bind("id", userId).bind("email", normalizedEmail).bind("hash", hash)
	...
	private Mono<Void> awardRegistrationCredits(String userId) {
		return financeCredits.award(userId, 3, "新用户注册赠送", "registration:" + userId).onErrorResume(error -> {
