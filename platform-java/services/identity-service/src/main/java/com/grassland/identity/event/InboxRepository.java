 ...
 *
 * <p>镜像 marketplace {@code InboxRepository}：{@code recordIfAbsent} 用 {@code ON CONFLICT DO NOTHING}
 * 插入，冲突（同 {@code (consumer_name, event_id)}）时回查校验内容是否一致——
 * 同 ID 异内容（payload SHA-256 不符）抛 {@link EventContractException}，进 DLT，不静默覆盖。
 */
@Component
public class InboxRepository {
    ...
    /**
     * 仅在单测里失败注入时使用：模拟「领域写（notification 插入）失败」以证明 inbox 行随之回滚。
     * 主路径不经此方法——消费者在 {@link NotificationEventConsumer} 的同一事务里完成 inbox + 通知插入。
     */
    ...
    private Mono<Boolean> validateExisting(
            String consumerName, IdentityEventEnvelope envelope, String payloadSha256) {
        return db.sql("""
