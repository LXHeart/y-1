 ...
 *
 * <p>
 * 镜像 marketplace {@code KafkaConsumerReliabilityConfig}，但<b>不重复定义事务 bean</b>——
 * identity 已有 {@code identityReactiveTransactionManager} /
 * {@code identityTransactionalOperator}
 * （{@code IdentityServiceApplication}），processor 直接注入后者。
 *
 * <p>
 * 关键策略（与 marketplace 一致）：
 * <ul>
 * <li>{@link EventContractException} 标记不可重试——坏数据直接进 DLT，不阻塞分区。</li>
 * <li>{@link FixedBackOff}(500ms, 2)：瞬时故障重试 2 次后进 DLT。</li>
 * <li>{@code AckMode.MANUAL} + {@code asyncAcks}：事务提交后才 ack，未提交前 offset
 * 不前移。</li>
 * </ul>
 */
@Configuration
public class NotificationKafkaReliabilityConfig {
	...
	@Bean
	DefaultErrorHandler notificationErrorHandler(DeadLetterPublishingRecoverer recoverer, FixedBackOff backOff) {
		DefaultErrorHandler errorHandler = new DefaultErrorHandler(recoverer, backOff);
		errorHandler.addNotRetryableExceptions(EventContractException.class);
		errorHandler.setRetryListeners(new RetryListener() {
			@Override
			public void failedDelivery(org.apache.kafka.clients.consumer.ConsumerRecord<?, ?> record, Exception error,
					int deliveryAttempt) {
				// 预留：如需重试计数可在此埋点。
			}

			@Override
			public void recovered(org.apache.kafka.clients.consumer.ConsumerRecord<?, ?> record, Exception error) {
				// 预留：恢复计数。
			}
