 ...
 * historyTable=flyway_schema_history；identity 用 Flyway 默认历史表名（历史遗留，其余服务为
 * <svc>_flyway_schema）。
 */
@Configuration
...
public class DataSourceConfig {
	...
	Flyway flyway(DataSource dataSource, Environment env) {
		return FlywayBootstrap.flyway(dataSource, "flyway_schema_history",
				env.getProperty("identity.flyway.locations", "classpath:db/migration"), false);
	}
}
