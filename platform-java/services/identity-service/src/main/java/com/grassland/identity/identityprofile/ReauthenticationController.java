...
import com.grassland.identity.auth.PasswordVerificationExecutor;
 ...
 *
 * <p>
 * <b>解决的问题</b>：trust 的 {@code final-decision} 要求断言 {@code reauthenticatedAt} 在
 * 5 分钟内， 但此前登录链路不产生该字段（edge-bff 签发时硬编码 null）→ 客服终审恒 403。
 *
 * <p>
 * 链路：本端点校验密码 → 写 {@code identity_session.reauthenticated_at/auth_strength} →
 * edge-bff 直读 session 时取出 → 签进断言 → trust 校验近期性。
 *
 * <p>
 * <b>按 session 记录</b>（非按账号）：一个设备重认证不提升另一设备的权限，与活动身份同粒度。
 *
 * <p>
 * <b>本轮范围</b>：以密码作为第二因子（re-auth by password）。真正的多因子（TOTP / 短信码）
...
public class ReauthenticationController {
	...
	public ReauthenticationController(CurrentAccountResolver accounts, UserLookup users, PasswordVerifier passwords,
			IdentitySessionRepository sessions, PasswordVerificationExecutor passwordExecutor) {
	...
	public Mono<ResponseEntity<Map<String, Object>>> reauthenticate(@RequestBody ReauthenticateRequest body,
		...
		return accounts.resolvePrincipal(request)
				.flatMap(principal -> users.findLoginUserById(principal.user().id())
						.switchIfEmpty(Mono.error(new IdentityException(401, "未登录"))).flatMap(user -> passwordExecutor
								.verify(passwords, body.password(), user.passwordHash()).flatMap(passwordOk -> {
									if (!passwordOk) {
						...
						.map(session -> {
							...
							data.put("reauthenticatedAt",
									session.reauthenticatedAt() == null
											? null
											: session.reauthenticatedAt().toString());
