 ...
 *
 * <p>
 * 流程（镜像 marketplace {@code TrustEventProcessor}，但「副作用 = 插通知」而非「启对账 workflow」）：
 * <ol>
 * <li>解 envelope——形状/必填字段不符抛 {@link EventContractException}（不可重试 → DLT）。</li>
 * <li>非关注类型 → {@link NotificationProcessingResult#IGNORED}，<b>不写 inbox</b>（与
 * marketplace 一致）。</li>
 ...
 *
 * <p>
 * <b>两道幂等</b>：① {@code (consumer_name, event_id)} inbox 吸收 at-least-once 重投； ②
...
public class NotificationEventProcessor {
	...
	/**
	 * 解析收件人并逐条插通知（同事务），随后入队事务邮件（同事务）。无收件人（如邀请邮箱未注册）时仍视为 PROCESSED——inbox 已记录；邮件
	...
	private IdentityEventEnvelope parseEnvelope(ConsumerRecord<String, String> record) {
		...
		return new IdentityEventEnvelope(requiredText(root, "eventId"), requiredText(root, "eventType"),
				requiredText(root, "aggregateType"), requiredText(root, "aggregateId"), payload);
	}
