 ...
 *
 * <p>读多值 {@code backend_role} 表（V26）；app_users.role 单值由调用方作兜底合并。
...
public class BackendRoleRepository {
    ...
    public BackendRoleRepository(DatabaseClient db) {
        this.db = db;
    }
    ...
    public Mono<Void> revoke(String accountId, BackendRole role) {
        return db.sql("DELETE FROM backend_role WHERE account_id = CAST(:acct AS uuid) AND role = :role")
                .bind("acct", accountId)
                .bind("role", role.dbValue())
                .then();
    }
