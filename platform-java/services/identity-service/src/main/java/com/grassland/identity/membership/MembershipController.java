 ...
 *
 * <ul>
 *   <li>POST — 新增成员（role∈admin/member），需 OWNER；UNIQUE(org,account) 冲突 → 409；写 outbox {@code MembershipGranted}。</li>
...
public class MembershipController {
    ...
    public Mono<ResponseEntity<Map<String, Object>>> list(@PathVariable String orgId, ServerHttpRequest request) {
        return authz.requireRole(request, orgId, MembershipRole.MEMBER)
                .flatMap(account -> memberships.findByOrganization(orgId).collectList()
    ...
    public Mono<ResponseEntity<Map<String, Object>>> remove(@PathVariable String orgId,
                                                            ...
                                                            ServerHttpRequest request) {
        return authz.requireRole(request, orgId, MembershipRole.OWNER)
                .flatMap(owner -> guardLastOwner(orgId, accountId)
    ...
    private Mono<Void> guardLastOwner(String orgId, String accountId) {
        return memberships.findRole(orgId, accountId)
                .flatMap(role -> {
