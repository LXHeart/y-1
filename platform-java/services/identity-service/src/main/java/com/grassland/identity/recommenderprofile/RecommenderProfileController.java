 ...
 *
 * <p>
 * <b>可见性</b>：画像是给商家做撮合判断用的，故任何登录用户都能按 accountId 读——
 * 但只回画像字段，<b>不回邮箱等账号信息</b>，也没有「按条件搜人」的入口（那会变成人肉数据库）。 商家拿到 accountId
 * 的唯一途径是对方主动报名了自己的任务。收入统计/月度账单是私有数据（D7）， 不在本响应内，永不出现在公开端点。
 *
 * <p>
 * <b>头像（D6）</b>：PUT 时若带 {@code avatarMediaId}，先经 intelligence
 * 复验（owner==account、 purpose=avatar、active）才落库；读取时把 {@code avatar_media_id} 换成短
 * TTL presigned GET （字段 {@code avatarUrl}），公开端点不外泄 media id。换取是 IO，失败时优雅降级为
 * null，不阻断画像读取。
 */
@RestController
public class RecommenderProfileController {
	...
	public RecommenderProfileController(CurrentAccountResolver accounts, RecommenderProfileRepository profiles,
			IdentityProfileRepository identities, RecommenderVerificationRepository verifications,
	...
	public Mono<ResponseEntity<Map<String, Object>>> updateMyProfile(@RequestBody UpdateRecommenderProfileRequest body,
		...
		return accounts.resolve(request)
				...
				.flatMap(account -> identities.findByAccountAndType(account.id(), IdentityType.RECOMMENDER.dbValue())
						...
						.then(requireAvatarUsable(body, account.id())).then(profiles.upsert(account.id(), body))
						.flatMap(profile -> verificationBody(account.id(), profile, true)));
	}
	...
	public Mono<ResponseEntity<Map<String, Object>>> profileOf(@PathVariable String accountId,
			ServerHttpRequest request) {
		return accounts.resolve(request).flatMap(viewer -> profiles.findByAccount(accountId)
	...
	/**
	 * 组装画像响应：先取 verification 状态，再换头像 presigned URL，最后包信封。 {@code includeMediaId} 为
	 * true 时（self 读）回 {@code avatarMediaId} 供编辑表单回填； 公开读只回 {@code avatarUrl}，不外泄
	 * media id。
	 */
	private Mono<ResponseEntity<Map<String, Object>>> verificationBody(String accountId, RecommenderProfile profile,
			boolean includeMediaId) {
		return verifications.findLatestByAccount(accountId)
				...
				.flatMap(verification -> avatarUrl(profile).defaultIfEmpty("").map(url -> {
	...
	private Mono<String> avatarUrl(RecommenderProfile profile) {
		...
		return avatars.issueDownloadUrl(mediaId).map(download -> download.downloadUrl().toString())
				.onErrorResume(error -> {
					log.warn("avatar presigned url exchange failed: accountId={}, mediaId={}", profile.accountId(),
							profile.avatarMediaId(), error);
					return Mono.empty();
				});
	}
	...
	private static Map<String, Object> toBody(RecommenderProfile profile, String avatarUrl, boolean includeMediaId) {
		...
		map.put("socialAccounts",
				profile.socialAccounts().stream().map(RecommenderProfileController::socialBody).toList());
		...
		map.put("workSamples",
				profile.workSamples().stream().map(RecommenderProfileController::workSampleBody).toList());
