 ...
 * 断言领域写（争议 / 裁决状态迁移）随之回滚——而非「写了领域态却丢了事件」的静默缺口。 覆盖 DisputeController 的
 * open/decide 两种写形态；AdjudicationController 的 assign/appeal/finalize 同形态（均
 * {@code transactions.transactional(写+outbox)}），由既有 AdjudicationControllerIT 守
 * happy path。
 */
class OutboxAtomicityIT extends TrustItSupport {
	...
	private long disputeCountByEngagement(String eng) {
		Long c = db.sql("SELECT COUNT(*)::bigint AS c FROM dispute_case WHERE engagement_ref = :eng").bind("eng", eng)
				.map(row -> row.get("c", Long.class)).one().block();
		return c == null ? 0L : c;
	}

	private String disputeStatus(String id) {
		return db.sql("SELECT status FROM dispute_case WHERE id = CAST(:id AS uuid)").bind("id", id)
