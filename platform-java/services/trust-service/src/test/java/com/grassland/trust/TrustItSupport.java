...
/**
 * trust 集成测试公共基座（草场 Epic 6 Slice 6A）。镜像 {@code FinanceItSupport}：单例
 * testcontainers postgres； {@code trust.datasource.from-database-url=true} 触发
 * {@code TrustDataSourceConfig} → Flyway V1 建 dispute_case/trust_outbox；
 * {@code identity-assertion} 注入 signer。提供 {@link #sign}（用户断言）+
 * {@link #signService}（服务断言）。
 *
 * <p>
 * 切片 12 起 {@code POST /api/trust/disputes} 先调 marketplace 授权；这里以
 * {@link MockitoBean} 替换该出站客户端， 默认对任意 applicationId 放行并回 canonical
 * {@link #MARKETPLACE_ORG}（单测无 marketplace）。需拒绝时在用例内重置桩。
 */
...
public abstract class TrustItSupport {
	...
	void authorizeByDefault() {
		lenient().when(authorizer.authorize(anyString(), anyString(), anyString())).thenAnswer(
				inv -> Mono.just(new MarketplaceEngagementAuthorizationClient.Authorization(inv.getArgument(0),
						MARKETPLACE_ORG, inv.getArgument(1), false)));
		lenient().when(reputationClient.getLevel(anyString())).thenAnswer(
				inv -> Mono.just(new MarketplaceReputationClient.LevelResult(inv.getArgument(0), "Lv5", 5, true, 0L)));
		lenient().when(identityMemberships.organizationIds(anyString())).thenReturn(Mono.just(Set.of()));
	}
	...
	static void props(DynamicPropertyRegistry r) {
		...
		// 6C 起 trust-service 引入 Temporal：IT 用内存 test-server（免 temporal 容器），镜像
		// marketplace。
	...
	protected WebTestClient client() {
		// responseTimeout 30s：默认 5s 在 CI 慢 runner
		// 负载下偶发超时（IdentityItSupport/FinanceItSupport 同口径）。
	...
	protected String sign(String accountId, String activeIdentityType, String organizationId, String permissionTier) {
		...
		return userSigner("edge-bff", "grassland-trust").sign(new IdentityAssertion(accountId, activeIdentityType,
	...
	protected String signServiceWithRole(String organizationId, String principal, String role) {
		Instant now = Instant.now();
		return serviceSigner(principal, "grassland-trust").sign(
				new IdentityAssertion("service:" + principal, null, null, organizationId, null, "service", "internal",
						null, "r", "t", "grassland-trust", now, now.plusSeconds(30), "service", principal, role));
	}
}
