 ...
 * 覆盖：open（成功+幂等/角色门禁）、活跃争议查询（marketplace 服务断言 200 / 404 / org
 * 自查）、decide（open→final 手动终局/重复 409）、 异议 48h 窗口（任务书 #70 卡B，PRD §7.1）。
 */
...
class DisputeControllerIT extends TrustItSupport {
	...
	/** 窗口配置 spy（任务书 #70 卡B）：默认透传真实值（yml 48h）；禁用/秒级用例按需改写 effective 值。 */
	@MockitoSpyBean
	...
	void openPersistsCanonicalPremiumSupportSnapshotFromMarketplace() {
		...
		String org = MARKETPLACE_ORG;
		String eng = UUID.randomUUID().toString();
		when(authorizer.authorize(eng, merchant, "merchant")).thenReturn(
				Mono.just(new MarketplaceEngagementAuthorizationClient.Authorization(eng, org, recommender, true)));
		...
		Boolean persistedPremium = db.sql("SELECT premium_support FROM dispute_case WHERE id=CAST(:id AS uuid)")
				.bind("id", data.get("id")).map((row, metadata) -> row.get("premium_support", Boolean.class)).one()
				.block();
		Integer persistedPriority = db.sql("SELECT support_priority FROM dispute_case WHERE id=CAST(:id AS uuid)")
				.bind("id", data.get("id")).map((row, metadata) -> row.get("support_priority", Integer.class)).one()
				.block();
	...
	void openRequiresPartyRole() {
		client().post().uri("/api/trust/disputes")
				...
				.bodyValue(Map.of("engagementRef", UUID.randomUUID().toString())).exchange().expectStatus()
				.isForbidden();
	}
	...
	void openRejectsNonUuidEngagementRef() {
		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity",
						sign(UUID.randomUUID().toString(), "merchant", MARKETPLACE_ORG, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("engagementRef", "not-a-uuid")).exchange()
				.expectStatus().isBadRequest();
	}

	/**
	 * D-03 审阅 F2：并发 create 撞唯一键时必须回读既有争议返回带 id 的 200， 不能返回空 200 体（marketplace 侧解析不到
	 * data.id → 商家看到 500）。
	 */
	@Test
	void concurrentCreateReturnsExistingDisputeInsteadOfEmptyBody() {
		...
		String org = MARKETPLACE_ORG;
		String eng = UUID.randomUUID().toString();
		Map<String, Object> body = Map.of("engagementRef", eng, "kind", "merchant_rejection", "openedByAccountId",
				merchant, "organizationId", org, "reason", "系统核实与实际不符");
		...
		client().post().uri("/api/trust/disputes").header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(body).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.id").isEqualTo(disputeId).jsonPath("$.data.kind").isEqualTo("merchant_rejection");
		assertThat(outboxCount("DisputeOpened", eng)).isEqualTo(1);

		// 真正的唯一键冲突路径：预查为空（对手尚未提交）→ create 撞键返回空 → 必须回读。
		String eng2 = UUID.randomUUID().toString();
		doAnswer(inv -> ((Mono<?>) inv.callRealMethod()).then(Mono.empty())).when(disputeRepo).create(eq(eng2),
				anyString(), anyString(), anyString(), any(), anyString(), anyBoolean());
		client().post().uri("/api/trust/disputes").header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", eng2, "kind", "merchant_rejection", "openedByAccountId", merchant,
						"organizationId", org, "reason", "并发"))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.id").isNotEmpty()
				.jsonPath("$.data.kind").isEqualTo("merchant_rejection");
	}

	/**
	 * D-03：marketplace 服务可代商家开 merchant_rejection 案，并由 SLA 内部端点默认 for_recommender
	 * 终局。
	 */
	@Test
	void marketplaceOpensAndAutoFinalizesMerchantRejection() {
		...
		Map<String, Object> resp = client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", signService(org, "marketplace")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", eng, "kind", "merchant_rejection", "openedByAccountId", merchant,
						"organizationId", org, "recommenderAccountId", UUID.randomUUID().toString(),
						"premiumSupportAtAccept", true, "reason", "系统核实与实际不符"))
				.exchange().expectStatus().isCreated().expectBody(Map.class).returnResult().getResponseBody();
		...
		client().get().uri("/api/trust/engagements/" + eng + "/open-dispute")
				...
				.expectBody().jsonPath("$.data.kind").isEqualTo("merchant_rejection")
				.jsonPath("$.data.openedByAccountId").doesNotExist().jsonPath("$.data.openedByAlias")
				.value(value -> assertThat((String) value).startsWith("participant-"));

		client().post().uri("/api/trust/internal/disputes/" + disputeId + "/auto-finalize")
				.header("X-Grassland-Identity", signService(org, "marketplace")).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.status").isEqualTo("final").jsonPath("$.data.finalDecision")
				.isEqualTo("for_recommender");
	...
	void merchantRejectionInternalEndpointsRejectWrongCallerAndKind() {
		...
		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", another, "kind", "merchant_rejection")).exchange().expectStatus()
				.isCreated().expectBody().jsonPath("$.data.kind").isEqualTo("standard");
	}
	...
	void autoFinalizePromotesDeferredObjectionToStandardSuccessor() {
		...
		Map<?, ?> opened = client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", signService(org, "marketplace")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", eng, "kind", "merchant_rejection", "openedByAccountId", merchant,
						"organizationId", org, "reason", "商家异议", "recommenderAccountId", recommender,
		...
		String successorKind = db
				.sql("SELECT kind FROM dispute_case" + " WHERE engagement_ref = :ref AND status <> 'final'")
				.bind("ref", eng).map(r -> r.get("kind", String.class)).one().block();
		String successorReason = db
				.sql("SELECT reason FROM dispute_case" + " WHERE engagement_ref = :ref AND status <> 'final'")
	...
	void openDisputeQuery404WhenNone() {
		client().get().uri("/api/trust/engagements/app-missing/open-dispute")
				.header("X-Grassland-Identity", signService(UUID.randomUUID().toString(), "marketplace")).exchange()
	...
	void decideFlipsToFinalAndClearsActive() {
		...
		client().post().uri("/api/trust/disputes/" + id + "/decide")
				...
				.expectStatus().isOk().expectBody().jsonPath("$.data.status").isEqualTo("final")
				.jsonPath("$.data.decision").isEqualTo("in_merchant_favor").jsonPath("$.data.finalDecision")
				.isEqualTo("in_merchant_favor");
	...
	private String outboxPayloadField(String eventType, String disputeId, String field) {
		return db
				.sql("SELECT payload->>'" + field + "' AS v FROM trust_outbox"
						+ " WHERE event_type = :et AND payload->>'disputeId' = :id")
				.bind("et", eventType).bind("id", disputeId).map(r -> r.get("v", String.class)).one().block();
	}
	...
	private long outboxCount(String eventType, String engagementRef) {
		return db
				...
				.bind("et", eventType).bind("ref", engagementRef).map(r -> r.get("c", Integer.class)).one().block()
				.longValue();
	}
	...
	void recommenderObjectionIsDeferredAndIdempotentWhileMerchantRejectionIsActive() {
		...
		client().post().uri("/api/trust/disputes").header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", eng, "kind", "merchant_rejection", "openedByAccountId", merchant,
						"organizationId", org, "reason", "系统与实际不符"))
				.exchange().expectStatus().isCreated();

		Map<?, ?> first = client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(recommender, "recommender", null, "basic"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", eng, "reason", "  我也不同意，证据 A/B  ")).exchange().expectStatus()
				.isAccepted().expectBody(Map.class).returnResult().getResponseBody();
		...
		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(recommender, "recommender", null, "basic"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", eng, "reason", "重试时的新文本不得覆盖")).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.requestId").isEqualTo(requestId).jsonPath("$.data.reason")
				.isEqualTo("  我也不同意，证据 A/B  ");

		client().get().uri("/api/trust/dispute-requests/" + requestId)
				.header("X-Grassland-Identity", sign(recommender, "recommender", null, "basic")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.status").isEqualTo("pending")
				.jsonPath("$.data.disputeId").isEqualTo("").jsonPath("$.data.workflowId").isEqualTo("")
	...
	/**
	 * F8 对称情形：marketplace 尝试在活跃 standard 争议上开 merchant_rejection → 409。 虽此路径当前只由
	 * marketplace contest 调（不会与推荐官并发），但守卫对称性防止未来误用。
	 */
	@Test
	void marketplaceCannotOpenMerchantRejectionWhileStandardDisputeIsActive() {
		...
		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(recommender, "recommender", null, "basic"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("engagementRef", eng, "reason", "未收到报酬"))
				.exchange().expectStatus().isCreated().expectBody().jsonPath("$.data.kind").isEqualTo("standard");

		// marketplace 尝试开 merchant_rejection → 409
		client().post().uri("/api/trust/disputes").header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("engagementRef", eng, "kind",
						"merchant_rejection", "openedByAccountId", merchant, "organizationId", org, "reason", "质量不符"))
	...
	// ---------------- 任务书 #70 卡B：异议 48h 窗口（PRD §7.1） ----------------

	/** 授权锚点控制：re-stub authorizer 返回带 resultAnchorAt 的五参 Authorization。 */
	private void authorizeWithAnchor(String engagementRef, String merchant, Instant anchor) {
		when(authorizer.authorize(engagementRef, merchant, "merchant"))
				.thenReturn(Mono.just(new MarketplaceEngagementAuthorizationClient.Authorization(engagementRef,
						MARKETPLACE_ORG, UUID.randomUUID().toString(), false, anchor)));
	}

	/** a. 窗口内（anchor=1h 前，默认 48h）→ 201 创建。 */
	@Test
	void openAllowedWithinDisputeWindow() {
		String merchant = UUID.randomUUID().toString();
		String eng = UUID.randomUUID().toString();
		authorizeWithAnchor(eng, merchant, Instant.now().minus(Duration.ofHours(1)));
	...
	/** b. 超窗（anchor=49h 前）→ 409 且文案含「异议期已过」，不建案不写事件。 */
	@Test
	void openRejectedAfterDisputeWindowExpires() {
		String merchant = UUID.randomUUID().toString();
		String eng = UUID.randomUUID().toString();
		authorizeWithAnchor(eng, merchant, Instant.now().minus(Duration.ofHours(49)));

		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(merchant, "merchant", MARKETPLACE_ORG, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("engagementRef", eng, "reason", "未履约"))
				.exchange().expectStatus().isEqualTo(409).expectBody().jsonPath("$.error")
				.value(org.hamcrest.Matchers.containsString("异议期已过"));
		assertThat(outboxCount("DisputeOpened", eng)).isZero();
	}

	/** c. anchor=null（无任何结果性事件，存量/未确认边缘）→ fail-open 创建。 */
	@Test
	void openAllowedWithoutResultAnchor() {
		String merchant = UUID.randomUUID().toString();
		String eng = UUID.randomUUID().toString();
		authorizeWithAnchor(eng, merchant, null);
	...
	/** d. 窗口禁用（effective=0 哨兵，等价 dispute-open-window-hours=0）→ 超窗 anchor 也可开。 */
	@Test
	void expiredAnchorStillOpensWhenWindowDisabled() {
		when(adjudicationProps.disputeOpenWindowSecondsEffective()).thenReturn(0L);
		String merchant = UUID.randomUUID().toString();
		String eng = UUID.randomUUID().toString();
		authorizeWithAnchor(eng, merchant, Instant.now().minus(Duration.ofHours(49)));
	...
	/**
	 * e. 秒级覆盖（effective=2s，等价 dispute-open-window-seconds=2 dev 通道）→ anchor=10s 前
	 * 409、窗口内 201。
	 */
	@Test
	void secondsOverrideDrivesSubHourWindow() {
		when(adjudicationProps.disputeOpenWindowSecondsEffective()).thenReturn(2L);
		String merchant = UUID.randomUUID().toString();
		String oldEng = UUID.randomUUID().toString();
		authorizeWithAnchor(oldEng, merchant, Instant.now().minus(Duration.ofSeconds(10)));

		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(merchant, "merchant", MARKETPLACE_ORG, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("engagementRef", oldEng, "reason", "未履约"))
				.exchange().expectStatus().isEqualTo(409).expectBody().jsonPath("$.error")
				.value(org.hamcrest.Matchers.containsString("异议期已过"));

		String freshEng = UUID.randomUUID().toString();
		authorizeWithAnchor(freshEng, merchant, Instant.now().minus(Duration.ofSeconds(1)));
		client().post().uri("/api/trust/disputes")
				.header("X-Grassland-Identity", sign(merchant, "merchant", MARKETPLACE_ORG, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("engagementRef", freshEng, "reason", "未履约"))
				.exchange().expectStatus().isCreated();
	}

	/** f. merchant_rejection 服务断言路径不受窗口约束（自带确认窗口+客服 SLA，D6 连坐豁免）。 */
	@Test
	void merchantRejectionServicePathBypassesDisputeWindow() {
		String merchant = UUID.randomUUID().toString();
		String org = MARKETPLACE_ORG;
		client().post().uri("/api/trust/disputes").header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", UUID.randomUUID().toString(), "kind", "merchant_rejection",
						"openedByAccountId", merchant, "organizationId", org, "reason", "系统核实与实际不符"))
				.exchange().expectStatus().isCreated().expectBody().jsonPath("$.data.kind")
				.isEqualTo("merchant_rejection");
	}
}
