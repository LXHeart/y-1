...
class OutboxRepositoryIT extends TrustItSupport {
    ...
    void claimBatch_claimsOnlyDueRows_andReclaimsOnlyExpiredLeases() {
        ...
        db.sql("UPDATE trust_outbox SET claimed_until = now() - interval '1 second' WHERE event_id = 'due'")
                .then().block();
    ...
    void conditionalTransitions_rejectStaleOwners_andTrackPendingAge() {
        ...
        db.sql("UPDATE trust_outbox SET next_attempt_at = now() - interval '1 second' WHERE event_id = 'old'")
                .then().block();
