 ...
 *
 * <p>
 * 覆盖：adjudicate（202 分配面板 + DisputeAssigned / 幂等 200 / 角色门禁 / 409 终局 / 503
 * 无审判官）、 votes（201 + tally 累计 / 幂等 200 / 非面板 403 / 非审判官 403 / 非 voting 409 /
 * 非法选项 400 / 4-of-7 多数）、 getAdjudication（快照 + 脱敏 + 服务断言 / 跨 org 403 / 404）、
 * appeal（decided→appealed / 非 decided 409 / 重复 409）、final-decision（客服覆盖
 * appealed/escalated + MFA / 非客服 403 / MFA 过期 403）。
 */
class AdjudicationControllerIT extends TrustItSupport {
	...
	void adjudicateAssignsPanelAndFlipsToVoting() {
		...
		String id = openExpired(merchant, org, eng);
		...
		client().post().uri("/api/trust/disputes/" + id + "/adjudicate")
				...
				.expectStatus().isAccepted().expectBody().jsonPath("$.data.status").isEqualTo("voting")
				.jsonPath("$.data.round").isEqualTo(1).jsonPath("$.data.panel.size").isEqualTo(PANEL_SIZE)
				.jsonPath("$.data.panel.voted").isEqualTo(0).jsonPath("$.data.tallies.majority").isEmpty();
	...
	void adjudicateRepairsIncompletePanelToExactConfiguredSize() {
		...
		String id = openExpired(merchant, org, UUID.randomUUID().toString());
		disputes.startAdjudication(id, 1, 0).block();
		db.sql("INSERT INTO dispute_panel_assignment(dispute_id, round, judge_account_id)"
				+ " VALUES (CAST(:disputeId AS uuid), 1, CAST(:accountId AS uuid))").bind("disputeId", id)
				.bind("accountId", candidates.get(0)).then().block();

		client().post().uri("/api/trust/disputes/" + id + "/adjudicate")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.panel.size").isEqualTo(PANEL_SIZE);
	...
	void adjudicateRejectsFinalDispute() {
		...
		db.sql("UPDATE dispute_case SET status = 'open' WHERE id = CAST(:id AS uuid)").bind("id", id).then().block();
	...
	void adjudicateRejectsNonParty() {
		...
		String id = open(merchant, org, UUID.randomUUID().toString());
		client().post().uri("/api/trust/disputes/" + id + "/adjudicate")
				.header("X-Grassland-Identity", sign(UUID.randomUUID().toString(), "consumer", null, null)).exchange()
	...
	void adjudicateReturns503WhenNoJudges() {
		...
		String merchant = UUID.randomUUID().toString();
		String org = MARKETPLACE_ORG;
		String id = openExpired(merchant, org, UUID.randomUUID().toString()); // 不 seed 任何审判官
		client().post().uri("/api/trust/disputes/" + id + "/adjudicate")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
	...
	void adjudicateRequiresACompleteSevenJudgePanel() {
		clearJudges();
		String merchant = UUID.randomUUID().toString();
		String id = openExpired(merchant, MARKETPLACE_ORG, UUID.randomUUID().toString());
	...
	void adjudicateRevalidatesEveryCandidateAndExcludesDowngradedJudge() {
		...
		when(reputationClient.getLevel(downgraded))
				.thenReturn(Mono.just(new com.grassland.trust.judge.MarketplaceReputationClient.LevelResult(downgraded,
						"Lv4", 4, false, 9L)));
	...
	void adjudicateRollsBackWhenAdmissionIsRevokedAfterRemoteValidation() {
		...
		var eligible = new com.grassland.trust.judge.MarketplaceReputationClient.LevelResult(revoked, "Lv5", 5, true,
				12L);
		when(reputationClient.getLevel(revoked)).thenReturn(db
				.sql("UPDATE judge SET ops_admitted=false, ops_admitted_at=NULL, ops_admitted_by=NULL,"
	...
	void judgeCastsVoteAndTallyAccumulates() {
		...
		vote(panel.id, first, "for_merchant", null).expectStatus().isCreated().expectBody().jsonPath("$.data.vote")
				.isEqualTo("for_merchant").jsonPath("$.data.tallies.forMerchant").isEqualTo(1)
				.jsonPath("$.data.tallies.majority").isEmpty();
		vote(panel.id, second, "for_recommender", "对方提交的履约凭证链完整，验收记录可查，我支持推荐官一方。").expectStatus().isCreated()
				.expectBody().jsonPath("$.data.tallies.forRecommender").isEqualTo(1)
	...
	void voteRejectsNonJudgeCaller() {
		...
		client().post().uri("/api/trust/disputes/" + panel.id + "/votes")
				...
				.bodyValue(Map.of("vote", "for_merchant", "rationale", "综合双方提交的凭证与陈述，我认为该方主张更可信。")).exchange()
				.expectStatus().isForbidden();
	}
	...
	void voteRejectsJudgeWhoLeftPool() {
		...
		String judge = panel.judges.get(0);
		// 退池后即失去投票权（入池状态即权限开关）
		db.sql("UPDATE judge SET active = false WHERE account_id = CAST(:a AS uuid)").bind("a", judge).then().block();
		client().post().uri("/api/trust/disputes/" + panel.id + "/votes")
				.header("X-Grassland-Identity", sign(judge, "recommender", null, null))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("vote", "for_merchant", "rationale", "综合双方提交的凭证与陈述，我认为该方主张更可信。")).exchange()
	...
	void voteRejectsJudgeWhoseOperationsAdmissionWasRevoked() {
		...
		db.sql("UPDATE judge SET ops_admitted=false, ops_admitted_at=NULL, ops_admitted_by=NULL, version=version+1"
				+ " WHERE account_id=CAST(:a AS uuid)").bind("a", judge).then().block();
	...
	void voteUsesAssignmentSnapshotWhenMarketplaceEligibilityChanges() {
		...
		when(reputationClient.getLevel(judge)).thenReturn(Mono.just(
				new com.grassland.trust.judge.MarketplaceReputationClient.LevelResult(judge, "Lv4", 4, false, 11L)));
	...
	void voteRejectsInvalidChoice() {
		...
		client().post().uri("/api/trust/disputes/" + panel.id + "/votes")
				...
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("vote", "bogus", "rationale", "综合双方提交的凭证与陈述，我认为该方主张更可信。")).exchange().expectStatus()
				.isBadRequest();
	}
	...
	void fourOfSevenFinalizesImmediatelyAndBlocksLaterVotes() {
		...
		vote(panel.id, panel.judges.get(3), "for_merchant", null).expectStatus().isCreated().expectBody()
				.jsonPath("$.data.tallies.forMerchant").isEqualTo(4).jsonPath("$.data.tallies.majority")
				.isEqualTo("for_merchant");
		client().get().uri("/api/trust/disputes/" + panel.id + "/adjudication")
				.header("X-Grassland-Identity", sign(panel.merchant, "merchant", panel.org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.status").isEqualTo("decided")
	...
	void getAdjudicationSnapshotIsDesensitizedAndServiceAccessible() {
		...
		// marketplace 服务断言也可查
		client().get().uri("/api/trust/disputes/" + panel.id + "/adjudication")
				.header("X-Grassland-Identity", signService(panel.org, "marketplace")).exchange().expectStatus().isOk();
	}

	/**
	 * 面板审判官必须能读快照——否则「能投票却看不见要投什么」。
	 *
	 * 浏览器实测抓到的集成缺口：castVote 用 requireJudge + 面板成员放行， 而 getAdjudication 只认「当事方 org /
	 * marketplace 服务」，审判官恒 403， 前端看板显示「无权查询该争议」，投票按钮组渲染不出来。 这与 judge
	 * 身份、customer_service 角色属同一类跨服务不一致——写路径放行、读路径没跟上。
	 */
	@Test
	void getAdjudicationAllowsPanelJudge() {
		...
		String body = client().get().uri("/api/trust/disputes/" + panel.id + "/adjudication")
				...
				.header("X-Grassland-Identity", sign(judge, "recommender", null, null)).exchange().expectStatus().isOk()
	 ...
	 * 前端看板恒显示「无权查询争议」，连「客服终审」折叠区都渲染不出来，终审在 UI 上完全不可达。 这是 judge 身份、customer_service
	 * 角色之后，同一类「写路径放行、读路径没跟上」的第三次出现。
	 */
	@Test
	...
	void partyAppealsDecidedDispute() {
		...
		client().post().uri("/api/trust/disputes/" + id + "/appeal")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("note", "不服判决")).exchange().expectStatus()
				.isOk().expectBody().jsonPath("$.data.status").isEqualTo("appealed");
	...
	void customerServiceFinalDecisionOnOpenMerchantRejection() {
		...
		String org = MARKETPLACE_ORG;
		String id = disputes
				.create(UUID.randomUUID().toString(), org, merchant, "merchant", "系统核实与实际不符", "merchant_rejection")
				.block().id();
		...
		client().post().uri("/api/trust/disputes/" + id + "/final-decision")
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("decision", "for_merchant")).exchange()
	...
	void customerServiceFinalDecisionPromotesDeferredObjection() {
		...
		DisputeCase source = disputes.create(engagement, org, merchant, "merchant", "商家异议", "merchant_rejection")
				.block();
		DeferredDisputeRequest request = deferredRequests.createOrFind(source, recommender, "  人工终审后仍需七官裁定  ").block();
		...
		assertThat(outboxPayloadField("DisputeFinalized", source.id(), "successorDisputeId")).isEqualTo(successor.id());
	...
	void customerServiceFinalDecisionOverridesAppealed() {
		...
		client().post().uri("/api/trust/disputes/" + id + "/final-decision")
				.header("X-Grassland-Identity", signCs(cs, Instant.now())).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "for_recommender")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.status").isEqualTo("final").jsonPath("$.data.finalDecision")
	...
	void customerServiceFinalDecisionOnEscalated() {
		...
		client().post().uri("/api/trust/disputes/" + id + "/final-decision")
				.header("X-Grassland-Identity", signCs(cs, Instant.now())).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "for_merchant")).exchange().expectStatus().isOk().expectBody()
	...
	void adminRoleCanAlsoFinalize() {
		...
		String adminAssertion = userSigner("edge-bff", "grassland-trust").sign(
				new IdentityAssertion(UUID.randomUUID().toString(), null, "sid-admin", null, null, "cookie-session",
						"level2", now, "r", "t", "grassland-trust", now, now.plusSeconds(60), null, null, "admin"));

		client().post().uri("/api/trust/disputes/" + id + "/final-decision")
				.header("X-Grassland-Identity", adminAssertion).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "for_merchant")).exchange().expectStatus().isOk();
	}
	...
	void serviceAssertionCannotImpersonateCustomerService() {
		...
		client().post().uri("/api/trust/disputes/" + id + "/final-decision")
				.header("X-Grassland-Identity", serviceWithRole).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "for_merchant")).exchange().expectStatus().isForbidden();
	}
	...
	private void adjudicate(String merchant, String org, String id) {
		backdateEvidenceDeadline(id); // C02-D：满足开庭条件（到期）后再手动开庭
		client().post().uri("/api/trust/disputes/" + id + "/adjudicate")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isAccepted();
	}

	private org.springframework.test.web.reactive.server.WebTestClient.ResponseSpec vote(String disputeId,
			String judgeAccountId, String vote, String rationale) {
	...
	/** C02-D：开争议并把质证截止回拨为已过期——需要「立即可开庭」的用例用这个。 */
	private String openExpired(String merchant, String org, String eng) {
		String id = open(merchant, org, eng);
	...
	private void toDecided(String id) {
		backdateEvidenceDeadline(id); // C02-D：先满足开庭条件
	 ...
	 */
	/**
	 * 客服断言：按<b>平台角色</b>（末位 role 参数）而非 activeIdentityType—— customer_service 不是
	 * identity 支持的业务身份（与 judge 同类问题，e2e 联调发现）。
	 */
