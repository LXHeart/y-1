...
/**
 * 审判数据层（草场 Epic 6 Slice 6C Phase B）。继承 {@link TrustItSupport}，直接注
 * {@link DisputeCaseRepository} / {@link JudgeRepository}，覆盖端点脱敏后不可见、且 Phase C
 * workflow 才会调的：
 * <ul>
 * <li>审判官池抽签——同组织 / 显式 {@code judge_conflict} / 低 tier 排除。</li>
 * <li>状态机 5 态迁移 +
 * 守卫（startAdjudication/reopen/recordDecision/markAppealed/finalize）。</li>
...
class AdjudicationRepositoryIT extends TrustItSupport {
	...
	void assignPanelRechecksOrganizationAndDeclaredConflictsAtFinalInsert() {
		...
		List<String> initiallyEligible = judges.drawEligiblePool(5, disputeOrg, 10).map(Judge::accountId).collectList()
				.block();
		assertThat(initiallyEligible).contains(sameOrgAfterDraw, conflictedAfterDraw);

		db.sql("UPDATE judge SET organization_id=CAST(:org AS uuid)" + " WHERE account_id=CAST(:accountId AS uuid)")
				.bind("org", disputeOrg).bind("accountId", sameOrgAfterDraw).then().block();
		seedConflict(conflictedAfterDraw, disputeOrg);

		assertThat(judges.assignPanel(disputeId, 1, List.of(sameOrgAfterDraw, conflictedAfterDraw)).block()).isZero();
	...
	void recordVoteAndTally() {
		...
		assertThat(judges.recordVote(disputeId, 1, b, "for_recommender", "r").block().vote())
	...
	void recordVoteRechecksLocalAdmissionAtInsertTime() {
		...
		db.sql("UPDATE judge SET ops_admitted=false, ops_admitted_at=NULL, ops_admitted_by=NULL"
				+ " WHERE account_id=CAST(:accountId AS uuid)").bind("accountId", accountId).then().block();

		assertThat(judges.recordVote(disputeId, 1, accountId, "for_merchant", null).block()).isNull();
		Integer votes = db
				.sql("SELECT COUNT(*)::int AS count FROM dispute_vote" + " WHERE dispute_id=CAST(:disputeId AS uuid)")
				.bind("disputeId", disputeId).map(row -> row.get("count", Integer.class)).one().block();
	...
	void stateMachineOpenToFinalViaAdjudication() {
		...
		DisputeCase voting = disputes.startAdjudication(id, 1, 0).block(); // open→voting round 1
	...
	void startAdjudicationGuardsOnOpen() {
		...
		assertThat(disputes.startAdjudication(id, 1, 0).block()).isNull();
	}
	...
	private String openDispute(String org) {
		return disputes
				.create("eng-" + UUID.randomUUID(), org, UUID.randomUUID().toString(), "merchant", "未履约", "standard")
				.block().id();
	}
