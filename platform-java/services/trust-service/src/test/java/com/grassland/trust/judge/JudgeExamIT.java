 ...
 *
 * <p>
 * 覆盖：Lv4+考试及格可被抽进面板（V14 触发器新分支）/ Lv4 未考试被触发器拒（23514）/ 见习席 >2 换重抽 / 10 轮转正 +
 * audit / 挂起后触发器拒票、恢复后可投 / 题库 CRUD version 递增 / 考核看板「建议暂停」标记。迁移重放由
 * {@code JudgeAdmissionMigrationTest} 风格覆盖（V14 幂等 DDL）。
 */
class JudgeExamIT extends TrustItSupport {
	...
	void examFlowGradesPassesAndMarksProbation() {
		...
		client().post().uri("/api/trust/judges/exam")
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("answers", answers.toString())).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.score")
				.value(org.hamcrest.Matchers.greaterThanOrEqualTo(80)).jsonPath("$.data.passed").isEqualTo(true);
		...
		// audit 'probation'
		Long audits = db
				.sql("SELECT COUNT(*)::bigint AS c FROM judge_admission_audit"
		...
		Long events = db
				.sql("SELECT COUNT(*)::bigint AS c FROM trust_outbox"
		...
		client().post().uri("/api/trust/judges/exam")
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("answers", wrong.toString())).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.passed").isEqualTo(false)
	...
	void lv4WithExamJoinPanelViaTriggerLv4WithoutExamRejected() {
		...
		db.sql("UPDATE judge SET exam_passed_at = now(), admission_level = 'probation', probation_since = now(),"
				+ " version = version + 1 WHERE account_id = CAST(:a AS uuid)").bind("a", withExam).then().block();
		...
		var picks = judges.drawEligiblePool(4, MARKETPLACE_ORG, 20).map(Judge::accountId).collectList().block();
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() -> db
	...
	void suspendedJudgeCannotVoteUntilReinstated() {
		...
		// 开 voting 案件 + 面板
		backdateEvidenceDeadline(disputeId); // C02-D：HTTP 开案 deadline 在未来，先满足开庭条件
		disputes.startAdjudication(disputeId, 1, 0).block();
		...
		db.sql("UPDATE judge SET suspended_until = now() + interval '30 days', suspension_reason = 'abstain_rate'"
				+ " WHERE account_id = CAST(:a AS uuid)").bind("a", judgeAccount).then().block();
		...
		org.assertj.core.api.Assertions
				.assertThatThrownBy(() -> db
						.sql("INSERT INTO dispute_vote(dispute_id, round, judge_account_id, vote, rationale)"
	...
	void assessmentFlagsHighAbstentionAndSuspensionNotifies() {
		...
		for (int i = 0; i < 6; i++) {
			...
			db.sql("INSERT INTO dispute_panel_assignment(dispute_id, round, judge_account_id, assigned_at)"
					+ " VALUES (CAST(:d AS uuid), 1, CAST(:a AS uuid), now())").bind("d", disputeId)
					.bind("a", judgeAccount).then().block();
			if (i < 3) {
		...
		String body = client().get().uri("/api/admin/trust/judges/assessment")
				.header("X-Grassland-Identity", signAdmin()).exchange().expectStatus().isOk().expectBody(String.class)
				.returnResult().getResponseBody();
		assertThat(body).contains("suggestSuspension\":true");

		// 挂起 + audit + 通知事件
		client().post().uri("/api/admin/trust/judges/" + judgeAccount + "/suspension")
				.header("X-Grassland-Identity", signAdmin()).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("suspend", true, "reason", "弃权率过高")).exchange().expectStatus().isOk().expectBody()
		...
		client().post().uri("/api/admin/trust/judges/" + judgeAccount + "/suspension")
				.header("X-Grassland-Identity", signAdmin()).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("suspend", false)).exchange().expectStatus().isOk().expectBody()
	...
	void questionCrudBumpsVersion() {
		String body = client().post().uri("/api/admin/trust/judge-exam/questions")
				.header("X-Grassland-Identity", signAdmin()).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("category", "规则", "question", "平票后案件如何处理？", "options",
						java.util.List.of("进入下一轮", "直接终局"), "answerIndex", 0))
		...
		// 列表口径同形状
		client().get().uri("/api/admin/trust/judge-exam/questions").header("X-Grassland-Identity", signAdmin())
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.items[0].options[0]")
				.isEqualTo("进入下一轮");

		// 更新（乐观锁 version 0→1）
		client().put().uri("/api/admin/trust/judge-exam/questions/" + id).header("X-Grassland-Identity", signAdmin())
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("question", "平票后案件如何处理？（修订）", "expectedVersion", 0)).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.version").isEqualTo(1);
		// 旧版本重放 → 409
		client().put().uri("/api/admin/trust/judge-exam/questions/" + id).header("X-Grassland-Identity", signAdmin())
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("question", "再次修订", "expectedVersion", 0))
				.exchange().expectStatus().isEqualTo(409);
		// 下线
		client().delete().uri("/api/admin/trust/judge-exam/questions/" + id).header("X-Grassland-Identity", signAdmin())
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.active").isEqualTo(false);
	}
	...
	private void seedExamQuestions(int count) {
		for (int i = 0; i < count; i++) {
			exams.insertQuestion(new JudgeExamQuestion(UUID.randomUUID().toString(), "规则", "题目 " + i + "：审判官应当如何行为？",
	...
	private void seedVoteRounds(String judgeAccount, int rounds) {
		...
		for (int i = 0; i < rounds; i++) {
			...
			db.sql("INSERT INTO dispute_panel_assignment(dispute_id, round, judge_account_id)"
					+ " VALUES (CAST(:d AS uuid), 1, CAST(:a AS uuid))").bind("d", disputeId).bind("a", judgeAccount)
					.then().block();
			db.sql("INSERT INTO dispute_vote(dispute_id, round, judge_account_id, vote, rationale)"
					+ " VALUES (CAST(:d AS uuid), 1, CAST(:a AS uuid), 'for_merchant', '理由充分。')").bind("d", disputeId)
					.bind("a", judgeAccount).then().block();
		}
	}
	...
	private String openVotingDispute() {
		...
		backdateEvidenceDeadline(id); // C02-D：HTTP 开案 deadline 在未来，先满足开庭条件
	...
	private String signAdmin() {
		...
		return com.grassland.identity.assertion.TestAssertionHelper.userSigner("edge-bff", "grassland-trust")
				.sign(new com.grassland.identity.assertion.IdentityAssertion(UUID.randomUUID().toString(), null,
						"sid-admin", null, null, "cookie-session", "level2", now, "r", "t", "grassland-trust", now,
						now.plusSeconds(60), null, null, "platform_admin"));
	}
}
