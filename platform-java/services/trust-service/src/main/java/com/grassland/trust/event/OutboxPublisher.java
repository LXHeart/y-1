...
/**
 * trust outbox → Kafka 发布器（草场 Epic 6 Slice 6B）。复刻 identity/marketplace/finance OutboxPublisher。
 * 轮询未发布（published_at IS NULL）行发 topic（默认 grassland.trust.events），成功后 markPublished。
...
public class OutboxPublisher {
    ...
    public OutboxPublisher(OutboxRepository repo,
                           ...
                           @Value("${trust.outbox.topic:grassland.trust.events}") String topic,
                           @Value("${trust.outbox.enabled:true}") boolean enabled) {
        this.repo = repo;
        this.kafka = kafka;
        this.topic = topic;
