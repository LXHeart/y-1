 ...
 *
 * <p>
 * 抽面板（{@link #streamEligibleCandidates}）：流式读取 active + ops_admitted 且
 * {@code eligibility_tier >= minTier} 的审判官，排除与争议组织利益冲突者 （同
 * {@code organization_id} 或显式 {@code judge_conflict}），随机排序供上层逐个复验。
 *
 * <p>
 * 分配（{@link #assignPanel}）/ 投票（{@link #recordVote}）均 <b>幂等</b>：UNIQUE 约束 + ON
 * CONFLICT DO NOTHING， Phase C workflow activity 重试不会重复写。{@link #tallyVotes} 读
 * {@code dispute_vote} 按 choice 聚合计票， 多数决阈值见 {@link VoteTally}。
 *
 * <p>
 * 本仓库在 workflow activity 线程（非 workflow 线程）执行，{@code ORDER BY random()} 等非确定性安全
 * （Temporal 确定性铁律只约束 workflow 主体，不约束 activity）。
 */
@Component
public class JudgeRepository {

	private static final String JUDGE_COLS = "j.id::text, j.account_id::text, j.organization_id::text, j.eligibility_tier, j.active,"
	...
	private static final String JUDGE_RETURNING = "id::text, account_id::text, organization_id::text, eligibility_tier, active,"
	...
	/**
	 * 任务书 #74 卡 E（V14 触发器同口径）：可被抽中的资格谓词——Lv5 直入，或 Lv4+考试及格（见习通道）， 且未处于挂起期。
	 */
	public static final String DRAWABLE_PREDICATE = "(j.eligibility_tier >= 5 OR (j.eligibility_tier >= 4 AND j.exam_passed_at IS NOT NULL)) "
	 ...
	 *
	 * <p>
	 * 调用方已从 marketplace 验证有效 Lv5。active（用户参选意愿）与 ops_admitted（运营准入）正交：
	 * 重复报名或退池后重报仅恢复 active，不得静默撤销或授予运营准入。
	 *
	 * <p>
	 * 幂等：{@code UNIQUE(account_id)} 冲突 → 复活并更新 tier（允许声誉为动态值）。
	 */
	public Mono<Judge> enrollWithTier(String accountId, String organizationId, int eligibilityTier) {
		...
		var spec = db.sql("""
				...
				RETURNING %s
				""".formatted(JUDGE_RETURNING)).bind("id", UUID.randomUUID().toString()).bind("acct", accountId)
				.bind("tier", eligibilityTier);
	...
	public Mono<Judge> findByAccountId(String accountId) {
		return db.sql("SELECT " + JUDGE_COLS + " FROM judge j WHERE j.account_id = CAST(:acct AS uuid)")
				.bind("acct", accountId).map(JudgeRepository::mapJudge).one();
	}

	/** 后台列表：精确账号搜索或按 created_at + id 做稳定的 keyset 分页。 */
	public Flux<Judge> listForAdmin(int limit, Instant beforeCreatedAt, String beforeId, String accountId) {
		...
		if (accountId != null) {
			...
			spec = spec.bind("beforeCreatedAt", OffsetDateTime.ofInstant(beforeCreatedAt, java.time.ZoneOffset.UTC))
					.bind("beforeId", beforeId);
		}
		return spec.map(JudgeRepository::mapJudge).all();
	}

	/** 乐观锁更新运营准入；版本不匹配或目标状态未变化均返回 empty。 */
	public Mono<Judge> updateAdmission(String accountId, boolean admitted, long expectedVersion, String admittedBy) {
		var spec = db.sql("""
				UPDATE judge
				...
				RETURNING %s
				""".formatted(JUDGE_RETURNING)).bind("admitted", admitted).bind("acct", accountId).bind("expected",
				expectedVersion);
	...
	public Mono<Judge> deactivate(String accountId) {
		return db.sql("""
				...
				RETURNING %s
				""".formatted(JUDGE_RETURNING)).bind("acct", accountId).map(JudgeRepository::mapJudge).one();
	}

	/**
	 * 流式读取本地符合资格且与争议组织无冲突的全部候选（随机顺序）。 不在 SQL 层预设候选上限，避免前段远端复验失败时遗漏后续有效候选。
	 */
	public Flux<Judge> streamEligibleCandidates(int minTier, String disputeOrgId) {
		return db.sql("""
				...
				ORDER BY random()
				""".formatted(JUDGE_COLS, DRAWABLE_PREDICATE)).bind("minTier", minTier).bind("org", disputeOrgId)
	 ...
	 * {@code matchedPlatformAccounts} 中的成员写 matched_platform=true（涉案平台完成 ≥3
	 * 任务的熟手席）。 调用方必须校验返回行数与目标面板人数完全一致，并在同一事务内回滚状态、分配和 outbox。
	 */
	public Mono<Integer> assignPanel(String disputeId, int round, List<String> judgeAccountIds,
	...
	private Mono<Integer> insertPanelMember(String disputeId, int round, String judgeAccountId, boolean matched) {
		return db.sql("""
				...
				""".formatted(DRAWABLE_PREDICATE)).bind("d", disputeId).bind("round", round).bind("j", judgeAccountId)
				.bind("matched", matched).fetch().rowsUpdated().map(Long::intValue).defaultIfEmpty(0);
	}
	...
	public Mono<Integer> countMatchedPanel(String disputeId, int round) {
		return db
				.sql("SELECT COUNT(*)::int AS c FROM dispute_panel_assignment"
						+ " WHERE dispute_id = CAST(:id AS uuid) AND round = :round AND matched_platform = true")
				.bind("id", disputeId).bind("round", round).map(r -> r.get("c", Integer.class)).one().defaultIfEmpty(0);
	}
	...
	public Mono<Integer> countPanelProbation(String disputeId, int round) {
		return db.sql("""
				...
				""").bind("id", disputeId).bind("round", round).map(r -> r.get("c", Integer.class)).one()
				.defaultIfEmpty(0);
	}
	...
	public Mono<Integer> countDistinctVotingRounds(String judgeAccountId) {
		return db
				.sql("SELECT COUNT(DISTINCT (dispute_id, round))::int AS c FROM dispute_vote"
						+ " WHERE judge_account_id = CAST(:j AS uuid)")
				.bind("j", judgeAccountId).map(r -> r.get("c", Integer.class)).one().defaultIfEmpty(0);
	}

	/** 卡 F：历轮全部面板成员（重抽面板排除集）。 */
	public Flux<String> listPanelAccountsAllRounds(String disputeId) {
		return db
				.sql("SELECT DISTINCT judge_account_id::text AS account_id FROM dispute_panel_assignment"
						+ " WHERE dispute_id = CAST(:id AS uuid)")
				.bind("id", disputeId).map(row -> row.get("account_id", String.class)).all();
	}
	...
	public Mono<Judge> promote(String judgeAccountId) {
		return db.sql("""
				...
				RETURNING %s
				""".formatted(JUDGE_RETURNING)).bind("j", judgeAccountId).map(JudgeRepository::mapJudge).one();
	}
	...
	public Mono<Judge> updateSuspension(String judgeAccountId, boolean suspend, java.time.Instant until,
		...
		var spec = db.sql("""
				...
				RETURNING %s
				""".formatted(JUDGE_RETURNING)).bind("j", judgeAccountId).bind("suspend", suspend);
		spec = suspend
	...
	public Flux<Judge> findPanel(String disputeId, int round) {
		return db.sql("""
				...
				""".formatted(JUDGE_COLS)).bind("id", disputeId).bind("round", round).map(JudgeRepository::mapJudge)
				.all();
	}
	...
	public Mono<Void> lockPanel(String disputeId, int round) {
		...
		return db.sql("SELECT pg_advisory_xact_lock(hashtextextended(:key, 0::bigint)) AS locked").bind("key", key)
	...
	public Flux<String> findPanelAccountIds(String disputeId, int round) {
		return db
				.sql("SELECT judge_account_id::text AS account_id FROM dispute_panel_assignment"
						+ " WHERE dispute_id = CAST(:id AS uuid) AND round = :round" + " ORDER BY judge_account_id")
				.bind("id", disputeId).bind("round", round).map(row -> row.get("account_id", String.class)).all();
	}

	/** 该审判官是否在该争议该轮面板上（投票前资格自查）。 */
	public Mono<Boolean> isPanelMember(String disputeId, int round, String judgeAccountId) {
		return db
				.sql("SELECT EXISTS(SELECT 1 FROM dispute_panel_assignment"
						+ " WHERE dispute_id = CAST(:id AS uuid) AND round = :round"
	...
	/**
	 * 记票（幂等）：UNIQUE(dispute_id, round, judge_account_id) 冲突 → empty（调用方据 findVote
	 * 返回既有票）。
	 */
	public Mono<JudgeVote> recordVote(String disputeId, int round, String judgeAccountId, String vote,
			String rationale) {
		var spec = db.sql("""
		...
		spec = (rationale == null || rationale.isBlank())
				? spec.bindNull("rationale", String.class)
				: spec.bind("rationale", rationale);
	...
	public Mono<JudgeVote> findVote(String disputeId, int round, String judgeAccountId) {
		return db.sql("SELECT dispute_id::text, round, judge_account_id::text, vote, rationale, voted_at"
				...
				.bind("d", disputeId).bind("round", round).bind("j", judgeAccountId).map(JudgeRepository::mapVote)
				.one();
	}

	/**
	 * 任务书 #31 / ADR-D15 D2：该轮**实际投出**（含弃权）的审判官账号——奖励对象。 早结论未投票者不在 dispute_vote
	 * 中，天然无奖励；客服终审轮无投票行，同样天然不发。
	 */
	public Flux<String> findVoterAccountIds(String disputeId, int round) {
		return db.sql("""
				...
				""").bind("d", disputeId).bind("round", round).map(r -> r.get(0, String.class)).all();
	}
	...
	public Flux<String> listVoteRationales(String disputeId, int round) {
		return db.sql("""
				...
				""").bind("d", disputeId).bind("round", round).map(r -> r.get("rationale", String.class)).all();
	}
	...
	public Mono<VoteTally> tallyVotes(String disputeId, int round) {
		return db.sql("""
				...
				.map(r -> new VoteTally(nvl(r.get("fm", Integer.class)), nvl(r.get("fr", Integer.class)),
						nvl(r.get("ab", Integer.class)), nvl(r.get("panel_size", Integer.class))))
				.one().defaultIfEmpty(new VoteTally(0, 0, 0, 0));
	}
	...
	private static Judge mapJudge(Readable row) {
		return new Judge(row.get("id", String.class), row.get("account_id", String.class),
				row.get("organization_id", String.class), row.get("eligibility_tier", Integer.class),
				row.get("active", Boolean.class), row.get("ops_admitted", Boolean.class),
				row.get("version", Long.class), toInstant(row.get("ops_admitted_at", OffsetDateTime.class)),
				row.get("ops_admitted_by", String.class), toInstant(row.get("created_at", OffsetDateTime.class)),
				toInstant(row.get("exam_passed_at", OffsetDateTime.class)), row.get("admission_level", String.class),
	...
	private static JudgeVote mapVote(Readable row) {
		return new JudgeVote(row.get("dispute_id", String.class), row.get("round", Integer.class),
				row.get("judge_account_id", String.class), row.get("vote", String.class),
				row.get("rationale", String.class), toInstant(row.get("voted_at", OffsetDateTime.class)));
	}
