...
public class RiskService {
    ...
    public Mono<Registration> register(RegisterSignalRequest request, Caller actor) {
        ...
        return transactions.transactional(repository.createSignal(request, evaluation).flatMap(created -> {
            if (!created.created()) return Mono.just(new Registration(created.signal(), null, false));
            if (!evaluation.opensCase()) return Mono.just(new Registration(created.signal(), null, true));
            return repository.createOrFindActiveCase(created.signal(), evaluation)
                    .flatMap(riskCase -> repository.attachSignal(riskCase.id(), created.signal().id())
    ...
    public Mono<RiskCase> act(String caseId, CaseActionRequest request, Caller actor) {
        ...
        return transactions.transactional(repository.transition(caseId, request.action(), actor.accountId(), request.note())
                .switchIfEmpty(Mono.error(new TrustException(409, "案件状态不允许该动作")))
                .flatMap(updated -> repository.appendAudit(caseId, request.action(), actor.accountId(), actor.role(), request.note())
                        .then(outbox.append(event("RiskCaseUpdated", updated, request.action())))
                        .thenReturn(updated)));
    }
    ...
    private static EventEnvelope event(String type, RiskCase riskCase, String detail) {
        ...
        return new EventEnvelope(type + ":" + riskCase.id() + ":" + detail, type, "risk_case", riskCase.id(),
                now.toEpochMilli(), now, UUID.randomUUID().toString(), Map.of(
                        "caseId", riskCase.id(), "subjectKind", riskCase.subjectKind(),
                        "subjectRef", riskCase.subjectRef(), "status", riskCase.status(), "detail", detail));
    }
}
