...
import com.grassland.trust.adjudication.HearingConditions;
...
/**
 * 七官审判 workflow 共享启动器：HTTP 手动启动 / court 通道开争议自动启动（任务书 #74 卡 B）/ cs_direct SLA
 * workflow（卡 A）/ 发回重审重启（卡 F）共用。
 *
 * <p>
 * 入参按争议当前状态折算（workflow 内不读 env/DB）：受理期（open|evidence）court 新案
 * evidencePhase=true； 发回重审重启 evidencePhase=false 且 startRound=新轮次（老 run 已
 * terminate，卡 F）。
 */
@Component
public class AdjudicationWorkflowStarter {
	...
	private Mono<String> startWithState(DisputeCase d) {
		...
		// 审查修复 02 / R06：质证计时锚定冻结的 evidence_deadline（空 → created_at+窗口 兼容锚点）——
		// 手动重试/工作流重启只等锚点剩余时间，不重新给整个 48h；已满足开庭条件（双方 done/到期）→ 0 秒直进开庭。
		long evidenceWindow = evidencePhase
				? HearingConditions.remainingSeconds(d, Instant.now(), props.evidenceWindowSecondsEffective())
				: 0;
		int startRound = Math.max(1, d.round());
		if (pending) {
			startRound = 1; // 受理期首启从第 1 轮开始
		}
		AdjudicationInput input = new AdjudicationInput(d.id(), props.voteWindowSecondsEffective(),
				props.appealWindowSecondsEffective(), props.maxRounds(), Math.max(0, props.csAwaitHours()) * 3600L,
				Math.max(1, props.csPollSeconds()), evidencePhase, evidenceWindow, startRound);
		return Mono.fromCallable(() -> {
			DisputeAdjudicationWorkflow stub = workflowClient.newWorkflowStub(DisputeAdjudicationWorkflow.class,
			...
			return workflowId;
		}).subscribeOn(Schedulers.boundedElastic()).onErrorResume(WorkflowExecutionAlreadyStarted.class,
				already -> Mono.just(workflowId));
	}
	...
	public Mono<String> startCsSla(String disputeId, long slaSeconds) {
		...
		return Mono.fromCallable(() -> {
			CsDirectSlaWorkflow stub = workflowClient.newWorkflowStub(CsDirectSlaWorkflow.class,
					WorkflowOptions.newBuilder().setWorkflowId(workflowId)
							.setTaskQueue(DisputeAdjudicationWorkflowImpl.TASK_QUEUE)
							.setWorkflowIdReusePolicy(
									WorkflowIdReusePolicy.WORKFLOW_ID_REUSE_POLICY_ALLOW_DUPLICATE_FAILED_ONLY)
	...
	/**
	 * 卡 F：发回重审重启。上一 run 多停在「等客服终审」轮询段（retrial 只对 appealed 案开放）， 必须先 terminate（固定
	 * workflowId 在 ALLOW_DUPLICATE_FAILED_ONLY 下 completed run 不允许重启）； terminate
	 * 不存在/已结束的 run 的异常吞掉。新 run evidencePhase=false、startRound=新轮次。
	 */
	public Mono<String> restartForRetrial(String disputeId) {
		return disputes.findById(disputeId).flatMap(d -> Mono.<Void>fromCallable(() -> {
			try {
			...
			return null;
		}).subscribeOn(Schedulers.boundedElastic()).onErrorResume(e -> Mono.empty()).then(startWithState(d)));
	}
}
