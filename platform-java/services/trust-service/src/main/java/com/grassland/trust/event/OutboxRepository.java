...
public class OutboxRepository {
    ...
    public Flux<OutboxRow> claimBatch(int limit, UUID claimToken, Duration leaseDuration) {
        ...
        return db.sql("""
                WITH candidates AS (
                    SELECT candidate.id
                    FROM trust_outbox AS candidate
                    WHERE published_at IS NULL
                      AND NOT EXISTS (
                          SELECT 1 FROM trust_outbox AS earlier
    ...
    public Mono<Long> pendingCount() {
        return db.sql("SELECT COUNT(*)::bigint AS pending_count FROM trust_outbox WHERE published_at IS NULL")
