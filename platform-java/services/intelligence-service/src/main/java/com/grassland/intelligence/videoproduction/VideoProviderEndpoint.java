 ...
 *
 * <p>控制面解析（platform_model_config + platform_provider_credential）后由
 * {@code VideoGenerationProviderResolver} 构造——vendor adapter 不再读全局
 * {@code VideoGenerationProperties} 的 env 型字段，改为按解析结果实例化的纯对象。
 * {@code apiKey} 是解密后的明文，只活在进程内调用链，绝不入日志/响应/落库。
 */
public record VideoProviderEndpoint(
        String baseUrl,
        String apiKey,
        String createPath,
        String pollPath,
    ...
    public static VideoProviderEndpoint minimax(String baseUrl, String apiKey, Duration timeout) {
        return new VideoProviderEndpoint(baseUrl, apiKey,
                "/v1/video_generation", "/v1/query/video_generation", "/v1/files/retrieve", timeout);
    }
    ...
    public static VideoProviderEndpoint seedance(String baseUrl, String apiKey, Duration timeout) {
        return new VideoProviderEndpoint(baseUrl, apiKey,
                "/api/v3/contents/generations/tasks", "/api/v3/contents/generations/tasks/{taskId}",
                "/api/v3/contents/generations/tasks/{taskId}", timeout);
    }
    ...
    public VideoProviderEndpoint withPathOverrides(String createPath, String pollPath, String retrievePath) {
        return new VideoProviderEndpoint(baseUrl, apiKey,
                overrideOrDefault(createPath, this.createPath),
                overrideOrDefault(pollPath, this.pollPath),
                overrideOrDefault(retrievePath, this.retrievePath),
                requestTimeout);
    }
