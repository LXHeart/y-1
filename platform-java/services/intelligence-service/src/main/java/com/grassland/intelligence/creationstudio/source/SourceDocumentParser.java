 ...
 *
 * <ul>
 * <li>统一 CRLF/CR 为 LF；plain-text 分支仅转义 Markdown 控制字符以保持字面呈现，
 * markdown 分支保留原标记。</li>
 * <li>CommonMark 启用 source spans（BLOCKS）+ GFM 表格扩展；顶层节点跨度对应
 * normalizedMarkdown 的 <b>code point</b> 区间 {@code [start,end)}（不是 UTF-16 索引）；
 * 嵌套列表仍属于一个顶层块。</li>
...
public final class SourceDocumentParser {
    ...
    public static ParsedSource parse(UUID documentId, String kind, String text) {
        ...
        for (Node child = document.getFirstChild(); child != null; child = child.getNext()) {
            ...
            blocks.add(new SourceDocument.Block(
                    blockId(documentId, blocks.size()),
                    blockKind,
                    blocks.size(),
                    startCp,
                    endCp,
    ...
    /**
     * plain-text 分支：转义 Markdown 控制字符保持字面呈现。转义只加反斜杠前缀，不删改任何字符；
     * 行中句点无需转义，但「行首 1~9 位数字 + 句点」会被解析为有序列表标记，需要转义句点。
     */
