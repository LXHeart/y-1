...
/**
 * 创作 style skill 仓储（任务书 #57）。{@code DatabaseClient} 裸 SQL +
 * {@code R2dbcBindings} 惯例 （照 {@code HomepageHotConfigRepository}）。
 *
 * <p>
 * 排序口径：category 升序（TITLE_FORMULA/GENRE/STYLE 枚举序）内按 {@code sort_order, code}。
 */
@Component
public class CreationStyleSkillRepository {
	...
	private static final String COLS = "id, category, code, name, description, prompt_content, "
			+ "enabled, sort_order, applicable_platforms, version, updated_by, updated_at";
	...
	public Mono<Long> count() {
		return db.sql("SELECT count(*) AS n FROM creation_style_skill").map((row, meta) -> row.get("n", Long.class))
				.one();
	}
	...
	public Flux<CreationStyleSkill> listEnabled(CreationStyleSkillCategory category) {
		String sql = "SELECT " + COLS + " FROM creation_style_skill WHERE enabled = true"
				+ (category == null ? "" : " AND category = :category") + " ORDER BY category, sort_order, code";
		var spec = db.sql(sql);
		if (category != null) {
	...
	public Mono<CreationStyleSkill> findById(UUID id) {
		return db.sql("SELECT " + COLS + " FROM creation_style_skill WHERE id = :id").bind("id", id)
	...
	public Mono<CreationStyleSkill> findByCode(CreationStyleSkillCategory category, String code) {
		return db.sql("SELECT " + COLS + " FROM creation_style_skill WHERE category = :category AND code = :code")
				.bind("category", category.key()).bind("code", code).map(CreationStyleSkillRepository::map).one();
	}

	/** 启动种子逐条插入；UNIQUE(category,code) 冲突静默跳过（ON CONFLICT DO NOTHING）。 */
	public Mono<Void> insertSeed(CreationStyleSkillCategory category, String code, String name, String description,
			String promptContent, int sortOrder, java.util.List<String> applicablePlatforms) {
		return db.sql("""
				INSERT INTO creation_style_skill(category, code, name, description, prompt_content, sort_order,
				                                 applicable_platforms)
				VALUES (:category, :code, :name, :description, :promptContent, :sortOrder, :platforms)
				...
				.bind("sortOrder", sortOrder).bind("platforms", CreationStyleSkill.joinPlatforms(applicablePlatforms))
				.then();
	}
	 ...
	 *            {@code null} = 未提交，保持原归属；空列表 = 显式改为全平台通用。
	 */
	public Mono<CreationStyleSkill> updateRow(UUID id, String name, String description, String promptContent,
			boolean enabled, java.util.List<String> applicablePlatforms, int expectedVersion, UUID updatedBy) {
		return db.sql("""
				...
				SET name = :name,
				    ...
				    enabled = :enabled,
				    applicable_platforms = COALESCE(:platforms, applicable_platforms),
				    version = version + 1,
				    updated_by = :updatedBy,
				    updated_at = now()
				WHERE id = :id AND version = :expectedVersion
				RETURNING """ + " " + COLS).bind("id", id).bind("name", name)
				.bind("description", description == null ? "" : description).bind("promptContent", promptContent)
				.bind("enabled", enabled)
				// null = 调用方未提交归属 → COALESCE 保持原值（旧治理台前端不传时不静默清空归属）
				.bind("platforms", nullable(
						applicablePlatforms == null ? null : CreationStyleSkill.joinPlatforms(applicablePlatforms),
						String.class))
				.bind("expectedVersion", expectedVersion).bind("updatedBy", nullable(updatedBy, java.util.UUID.class))
				.map(CreationStyleSkillRepository::map).one();
	}

	private static CreationStyleSkill map(Row row, RowMetadata meta) {
		return new CreationStyleSkill(row.get("id", UUID.class),
				CreationStyleSkillCategory.fromKey(row.get("category", String.class)), row.get("code", String.class),
				row.get("name", String.class), row.get("description", String.class),
				row.get("prompt_content", String.class), Boolean.TRUE.equals(row.get("enabled", Boolean.class)),
				row.get("sort_order", Integer.class),
				CreationStyleSkill.parsePlatforms(row.get("applicable_platforms", String.class)),
