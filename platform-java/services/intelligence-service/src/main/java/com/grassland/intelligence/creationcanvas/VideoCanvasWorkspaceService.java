 ...
 * <li>不带 draftId → 存量引用精确单一匹配则采纳；多个候选 409 要求从最近项目进入； 无匹配则从可信分镜行创建最小视频草稿（不复制
 * base64、不构造浏览器身份）；
...
public class VideoCanvasWorkspaceService {
	...
	private final CanvasVariantRepairService repairs;

	public VideoCanvasWorkspaceService(VideoStoryboardRepository storyboards, VideoCanvasWorkspaceRepository bindings,
			CreationDraftRepository drafts, CreationDraftService draftService, TransactionalOperator transactions,
			CanvasProjectAccess projects, CanvasVariantRepairService repairs) {
	...
	public Mono<Map<String, Object>> bind(Caller caller, UUID storyboardId, BindingRequest request) {
		...
		return storyboards.findById(storyboardId, caller.accountId())
				.switchIfEmpty(Mono.error(new IntelligenceException(404, "分镜不存在"))).flatMap(storyboard -> bindings
						.findByAccountAndOperation(caller.accountId(), operationId).flatMap(existing -> {
							if (!requestHash.equals(existing.requestHash()))
								return Mono.error(new IntelligenceException(409, "CANVAS_OPERATION_CONFLICT",
										"操作参数已变化，请使用原请求重试"));
							return confirmReplay(existing, storyboard, requestedDraftId);
						})
				...
				.flatMap(binding -> buildResult(caller, binding, storyboardId)).as(transactions::transactional);
	}
	...
	private Mono<VideoCanvasWorkspaceRepository.WorkspaceBinding> confirmReplay(
		...
		if (!sameStoryboard) {
			return Mono.error(
					new IntelligenceException(409, "WORKSPACE_OPERATION_REUSED", "该 operationId 已绑定到其他分镜，请刷新后重试"));
		}
		if (!draftMatches) {
			return Mono.error(new IntelligenceException(409, "WORKSPACE_BINDING_CONFLICT", "该分镜已关联其他草稿，请从最近项目选择草稿进入"));
		}
	...
	private Mono<VideoCanvasWorkspaceRepository.WorkspaceBinding> bindFresh(Caller caller, VideoStoryboard storyboard,
		...
		return bindings
				...
				.switchIfEmpty(Mono.defer(
						() -> resolveDraft(caller, storyboard, operationId, requestedDraftId, expectedDraftVersion)
								.flatMap(draftId -> projects
										.lockForBinding(caller.accountId(),
												new CanvasProjectAccess.ProjectKey(draftId, storyboard.id()))
										.flatMap(project -> {
											projects.checkWritable(project, CanvasProjectAccess.WriteKind.CANVAS);
											if (requestedDraftId != null
													&& project.draft().version() != expectedDraftVersion)
												return Mono.error(new IntelligenceException(409,
														"DRAFT_VERSION_CONFLICT", "草稿已变化，请刷新后重试"));
											return draftService
													.validateVideoSource(project.draft(), project.storyboard())
													.then(insertBinding(project.storyboard(), draftId,
															caller.accountId(), operationId, requestHash));
										}))));
	}

	private Mono<UUID> resolveDraft(Caller caller, VideoStoryboard storyboard, UUID operationId, UUID requestedDraftId,
			Integer expectedDraftVersion) {
		if (requestedDraftId != null) {
			return draftService.loadOwned(requestedDraftId.toString(), caller.accountId()).flatMap(draft -> {
				...
				if (draft.version() != expectedDraftVersion) {
					return Mono.error(new IntelligenceException(409, "DRAFT_VERSION_CONFLICT", "草稿已被其他设备修改，请刷新后重试"));
				}
		...
		return bindings.findCandidateDraftIds(caller.accountId(), storyboard.id()).collectList().flatMap(ids -> {
			if (ids.size() > 1) {
				return Mono.error(
						new IntelligenceException(409, "WORKSPACE_BINDING_AMBIGUOUS", "该分镜匹配到多个草稿，请从最近项目选择草稿进入画布"));
			}
			if (ids.size() == 1) {
	...
	/**
	 * 平台兜底（仅当分镜行自身带 platform 时透传，可信来源；request_payload 是 JSON 文本）。 无 platform
	 * 是合法常态——Optional 包装防 fromSupplier 对 null 收敛成空 Mono （C100-08 e2e 实测：无 platform
	 * 的旧深链绑定会静默 200 空体、零写入）。
	 */
	private Mono<UUID> createMinimalDraft(Caller caller, VideoStoryboard storyboard, UUID operationId) {
		return Mono.fromSupplier(() -> java.util.Optional.ofNullable(parsePlatform(storyboard.requestPayload())))
				.flatMap(platform -> draftService.createMinimalVideoDraft(caller, storyboard.id(),
						storyboard.organizationId(), storyboard.contextSnapshotId(), platform.orElse(null),
						storyboard.id()))
	...
	private static Map<String, Object> result(CreationDraftView view, VideoStoryboard storyboard) {
		...
		if (view.draft().workspace() instanceof Map<?, ?> workspace
				&& workspace.get("inputs") instanceof Map<?, ?> inputs && inputs.get("video") instanceof Map<?, ?> video
				&& video.get("productionTaskId") instanceof String id && !id.isBlank()) {
			taskRef = id;
		}
		...
		return bindings.insert(binding)
				.flatMap(inserted -> inserted > 0
						...
						: bindings.findByStoryboard(storyboard.id())
								.flatMap(existing -> existing.draftId().equals(draftId)
										? Mono.just(existing)
	...
	private Mono<Map<String, Object>> buildResult(Caller caller,
		...
		return draftService.loadOwned(binding.draftId().toString(), caller.accountId()).onErrorResume(
				com.grassland.intelligence.security.IntelligenceException.class,
				error -> error.status() == 404
						? Mono.error(
								new IntelligenceException(409, "WORKSPACE_BINDING_DRAFT_GONE", "关联草稿已删除，请从最近项目重新进入"))
						: Mono.error(error))
				.flatMap(draft -> repairs.repairIfProvable(caller.accountId(), storyboardId)
						.then(projects.lockProjects(caller.accountId(),
								java.util.List.of(new CanvasProjectAccess.ProjectKey(draft.id(), storyboardId))))
						.map(locked -> {
							var project = locked.getFirst();
							var response = result(CreationDraftView.of(project.draft()), project.storyboard());
							response.put("sourceIntegrity", project.trustedSource() ? "verified" : "unverified");
							return response;
						}));
	}
