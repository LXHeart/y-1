...
public class TextProposalRepository {
	...
	public Mono<Boolean> insertPlaceholder(ProposalRow row) {
		var spec = db.sql("""
				...
				.bind("inputSnapshotJson", orEmptyObject(row.inputSnapshotJson())).bind("expiresAt", row.expiresAt());
		spec = row.sourceDocumentId() == null
				? spec.bindNull("sourceDocumentId", java.util.UUID.class)
				: spec.bind("sourceDocumentId", row.sourceDocumentId());
		spec = row.promptCiphertext() == null
				? spec.bindNull("promptCiphertext", String.class)
				: spec.bind("promptCiphertext", row.promptCiphertext());
		spec = row.promptHash() == null
				? spec.bindNull("promptHash", String.class)
	...
	public Mono<Integer> attachRun(UUID id, UUID runId) {
		return db
				.sql("UPDATE creation_text_proposal SET run_id = CAST(:runId AS uuid) "
	...
	public Mono<ProposalRow> findByOwnerAndRequestId(String ownerAccountId, String requestId) {
		return db.sql(SELECT + " WHERE owner_account_id = :owner AND request_id = :requestId")
				.bind("owner", ownerAccountId).bind("requestId", requestId).map(TextProposalRepository::map).one();
	}
	...
	public static ProposalRow map(Readable row) {
		...
		return new ProposalRow(uuid(row.get("id", String.class)), row.get("owner_account_id", String.class),
				...
				row.get("request_hash", String.class), row.get("action", String.class), row.get("status", String.class),
				row.get("base_draft_version", Integer.class), row.get("base_content_hash", String.class),
				uuid(row.get("source_document_id", String.class)), row.get("selected_blocks_json", String.class),
				row.get("input_snapshot_json", String.class), row.get("prompt_ciphertext", String.class),
				row.get("prompt_hash", String.class), result, uuid(row.get("run_id", String.class)),
				row.get("error_code", String.class), row.get("created_at", OffsetDateTime.class),
				row.get("expires_at", OffsetDateTime.class), row.get("applied_draft_version", Integer.class),
				row.get("apply_request_id", String.class), applyResult);
	}
