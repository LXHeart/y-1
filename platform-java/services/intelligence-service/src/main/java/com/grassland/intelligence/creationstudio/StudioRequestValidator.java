 ...
 *
 * <p>
 * Spring Boot 4 的 HTTP codec 走 Jackson 3（{@code tools.jackson}），控制器以
 * {@code Map<String, Object>} 接请求（house style，见 VideoRecreationController）；本工具在
 * Map 上检查<b>真实 JSON 类型</b>再构造 DTO：禁止把浮点数截成版本整数、把数字转成文本、把字符串 "true"
 * 转成布尔；未知字段、未声明 nullable 的显式 null 均为 400。解析错误只返回字段名和原因， 不回显原值。
 */
