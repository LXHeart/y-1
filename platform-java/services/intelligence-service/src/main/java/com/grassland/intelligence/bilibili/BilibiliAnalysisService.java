...
/**
 * Bilibili 视频分析编排（草场 Slice 13 Stage 5）。移植 legacy
 * {@code bilibili-video-analysis.service.ts} 的
 * {@code analyzeBilibiliVideoByProxyUrl}（content 提取）与
 * {@code analyzeBilibiliVideoForRecreation}（复刻场景）。
 *
 * <p>
 * <b>Java 分析路径</b>：
 * <ul>
 ...
 *
 * <p>
 * provider 解析经执行环控制面（个人 BYOK &gt; 组织 BYOK &gt; 平台回落，与任务模式同机器）。
 */
@Service
public class BilibiliAnalysisService {
	...
	public BilibiliAnalysisService(BilibiliProxyToken tokenCodec, Environment environment, PlatformMediaService media,
	...
	public Mono<BilibiliAnalysisOutcome> analyze(String proxyVideoUrl, String accountId, ServerWebExchange exchange) {
	...
	public Mono<BilibiliAnalysisOutcome> analyzeTask(String proxyVideoUrl, String accountId,
		...
		return media.prepareBilibili(target)
				.flatMap(sourceId -> (duration > maxSingleSegmentSeconds
						? media.createClips(sourceId, duration, 30)
						: Mono.just(List.of(sourceId)))
						.flatMap(ids -> taskAnalysis.analyzeSegments("bilibili", ids, accountId, task, exchange)
								.map(BilibiliAnalysisOutcome::new).doFinally(signal -> ids.forEach(media::remove)))
						.doFinally(signal -> media.remove(sourceId)));
	}

	/**
	 * 复刻分镜场景分析（{@code POST /api/bilibili/analyze-video} body
	 * {@code mode:"recreation"}）。 与 {@link #analyze} 同路由决策；用
	 * {@link VideoAnalysisPrompts#recreation()} + 复刻归一； legacy
	 * {@code analyzeBilibiliVideoForRecreation} 在切流期无路由，本方法即其正式暴露。
	 */
	public Mono<BilibiliAnalysisOutcome> analyzeForRecreation(String proxyVideoUrl, String accountId,
		...
		return frozenText
				.executeIndependent(exchange, List.of(ChatMessage.user(parts)), ANALYSIS_MAX_TOKENS,
						...
						completion -> new BilibiliAnalysisOutcome(VideoRecreationResultNormalizer
	...
	public Mono<BilibiliAnalysisOutcome> analyzeTaskForRecreation(String proxyVideoUrl, String accountId,
			VideoRecreationTaskRequest task, ServerWebExchange exchange) {
		String token = extractToken(proxyVideoUrl);
		BilibiliMediaTarget target = tokenCodec.parse(token);
		long duration = assertAnalysisDuration(target.durationSeconds());
	...
	private Mono<BilibiliAnalysisOutcome> analyzeTarget(String token, BilibiliMediaTarget target, long duration,
		...
		if (target instanceof BilibiliMediaTarget.Progressive && duration <= maxSingleSegmentSeconds) {
			...
			return frozenText
					.executeIndependent(exchange, List.of(ChatMessage.user(parts)), ANALYSIS_MAX_TOKENS,
							CreditFeature.VIDEO_ANALYSIS, timeout,
							completion -> new BilibiliAnalysisOutcome(VideoAnalysisResultNormalizer
									.normalize(completion.content(), completion.providerRunId())))
					.map(FrozenTextExecutionService.Traced::value);
		}
		return media.prepareBilibili(target)
				.flatMap(sourceId -> (duration > maxSingleSegmentSeconds
						? media.createClips(sourceId, duration, 30)
						: Mono.just(List.of(sourceId)))
						.flatMap(ids -> segmented.analyze("bilibili", ids, timeout, exchange)
								.map(BilibiliAnalysisOutcome::new).doFinally(signal -> ids.forEach(media::remove)))
						.doFinally(signal -> media.remove(sourceId)));
	}
	...
	private String extractToken(String proxyVideoUrl) {
		...
		// token 为 base64url + '.'，URL 安全字符，无需 decode（与 BilibiliExtractController
		// 拼接契约一致）。
