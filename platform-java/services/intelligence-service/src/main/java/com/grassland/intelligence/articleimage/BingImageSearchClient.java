...
public class BingImageSearchClient {
	...
	public BingImageSearchClient(
		...
		this(endpoint, Duration.ofMillis(timeoutMs), () -> ThreadLocalRandom.current().nextInt(1, 31), dnsPinning);
	}
	...
	BingImageSearchClient(String endpoint, Duration timeout, IntSupplier offsetSupplier,
		...
		// GL-P3-AI-001 尾巴（覆盖扩展）：固定搜索端点钉扎；保留 followRedirect(true)（Bing 搜索
		// 会 302 到区域端点，工厂口径的 false 会破坏真实搜索）
		HttpClient http = HttpClient.create().option(ChannelOption.CONNECT_TIMEOUT_MILLIS, 5_000)
				.responseTimeout(timeout).followRedirect(true)
				.resolver(com.grassland.intelligence.ai.PinnedOutboundClients.resolverFor(endpoint, dnsPinning));
		this.webClient = WebClient.builder().clientConnector(new ReactorClientHttpConnector(http))
	...
	public Mono<List<ImageSearchResult>> search(String keywords, int count) {
		String uri = UriComponentsBuilder.fromUriString(endpoint).queryParam("q", keywords)
				.queryParam("first", offsetSupplier.getAsInt()).build().encode().toUriString();
		return webClient.get().uri(uri).header("User-Agent", "Mozilla/5.0 (compatible; ArticleBot/1.0)")
				.header("Accept", "text/html,application/json;q=0.9,*/*;q=0.8").exchangeToMono(response -> {
					if (!response.statusCode().is2xxSuccessful()) {
						return Mono.error(new IntelligenceException(
								response.statusCode().is5xxServerError() ? 502 : 400, "搜图失败，请稍后重试"));
					}
				...
				}).timeout(timeout).onErrorMap(error -> {
	...
	private List<ImageSearchResult> parseJson(String body, int count) {
		try {
			...
			for (JsonNode item : root) {
				add(results, seen, result(text(item, "url"), text(item, "thumbnailUrl"), text(item, "sourceUrl"),
	...
	private List<ImageSearchResult> parseMetadata(Document document, int count) {
		...
		for (Element element : document.select(".iusc[m]")) {
			try {
				...
				add(results, seen, result(text(item, "murl"), text(item, "turl"), text(item, "purl"),
						firstNonBlank(text(item, "desc"), text(item, "title")), integer(item, "w"), integer(item, "h")),
						count);
	...
	private List<ImageSearchResult> parseFallback(Document document, int count) {
		...
		for (Element container : document.select(".iusc, .imgpt, .dgControl, .img_cont, .mimg")) {
			...
			add(results, seen, result(firstNonBlank(image.attr("data-src"), image.attr("src")),
					firstNonBlank(image.attr("src"), image.attr("data-src")), link == null ? null : link.attr("href"),
					firstNonBlank(image.attr("alt"), container.attr("aria-label")), parseInt(image.attr("width")),
					parseInt(image.attr("height"))), count);
			if (results.size() >= count) {
				break;
			}
		}
	...
	private static void add(List<ImageSearchResult> results, Set<String> seen, ImageSearchResult candidate, int count) {
	...
	private static ImageSearchResult result(String rawUrl, String rawThumbnail, String rawSource, String description,
