...
public class AiRunRepository {
	...
	private static final String SELECT_COLS = "id::text, organization_id::text, account_id::text, capability, provider, model, run_type, "
			...
			+ "credits_cents_policy_version, byok_organization_id::text, credential_version";
	...
	public Mono<UUID> create(AiRun run) {
		return db.sql("""
				...
				RETURNING id::text
				""").bind("orgId", nullable(run.organizationId(), String.class)).bind("accountId", run.accountId())
				.bind("capability", run.capability()).bind("provider", run.provider())
				.bind("model", nullable(run.model(), String.class)).bind("runType", run.runType())
				.bind("budgetCents", run.budgetCents()).bind("operationId", run.operationId().toString())
				.bind("platformModelVersion", nullable(run.platformModelVersion(), Integer.class))
				.bind("fallbackAuthorized", run.fallbackAuthorized())
				.bind("contextSnapshotId",
						nullable(run.contextSnapshotId() == null ? null : run.contextSnapshotId().toString(),
								String.class))
				...
				.map((r, meta) -> r.get("id", String.class)).one().map(UUID::fromString);
	}
	...
	public Mono<Boolean> complete(UUID id, int actualCents, Integer inputTokens, Integer outputTokens,
		...
		return db.sql("""
				...
				RETURNING id::text
				""").bind("id", id.toString()).bind("actualCents", actualCents)
				.bind("inputTokens", nullable(inputTokens, Integer.class))
				.bind("outputTokens", nullable(outputTokens, Integer.class)).bind("imagesGenerated", imagesGenerated)
				.bind("videoSeconds", videoSeconds).map((r, meta) -> r.get("id", String.class)).one().hasElement();
	}
	...
	public Mono<Boolean> fail(UUID id, String reason) {
		return db.sql("""
				...
				WHERE id = CAST(:id AS uuid) AND status = 'running'
				RETURNING id::text
				""").bind("id", id.toString()).bind("reason", reason).map((r, meta) -> r.get("id", String.class)).one()
				.hasElement();
	}
	...
	public Mono<Boolean> cancel(UUID id) {
		return db.sql("""
				...
				WHERE id = CAST(:id AS uuid) AND status = 'running'
				RETURNING id::text
				""").bind("id", id.toString()).map((r, meta) -> r.get("id", String.class)).one().hasElement();
	}
	...
	public Mono<Boolean> setRefundOperation(UUID id, UUID refundOpId) {
		return db.sql("""
				...
				RETURNING id::text
				""").bind("id", id.toString()).bind("refundOpId", refundOpId.toString())
				.map((r, meta) -> r.get("id", String.class)).one().hasElement();
	}
	...
	public Mono<AiRun> findByOperationId(UUID operationId) {
		return db.sql("SELECT " + SELECT_COLS + " FROM ai_run WHERE operation_id = CAST(:id AS uuid)")
				.bind("id", operationId.toString()).map(AiRunRepository::map).one();
	}

	/**
	 * 任务书 #101 C101-08：owner + operation 安全回读（恢复结算入口用）——执行操作 ID 难猜
	 * 不作为授权（§5.6），owner 不符返回 empty。
	 */
	...
	public Mono<AiRun> findById(UUID id) {
		return db.sql("SELECT " + SELECT_COLS + " FROM ai_run WHERE id = CAST(:id AS uuid)").bind("id", id.toString())
				.map(AiRunRepository::map).one();
	}
	...
	public Flux<AiRun> findByAccount(String accountId, int limit) {
		return db
				...
				.bind("accountId", accountId).bind("limit", limit).map(AiRunRepository::map).all();
	}

	private static AiRun map(Row row, RowMetadata meta) {
		return new AiRun(uuidFromString(row.get("id", String.class)), row.get("organization_id", String.class),
				row.get("account_id", String.class), row.get("capability", String.class),
				row.get("provider", String.class), row.get("model", String.class), row.get("run_type", String.class),
				row.get("input_tokens", Integer.class), row.get("output_tokens", Integer.class),
				row.get("images_generated", Integer.class), row.get("video_seconds", Integer.class),
				row.get("budget_cents", Integer.class), row.get("actual_cents", Integer.class),
				row.get("status", String.class), row.get("failure_reason", String.class),
				toInstant(row.get("started_at", OffsetDateTime.class)),
				toInstant(row.get("completed_at", OffsetDateTime.class)), row.get("price_table_version", String.class),
				...
				row.get("platform_model_version", Integer.class), row.get("fallback_authorized", Boolean.class),
				uuidFromString(row.get("context_snapshot_id", String.class)),
				row.get("credits_cents_policy_version", String.class), row.get("byok_organization_id", String.class),
