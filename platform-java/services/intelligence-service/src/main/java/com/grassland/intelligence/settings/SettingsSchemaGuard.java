 ...
 *
 * <p>
 * legacy 时代 settings_json 是无 schema 自由 jsonb：{@code mergeFeature} 会照抄任意未知键，
 ...
 *
 * <p>
 * analysis 类型白名单（任务书 #88 起）：仅
 * {@code integrations.feishu.{appId,appSecret,folderToken}}——
 * {@code features.*} 已退役（normalize 直接丢弃、validate 不再校验；请求携带 features 由
 * {@code AnalysisSettingsService.update} 整键忽略，存量存储 preserve-on-write 另见其
 * javadoc）。
 *
 * <p>
 * 读路径也过 {@link #normalize}：存量 legacy 脏数据（未知键/越界值）在响应里永远是规整后的形态。
...
final class SettingsSchemaGuard {
	...
	private static Map<String, Object> normalizeAnalysis(Map<String, Object> settings) {
		...
		if (integrationsObj instanceof Map<?, ?> integrations
			...
			Map<String, Object> normalizedFeishu = keepKnownKeys(asMap(feishu),
					Set.of("appId", "appSecret", "folderToken"));
