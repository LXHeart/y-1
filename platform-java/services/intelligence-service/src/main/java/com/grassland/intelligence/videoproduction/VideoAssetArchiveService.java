...
/** Copies provider output into private object storage before exposing any result to clients. */
@Service
public class VideoAssetArchiveService {
    ...
    public Mono<String> archive(VideoGenerationJob job, String providerUrl) {
        ...
        if (properties.getMode().equalsIgnoreCase("sandbox")) {
            return store(job, storage, mediaId, key,
                    new byte[] {0, 0, 0, 8, 'f', 't', 'y', 'p'}, "video/mp4");
        }
        ...
        return client.get().uri(providerUrl).exchangeToMono(response -> {
            ...
            if (declared > MAX_BYTES) return Mono.error(new IllegalStateException("视频结果大小超出归档限制"));
    ...
    private Mono<String> store(VideoGenerationJob job, ObjectStorageAdapter storage, UUID mediaId,
                               String key, byte[] bytes, String mime) {
        return Mono.fromRunnable(() -> storage.putObject(key, bytes, mime))
                    .subscribeOn(Schedulers.boundedElastic())
                    .then(transactions.transactional(mediaRefs.insert(new MediaReference(
                            mediaId, job.accountId(), job.organizationId(),
                            MediaPurpose.VIDEO_ASSET.db(), "video_generation_job", job.id().toString(),
                            key, mime, bytes.length, VideoArchiveChecksums.sha256(bytes),
