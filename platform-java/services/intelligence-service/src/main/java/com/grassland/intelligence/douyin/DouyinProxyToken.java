...
public final class DouyinProxyToken {
    ...
    public DouyinProxyToken(DouyinProxyTokenProperties properties) {
    ...
    public String create(DouyinMediaTarget target) {
        ensureSecret();
        ...
        payload.put("playableVideoUrl", target.playableVideoUrl());

        String encoded = encodeJson(payload);
        return encoded + "." + sign(encoded);
    }

    public DouyinMediaTarget parse(String token) {
        ensureSecret();
        if (token == null) {
        ...
        requireHttps(url);
        return DouyinMediaTarget.progressive(url, headers, filename, duration);
    }

    private void ensureSecret() {
        if (secret == null || secret.length < MIN_SECRET_LENGTH) {
            throw new IntelligenceException(500, "未配置抖音代理密钥（DOUYIN_PROXY_TOKEN_SECRET 须 ≥32 字符）");
        }
    }
