 ...
 * base URL 硬编码 {@code https://open.feishu.cn}（与 legacy 一致，不可配）；超时 {@code feishu.export.api-timeout-ms}（默认 30s）。
 * 错误按 legacy {@code validateFeishuResponse} 映射：上传图片遇 code 99991672 → 固定权限消息；其余 code!=0 →「飞书{op}失败」。
 * 飞书以 body 中 {@code code} 判定成败（非 HTTP 状态），与 legacy 一致——故不检查 HTTP 状态，只解析 body code。
 */
@Component
public class FeishuClient {
    ...
    static final String IMAGE_UPLOAD_PERMISSION_MESSAGE =
            "飞书图片上传失败：当前飞书应用缺少图片上传权限，请在飞书开放平台为该应用开通 docs:document.media:upload 权限后重试";
    ...
    public Mono<String> tenantAccessToken(String appId, String appSecret) {
        return client.post().uri("/open-apis/auth/v3/tenant_access_token/internal")
                ...
                .map(body -> parseToken(body, "获取访问凭证"))
                .onErrorMap(FeishuClient::wrap);
    }
    ...
    public Mono<String> createDocument(String token, String title, String folderToken) {
        ...
        return postJson("/open-apis/docx/v1/documents", token, body, "创建文档")
                .map(node -> nonEmpty(node.path("data").path("document").path("document_id").asText(), "创建文档", "document_id"));
    }

    /** 返回首个子块的 block_id（图片占位块用）。 */
    public Mono<String> appendBlocks(String token, String documentId, List<Map<String, Object>> blocks) {
        return postJson("/open-apis/docx/v1/documents/" + documentId + "/blocks/" + documentId + "/children",
                token, Map.of("children", blocks), "写入文档内容")
                .map(node -> nonEmpty(node.path("data").path("children").path(0).path("block_id").asText(), "写入文档内容", "block_id"));
    }
    ...
    public Mono<String> uploadMedia(String token, FeishuUploadImage image, String parentBlockId) {
        ...
        return client.post().uri("/open-apis/drive/v1/medias/upload_all")
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.MULTIPART_FORM_DATA)
                .body(BodyInserters.fromMultipartData(form))
                .exchangeToMono(resp -> resp.bodyToMono(String.class).defaultIfEmpty(""))
                .map(body -> parseUpload(body, "上传图片"))
                .onErrorMap(FeishuClient::wrap);
    }

    public Mono<Void> replaceImageBlock(String token, String documentId, String blockId, String fileToken) {
        return patchJson("/open-apis/docx/v1/documents/" + documentId + "/blocks/" + blockId,
                token, Map.of("replace_image", Map.of("token", fileToken)), "设置图片内容")
                .then();
    }
    ...
    private Mono<JsonNode> postJson(String uri, String token, Object body, String operation) {
        return client.post().uri(uri)
                ...
                .bodyValue(body)
                .exchangeToMono(resp -> resp.bodyToMono(String.class).defaultIfEmpty(""))
                .map(raw -> validate(raw, operation))
                .onErrorMap(FeishuClient::wrap);
    }
