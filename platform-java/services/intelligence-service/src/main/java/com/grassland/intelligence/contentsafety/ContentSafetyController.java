 ...
 *
 * <p>
 * {@code POST /api/content-safety/check}（需登录；请求体 ≤50KB；platform 可选做 overlay 预留，
 * v1 不改变结果）。findings 是 advisory（警告 + 建议），词库服务端独占（D9）——响应只回 findings 与版本，不回词表。
 */
@RestController
public class ContentSafetyController {
	...
	public Mono<Map<String, Object>> check(@RequestBody CheckRequest body, ServerWebExchange exchange) {
		return callers.requireUser(exchange.getRequest()).flatMap(caller -> Mono.defer(() -> {
			if (body.text() == null || body.text().isBlank()) {
				return Mono.just(ok(SafetyReport.emptyShallow()));
			}
			if (body.text().length() > MAX_TEXT_CHARS) {
				return Mono.error(new IntelligenceException(400, "文本超长（上限 " + MAX_TEXT_CHARS + " 字符）"));
			}
			return safety.check(exchange, body.text(), body.platform(), body.industry(),
					new OriginalityChecker.Context(caller.accountId(), body.taskId(), body.applicationId(),
	...
	private static Map<String, Object> ok(SafetyReport report) {
		...
		safetyBody.put("findings", report.findings().stream().map(f -> {
		...
		// 任务书 #78 卡 B：BYOK 绕过标识（与 SSE 帧 reportBody 同构；仅绕过时才带，缺省即 false）
		if (report.deepCheckSkipped()) {
			safetyBody.put("deepCheckSkipped", true);
		}
		return Map.of("success", true, "data", Map.of("safety", safetyBody));
	}

	/** 请求体：text 必填；platform 可选（overlay 预留，v1 不参与判定）。 */
	record CheckRequest(String text, String platform, String industry, String taskId, String applicationId,
			String contentForm) {
	}
}
