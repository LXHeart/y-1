 ...
 *
 * <p>
 * 独立画布文档：draft 唯一、revision CAS（expectedRevision=0 仅创建）、document ≤256KiB / 200
 * 节点 / 500 边；storyboardId 必须与草稿存在 API-07 绑定；归档草稿只读。画布写不提升
...
public class CreationCanvasController {
	...
	public CreationCanvasController(IntelligenceCallerResolver callers, CreationDraftRepository drafts,
			VideoCanvasWorkspaceRepository bindings, CreationCanvasRepository canvas, CanvasProjectAccess projects,
			TransactionalOperator transactions, CanvasReferenceAccess references) {
	...
	public Mono<ResponseEntity<Map<String, Object>>> getCanvas(@PathVariable String id, ServerWebExchange exchange) {
		UUID draftId = parseId(id);
		return callers.resolve(exchange.getRequest()).flatMap(caller -> requireOwnedDraft(caller.accountId(), draftId))
				.flatMap(draft -> canvas.findByDraftId(draftId).map(row -> ResponseEntity.ok(envelope(view(row))))
	...
	public Mono<ResponseEntity<Map<String, Object>>> putCanvas(@PathVariable String id,
			@RequestBody Map<String, Object> raw, ServerWebExchange exchange) {
		UUID draftId = parseId(id);
		SaveCanvasRequest body = parseRequest(raw);
		...
		if (body.expectedRevision() < 0 || body.expectedRevision() > 9_007_199_254_740_991L) {
			return Mono.error(new IntelligenceException(400, "CANVAS_INVALID_INPUT", "expectedRevision 必须为 ≥0 的安全整数"));
		}
		String documentJson;
		try {
		...
		return callers.resolve(exchange.getRequest())
				.flatMap(caller -> requireOwnedDraft(caller.accountId(), draftId).flatMap(draft -> {
					...
					return requireBoundStoryboard(draftId, storyboardId)
							.then(projects.lockProjects(caller.accountId(), List
									.of(new CanvasProjectAccess.ProjectKey(draftId, UUID.fromString(storyboardId)))))
							.flatMap(locked -> {
								var project = locked.getFirst();
								projects.checkWritable(project, CanvasProjectAccess.WriteKind.CANVAS);
								return references.validateChanges(caller.accountId(),
										project.canvas() == null ? null : project.canvas().documentJson(), validated);
							}).then(crossProjectCheck).flatMap(crossProject -> {
								if (crossProject) {
									throw new IntelligenceException(400, "CANVAS_INVALID_INPUT", "canonical 引用指向其他项目");
								}
								return save(draftId, caller.accountId(), body.expectedRevision(), validated);
							});
				}).as(transactions::transactional)).map(row -> ResponseEntity.ok(envelope(view(row))));
	}

	@SuppressWarnings("unchecked")
	private static SaveCanvasRequest parseRequest(Map<String, Object> raw) {
		if (raw == null || !java.util.Set.of("expectedRevision", "document").containsAll(raw.keySet())
				|| !(raw.get("expectedRevision") instanceof Integer || raw.get("expectedRevision") instanceof Long)
				|| !(raw.get("document") instanceof Map<?, ?>))
			throw new IntelligenceException(400, "CANVAS_INVALID_INPUT", "必须提供整数 expectedRevision 和 document，不能增加未知字段");
		return new SaveCanvasRequest(((Number) raw.get("expectedRevision")).longValue(),
				(Map<String, Object>) raw.get("document"));
	}

	private Mono<CreationCanvasRepository.CanvasRow> save(UUID draftId, String accountId, long expectedRevision,
			String documentJson) {
		if (expectedRevision == 0) {
			return canvas.insert(draftId, accountId, documentJson).switchIfEmpty(Mono.defer(
					() -> canvas.findByDraftId(draftId).flatMap(existing -> Mono.error(new IntelligenceException(409,
							"CANVAS_VERSION_CONFLICT", "画布已存在（revision=" + existing.revision() + "），不能以 0 重复创建")))));
		}
		return canvas.findByDraftId(draftId).flatMap(existing -> {
			...
			return canvas.casUpdate(draftId, expectedRevision, documentJson);
				...
				new IntelligenceException(409, "CANVAS_VERSION_CONFLICT", "画布不存在：expectedRevision>0 只能更新已有文档（首建用 0）")));
	}

	/** 草稿可用性闸（§6.3）：不存在/已删/不归本人 404 同口径（不泄露归属）。 */
	private Mono<com.grassland.intelligence.creationassistant.CreationDraft> requireOwnedDraft(String accountId,
			UUID draftId) {
		return drafts.findById(draftId)
				.filter(draft -> draft.ownerAccountId().equals(accountId) && draft.deletedAt() == null)
				.switchIfEmpty(Mono.error(new IntelligenceException(404, "CANVAS_RESOURCE_NOT_FOUND", "草稿不存在")));
	}
	...
	private Mono<Void> requireBoundStoryboard(UUID draftId, String storyboardId) {
		return bindings.findByStoryboard(UUID.fromString(storyboardId))
				...
				.switchIfEmpty(Mono.error(
						new IntelligenceException(400, "CANVAS_INVALID_INPUT", "document.storyboardId 与草稿无绑定关系")))
				.then();
	}
	...
	private static Map<String, Object> view(CreationCanvasRepository.CanvasRow row) {
		...
		return Map.of("id", row.id().toString(), "draftId", row.draftId().toString(), "revision", row.revision(),
				"updatedAt", row.updatedAt().toString(), "document", document);
	}
