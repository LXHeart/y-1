 ...
 *
 * <p>
 * 写路径两条：{@link #insert}（首建，draft_id 唯一冲突返回 0 行）与 {@link #casUpdate} （revision
 * 匹配才 +1 落库）。画布写不触碰 creation_draft（正文/版本不受布局影响，§7.3）。
 */
@Component
public class CreationCanvasRepository {
	...
	public Mono<CanvasRow> insert(UUID draftId, String accountId, int schemaVersion, String documentJson) {
		...
		return db
				.sql("INSERT INTO creation_canvas_document(id, draft_id, account_id, schema_version, revision, "
						+ "document) VALUES (CAST(:id AS uuid), CAST(:draft AS uuid), :account, "
						+ ":schemaVersion, 1, CAST(:document AS jsonb)) " + "ON CONFLICT (draft_id) DO NOTHING")
				.bind("id", id).bind("draft", draftId.toString()).bind("account", accountId)
				.bind("schemaVersion", schemaVersion).bind("document", documentJson).fetch().rowsUpdated()
	...
	public Mono<CanvasRow> casUpdate(UUID draftId, long expectedRevision, String documentJson) {
		return db.sql(
				"UPDATE creation_canvas_document SET revision=revision + CASE WHEN document=CAST(:document AS jsonb) THEN 0 ELSE 1 END, "
						+ "schema_version=:schemaVersion, "
						+ "document=CAST(:document AS jsonb), updated_at=CASE WHEN document=CAST(:document AS jsonb) THEN updated_at ELSE now() END "
						+ "WHERE draft_id=CAST(:draft AS uuid) AND revision=:expected " + "RETURNING " + COLS)
				.bind("draft", draftId.toString()).bind("expected", expectedRevision).bind("schemaVersion", 1)
				.bind("document", documentJson).map(CreationCanvasRepository::map).one();
	}
	...
	public Mono<CanvasRow> findByDraftId(UUID draftId) {
		return db.sql("SELECT " + COLS + " FROM creation_canvas_document WHERE draft_id=CAST(:id AS uuid)")
				.bind("id", draftId.toString()).map(CreationCanvasRepository::map).one();
	}

	public Mono<CanvasRow> lockByDraftId(UUID draftId) {
		return db.sql("SELECT " + COLS + " FROM creation_canvas_document WHERE draft_id=CAST(:id AS uuid) FOR UPDATE")
				.bind("id", draftId.toString()).map(CreationCanvasRepository::map).one();
	}

	/**
	 * 跨项目 canonical 防线（§6.3）：shot/take 引用存在但归属其他分镜 → true。 不存在（历史失效引用）→
	 * false，由读侧以不可用态呈现（§7.3）。
	 */
	public Mono<Boolean> belongsToOtherStoryboard(String kind, String refId, String storyboardId) {
		...
		return db.sql(sql).bind("ref", refId).bind("storyboard", storyboardId)
				.map((row, meta) -> row.get(0, Long.class) != null && row.get(0, Long.class) > 0).one();
	}
	...
	public Mono<Boolean> anyCrossProjectReference(java.util.List<String[]> kindAndRefIds, String storyboardId) {
		return reactor.core.publisher.Flux.fromIterable(kindAndRefIds)
				.concatMap(pair -> belongsToOtherStoryboard(pair[0], pair[1], storyboardId)).any(Boolean::booleanValue)
