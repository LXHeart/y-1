 ...
 *
 * <p><b>缓存语义</b>：{@link PlatformProviderPolicy#validate} 是同步调用（WebFlux 事件循环上也有），
 ...
 *
 * <p><b>单实例语义</b>：多实例部署时其它副本感知不到本副本的写事件，各自有最长「到重启为止」的
 * 陈旧窗口。当前 intelligence 单实例部署；扩副本前需改为共享存储订阅（如 outbox/notify）——TODO(#58)。
 */
@Service
public class TrustedOriginService implements ApplicationRunner {
    ...
    public TrustedOriginService(
            PlatformTrustedOriginRepository repository, ApplicationEventPublisher events) {
    ...
    public void onOriginsChanged(TrustedOriginsChangedEvent event) {
        refresh().subscribe(
                null,
                error -> logger.warn("Trusted origin cache reload failed; keeping previous snapshot: {}",
                        error.getMessage()));
    }

    /** 重拉启用中的 origin 集（启动预热与写后失效共用），并归一为 scheme://host:port 形态。 */
    public Mono<Void> refresh() {
        return repository.listEnabledOrigins()
                .map(TrustedOriginService::normalize)
                .collectList()
    ...
    /**
     * 新增。语义重复（归一化后与既有行相同，含种子行的无端口写法）→ 409；
     * raw 字符串重复由唯一索引兜底 → 同样 409。
     */
    public Mono<PlatformTrustedOrigin> create(String origin, String label, String adminId) {
        return assertOriginFree(normalize(origin), null)
                .then(Mono.defer(() -> repository.create(origin, label, adminId)))
                .doOnNext(saved -> publish(saved))
                .onErrorMap(DataIntegrityViolationException.class,
                        error -> new com.grassland.intelligence.security.IntelligenceException(
                                409, "该端点已在受信列表中"));
    }
    ...
    public Mono<PlatformTrustedOrigin> update(
        ...
        return assertOriginFree(normalize(origin), id)
                ...
                .switchIfEmpty(Mono.defer(() -> repository.findById(id)
                        .<PlatformTrustedOrigin>flatMap(existing -> Mono.error(
                                new com.grassland.intelligence.security.IntelligenceException(
                                        409, "该端点已被他人修改（版本冲突），请刷新后重试")))
                        .switchIfEmpty(Mono.error(new com.grassland.intelligence.security.IntelligenceException(
                                404, "未找到受信端点: " + id)))))
                .doOnNext(saved -> publish(saved));
    }

    /** 删除（硬删）。仍被模型行引用的 origin 删除后，那些行的下次校验将拒绝——运营自行取舍。 */
    public Mono<Boolean> delete(UUID id) {
        return repository.delete(id).doOnNext(deleted -> {
            if (deleted) {
    ...
    private Mono<Void> assertOriginFree(String normalizedOrigin, UUID excludeId) {
        return repository.listAll()
                .filter(row -> !row.id().equals(excludeId))
                .map(row -> normalize(row.origin()))
                .collectList()
                .flatMap(existing -> existing.contains(normalizedOrigin)
                        ? Mono.error(new com.grassland.intelligence.security.IntelligenceException(
                                409, "该端点已在受信列表中"))
                        : Mono.empty());
    }
