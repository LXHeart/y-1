 ...
 *
 * <p>
 * 先 claim deleting，再删最终/临时两把 key，成功后写 deleted_at 审计；任何对象删除失败都保留 deleting， stale
 * deleting 会在下轮重新 claim 并重试。文章生成图同时有
 * {@code S3GeneratedImageStore.cleanupExpired} 按对象 lastModified
 * 巡检：两者故意幂等重叠——store 巡检清理无元数据的历史/孤儿对象，本类清理有 media_reference 的资产并写删除审计。
 */
@Component
...
public class MediaCleanup {
	...
	public MediaCleanup(MediaReferenceRepository mediaRefs, ObjectStorageAdapter storage, OutboxRepository outbox,
			TransactionalOperator transactions, @Value("${media.pending-grace-seconds:3600}") long pendingGraceSeconds,
			@Value("${media.upload-url-ttl-seconds:900}") long uploadUrlTtlSeconds) {
		this.mediaRefs = mediaRefs;
		this.storage = storage;
		this.outbox = outbox;
		this.transactions = transactions;
		this.pendingGrace = Duration
				.ofSeconds(Math.max(Math.max(pendingGraceSeconds, 1L), Math.max(uploadUrlTtlSeconds, 1L) + 60L));
	}
	...
	public void cleanupExpired() {
		...
		cleanup().doFinally(signal -> running.set(false)).subscribe(ignored -> {
	...
	Mono<Void> cleanup() {
		return mediaRefs.findCleanupCandidates(pendingGrace).concatMap(this::claimDeleteAndAudit)
				.then(cleanOrphanedTemporaryObjects());
	}

	private Mono<Void> claimDeleteAndAudit(MediaReference candidate) {
		return mediaRefs.claimCleanup(candidate.id()).flatMap(this::deleteAndAudit).onErrorResume(error -> {
			log.warn("media cleanup failed: mediaId={}, failureStage=candidate_cleanup, "
					+ "exceptionType={}, errorCategory=cleanup_failed", candidate.id(), exceptionType(error));
			return Mono.empty();
		}).then();
	}
	...
	private Mono<Void> deleteAndAudit(MediaReference ref) {
		...
		return mediaRefs.erasureGuarded(ref.objectKey()).flatMap(guarded -> guarded
				? Mono.empty()
	...
	private Mono<Void> cleanOrphanedTemporaryObjects() {
		...
		return Mono.fromCallable(() -> storage.listObjects("media-pending/")).subscribeOn(Schedulers.boundedElastic())
				.flatMapMany(reactor.core.publisher.Flux::fromIterable)
				.filter(object -> object.lastModified() != null && object.lastModified().isBefore(cutoff))
				.concatMap(object -> deleteObject(object.key()).onErrorResume(error -> {
					log.warn(
							"media temporary object cleanup failed: "
