 ...
 *
 * <p>
 * 端点（{@code resolve}——任意登录用户；资源按账号作用域）：
 * <ul>
 * <li>POST /api/ai/runs — 执行一次 sync text run：预算→路由→(平台)扣分/(BYOK)解密→落库→provider
 * 调用→结算/退款。</li>
 ...
 *
 * <p>
 * 非整体 {@code @Conditional}：平台 run + 查询不依赖 KEK；仅 BYOK 解密分支在无 KEK 时
 * 503（{@code AiExecutionService}）。 真实 provider 凭据 / 浏览器真链路不在本 slice（IT 用
 * WireMock 证明解密→调用链）。
 */
@RestController
...
public class AiRunController {
	...
	public AiRunController(IntelligenceCallerResolver callers, AiExecutionService aiExecution,
			TextCompletionClient textClient, AiRunRepository runRepository,
	...
	@PostMapping
	public Mono<ResponseEntity<AiRunResponse>> execute(@Valid @RequestBody ExecuteRunRequest body,
		...
		return aiExecution
				.prepareExecution(exchange, body.capability(), CreditFeature.AI_RUN_TEXT, estimatedInputTokens,
						maxTokens, allowFallback, body.contextSnapshotId())
	...
	private Mono<ResponseEntity<AiRunResponse>> doExecute(AiExecutionService.ExecutionContext ctx, String prompt,
			int maxTokens) {
		// 任务书 #47 S2：平台凭据与 BYOK 统一走 decryptedKey——env 兜底已集中在 AiExecutionService
		String bearer = ctx.decryptedKey();
		return Mono
				.usingWhen(Mono.just(ctx),
						ignored -> executeWithLease(ctx, prompt, maxTokens, bearer)
								.map(completion -> aiExecution.normalizeProviderUsage(ctx, completion))
								.flatMap(completion -> aiExecution
										.settleSuccess(ctx, completion.inputTokens(), completion.outputTokens(), 0, 0)
				...
				.onErrorResume(error -> aiExecution.handleFailure(ctx, errorMessage(error)).then(Mono.error(error)));
	}

	private Mono<TextCompletionResult> executeWithLease(AiExecutionService.ExecutionContext ctx, String prompt,
			int maxTokens, String bearer) {
		return Mono.usingWhen(concurrencyLimiter.acquire(ctx.provider()),
				ignored -> textClient.complete(ctx.provider().provider(), ctx.provider().baseUrl(), bearer,
						ctx.provider().model(), prompt, maxTokens, ctx.provider().isByok()),
	...
	public Flux<AiRunResponse> list(ServerWebExchange exchange) {
		return callers.resolve(exchange.getRequest())
				.flatMapMany(c -> runRepository.findByAccount(c.accountId(), LIST_LIMIT).map(AiRunResponse::summary));
	}

	@GetMapping("/{id}")
	public Mono<ResponseEntity<AiRunResponse>> get(@PathVariable UUID id, ServerWebExchange exchange) {
		return callers.resolve(exchange.getRequest())
				.flatMap(c -> runRepository.findById(id).filter(run -> c.accountId().equals(run.accountId())) // 跨账号 → 空
																												// → 404
	...
	private static IntelligenceException deniedException(String reason) {
		return switch (reason) {
			...
			// 任务书 #78 卡 B（D3）：own 模式未配该能力密钥 → 禁用并引导（不回退平台）
