 ...
 *
 * <p>{@code video_generation} / {@code video_tts} 两个 capability 经
 * {@link PlatformModelControlPlaneService} 解析 primary（健康优先）+ 平台凭据，vendor adapter
 * 按行的 {@code provider} 值分派（minimax / seedance / sandbox；万相在卡6 接入）。
 * {@code VideoGenerationProperties} 的 mode/base-url/api-key/model 自此不再是生产配置来源
 * （字段保留供旧链 IT 注入 sandbox，见 application.yml 卡2 注释）。
 *
 * <p>两道 fail-closed 闸：凭据解密失败（含平台凭据缺密钥）→ Unavailable(reason)；模型在
 * 当前 active 价目表无行 → Unavailable（宁可 slideshow 也不误配 0 价）。解密与价目的明文/
 * 单价随 plan 返回，capabilities 显 available=false、建任务的卡6 拒绝建——不在这里抛 500。
 */
@Service
public class VideoGenerationProviderResolver {
    ...
    public Mono<VideoProviderResolution> resolveVideoGeneration() {
        return controlPlane.resolve(CAPABILITY_VIDEO_GENERATION)
                .map(row -> row.map(this::toVideoPlan)
                        .orElseGet(() -> VideoProviderResolution.unavailable("未配置视频生成模型")));
    }
    ...
    public Mono<TtsProviderResolution> resolveTts() {
        return controlPlane.resolve(CAPABILITY_VIDEO_TTS)
                .map(row -> row.map(this::toTtsPlan)
                        .orElseGet(() -> TtsProviderResolution.unavailable("配音模型未配置")));
    }
    ...
    private VideoProviderResolution toVideoPlan(ResolvedPlatformModel row) {
        ...
        return VideoProviderResolution.of(new VideoProviderResolution.Plan(
                adapter, toProviderResolution(row), unitPriceCents, priceTableVersion));
    }
    ...
    private ProviderResolution toProviderResolution(ResolvedPlatformModel row) {
        return ProviderResolution.platform(row.configId(), row.provider(), row.baseUrl(), row.model(),
                row.version(), row.maxConcurrency(), row.credentialEncryptedKey(), row.credentialVersion());
    }
    ...
    /**
     * 解析产物：adapter 已按解密凭据构造好；{@code resolution} 携带密文供执行环
     * （prepareMediaExecution 内部会再解密一次——确定性开销，明文不出本进程）。
     */
    public record VideoProviderResolution(Plan plan, String unavailableReason) {
        ...
        public record Plan(
                VideoGenerationProvider adapter,
    ...
    /**
     * 配音解析产物（卡5 构造 TTS adapter 用）。{@code apiKey} 是解密明文，只在进程内传递，
     * 绝不入日志/响应/落库。
     */
