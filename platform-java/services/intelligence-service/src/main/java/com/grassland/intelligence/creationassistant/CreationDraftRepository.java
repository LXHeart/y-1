 ...
 *
 * <p>
 * 用法复刻 {@code ContentAssetRepository}：注入 {@link DatabaseClient}，timestamptz 读为
 * {@link OffsetDateTime} 再转 {@link Instant}，uuid 经
 * {@code ::text}/{@code CAST}，可空字段用 {@code bindNull}。
 */
@Component
public class CreationDraftRepository {
	...
	public Mono<CreationDraft> create(CreationDraft draft) {
		...
		spec = bindNullableString(spec, "questionText", draft.questionText());
		spec = bindNullableString(spec, "questionRef", draft.questionRef());
	...
	public Mono<CreationDraft> save(UUID id, int expectedVersion, String title, String topic, String articleTitle,
		...
		spec = bindNullableString(spec, "questionText", questionText);
		spec = bindNullableString(spec, "questionRef", questionRef);
	...
	/**
	 * 落不可变历史快照（镜像 task_version / content_asset_version）。每次保存前调用， 把当前行整行镜像到
	 * creation_draft_version。与 save 同事务（由 controller 串）。
	 *
	 * <p>
	 * {@code ON CONFLICT DO NOTHING} 是并发自动保存的必需项，不是防御性冗余：跨设备/debounce 抖动下 两个 PUT
	 * 可能读到同一 version 并各自 insert 同一 {@code (draft_id, version)} 主键。后到者会阻塞在
	...
	public Flux<CreationDraftVersion> listVersions(UUID draftId, Integer beforeVersion, int fetchLimit) {
		...
		DatabaseClient.GenericExecuteSpec spec = db
				.sql("SELECT * FROM (" + VERSION_UNION + ") versions" + cursorClause
						+ " ORDER BY version DESC LIMIT :fetchLimit")
				.bind("draftId", draftId.toString()).bind("fetchLimit", fetchLimit);
	...
	public Mono<CreationDraftVersion> findVersion(UUID draftId, int version) {
		return db.sql("SELECT * FROM (" + VERSION_UNION + ") versions WHERE version=:version")
				.bind("draftId", draftId.toString()).bind("version", version).map(CreationDraftRepository::mapVersion)
				.one();
	}

	private static CreationDraft map(Readable row) {
		return new CreationDraft(UUID.fromString(row.get("id", String.class)),
				row.get("owner_account_id", String.class), row.get("organization_id", String.class),
				row.get("title", String.class), DraftSourceType.fromRequest(row.get("source_type", String.class)),
				row.get("task_id", String.class), row.get("task_version", Integer.class),
				row.get("store_id", String.class), row.get("platform", String.class),
				row.get("content_form", String.class), row.get("topic", String.class),
				row.get("article_title", String.class), row.get("outline", String.class),
				row.get("content", String.class), DraftContentMode.orDefault(row.get("content_mode", String.class)),
	...
	private static CreationDraftVersion mapVersion(Readable row) {
		return new CreationDraftVersion(UUID.fromString(row.get("draft_id", String.class)),
				intValue(row.get("version", Integer.class), 1), row.get("title", String.class),
				DraftSourceType.fromRequest(row.get("source_type", String.class)), row.get("task_id", String.class),
				row.get("task_version", Integer.class), row.get("store_id", String.class),
				row.get("platform", String.class), row.get("content_form", String.class),
				row.get("topic", String.class), row.get("article_title", String.class),
				row.get("outline", String.class), row.get("content", String.class),
	...
	private static DatabaseClient.GenericExecuteSpec bindNullableInt(DatabaseClient.GenericExecuteSpec spec,
