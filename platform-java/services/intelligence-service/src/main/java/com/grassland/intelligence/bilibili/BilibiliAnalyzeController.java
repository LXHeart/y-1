...
/**
 * Bilibili 视频内容分析（草场 Slice 13 Stage
 * 5）：{@code POST /api/bilibili/analyze-video}。
 *
 * <p>
 * 需登录；短 progressive 视频直接交给 Qwen，DASH/长视频由 Java mux/切片并经 analysis-media 回源。
 * 整次分析只扣一次积分。Edge 开关默认开启，显式关闭会 fail-closed 404。
 *
 * <p>
 * body {@code mode} 缺省为内容提取；{@code "recreation"} 走复刻分镜场景分析（PRD §4.4）， 仅支持短
 * progressive 视频；任务模式复刻分析走冻结快照执行。
 */
@RestController
public class BilibiliAnalyzeController {
	...
	public BilibiliAnalyzeController(IntelligenceCallerResolver resolver, BilibiliAnalysisService service) {
	...
	private Mono<BilibiliAnalysisOutcome> route(String proxyVideoUrl, IntelligenceCallerResolver.Caller caller,
