...
/** 组织 AI 全局预算管理入口（任务书 #37 / PRD §4.11）。 */
@RestController
@RequestMapping("/api/ai/organizations/{organizationId}/budget")
public class AiOrgBudgetController {

    private final IntelligenceCallerResolver callers;
    private final IdentityOrgAuthorizationClient orgAuthorization;
    private final AiModelBudgetRepository budgets;

    public AiOrgBudgetController(
    ...
    public Mono<ResponseEntity<Map<String, Object>>> get(
        ...
        return requireAdmin(organizationId, exchange)
                .then(budgets.findOrganizationBudget(organizationId)
                        .map(AiOrgBudgetController::toResponse)
                        .defaultIfEmpty(unlimitedResponse()))
                .map(AiOrgBudgetController::success);
    }

    @PutMapping
    public Mono<ResponseEntity<Map<String, Object>>> update(
            @PathVariable String organizationId,
            @RequestBody UpdateBudgetRequest body,
            ServerWebExchange exchange) {
        return requireAdmin(organizationId, exchange)
                .then(Mono.defer(() -> updateBudget(organizationId, body)))
                .map(AiOrgBudgetController::success);
    }
    ...
    private Mono<Map<String, Object>> updateBudget(String organizationId, UpdateBudgetRequest body) {
        validate(body);
        long expectedVersion = body.expectedVersion();
        if (body.allLimitsEmpty()) {
            if (expectedVersion == 0) {
                return budgets.findOrganizationBudget(organizationId)
                        .flatMap(ignored -> AiOrgBudgetController.<Map<String, Object>>conflict())
                        .switchIfEmpty(Mono.just(unlimitedResponse()));
            }
            return budgets.deleteOrganizationBudget(organizationId, expectedVersion)
                    .filter(Boolean::booleanValue)
                    .switchIfEmpty(Mono.defer(AiOrgBudgetController::conflict))
                    .thenReturn(unlimitedResponse());
        }

        Mono<AiModelBudget> saved = expectedVersion == 0
                ? budgets.createOrganizationBudget(
                        organizationId,
                ...
                : budgets.updateOrganizationBudget(
                        organizationId, expectedVersion,
                        body.maxTokensPerRun(), body.maxTokensDaily(), body.maxTokensMonthly(),
                        body.maxCentsPerRun(), body.maxCentsDaily(), body.maxCentsMonthly());
        return saved.map(AiOrgBudgetController::toResponse)
                .switchIfEmpty(Mono.defer(AiOrgBudgetController::conflict));
    }
    ...
    private static Map<String, Object> toResponse(AiModelBudget budget) {
        ...
        data.put("version", budget.version());
    ...
    private static <T> Mono<T> conflict() {
        return Mono.error(new IntelligenceException(409, "AI 预算已被其他管理员修改，请重新载入"));
    }
