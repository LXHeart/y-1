...
public class OutboxRepository {
    ...
    public Mono<Void> append(EventEnvelope event) {
        try {
            ...
            return db.sql("""
                    ...
                    VALUES (:eventId, :eventType, :aggregateType, :aggregateId, CAST(:payload AS json))
    ...
    public Flux<OutboxRow> claimBatch(int limit, UUID claimToken, Duration leaseDuration) {
        ...
        return db.sql("""
                WITH candidates AS (
                    SELECT candidate.id
                    FROM intelligence_outbox AS candidate
                    WHERE published_at IS NULL
                      AND NOT EXISTS (
                          SELECT 1 FROM intelligence_outbox AS earlier
                ...
                UPDATE intelligence_outbox AS target
    ...
    public Mono<Boolean> markFailure(
            String id, UUID claimToken, Duration retryDelay, String errorCode) {
        return db.sql("""
                UPDATE intelligence_outbox
    ...
    public Mono<Long> pendingCount() {
        return db.sql("SELECT COUNT(*)::bigint AS pending_count FROM intelligence_outbox WHERE published_at IS NULL")
                .map(row -> value(row.get("pending_count", Long.class), 0L))
                .one()
                .defaultIfEmpty(0L);
    }
    ...
    public Mono<Long> oldestPendingAgeSeconds() {
        return db.sql("""
                ...
                WHERE published_at IS NULL
                """)
                .map(row -> value(row.get("age_seconds", Long.class), 0L))
                .one()
                .defaultIfEmpty(0L);
    }
