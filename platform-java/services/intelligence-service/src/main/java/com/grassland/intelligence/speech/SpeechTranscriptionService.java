...
public final class SpeechTranscriptionService {
    ...
    public Mono<SpeechTranscription> create(
            ServerHttpRequest request, UUID mediaId, String requestedLanguage) {
        return callers.requireUser(request)
                .flatMap(caller -> {
                    String language = language(requestedLanguage);
                    return requireOwnedMedia(mediaId, caller.accountId())
                            .flatMap(this::readAndProbe)
                            .flatMap(audio -> createProcessing(
                                    caller.accountId(), caller.organizationId(), mediaId, language, audio));
                });
    }

    public Mono<SpeechTranscription> get(ServerHttpRequest request, UUID transcriptionId) {
        return callers.requireUser(request)
                .flatMap(caller -> transcriptions.findOwned(transcriptionId, caller.accountId())
    ...
    private Mono<MediaReference> requireOwnedMedia(UUID mediaId, String accountId) {
        ...
        return mediaReferences.findById(mediaId)
                .filter(media -> accountId.equals(media.ownerAccountId()))
                ...
                .filter(SpeechTranscriptionService::validMetadata)
                .switchIfEmpty(notFound());
    }
        ...
        return Mono.fromCallable(() -> {
                    ...
                    if (storage == null) {
                        throw new IntelligenceException(
                                503, "speech_storage_unavailable", "语音音频存储暂不可用");
                    }
                    byte[] bytes;
                    try {
                        bytes = storage.getObject(media.objectKey());
                    } catch (RuntimeException storageError) {
                        // Object storage implementations may include the internal key in their exception text.
    ...
    private Mono<SpeechTranscription> createProcessing(
            String accountId,
        ...
        SpeechTranscription processing = new SpeechTranscription(
                id, mediaId, accountId, organizationId, language, null, audio.durationMs(),
                "processing", null, null, null, null, null, null, null, null, null);
        return Mono.usingWhen(
                        transcriptions.createProcessing(processing),
                        transcription -> prepareAndExecute(
                                transcription, audio, accountId, organizationId),
                        ignored -> Mono.empty(),
                        (transcription, error) -> finalizeTranscriptionFailure(
                                transcription.id(), failureCode(error)),
                        transcription -> finalizeTranscriptionFailure(
                                transcription.id(), "execution_cancelled"))
                .onErrorMap(SpeechTranscriptionService::exposedError);
    }

    private Mono<SpeechTranscription> prepareAndExecute(
        ...
        return Mono.usingWhen(
                ...
                ignored -> Mono.empty(),
                (prepared, error) -> finalizeRunFailure(
                        transcription.id(), prepared, failureCode(error)),
                prepared -> finalizeRunFailure(
                        transcription.id(), prepared, "execution_cancelled"));
    }

    private Mono<SpeechTranscription> executePrepared(
            SpeechTranscription transcription,
            AudioInput audio,
    ...
    private Mono<SpeechTranscription> complete(
            SpeechTranscription transcription,
            AiExecutionService.ExecutionContext context,
    ...
    private Mono<Void> finalizeTranscriptionFailure(UUID transcriptionId, String failureCode) {
        return cleanupTranscription(transcriptionId, failureCode)
                .onErrorResume(cleanupError -> Mono.empty());
    }

    private Mono<Void> cleanupTranscription(UUID transcriptionId, String failureCode) {
        return Mono.defer(() -> transactions.transactional(requireChanged(
                                transcriptions.markFailed(transcriptionId, failureCode),
                                "语音转写失败状态保存失败"))
                        .then())
                .doOnError(error -> logFinalizationFailure(
                        transcriptionId, null, "transcription_mark_failed", error));
    }

    private Mono<Void> finalizeRunFailure(
            UUID transcriptionId,
            AiExecutionService.ExecutionResult prepared,
            String failureCode) {
        if (!prepared.allowed()) {
            return Mono.empty();
        }
        return Mono.defer(() -> {
                    ...
                    return requireChanged(executions.handleFailure(
                                        context, "speech transcription failed: " + failureCode),
                                    "语音转写 Run 失败状态保存失败")
                            .then()
                            .doOnError(error -> logFinalizationFailure(
                                    transcriptionId, context.runId(), "run_mark_failed", error));
                })
                .onErrorResume(cleanupError -> Mono.empty());
    }

    private static void logFinalizationFailure(
            UUID transcriptionId, UUID runId, String failureStage, Throwable error) {
        String exceptionType = error == null || error.getClass().getSimpleName().isBlank()
                ? "Unknown" : error.getClass().getSimpleName();
        log.warn("speech failure finalization failed: transcriptionId={}, runId={}, "
    ...
    private static Mono<Boolean> requireChanged(Mono<Boolean> change, String message) {
        return change.flatMap(changed -> changed
                ? Mono.just(true)
    ...
    private static boolean validMetadata(MediaReference media) {
        return media.objectKey() != null && !media.objectKey().isBlank()
                && media.checksum() != null && !media.checksum().isBlank()
                && media.sizeBytes() >= 1 && media.sizeBytes() <= MAX_AUDIO_BYTES
                && media.mimeType() != null
                && MIME_TYPES.contains(media.mimeType().trim().toLowerCase(Locale.ROOT));
    }
    ...
    private static IntelligenceException exposedError(Throwable error) {
        if (error instanceof IntelligenceException intelligence) {
            if (intelligence.status() == 429) {
                return new IntelligenceException(
                        429, "concurrency_unavailable", "语音识别并发已满，请稍后重试");
            }
            return intelligence;
        }
    ...
    private static IntelligenceException deniedException(String reason) {
        return switch (reason) {
            case "no_platform_model" ->
                    new IntelligenceException(503, "no_platform_model", "平台未配置语音识别模型");
            case "unpriced_model" ->
                    new IntelligenceException(503, "unpriced_model", "语音识别模型缺少价目配置");
