...
public class CreationExportRepository {
	...
	private static final String COLS = "id, owner_account_id, request_id, draft_id, version, format, theme,"
			...
			+ " content_hash, error_code, created_at, ready_at, failed_at, build_started_at, build_token";
	...
	public record ExportRow(UUID id, String ownerAccountId, String requestId, UUID draftId, int version, String format,
			...
			OffsetDateTime failedAt, OffsetDateTime buildStartedAt, UUID buildToken) {
	}
	...
	public Mono<ExportRow> claimOrGet(UUID id, String ownerAccountId, String requestId, UUID draftId, int version,
		...
		return db.sql("""
				INSERT INTO creation_export (id, owner_account_id, request_id, draft_id, version, format, theme,
				    include_title, cite_external_links, payload_hash, state, build_token)
				VALUES (CAST(:id AS uuid), :owner, :requestId, CAST(:draft AS uuid), :version, :format, :theme,
				    :includeTitle, :cite, CAST(:hash AS char(64)), 'building', CAST(:id AS uuid))
	...
	public Mono<Integer> markReady(UUID id, UUID token, String manifestJson, String contentHash) {
		return db.sql("""
				...
				WHERE id = CAST(:id AS uuid) AND state = 'building' AND build_token=:token
				""").bind("id", id.toString()).bind("token", token).bind("manifest", manifestJson)
				.bind("hash", contentHash).fetch().rowsUpdated().map(count -> count == null ? 0 : count.intValue());
	}

	public Mono<ExportRow> claimRebuild(ExportRow row) {
		return db
				.sql("UPDATE creation_export SET state='building', build_token=:token, build_started_at=now(),"
						+ " failed_at=NULL, error_code=NULL WHERE id=:id AND state='failed'")
				.bind("id", row.id()).bind("token", UUID.randomUUID()).fetch().rowsUpdated()
				.flatMap(count -> count > 0 ? findByIdAndOwner(row.id(), row.ownerAccountId()) : Mono.empty());
	}

	public Mono<Integer> markFailed(UUID id, UUID token, String errorCode, java.util.List<String> missing) {
		return db.sql("UPDATE creation_export SET state='failed', error_code=:code, failed_at=now(),"
				+ " manifest_json=CAST(:manifest AS jsonb) WHERE id=:id AND build_token=:token AND state='building'")
				.bind("id", id).bind("token", token).bind("code", errorCode)
				.bind("manifest",
						com.grassland.intelligence.creationstudio.plan.PlanJson
								.json(java.util.Map.of("missingItems", missing)))
				.fetch().rowsUpdated().map(Long::intValue);
	}

	public Mono<Boolean> expire(ExportRow row) {
		return db
				.sql("UPDATE creation_export SET state='failed', error_code='STUDIO_EXPORT_EXPIRED', failed_at=now()"
						+ " WHERE id=:id AND state='ready' AND ready_at=:readyAt")
				.bind("id", row.id()).bind("readyAt", row.readyAt()).fetch().rowsUpdated().map(count -> count > 0);
	}
	...
	public Mono<Integer> failStaleBuilding(OffsetDateTime cutoff) {
		return db
				.sql("UPDATE creation_export SET state='failed', error_code='STUDIO_EXPORT_TIMEOUT', failed_at=now()"
						+ " WHERE state='building' AND build_started_at < :cutoff")
	...
	private static ExportRow map(io.r2dbc.spi.Row row, io.r2dbc.spi.RowMetadata metadata) {
		return new ExportRow(row.get("id", UUID.class), row.get("owner_account_id", String.class),
				...
				row.get("build_started_at", OffsetDateTime.class), row.get("build_token", UUID.class));
	}
}
