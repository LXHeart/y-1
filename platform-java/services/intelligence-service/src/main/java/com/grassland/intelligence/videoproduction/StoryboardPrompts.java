...
final class StoryboardPrompts {
    ...
    private static final java.util.Map<String, String> PLATFORM_ADAPTATIONS = java.util.Map.of(
            "bilibili", """

            ## B站适配
            按「论点 → 证据 → 演示」组织内容：先给出本期要说明的问题或结论，再用可展示的材料支撑；
            镜头按章节推进（在 visual 里体现章节递进，如「第X部分」），避免平铺罗列。
            引用的数据与结论必须在旁白中说明来源（用户提供资料/画面可见），资料不支持的保持待核对表述。""",
            "douyin", """

            ## 抖音适配
            开场 2 秒直接给出「要说明的问题或场景」，随后逐镜递进；结尾给明确行动指引。
            发布描述与话题由用户另行编辑——旁白只负责口播内容，不要在旁白里堆砌话题标签。""",
            "kuaishou", """

            ## 快手适配
            突出人物、过程与真实结果：谁在做事、怎么做、做成什么样；演示步骤完整可跟做。
            口吻接地气，可按用户要求的方言/称呼表达，不用固定套话代替平台适配。""",
            "channels", """
    ...
    static ChatMessage system(int targetDurationSeconds, String targetPlatform, String industryType,
        ...
        String prompt = SYSTEM_TEMPLATE
                ...
                .replace("{industryType}", safe(industryType, "其他"))
                .replace("{videoStyle}", safe(videoStyle, "自然纪实"));
    ...
    static ChatMessage user(VideoProductionController.StoryboardRequest request) {
        ...
        if (VideoProductionController.StoryboardRequest.INPUT_SCRIPT.equals(mode)) {
            ...
            for (VideoProductionController.StoryboardRequest.OwnMediaRef ref : request.ownMediaRefs()) {
                StringBuilder item = new StringBuilder("- 素材").append(index++).append("（mediaId=")
                        .append(ref.mediaId()).append("）");
                if (ref.label() != null) {
                    item.append("：").append(ref.label());
                }
                if (ref.trimStartSeconds() != null || ref.trimEndSeconds() != null) {
                    item.append("；裁剪范围 ")
                            .append(ref.trimStartSeconds() == null ? 0 : ref.trimStartSeconds()).append("-")
                            .append(ref.trimEndSeconds() == null ? "结尾" : ref.trimEndSeconds()).append(" 秒");
                }
        ...
        if (!brief.isEmpty()) lines.add(brief);
