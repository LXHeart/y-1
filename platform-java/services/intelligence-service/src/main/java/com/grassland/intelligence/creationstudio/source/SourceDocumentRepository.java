 ...
 * timestamptz 读 {@link OffsetDateTime} 转 {@link Instant}，jsonb 以 text 读写、service-local
 * mapper 序列化。幂等键 UNIQUE(owner_account_id, request_id)——同键冲突由 service 比较
 * request_hash 决定重放或 409。
 */
@Component
public class SourceDocumentRepository {
    ...
    public Mono<SourceDocument> findByOwnerAndRequestId(String ownerAccountId, String requestId) {
        return db.sql("""
                ...
                .map(SourceDocumentRepository::map).one();
    }
    ...
    private static SourceDocument map(Readable row) {
        List<SourceDocument.Block> blocks = readJson(row.get("blocks_json", String.class)).stream()
                .map(item -> new SourceDocument.Block(text(item.get("id")), text(item.get("kind")),
                        ...
                        ((Number) item.get("startCodePoint")).intValue(),
        ...
        return new SourceDocument(UUID.fromString(row.get("id", String.class)),
                ...
                row.get("kind", String.class), row.get("schema_version", Integer.class),
                row.get("title", String.class), row.get("raw_text", String.class),
                row.get("normalized_markdown", String.class), row.get("content_hash", String.class), blocks,
                sourceRefs, readJson(row.get("warnings_json", String.class)).stream()
