...
public class CreditUsageSettlementDispatcher {
	...
	public CreditUsageSettlementDispatcher(CreditUsageSettlementRepository repository, CreditsClient credits,
	...
	public void dispatchPending() {
		...
		dispatchBatch().doOnError(error -> logger.error("AI credit usage settlement batch failed", error))
				.onErrorResume(error -> Mono.empty()).doFinally(signal -> dispatching.set(false)).subscribe();
	}
	...
	Mono<Void> dispatchBatch() {
		...
		return repository.claimBatch(batchSize, claimToken, claimLease).flatMap(this::process, batchSize).then();
	}
	...
	private Mono<Void> process(CreditUsageSettlementRepository.SettlementClaim claim) {
		...
		CreditCharge reservation = new CreditCharge(claim.accountId(), feature, claim.consumeOperationId().toString(),
	...
	private Mono<Void> complete(CreditUsageSettlementRepository.SettlementClaim claim, CreditSettlement settlement) {
	...
	private Mono<Void> markFailed(CreditUsageSettlementRepository.SettlementClaim claim, Throwable error) {
		return repository.markFailed(claim.id(), claim.claimToken(), backoff(claim.attemptCount()), errorCode(error))
				.then();
	}

	private Mono<Void> markTerminalFailed(CreditUsageSettlementRepository.SettlementClaim claim, Throwable error) {
		String code = errorCode(error);
		return repository.markTerminalFailed(claim.id(), claim.claimToken(), code).doOnNext(updated -> {
			if (updated) {
				logger.error(
	...
	private boolean isTerminal(CreditUsageSettlementRepository.SettlementClaim claim, Throwable error) {
		...
		return intelligenceError.status() == 400 || intelligenceError.status() == 404
				|| intelligenceError.status() == 409 || intelligenceError.status() == 422;
	}
	...
	private static EventEnvelope usageAdjustedEvent(CreditUsageSettlementRepository.SettlementClaim claim,
		...
		return new EventEnvelope(
				UUID.nameUUIDFromBytes(
						("UsageAdjusted:" + claim.runId()).getBytes(java.nio.charset.StandardCharsets.UTF_8))
						.toString(),
				"UsageAdjusted", "ai_run", claim.runId().toString(), 1, Instant.now(),
				claim.consumeOperationId().toString(), payload);
	}
}
