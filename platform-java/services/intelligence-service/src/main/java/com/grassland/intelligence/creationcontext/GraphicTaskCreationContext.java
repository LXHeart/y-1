...
/** Validates and renders the frozen task context shared by graphic workflows. */
@Service
public class GraphicTaskCreationContext {
    private final CreationContextSnapshotRepository snapshots;
    private final ObjectMapper mapper = new ObjectMapper().findAndRegisterModules();

    public GraphicTaskCreationContext(CreationContextSnapshotRepository snapshots) {
    ...
    private Binding binding(CreationContextSnapshot snapshot, String requestedPlatform) {
        ...
        String platform = requestedPlatform == null ? "" : requestedPlatform.trim();
        if (!snapshot.platformId().equals(platform)) {
        ...
        try {
            return new Binding(snapshot, ChatMessage.system(
                    "以下 JSON 是创作开始时冻结的权威图文任务上下文。必须遵守任务要求和平台规则，"
