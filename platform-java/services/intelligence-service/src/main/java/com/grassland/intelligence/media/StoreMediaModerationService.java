...
/**
 * 门店公开媒体多模态审核（缺口清偿之五，#42 D9 登记）：store_media 用途图片在 confirm 服务端字节可得时送视觉模型审一次，结论落
 * {@code store_media_moderation}。
 *
 * <p>
...
public class StoreMediaModerationService {
	...
	/**
	 * confirm 后审核一次（已有结论不重跑）。{@code bytes} 为惰性对象字节拉取——仅在确实要送审 （store_media
	 * 图片/视频且无既有结论）时才订阅。返回落库后的行；未审（未配置/失败）返回 empty。
	 */
	public Mono<StoreMediaModerationRepository.ModerationRow> moderateOnce(MediaReference ref, Mono<byte[]> bytes) {
		if (!"store_media".equals(ref.purpose())
	...
	private Mono<List<ContentPart>> moderationParts(MediaReference ref, byte[] bytes) {
		if (isSupportedImage(ref.mimeType())) {
			...
			return Mono.just(List.of(ContentPart.image(dataUrl), ContentPart.text(rubric())));
		}
		return Mono.fromCallable(() -> frameExtractor.extract(bytes)).subscribeOn(Schedulers.boundedElastic())
				.map(frames -> {
					...
					parts.add(ContentPart.text(videoRubric()));
