 ...
 *
 * <p>聚合型 SSE（moments/card-series 同款契约）：校验、鉴权与预算拒绝都以非 2xx JSON 先于流；
 * 流内只有 progress/result 帧。修复不收积分（P2 拍板，feature=null 平台资助），但
 * 「未配置 content_fix 模型」必须显式报错（503），不静默降级。
 */
@RestController
public class ContentSafetyFixController {
    ...
    public Mono<ResponseEntity<Flux<DataBuffer>>> fix(@RequestBody FixRequest body, ServerWebExchange exchange) {
        return callers.requireUser(exchange.getRequest())
                .flatMap(caller -> service.fix(body, exchange).map(frames -> sseEntity(frames, exchange)))
                // 执行环 denied 的通用映射是 403「执行被拒绝：no_platform_model」——修复场景按
                // 任务书 #63 定死契约升级为 503 + 明确配置指引（其余 402 预算类原样透出）。
                .onErrorMap(error -> error instanceof IntelligenceException denied
                        && denied.status() == 403
    ...
    public record FixRequest(
            String text, List<FindingInput> findings, String platform,
            String contentForm, String genre, String style) {
        ...
        public FixRequest {
            ...
            for (FindingInput finding : findings) {
                ...
                normalized.add(new FindingInput(
                        finding.category().trim(), finding.match().trim(),
            ...
            if (contentForm != null && !contentForm.isBlank()
                    && !"answer".equals(contentForm) && !"article".equals(contentForm)) {
