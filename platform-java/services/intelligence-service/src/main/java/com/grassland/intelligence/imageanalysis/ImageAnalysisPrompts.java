...
/**
 * 图片评价文案 prompts（草场 intelligence Slice 6）。逐字移植 legacy {@code qwen-provider.ts}
 * 的
 ...
 * 及风格 summary/merge prompt（{@code image-review-style.service.ts}）。保持 legacy
 * 中未受信内容的 {@code <<<...>>>} 围栏、 字数规则、去 AI 化红线、平台差异（淘宝单段 vs 大众点评标题/正文/标签）。
 */
public final class ImageAnalysisPrompts {
	...
	public static String buildImageReviewPrompt(ImageReviewInput input) {
		...
		String prompt = platformContext + "你必须且只能返回合法的 JSON 对象，不要返回任何其他文字。\n\n" + "要求：\n"
				...
				+ input.reviewLength() + " 字；资料不足时保持简短，不得为凑字数虚构细节\n" + "- 最长不要超过 "
				+ calculateImageReviewMaxLength(input.reviewLength()) + " 字\n"
				+ (dianping ? "- 输出标题、正文和标签三个字段，不要只输出一段评价" : "- 只输出一段完整评价，不要分点，不要加标题，不要解释生成过程") + "\n" + platformRules
				+ "\n" + HUMANIZER_ZH_RULES + "\n\n" + "返回 JSON 格式：\n" + jsonFormat;
		return appendStylePreferences(prompt, input.stylePreferences()) + factualReviewRules()
				+ CreationBriefInput.render(input.brief());
	}
	...
	public static String buildImageReviewOptimizationPrompt(ImageReviewInput input, String draft, int round) {
		...
		String prompt = "你正在进行图片评价文案的第 " + round + " 轮优化。请把下面这段评价继续改得更像真人顺手写下的评论。你必须且只能返回合法的 JSON 对象，不要返回任何其他文字。\n\n"
				...
				+ (dianping ? "- 保持标题、正文和标签结构完整，优化时三个字段都要保留" : "- 只输出一段完整评价，不要分点，不要加标题，不要解释修改过程") + "\n" + platformRules
				+ "\n" + HUMANIZER_ZH_RULES + "\n\n" + "返回 JSON 格式：\n" + jsonFormat;
		return appendStylePreferences(prompt, input.stylePreferences()) + factualReviewRules()
				+ CreationBriefInput.render(input.brief());
	}
	...
	private static String factualReviewRules() {
		return "\n身份与事实底线：商家、商业合作作者或其他利益相关者必须按真实身份表达，不得伪装普通消费者。" + "没有用户确认的体验时，只描述图片可见信息；不得新增购买、到店、口味、服务或使用效果。"
				+ "不得把用户明确的不满改为推荐；文风偏好与字数要求不能覆盖事实约束。";
	}

	/**
	 * 风格总结 prompt（镜像 legacy {@code buildStyleSummaryPrompt}）。original/edited 为快照
	 * JSON（pretty）。
	 */
	...
	/**
	 * 图片评价生成输入（镜像 legacy
	 * {@code ImageReviewGenerationInput}）。{@code stylePreferences} 为预构建附录串。
	 */
