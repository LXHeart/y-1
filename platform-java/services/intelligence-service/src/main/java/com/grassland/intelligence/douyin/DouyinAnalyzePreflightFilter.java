...
/**
 * Douyin {@code POST /api/douyin/analyze-video} 的 body 前鉴权 + 限流（草场 GL-P3-MEDIA-001）。
 *
 * <p>逐字延续 legacy {@code app.ts} 的 {@code douyin-analyze} 防滥用成本闸：固定窗口
 ...
 */
@Component
public class DouyinAnalyzePreflightFilter implements WebFilter, Ordered {
    ...
    @Autowired
    public DouyinAnalyzePreflightFilter(IntelligenceCallerResolver callers) {
        this(callers, Clock.systemUTC());
    }

    DouyinAnalyzePreflightFilter(IntelligenceCallerResolver callers, Clock clock) {
    ...
    private static String bucketKey(String accountId) {
        return "douyin-analyze:user:" + accountId;
    }
