 ...
 * <li>同 (owner, requestId) 同输入返回原记录（重放 200）；同键异参 409
 * STUDIO_OPERATION_CONFLICT。</li>
...
public class SourceDocumentService {
	...
	private Mono<CreateResult> createNew(Caller caller, CreateCommand command) {
		...
		return drafts.loadOwned(command.draftId().toString(), caller.accountId()).flatMap(draft -> {
			...
			if ("draft-content".equals(command.kind())) {
				if (rawText != null) {
					return Mono.error(
							new IntelligenceException(400, "STUDIO_INVALID_INPUT", "kind=draft-content 不接受 text 字段"));
				}
	...
	public Mono<SourceDocument> loadOwned(String id, Caller caller) {
		...
		return sources.findById(documentId).filter(document -> caller.accountId().equals(document.ownerAccountId()))
				.flatMap(document -> drafts.loadOwned(document.draftId().toString(), caller.accountId())
						.thenReturn(document))
