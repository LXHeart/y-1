...
/**
 * 对象存储（MinIO/S3）实现：UUID key、30 分钟 TTL（按对象 lastModified 判过期）、访问不续期。 多副本共享同一
 * bucket，解决本地卷无法跨实例的问题。生产路径（{@code object-storage.enabled=true}）。
 *
 * <p>
 * 外部契约与 {@link LocalGeneratedImageStore} 一致：store 返回裸 UUID（URL 用），find 按 UUID
 * 还原字节。
 */
@Component
...
public class S3GeneratedImageStore implements GeneratedImageStore {
	...
	@Autowired
	public S3GeneratedImageStore(ObjectStorageAdapter storage,
	...
	private boolean isExpired(Instant lastModified) {
		return lastModified != null && Duration.between(lastModified, clock.instant()).compareTo(ttl) > 0;
	}
