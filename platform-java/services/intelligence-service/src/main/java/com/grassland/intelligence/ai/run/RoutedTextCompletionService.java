 ...
 *
 * <p>
 * 此前创作域一批遗留流（图片评价 step/*、文章 outline/content、朋友圈文案、视频复刻改编、 游客试用等）直连 env 版
 * {@code QwenClient}，绕过「模型密钥」面板开关与管理后台平台模型配置。
 * 本服务把这些流全部接到既有路由体系上，语义与执行环（{@code AiExecutionService}）一致：
 *
 * <ul>
 * <li>商家活动身份：组织 BYOK &gt; 平台控制面；推荐官/消费者：个人 BYOK（受「使用自定义模型」 开关控制）&gt;
 * 平台控制面；匿名：平台控制面直连。</li>
 ...
 *
 * <p>
 * <b>刻意不带计费闸</b>：本服务只决定「用哪个模型」，不改变各调用方既有计费语义——免费流
 * （step/*、游客试用、文章独立模式）继续免费，走执行环的计费流照旧。BYOK 分支沿用 TextCompletionClient 的 SSRF
 * 防护（仅 HTTPS + 全量公网 DNS 钉扎）。
 *
 * <p>
 * 错误文案：调用方各自的 {@code failureMessage} 落在 502 上（镜像 legacy
 * {@code TextCompletionCommand} 语义）；503（凭据缺失 fail-closed）与
 * 504（超时）保留具体文案——用户需要知道是平台配置问题而不是含糊失败。
 */
@Component
public class RoutedTextCompletionService {
	...
	private Mono<Routed> resolveRouted(Mono<ProviderResolution> resolution) {
		...
		return Mono.defer(() -> resolution.map(provider -> {
			if (provider.isDenied()) {
				if ("own_key_missing".equals(provider.denialReason())) {
					throw new IntelligenceException(422, "该能力未配置自有模型密钥，请在 AI 与治理中配置或切回平台统一模型");
				}
	...
	private static String deniedMessage(ProviderResolution provider) {
		return "no_platform_model".equals(provider.denialReason()) ? "平台未配置可用的内置模型，请联系管理员" : "当前设置不允许使用平台内置模型";
	}

	// ---------------- 非流式完成 ----------------

	public Mono<TextCompletionResult> complete(ServerWebExchange exchange, List<ChatMessage> messages, int maxTokens,
			Duration timeout, String failureMessage) {
		return resolve(exchange).flatMap(r -> execute(r, messages, maxTokens, timeout, failureMessage));
	}

	public Mono<TextCompletionResult> completeFor(String accountId, String organizationId, List<ChatMessage> messages,
	...
	public Mono<TextCompletionResult> completePlatformOnly(List<ChatMessage> messages, int maxTokens, Duration timeout,
	...
	public Mono<TextCompletionResult> completeWith(Routed routed, List<ChatMessage> messages, int maxTokens,
			Duration timeout, String failureMessage) {
		return execute(routed, messages, maxTokens, timeout, failureMessage);
	}

	private Mono<TextCompletionResult> execute(Routed routed, List<ChatMessage> messages, int maxTokens,
			Duration timeout, String failureMessage) {
	...
	public Flux<ChatChunk> stream(ServerWebExchange exchange, List<ChatMessage> messages, int maxTokens,
			Duration timeout, String failureMessage) {
		return resolve(exchange).flatMapMany(r -> streamWith(r, messages, maxTokens, timeout, failureMessage));
	}

	public Flux<ChatChunk> streamFor(String accountId, String organizationId, List<ChatMessage> messages, int maxTokens,
			Duration timeout, String failureMessage) {
		return resolveFor(accountId, organizationId)
	...
	public Flux<ChatChunk> streamWith(Routed routed, List<ChatMessage> messages, int maxTokens, Duration timeout,
			String failureMessage) {
		return textCompletion
