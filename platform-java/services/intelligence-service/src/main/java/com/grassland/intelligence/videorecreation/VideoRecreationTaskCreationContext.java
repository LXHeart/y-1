...
/** Validates and renders the frozen video-task context used throughout recreation. */
@Service
public class VideoRecreationTaskCreationContext {
    private final CreationContextSnapshotRepository snapshots;
    private final ObjectMapper mapper = new ObjectMapper().findAndRegisterModules();

    public VideoRecreationTaskCreationContext(CreationContextSnapshotRepository snapshots) {
        this.snapshots = snapshots;
    }

    public Mono<Binding> bind(UUID snapshotId, String accountId, String targetPlatform) {
        if (snapshotId == null) {
        ...
        return snapshots.findById(snapshotId)
                ...
                .map(snapshot -> binding(snapshot, targetPlatform));
    }

    private Binding binding(CreationContextSnapshot snapshot, String targetPlatform) {
        if (!"video".equals(snapshot.contentFormId())) {
            throw new IntelligenceException(409, "创作上下文不是视频任务");
        }
        if (targetPlatform == null || !snapshot.platformId().equals(targetPlatform.trim())) {
        ...
        try {
            return new Binding(snapshot, ChatMessage.system(
                    "以下 JSON 是创作开始时冻结的权威视频复刻任务上下文。必须遵守任务要求和平台规则，"
