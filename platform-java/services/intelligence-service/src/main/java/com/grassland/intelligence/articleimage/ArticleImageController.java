...
public class ArticleImageController {
	...
	private static final Set<MediaType> IMAGE_TYPES = Set.of(MediaType.IMAGE_JPEG, MediaType.IMAGE_PNG,
	...
	public Mono<Map<String, Object>> recommend(@RequestBody RecommendationRequest body, ServerWebExchange exchange) {
	...
	public Mono<Map<String, Object>> search(@RequestBody SearchRequest body, ServerWebExchange exchange) {
		return callers.resolve(exchange.getRequest()).flatMap(caller -> images.search(body.keywords(), body.count()))
				.map(result -> success(Map.of("images", result)));
	}

	@PostMapping(value = "/generate-image", consumes = MediaType.APPLICATION_JSON_VALUE)
	public Mono<Map<String, Object>> generateJson(@RequestBody GenerateJsonRequest body, ServerWebExchange exchange) {
		return callers.resolve(exchange.getRequest())
				.flatMap(
						caller -> body.isTaskMode()
								? taskImages.generate(new GenerateCommand(body.prompt(), body.size(), List.of()),
										caller.accountId(), body.contextSnapshotId(), body.targetPlatform())
								// 任务书 #56：独立分支接执行环（BYOK 路由 + 预算闸），不再走 legacy 免费路径
								: independentImages.generate(new GenerateCommand(body.prompt(), body.size(), List.of()),
	...
	public Mono<Map<String, Object>> generateMultipart(ServerWebExchange exchange) {
		return callers.resolve(exchange.getRequest()).flatMap(caller -> exchange.getMultipartData()
				.flatMap(this::parseMultipart)
				.flatMap(input -> input.taskMode()
						? taskImages.generate(input.command(), caller.accountId(), input.contextSnapshotId(),
								input.targetPlatform())
						: independentImages.generate(input.command(), caller.accountId(), caller.organizationId())))
				.map(ArticleImageController::success);
	}
	...
	public Mono<ResponseEntity<Resource>> generated(@PathVariable String id) {
		...
		return images.findGenerated(id)
				.map(stored -> ResponseEntity.ok().contentType(MediaType.IMAGE_PNG)
	...
	private Mono<GenerationInput> parseMultipart(MultiValueMap<String, Part> parts) {
		...
		return parsed.doFinally(signal -> Flux.fromIterable(parts.values()).flatMapIterable(value -> value)
	...
	private Mono<GenerationInput> parseMultipartValidated(MultiValueMap<String, Part> parts) {
		...
		for (String name : parts.keySet()) {
			if (!Set.of("prompt", "size", "images", "taskMode", "contextSnapshotId", "targetPlatform").contains(name)) {
	...
	private Mono<GenerationInput> parseMultipartParts(MultiValueMap<String, Part> parts, List<Part> promptParts,
			List<Part> sizeParts, List<Part> imageParts) {
		long fieldCount = parts.values().stream().flatMap(List::stream).filter(FormFieldPart.class::isInstance).count();
		...
		if (utf8Length(promptPart.value()) > MAX_FIELD_BYTES || (!sizeParts.isEmpty()
				&& utf8Length(((FormFieldPart) sizeParts.getFirst()).value()) > MAX_FIELD_BYTES)) {
			return Mono.error(new IllegalArgumentException("图片上传失败，请检查文件后重试"));
		}
		...
		if (taskMode && !imageParts.isEmpty()) {
			return Mono.error(new IllegalArgumentException("任务生图只能使用创作开始时冻结的授权素材"));
		}
		return Flux.fromIterable(imageParts).concatMap(this::readImage).collectList()
				.map(references -> new GenerationInput(new GenerateCommand(prompt, size, references), taskMode,
	...
	public record RecommendationRequest(String content, String outline, String platform) {
		public RecommendationRequest {
			...
			if (!Set.of("wechat", "zhihu", "xiaohongshu", "douyin").contains(platform)) {
	...
	public record GenerateJsonRequest(String prompt, String size, Boolean taskMode, UUID contextSnapshotId,
	...
	private record GenerationInput(GenerateCommand command, boolean taskMode, UUID contextSnapshotId,
			String targetPlatform) {
	}
	...
	private static void validateTaskBinding(boolean taskMode, UUID contextSnapshotId, String targetPlatform) {
