 ...
 *
 * <p>
 * 写入口 {@link #insert}（操作键唯一冲突 0 行 → 服务层复读重放）；每根至多 20 派生 由服务层在根行锁下计数闸。
 */
@Component
public class VideoStoryboardVariantRepository {
	...
	public Mono<Long> insert(Variant variant) {
		return db
				.sql("INSERT INTO video_storyboard_variant(storyboard_id, parent_storyboard_id, "
				...
				.bind("id", variant.storyboardId().toString()).bind("parent", variant.parentStoryboardId().toString())
				.bind("root", variant.rootStoryboardId().toString()).bind("account", variant.accountId())
				.bind("operation", variant.operationId().toString()).bind("hash", variant.requestHash())
				.bind("sourceEditVersion", variant.sourceEditVersion())
				.bind("sourceDraftVersion", variant.sourceDraftVersion()).bind("title", variant.title())
				.bind("shotMap", variant.shotIdMapJson()).fetch().rowsUpdated();
	}

	public Mono<Variant> findByStoryboard(UUID storyboardId) {
		return db
				.sql("SELECT " + COLS + " FROM video_storyboard_variant v "
						+ "LEFT JOIN video_storyboard_workspace w ON w.storyboard_id = v.storyboard_id "
						+ "WHERE v.storyboard_id=CAST(:id AS uuid)")
				.bind("id", storyboardId.toString()).map(VideoStoryboardVariantRepository::map).one();
	}
	...
	public Mono<Variant> findByAccountAndOperation(String accountId, UUID operationId) {
		return db
				...
				.map(VideoStoryboardVariantRepository::map).one();
	}
	...
	public Flux<Variant> findByRoot(UUID rootStoryboardId) {
		return db
				.sql("SELECT " + COLS + " FROM video_storyboard_variant v "
						+ "LEFT JOIN video_storyboard_workspace w ON w.storyboard_id = v.storyboard_id "
						+ "WHERE v.root_storyboard_id=CAST(:root AS uuid) " + "ORDER BY v.created_at, v.storyboard_id")
				.bind("root", rootStoryboardId.toString()).map(VideoStoryboardVariantRepository::map).all();
	}

	/** 根方案行的 draftId/createdAt（API-12 根行摘要；根分镜无绑定行时 draftId 为 null）。 */
	public Mono<RootRow> findRootRow(UUID rootStoryboardId) {
		return db
				.sql("SELECT w.draft_id::text, s.created_at, d.title FROM video_storyboard s "
						+ "LEFT JOIN video_storyboard_workspace w ON w.storyboard_id = s.id "
						+ "LEFT JOIN creation_draft d ON d.id = w.draft_id " + "WHERE s.id=CAST(:root AS uuid)")
				.bind("root", rootStoryboardId.toString())
				.map((row, meta) -> new RootRow(
						row.get("draft_id", String.class) == null
								? null
								: UUID.fromString(row.get("draft_id", String.class)),
						row.get("created_at", OffsetDateTime.class), row.get("title", String.class)))
				.one();
	}

	public record RootRow(UUID draftId, OffsetDateTime createdAt, String title) {
	}

	public Mono<Long> countByRoot(UUID rootStoryboardId) {
		return db.sql("SELECT COUNT(*) FROM video_storyboard_variant " + "WHERE root_storyboard_id=CAST(:root AS uuid)")
				.bind("root", rootStoryboardId.toString()).map((row, meta) -> row.get(0, Long.class)).one();
	}
	...
	private static Variant map(Row row, RowMetadata meta) {
		return new Variant(UUID.fromString(row.get("storyboard_id", String.class)),
				...
				UUID.fromString(row.get("root_storyboard_id", String.class)), row.get("account_id", String.class),
				UUID.fromString(row.get("operation_id", String.class)), row.get("request_hash", String.class).trim(),
				row.get("source_edit_version", Long.class), row.get("source_draft_version", Integer.class),
				row.get("title", String.class), row.get("shot_id_map", String.class),
