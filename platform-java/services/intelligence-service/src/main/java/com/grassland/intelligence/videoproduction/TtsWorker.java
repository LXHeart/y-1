 ...
 * provider submit/poll（跨领单周期，任务号落行）→ 音频归档私有存储（≤20MB、同 origin 校验） → cues 落行 →
 * settleSuccess。
 *
 * <p>
 * 不可达路径全部降级不阻断成片：旁白空 / TTS 渠道未配置 → {@code skipped}；预算闸拒绝 / provider 失败 → failed
 * + handleFailure（释放预留，卡8 合成按无配音继续）。 ExecutionContext 每 cycle 从行上句柄重建（video
 * worker 同款），重启不悬空 run。
 */
@Component
public class TtsWorker {
	...
	public Mono<Void> process(VideoShotAudio audio) {
		...
		return shots.findById(audio.shotId()).flatMap(
				shot -> storyboards.findById(shot.storyboardId()).flatMap(storyboard -> drive(audio, shot, storyboard)))
				.onErrorResume(error -> {
	...
	private Mono<Void> prepare(VideoShotAudio audio, VideoShot shot, String narration, VideoStoryboard storyboard,
				...
				.flatMap(result -> {
					...
					return audios
							...
							.then(execute(reload(audio, result.context().runId(), budget.budgetId(),
									budget.reservationDate(), budget.reservedCents()), shot, storyboard, narration,
									resolution));
				});
	}

	private Mono<Void> execute(VideoShotAudio audio, VideoShot shot, VideoStoryboard storyboard, String narration,
			VideoGenerationProviderResolver.TtsProviderResolution resolution) {
		...
		return Mono.usingWhen(concurrencyLimiter.acquire(toProviderResolution(resolution)),
				lease -> call.flatMap(result -> switch (result.state()) {
					case SUCCEEDED -> complete(audio, shot, storyboard, narration, resolution, result);
					case FAILED ->
						fail(audio, storyboard, result.errorCode() == null ? "tts_provider_failed" : result.errorCode(),
								result.errorMessage() == null ? "TTS provider 失败" : result.errorMessage());
					case QUEUED -> audios
							.updateProviderState(audio.id(), VideoShotAudio.STATUS_SUBMITTED, result.providerTaskId())
							.doOnSuccess(
									ignored -> events.emitAudioFor(storyboard, audio, VideoShotAudio.STATUS_SUBMITTED))
							.then();
					case PROCESSING -> audios
							.updateProviderState(audio.id(), VideoShotAudio.STATUS_PROCESSING, result.providerTaskId())
							.doOnSuccess(
									ignored -> events.emitAudioFor(storyboard, audio, VideoShotAudio.STATUS_PROCESSING))
							.then();
				}), PlatformConcurrencyLimiter.Lease::release, (lease, error) -> lease.release(),
				PlatformConcurrencyLimiter.Lease::release);
	}

	private Mono<Void> complete(VideoShotAudio audio, VideoShot shot, VideoStoryboard storyboard, String narration,
			VideoGenerationProviderResolver.TtsProviderResolution resolution, TtsProvider.TtsResult result) {
		return fetchAudio(audio, resolution, result).flatMap(audioBytes -> {
			long durationMs = result.durationMs() != null ? result.durationMs() : durationProbe.probe(audioBytes);
			...
			return archive(audio, storyboard, audioBytes, durationMs)
					.flatMap(reference -> audios.attachMedia(audio.id(), reference.id(), cues, (int) durationMs))
					.doOnSuccess(ignored -> {
						...
						log.info(
								...
								audio.id(), audio.provider(), durationMs, System.currentTimeMillis() - startedAt);
	...
	private Mono<byte[]> fetchAudio(VideoShotAudio audio,
		...
		if (result.audioUrl() == null || result.audioUrl().isBlank()) {
			...
			int durationMs = result.durationMs() == null ? SandboxTtsProvider.durationMsFor(null) : result.durationMs();
	...
	private Mono<MediaReference> archive(VideoShotAudio audio, VideoStoryboard storyboard, byte[] bytes,
			long durationMs) {
		...
		MediaReference reference = new MediaReference(mediaId, storyboard.accountId(), storyboard.organizationId(),
				MediaPurpose.SPEECH_AUDIO.db(), "video_shot_audio", audio.id().toString(), key, mime, bytes.length,
				VideoAssetArchiveService.VideoArchiveChecksums.sha256(bytes), "generated", MediaStatus.ACTIVE,
	...
	private Mono<Void> settle(VideoShotAudio audio, VideoStoryboard storyboard, int tokens) {
		return rebuildContext(audio, storyboard).flatMap(ctx -> executions.settleSuccess(ctx, tokens, 0, 0, 0).then())
				.onErrorResume(error -> {
	...
	private Mono<Void> skip(VideoShotAudio audio, VideoStoryboard storyboard, String reason) {
		return failRunIfBilled(audio, null, reason).then(audios.markSkipped(audio.id(), reason))
	...
	private Mono<Void> fail(VideoShotAudio audio, VideoStoryboard storyboard, String code, String message) {
		return failRunIfBilled(audio, storyboard, message).then(audios.markFailed(audio.id(), code, message))
				.doOnSuccess(ignored -> events.emitAudioFor(storyboard, audio, VideoShotAudio.STATUS_FAILED))
				.then(events.maybeEmitSelecting(storyboard));
	}

	private Mono<Void> failTerminal(VideoShotAudio audio, VideoStoryboard storyboard, String code, String message) {
	...
	private Mono<AiExecutionService.ExecutionContext> rebuildContext(VideoShotAudio audio, VideoStoryboard storyboard) {
		if (audio.runId() == null || storyboard == null) {
			return Mono.empty();
		}
		return runs.findById(audio.runId()).flatMap(run -> {
			var resolution = ProviderResolution.platform(null, audio.provider(), null, audio.model(), 0, null);
			var budget = ModelBudgetService.BudgetCheckResult.allowed(audio.budgetId(), audio.budgetReservationDate(),
					0, audio.reservedCents() == null ? 0 : audio.reservedCents());
			return Mono.just(new AiExecutionService.ExecutionContext(audio.runId(), storyboard.organizationId(),
					storyboard.accountId(), VideoGenerationProviderResolver.CAPABILITY_VIDEO_TTS, resolution, budget,
					audio.id(), null, null, false, null, null, 0, 0, run.creditsCentsPolicyVersion()));
		});
	}
	...
	private static ProviderResolution toProviderResolution(
			VideoGenerationProviderResolver.TtsProviderResolution resolution) {
		return ProviderResolution.platform(resolution.configId(), resolution.provider(), resolution.baseUrl(),
				resolution.model(), resolution.platformModelVersion(), resolution.maxConcurrency());
	}
	...
	private static VideoShotAudio reload(VideoShotAudio audio, UUID runId, UUID budgetId,
		...
		return new VideoShotAudio(audio.id(), audio.shotId(), audio.provider(), audio.model(), audio.providerTaskId(),
				audio.status(), audio.attempts(), audio.mediaId(), audio.cues(), audio.durationMs(), runId, budgetId,
				reservationDate, reservedCents, audio.errorCode(), audio.errorMessage(), audio.nextAttemptAt(),
				audio.claimedUntil(), audio.claimToken(), audio.createdAt(), audio.updatedAt(), audio.completedAt());
	}
}
