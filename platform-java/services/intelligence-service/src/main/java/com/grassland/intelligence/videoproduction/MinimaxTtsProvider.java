...
/**
 * MiniMax T2A 异步合成适配器（任务书 #64 卡5，P1）：submit（model/voice/text）→ taskId →
 * poll → 音频 file URL。契约形态对齐 {@link MinimaxVideoGenerationProvider} 的 file_id/URL 取法
 * （成功无直链时经 /v1/files/retrieve 兜底）；按控制面解析的 {@link VideoProviderEndpoint} 构造。
 *
 * <p>返回的字级/句级时间戳（若上游给）映射为 cues 的职责在 worker（TtsCues）；本类只搬运协议。
 */
public class MinimaxTtsProvider implements TtsProvider {
    ...
    private static final String CREATE_PATH = "/v1/t2a_v2_async";
    private static final String POLL_PATH = "/v1/query/t2a_v2_async";
        ...
        return client().post().uri(endpoint.createPath()).bodyValue(payload)
                ...
                .map(node -> {
                    ...
                    if (taskId == null) {
                        ...
                        String fileId = VideoProviderJson.text(node, "/data/audio/file_id",
                                "/audio/file_id", "/file_id");
                        if (fileId == null) {
                            throw new IllegalStateException("MiniMax TTS 创建任务响应缺少 task_id/file_id");
                        }
                        return retrieve(fileId).map(url -> new TtsResult(
                                TtsResult.State.SUCCEEDED, taskIdOf(fileId), url, null, null, null));
                    }
    ...
    private Mono<TtsResult> mapStatus(JsonNode node, String taskId) {
        ...
        if (List.of("fail", "failed", "error", "cancelled", "canceled").contains(status)) {
            return Mono.just(failed(taskId,
        ...
        return Mono.just(new TtsResult(
                queued ? TtsResult.State.QUEUED : TtsResult.State.PROCESSING,
    ...
    private static TtsResult failed(String taskId, String code, String message) {
        return new TtsResult(TtsResult.State.FAILED, taskId, null, null,
