 ...
 *
 * <p>
 * 用法复刻 {@code OutboxRepository} / {@code StylePreferencesRepository}：注入
 * {@link DatabaseClient}， timestamptz 读为 {@link OffsetDateTime} 再转
 * {@link java.time.Instant}，uuid 经 {@code ::text}/{@code CAST}， 可空字段用
 * {@code bindNull}。
 */
@Component
public class MediaReferenceRepository {
	...
	public Mono<MediaReference> insert(MediaReference ref) {
		DatabaseClient.GenericExecuteSpec spec = db.sql("""
				...
				""").bind("id", ref.id().toString()).bind("ownerAccountId", ref.ownerAccountId())
				.bind("purpose", ref.purpose()).bind("objectKey", ref.objectKey()).bind("mimeType", ref.mimeType())
				.bind("sizeBytes", ref.sizeBytes()).bind("source", ref.source()).bind("status", ref.status().db());
	...
	public Mono<MediaReference> claimFinalize(UUID id) {
		return db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id.toString()).map(MediaReferenceRepository::map).one();
	}

	/** finalizing→active，补全校验元数据；保留 upload key 名称，供删除/GC 清理由旧 PUT URL 重建的临时对象。 */
	public Mono<MediaReference> completeFinalize(UUID id, String mimeType, long sizeBytes, String checksum) {
		return db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id.toString()).bind("mimeType", mimeType)
				.bind("sizeBytes", sizeBytes).bind("checksum", checksum).map(MediaReferenceRepository::map).one();
	}
	...
	public Mono<MediaReference> findById(UUID id) {
		return db.sql("SELECT " + SELECT_COLS + " FROM media_reference WHERE id=CAST(:id AS uuid)")
				.bind("id", id.toString()).map(MediaReferenceRepository::map).one();
	}
	/** 任务书 #54：系列图卡持久化幂等——按 object_key 查活跃行（永久 key 唯一映射原卡）。 */
	public Mono<MediaReference> findByObjectKey(String objectKey) {
		return db
				.sql("SELECT " + SELECT_COLS + " FROM media_reference WHERE object_key=:objectKey "
						+ "AND status='active' ORDER BY created_at LIMIT 1")
				.bind("objectKey", objectKey).map(MediaReferenceRepository::map).one();
	}
	...
	public Mono<MediaReference> claimDelete(UUID id, String ownerAccountId) {
		return transactions.transactional(lockForLifecycle(id).flatMap(ignored -> db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", id.toString()).bind("ownerAccountId", ownerAccountId)
				.map(MediaReferenceRepository::map).one()));
	}

	/**
	 * 注销清理引用护栏（#104 §7.2）：exact object_key 仍在活动 manifest 中未完成判定（pending/failed）或
	 * 已按组织/共享引用保留 （retained）时，通用 GC 不得领取/物删，避免与注销 worker 竞争。 manifest
	 * completed/needs_review 终态后护栏自然解除 （retained 行由租约到期后的既有 GC 接管）。
	 * {@link #ERASURE_GUARD} 为可直接 AND 的排除形式。
	 */
	...
	public Mono<Boolean> erasureGuarded(String objectKey) {
		return db.sql("SELECT %s AS guarded FROM media_reference WHERE object_key = :k".formatted(ERASURE_GUARD_MATCH))
				.bind("k", objectKey).map(row -> Boolean.TRUE.equals(row.get("guarded", Boolean.class))).one()
				.defaultIfEmpty(false);
	}

	/**
	 * 与 KYB retention 写入统一锁顺序。锁取得后的下一条 UPDATE 使用 READ COMMITTED 新快照， 能看见等待期间提交的
	 * retention；单条 UPDATE 的固定语句快照做不到这一点。
	 */
	private Mono<Object> lockForLifecycle(UUID id) {
		return db.sql("SELECT id FROM media_reference WHERE id=CAST(:id AS uuid) FOR UPDATE").bind("id", id.toString())
				.map(row -> row.get(0)).one();
	}
	 ...
	 * 行锁串行化同 owner 的并发预留（MVCC 行锁，可靠），校验通过才递增计数并落 media 行。 两条语句（计数 UPSERT + media
	 * INSERT）在同一 data-modifying CTE 中原子执行。 返回 empty=配额已满（调用方应报 429）。
	 */
	public Mono<MediaReference> insertIfQuotaAllowed(MediaReference ref, long maxObjects, long maxTotalBytes) {
		DatabaseClient.GenericExecuteSpec spec = db.sql("""
				WITH seed AS (
				...
				FROM bump
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", ref.id().toString()).bind("ownerAccountId", ref.ownerAccountId())
				.bind("purpose", ref.purpose()).bind("objectKey", ref.objectKey()).bind("mimeType", ref.mimeType())
				.bind("sizeBytes", ref.sizeBytes()).bind("source", ref.source()).bind("status", ref.status().db())
				.bind("maxObjects", maxObjects).bind("maxTotalBytes", maxTotalBytes);
		spec = bindNullableString(spec, "uploadKey", ref.uploadKey());
		spec = bindNullableString(spec, "organizationId", ref.organizationId());
		spec = bindNullableString(spec, "domainType", ref.domainType());
		spec = bindNullableString(spec, "domainId", ref.domainId());
	 ...
	 * 「递减计数」。{@code quota_released} 保证每行 exactly-once 释放（GC 对 deleting 行重试不双扣）； 仅对
	 * {@code source='upload'} 的行递减（服务端生成/本地资产从未预留配额，见 {@code insert}）；
	...
	public Mono<Void> releaseQuota(UUID id) {
		return db.sql("""
				...
				""").bind("id", id.toString()).then();
	}

	/**
	 * owner 上传占用（与 media_owner_quota 计数同口径：仅 source='upload' 的
	 * pending/finalizing/active）。
	 */
	public Mono<OwnerUsage> usageByOwner(String ownerAccountId) {
		return db.sql("""
				...
				.map(row -> new OwnerUsage(value(row.get("object_count", Long.class), 0L),
	...
	public Flux<MediaReference> findCleanupCandidates(Duration pendingGrace) {
		return db.sql("""
				...
				.bind("pendingGraceMillis", Math.max(pendingGrace.toMillis(), 1L)).map(MediaReferenceRepository::map)
				.all();
	}

	/**
	 * 门店媒体批量换 URL 的四重过滤查询（#42 D5）+ 审核过滤（缺口清偿之五）：单条 SQL {@code id = ANY(:ids)} +
	 * purpose ∧ organization_id ∧ domain_type='store' ∧ domain_id=storeId ∧ active
	 * ∧ 未过期 ∧ 未被审核拦截（LEFT JOIN {@code store_media_moderation}，blocked 缺席），
	 * 仅返回通过过滤的子集（不逐项 findById）。过滤失败的 id 直接缺席，调用方据此实现子集语义。
	 */
	public Flux<MediaReference> findActiveStoreMedia(Collection<UUID> ids, String purpose, String organizationId,
			String storeId) {
		if (ids.isEmpty()) {
			return Flux.empty();
		}
		...
		return db.sql("""
				...
				""".formatted(columns)).bind("ids", ids.stream().map(UUID::toString).toArray(String[]::new))
				.bind("purpose", purpose).bind("organizationId", organizationId).bind("storeId", storeId)
	...
	public Flux<UUID> findAvailableIds(Collection<UUID> ids) {
		...
		return db.sql("""
				...
				.map((row, metadata) -> UUID.fromString(row.get("id", String.class))).all();
	}
	...
	private static MediaReference map(Readable row) {
		return new MediaReference(UUID.fromString(row.get("id", String.class)),
				row.get("owner_account_id", String.class), row.get("organization_id", String.class),
				row.get("purpose", String.class), row.get("domain_type", String.class),
				row.get("domain_id", String.class), row.get("object_key", String.class),
				row.get("upload_key", String.class), row.get("mime_type", String.class),
				value(row.get("size_bytes", Long.class), 0L), row.get("checksum", String.class),
				row.get("source", String.class), MediaStatus.fromDb(row.get("status", String.class)),
	...
	private static DatabaseClient.GenericExecuteSpec bindNullableOffsetDateTime(DatabaseClient.GenericExecuteSpec spec,
			String name, java.time.Instant value) {
		return value == null
