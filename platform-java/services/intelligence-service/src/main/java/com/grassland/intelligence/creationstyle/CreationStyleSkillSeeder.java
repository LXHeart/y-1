 ...
 * {@code /contracts/creation-style-skills.json} 全量插入（22 条 = 6 标题套路 + 9 体裁 + 7 文风）。
 *
 * <p>best-effort（照 {@code PlatformModelConfigSeeder} 姿态）：失败打 WARN 不阻断启动；
 * admin 后续经治理台 CRUD 修订，种子不回写文件。种子 JSON 解析用<b>自持</b> Jackson 实例
 * （intelligence 无 ObjectMapper bean，注入即炸整个上下文）。
 */
@Component
public class CreationStyleSkillSeeder {
    ...
    public void seedOnStartup() {
        try {
            ...
            for (SeedSkill seed : seeds) {
                inserts.add(repository.insertSeed(seed.category(), seed.code(), seed.name(),
                        seed.description(), seed.promptContent(), seed.sortOrder()));
            }
            reactor.core.publisher.Flux.concat(inserts).then().block(BLOCK);
            log.info("Seeded creation style skills: {} rows ({} title formulas / {} genres / {} styles)",
                    seeds.size(),
    ...
    private static List<SeedSkill> loadSeed() {
        try (var stream = CreationStyleSkillSeeder.class.getClassLoader()
            ...
            for (JsonNode node : root.path("skills")) {
                CreationStyleSkillCategory category = CreationStyleSkillCategory.fromKey(
                        node.path("category").asText(""));
                if (category == null) {
                    throw new IllegalStateException("creation-style-skills.json 未知 category: "
                            + node.path("category").asText());
                }
    ...
    private record SeedSkill(CreationStyleSkillCategory category, String code, String name,
