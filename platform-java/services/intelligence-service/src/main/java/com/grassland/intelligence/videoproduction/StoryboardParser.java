 ...
 *
 * <p>
 * 逐行解析（§4.1：逐行解析转发），容忍 LLM 对「每行一个 JSON」的常见偏离：代码围栏与空行、 前导/结尾散文行、编号列表前缀（{@code 1.
 * {...}}）、整段 JSON 数组、跨行 pretty-print 对象、 {@code {"shots":[...]}} 包裹——2026-09-03
 * MiniMax-M3 实跑即因偏离炸在逐行 readTree 上，故按 既有防御性姿态扩展（镜头字段校验仍严格）。seq 按到达顺序重编号（LLM 的
 * seq 不可信）； plannedSeconds 钳制到 4-6（§4.2 硬约束的防御性落地）；anchorImageIndex 超出 [0,
 * imageCount] 直接 400（卡3 验收项：图镜映射校验）。镜头数上限 30（#65 卡1；下界不硬拒——提示词层约束 3-30， LLM
 * 偶发短输出仍可进编辑步手工调整）。
 *
 * <p>
 * 可诊断性：跳过的偏离片段与终态失败都会 WARN 留原始输出（截断）——此前解析失败完全静默， ai_run/lineage
 * 均不落正文，线上无法定位是哪种污染。
 */
final class StoryboardParser {
	...
	record ParsedShot(int seq, String visual, String narration, int plannedSeconds, String cameraMove,
	...
	private static ParsedShot shotOf(JsonNode node, int seq, int imageCount) {
		...
		return new ParsedShot(seq, visual, node.path("narration").asText("").trim(),
				clamp(node.path("plannedSeconds").asInt(MIN_SHOT_SECONDS)),
				defaultCameraMove(node.path("cameraMove").asText("").trim()), anchor,
