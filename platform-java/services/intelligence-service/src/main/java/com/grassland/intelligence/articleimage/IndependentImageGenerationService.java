				...
				.usingWhen(Mono.just(context),
						ignored -> images.generate(command, owner, purpose, endpoint)
								.flatMap(response -> executions.settleSuccessWithCost(context, settleCents, 0, 0, 1, 0)
										.thenReturn(new Traced(response, context.runId(), context.provider().provider(),
												context.provider().model()))),
						ignored -> Mono.empty(),
						(ignored, error) -> executions
								.handleFailure(context,
										error.getMessage() == null ? "image generation failed" : error.getMessage())
								.then(),
