...
public final class MediaLifecycleEvents {
	...
	static EventEnvelope deleted(MediaReference ref, String reason) {
		return event(ref, "MediaDeleted", 3,
				Map.of("status", "deleted", "reason", reason == null ? "deleted" : reason));
	}

	private static EventEnvelope event(MediaReference ref, String eventType, long version, Map<String, Object> extra) {
		...
		return new EventEnvelope(ref.id() + ":" + eventType, eventType, "media_reference", ref.id().toString(), version,
				Instant.now(), ref.id().toString(), payload);
	}
