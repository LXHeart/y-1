...
public class SpeechTranscriptionRepository {
	...
	private static final String COLUMNS = """
			...
			segments_json::text, created_at, updated_at, completed_at
			""";
	...
	public Mono<SpeechTranscription> createProcessing(SpeechTranscription value) {
		if (value == null || value.id() == null || value.mediaReferenceId() == null || value.ownerAccountId() == null
				|| value.requestedLanguage() == null || value.durationMs() < 0) {
			return Mono.error(
					new IllegalArgumentException("A processing transcription requires identity and media fields"));
		}
		return db
				.sql("""
						...
						"""
						.formatted(COLUMNS))
				.bind("id", value.id().toString()).bind("mediaReferenceId", value.mediaReferenceId().toString())
				.bind("ownerAccountId", value.ownerAccountId())
				.bind("organizationId", nullable(value.organizationId(), String.class))
				.bind("requestedLanguage", value.requestedLanguage()).bind("durationMs", value.durationMs())
				...
				.map(SpeechTranscriptionRepository::map).one();
	}

	public Mono<SpeechTranscription> findOwned(UUID id, String ownerAccountId) {
		return db.sql("SELECT " + COLUMNS
				+ " FROM speech_transcription WHERE id = CAST(:id AS uuid) AND owner_account_id = :ownerAccountId")
				.bind("id", id.toString()).bind("ownerAccountId", ownerAccountId)
				.map(SpeechTranscriptionRepository::map).one();
	}
	...
	public Flux<SpeechTranscription> findRecentOwned(String ownerAccountId, int limit) {
		return db.sql("SELECT " + COLUMNS
				+ " FROM speech_transcription WHERE owner_account_id = :ownerAccountId ORDER BY created_at DESC LIMIT :limit")
				.bind("ownerAccountId", ownerAccountId).bind("limit", limit).map(SpeechTranscriptionRepository::map)
				.all();
	}

	public Mono<Boolean> storeProviderResult(UUID id, String text, String detectedLanguage, String provider,
			String model, Integer platformModelVersion, UUID runId,
			java.util.List<SpeechRecognitionProvider.Segment> segments) {
		if (text == null || provider == null || model == null || runId == null) {
			return Mono
					.error(new IllegalArgumentException("A provider result requires text, provider, model and run id"));
		}
		return changed(db.sql("""
				...
				SET transcript_text = :text, detected_language = :detectedLanguage, provider = :provider,
				    ...
				    ai_run_id = CAST(:runId AS uuid), failure_code = NULL,
				    segments_json = CAST(:segmentsJson AS jsonb), updated_at = now()
				WHERE id = CAST(:id AS uuid) AND status = 'processing'
				""").bind("id", id.toString()).bind("text", text)
				.bind("detectedLanguage", nullable(detectedLanguage, String.class)).bind("provider", provider)
				.bind("model", model).bind("platformModelVersion", nullable(platformModelVersion, Integer.class))
				.bind("runId", runId.toString()).bind("segmentsJson", nullable(segmentsJson(segments), String.class)));
	}

	private static final com.fasterxml.jackson.databind.ObjectMapper MAPPER = new com.fasterxml.jackson.databind.ObjectMapper();

	private static String segmentsJson(java.util.List<SpeechRecognitionProvider.Segment> segments) {
		if (segments == null || segments.isEmpty()) {
			return null;
		}
		try {
			var array = MAPPER.createArrayNode();
			for (SpeechRecognitionProvider.Segment segment : segments) {
			...
			throw new IllegalArgumentException("segments 序列化失败", error);
		}
	}

	private static java.util.List<SpeechRecognitionProvider.Segment> parseSegments(String json) {
		...
		try {
			...
			java.util.List<SpeechRecognitionProvider.Segment> parsed = new java.util.ArrayList<>();
			for (var item : root) {
				if (item.path("start").isNumber() && item.path("end").isNumber() && item.path("text").isTextual()) {
					parsed.add(new SpeechRecognitionProvider.Segment(item.path("start").asDouble(),
							item.path("end").asDouble(), item.path("text").asText()));
				}
			}
	...
	public Mono<Boolean> markFailed(UUID id, String failureCode) {
		return changed(db.sql("""
				UPDATE speech_transcription
				SET status = 'failed', failure_code = :failureCode, transcript_text = NULL, segments_json = NULL,
	...
	private static SpeechTranscription map(Readable row) {
		return new SpeechTranscription(uuid(row.get("id", String.class)),
				uuid(row.get("media_reference_id", String.class)), row.get("owner_account_id", String.class),
				row.get("organization_id", String.class), row.get("requested_language", String.class),
				row.get("detected_language", String.class), value(row.get("duration_ms", Long.class), 0L),
				row.get("status", String.class), row.get("transcript_text", String.class),
				row.get("provider", String.class), row.get("model", String.class),
				row.get("platform_model_version", Integer.class), uuid(row.get("ai_run_id", String.class)),
				row.get("failure_code", String.class), parseSegments(row.get("segments_json", String.class)),
