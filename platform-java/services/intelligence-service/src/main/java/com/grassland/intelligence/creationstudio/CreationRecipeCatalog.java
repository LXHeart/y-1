 ...
 *
 * <p>
 * {@link #resolve} 必须返回唯一合法定义或受控 400，不能回退到第一个模板； enabled=false
 * 的模板不出现在可见集合，也不能参与生成（后续卡按里程碑启用）。 公开响应不携带 prompt——契约本身不含内部提示词。
 */
public final class CreationRecipeCatalog {
	...
	public record RecipeDefinition(String id, String version, String label, boolean enabled, List<String> platformIds,
			List<String> contentForms, List<String> processingModes, int minItems, int maxItems, String defaultAspect,
			List<String> supportedStrategies) {

		public boolean applies(String platform, String contentForm, String processingMode) {
			return platformIds.contains(platform) && contentForms.contains(contentForm)
	...
	public static RecipeDefinition resolve(String id, String version, String platform, String contentForm,
		...
		if (!recipe.applies(platform, contentForm, processingMode)) {
			throw new IntelligenceException(400, "STUDIO_INVALID_INPUT", "模板「" + recipe.label() + "」不适用于当前平台／加工方式");
		}
		return recipe;
	}
