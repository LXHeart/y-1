 ...
 *
 * <p>迁入既有 12 风格 × 8 布局 × 3 配色 + 6 preset（全部旧 ID 保留）。服务端经
 * processResources 加载同一契约，禁止再维护一份手工 Java 样式表。palette.backgroundHex
 * 仅用于生成交付图补边，属于内容数据，不作为应用 UI 颜色。
 */
    ...
    private static Contract loadContract() {
        try (InputStream stream = CreationVisualPresetCatalog.class.getClassLoader()
            ...
            for (JsonNode node : root.path("styles")) {
                styles.add(new Style(node.path("id").asText(), node.path("label").asText(),
