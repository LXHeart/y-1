...
/**
 * Generates permanent public image assets, one isolated budgeted run per
 * requested item.
 */
@Service
public class PublicAssetBatchGenerationService {
	...
	public Mono<BatchResult> generate(String accountId, Command command) {
		return Flux.range(1, command.count())
				.concatMap(index -> generateOne(accountId, command, index).map(asset -> Item.success(index, asset.id()))
						.onErrorResume(error -> Mono.just(Item.failure(index, publicReason(error)))))
				.collectList().map(items -> new BatchResult(items, (int) items.stream().filter(Item::ok).count()));
	}
	...
	private Mono<ContentAsset> persist(String accountId, Command command, int index, GeneratedImage generated) {
		ObjectStorageAdapter storage = storageProvider.getIfAvailable();
		if (storage == null)
			return Mono.error(new IntelligenceException(503, "公共素材生成需要启用对象存储"));
		byte[] bytes;
		try {
		...
		MediaReference reference = new MediaReference(mediaId, accountId, null, MediaPurpose.CONTENT_ASSET.db(),
				"public_asset_generation", null, objectKey, "image/png", bytes.length, MediaChecksums.sha256(bytes),
				"generated", MediaStatus.ACTIVE, null, null, null);
		ContentAsset asset = new ContentAsset(UUID.randomUUID(), mediaId, LibraryType.PUBLIC, category(command.kind()),
				accountId, null, title(command, index), tags(command), "image/png", (long) bytes.length,
				command.validUntil(), AssetStatus.PENDING_REVIEW, 1, "platform_ai", "platform_authorized", null, null,
				null, null, null, null);
		return Mono.fromRunnable(() -> storage.putObject(objectKey, bytes, "image/png"))
				.subscribeOn(Schedulers.boundedElastic())
				.then(transactions.transactional(media.insert(reference).then(assets.create(asset))
						.flatMap(created -> outbox.append(ContentAssetController
								.assetEvent("ContentAssetSubmittedForReview", created, accountId, null, null))
								.thenReturn(created))));
	}
	...
	private static String prompt(Command command) {
		...
		return "生成公共内容素材。类型要求：" + subject + "。主题：" + command.theme() + "。视觉风格：" + style + "。不得出现品牌标识、文字、水印或受版权保护的角色。";
	}
	...
	private static AssetCategory category(Kind kind) {
		return kind == Kind.BACKGROUND || kind == Kind.MOOD ? AssetCategory.SCENE : AssetCategory.OTHER;
	}
	...
	public record Command(Kind kind, String theme, String style, int count, Instant validUntil) {
	}

	public enum Kind {
		ICON("icon", "图标"), DECORATION("decoration", "装饰"), BACKGROUND("background", "背景"), MOOD("mood", "氛围图");
		private final String db;
		private final String label;
		Kind(String db, String label) {
			this.db = db;
			this.label = label;
		}
		public String db() {
	...
	public record Item(int index, boolean ok, UUID assetId, String errorReason) {
		static Item success(int index, UUID assetId) {
			return new Item(index, true, assetId, null);
		}
		static Item failure(int index, String error) {
