 ...
 *
 * <p>
 * 核心是**掩码感知 merge**：前端永远拿到 masked 密钥（****xxxx），PUT 回来时：
 * <ul>
 * <li>字段缺失 → 保留当前值</li>
 ...
 * <li>明文 → 更新</li>
 * </ul>
 * 非密钥字段直接覆盖。复刻 legacy {@code analysis-settings.service.ts} 的
 * resolveUpdatedSecret/merge 逻辑。
 *
 * <p>
 * 2026-09 任务书 #88：{@code features} 已退役——请求携带 features 整键忽略（WARN 观测）、
...
public class AnalysisSettingsService {
	...
	public Mono<Map<String, Object>> get(String accountId) {
		return repo.findByAccountAndType(accountId, SETTINGS_TYPE).map(this::parseJson)
				.map(json -> SettingsSchemaGuard.normalize(SETTINGS_TYPE, json))
				.filter(normalized -> !normalized.isEmpty()).defaultIfEmpty(defaultAnalysisSettings())
	 ...
	 *
	 * <p>
	 * 任务书 #88：请求体 {@code features} 整键忽略（含坏值——不再 400，记一条 WARN 观测 sunset 遥测， WARN 只含
	 * accountId 不含任何 features 内容）；写路径把存量行中的 {@code features} deepCopy 后原样 放回待写
	 * JSON（preserve-on-write，回滚安全），响应映射永不含该键。8KB 校验作用于实际写库串。
	 */
	public Mono<Map<String, Object>> update(String accountId, Map<String, Object> partial) {
		...
		return repo.findByAccountAndType(accountId, SETTINGS_TYPE).map(this::parseJson).defaultIfEmpty(Map.of())
				.flatMap(raw -> {
					...
					Map<String, Object> merged = SettingsSchemaGuard.normalize(SETTINGS_TYPE,
							mergeAnalysisSettings(current, partial == null ? Map.of() : partial));
					...
					return repo.upsert(accountId, SETTINGS_TYPE, serialized).thenReturn(maskSecrets(merged));
				});
	}
	...
	/**
	 * mask 感知 merge：partial 覆盖 current，但密钥字段遵守掩码语义。 任务书 #88：请求体 {@code features}
	 * 整键忽略——返回值不含该键。
	 */
	@SuppressWarnings("unchecked")
	static Map<String, Object> mergeAnalysisSettings(Map<String, Object> current, Map<String, Object> partial) {
		...
		if (partialIntegrations.containsKey("feishu")) {
			Map<String, Object> currentIntegrations = (Map<String, Object>) result.getOrDefault("integrations",
					Map.of());
