...
/**
 * 组织级 BYOK 密钥管理（ADR-D17）集成测试：admin/owner 全 CRUD、member 与跨组织 404 隐藏、
 * 组织维唯一 409、密钥永不回显（只回掩码）。KEK fail-closed 条件与个人版同款。
 */
...
class AiOrgProviderKeyControllerIT extends IntelligenceItSupport {
    ...
    private static final String API_KEY = "sk-org-real-key-1234567890abcdef";
    ...
    private String createOrgKey(String capability) {
        byte[] body = client().post()
                .uri("/api/ai/organizations/" + ORG + "/keys")
                ...
                .expectBody()
                .jsonPath("$.maskedHint").value(hint -> assertThat(String.valueOf(hint)).startsWith("sk-"))
    ...
    void adminFullLifecycle() {
        ...
        client().get().uri("/api/ai/organizations/" + ORG + "/keys")
                ...
                .jsonPath("$[0].organizationId").isEqualTo(ORG)
                .jsonPath("$[0].encryptedKey").doesNotExist();

        client().put().uri("/api/ai/organizations/" + ORG + "/keys/" + id)
                .header("X-Grassland-Identity", sign(ADMIN, "merchant"))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(Map.of("baseUrl", "https://api.openai.com", "model", "gpt-4o"))
                .exchange()
                .expectStatus().isOk()
                .expectBody()
        ...
        client().put().uri("/api/ai/organizations/" + ORG + "/keys/" + id + "/key")
                .header("X-Grassland-Identity", sign(ADMIN, "merchant"))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(Map.of("apiKey", "sk-org-rotated-9876543210fedcba"))
                .exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.maskedHint").value(hint -> assertThat(String.valueOf(hint)).startsWith("sk-"));
    ...
    void nonAdminAndForeignOrgHidden() {
        ...
        client().post().uri("/api/ai/organizations/" + ORG + "/keys")
                .header("X-Grassland-Identity", sign(MEMBER, "merchant"))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(Map.of("capability", "text", "provider", "openai-compatible",
                        "baseUrl", "https://api.openai.com", "apiKey", API_KEY))
                .exchange()
                .expectStatus().isNotFound();
