 ...
 *
 * <p>
 * TC-038 两项修改版本闸（任一版本变化/任一动作失败全回滚，只成功一次升 editVersion； planId 唯一结果重放）；TC-039
 * prepare-generation 只准备零媒体调用；TC-040 越界 update（未选中镜头）拒绝、跨账号 404。
 */
...
class CanvasAgentApplyIT extends IntelligenceItSupport {
	...
	void atomicEditApplyWithVersionGate() {
		Prepared p = prepare();
		String action = "{\"edit\":{\"actions\":[" + updateAction(p.shotIds.get(0), "新画面一") + ","
				+ updateAction(p.shotIds.get(1), "新画面二") + "]}}";
		UUID planId = seedReadyPlan(p, action, List.of("shot:" + p.shotIds.get(0), "shot:" + p.shotIds.get(1)));
		...
		UUID third = seedReadyPlan(p, action, List.of("shot:" + p.shotIds.get(0), "shot:" + p.shotIds.get(1)));
		db.sql("DELETE FROM video_shot WHERE id=CAST(:id AS uuid)").bind("id", p.shotIds.get(1).toString()).then()
	...
	void prepareOnlyAndScopeGuards() {
		Prepared p = prepare();
		UUID prepare = seedReadyPlan(p, "{\"prepare-generation\":{\"mode\":\"initial\",\"shotId\":null}}",
				List.of("shot:" + p.shotIds.get(0)));
		Map<String, Object> prepared = apply(prepare, 200);
		assertThat((Map<?, ?>) prepared.get("preparedGeneration")).hasSize(2);
		assertThat(((Map<?, ?>) prepared.get("preparedGeneration")).get("mode")).isEqualTo("initial");
		assertThat(((Map<?, ?>) prepared.get("preparedGeneration")).get("shotId")).isNull();
		// 零媒体调用：任务表无行、镜头状态不变
		...
		UUID cross = seedReadyPlan(p, "{\"edit\":{\"actions\":[" + updateAction(p.shotIds.get(1), "越界") + "]}}",
				List.of("shot:" + p.shotIds.get(0)));
		apply(cross, 502);
	...
	private Prepared prepare() {
		...
		for (int seq = 1; seq <= 3; seq++) {
			shotIds.add(UUID.fromString(db
					.sql("INSERT INTO video_shot(storyboard_id, seq, visual, "
							...
							+ "(CAST(:sb AS uuid), :seq, :visual, '旁白', 5, '固定机位', 1, 'p') " + "RETURNING id::text")
		...
		db.sql("INSERT INTO video_storyboard_workspace(storyboard_id, draft_id, account_id, operation_id, "
				...
				.bind("account", ACCOUNT).bind("hash", "c".repeat(64)).then().block(Duration.ofSeconds(5));
		db.sql("INSERT INTO creation_canvas_document(id, draft_id, account_id, schema_version, revision, "
				...
				.bind("document", "{\"schemaVersion\":1,\"storyboardId\":\"" + storyboardId + "\","
						...
						+ shotIds.get(0) + "\",\"label\":null,\"text\":null,\"x\":40,\"y\":0}," + "{\"id\":\"shot:"
						+ shotIds.get(1) + "\",\"kind\":\"shot\",\"refType\":\"shot\"," + "\"refId\":\""
						+ shotIds.get(1) + "\",\"label\":null,\"text\":null,\"x\":360,\"y\":0}],"
	...
	private UUID seedReadyPlan(Prepared p, String actionJson, List<String> selectedNodeIds) {
		return UUID.fromString(db
				.sql("INSERT INTO creation_canvas_agent_plan(id, account_id, "
				...
				.bind("account", ACCOUNT).bind("hash", "d".repeat(64)).bind("draft", p.draftId().toString())
				.bind("sb", p.storyboardId().toString()).bind("selected", toJsonArray(selectedNodeIds))
				.bind("action", actionJson).map(row -> row.get(0, String.class)).one().block(Duration.ofSeconds(5)));
	}
		...
		client().post().uri("/api/creation-assistant/canvas/plans/{id}/apply", planId)
				...
				.bodyValue(Map.of()).exchange().expectBody(String.class).consumeWith(result -> {
					org.assertj.core.api.Assertions.assertThat(result.getStatus().value())
							.as("apply 响应体=" + result.getResponseBody()).isEqualTo(status);
					try {
