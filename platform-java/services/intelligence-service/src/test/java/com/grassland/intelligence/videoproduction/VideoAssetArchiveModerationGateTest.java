 ...
 * {@code GeneratedMediaModerationGateTest}）：归档成功（对象存储 + media_reference + outbox
 * activated）后，以归档行（purpose=video_asset）与归档字节异步送审——advisory，不阻塞归档返回。
 */
...
class VideoAssetArchiveModerationGateTest {
    ...
    void setUp() {
        ...
        service = new VideoAssetArchiveService(mediaRefs, storageProvider, outbox, transactions,
    ...
    void moderationHookReceivesReferenceOnlyAfterPersistSucceeds() {
        ...
        service.archive(job, "https://provider.example/out.mp4")
    ...
    private static VideoGenerationJob job() {
        return new VideoGenerationJob(UUID.randomUUID(), "acct-1", "org-1", "idem-1", null, null,
                "sandbox", "sandbox-video-v1", null, "succeeded", 100, null,
                "https://provider.example/out.mp4", 5, null, null, null, 0, 0, null, null, null,
