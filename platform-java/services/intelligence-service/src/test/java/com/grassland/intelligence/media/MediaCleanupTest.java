...
class MediaCleanupTest {
	...
	void cleanupKeepsDeletingWhenObjectDeletionFailsSoNextRoundCanRetry() {
		...
		doThrow(new RuntimeException(sensitiveMessage)).when(storage).deleteObject(ref.objectKey());
		...
		ILoggingEvent warning = appender.list.stream()
				.filter(event -> event.getFormattedMessage().contains("media cleanup failed")).findFirst()
				.orElseThrow();
		assertThat(warning.getFormattedMessage())
				.contains(ref.id().toString(), "failureStage=candidate_cleanup", "exceptionType=RuntimeException")
	...
	void orphanCleanupFailureLogDoesNotExposeObjectKeyOrExceptionDetails() {
		...
		when(storage.listObjects("media-pending/"))
				.thenReturn(List.of(new StoredObject(orphanKey, 1L, null, null, Instant.now().minusSeconds(7200))));
		...
		ILoggingEvent warning = appender.list.stream()
				.filter(event -> event.getFormattedMessage().contains("temporary object cleanup failed")).findFirst()
				.orElseThrow();
		String expectedHash = sha256Prefix(orphanKey);
		assertThat(warning.getFormattedMessage()).contains("failureStage=orphan_storage_delete",
				"exceptionType=RuntimeException", "objectKeyHash=" + expectedHash)
	...
	void cleanupRoundFailureLogDoesNotExposeExceptionDetails() {
		...
		ILoggingEvent warning = appender.list.stream()
				.filter(event -> event.getFormattedMessage().contains("media cleanup round failed")).findFirst()
				.orElseThrow();
		assertThat(warning.getFormattedMessage())
	...
	private static MediaReference expired() {
		...
		return new MediaReference(id, "acct", null, MediaPurpose.USER_UPLOAD.db(), null, null,
				"media/user_upload/" + id, "media-pending/" + id, "image/png", 1L, "checksum", "upload",
				MediaStatus.ACTIVE, Instant.now().minusSeconds(7200), Instant.now().minusSeconds(3600), null);
	}

	private static MediaReference withStatus(MediaReference ref, MediaStatus status) {
		return new MediaReference(ref.id(), ref.ownerAccountId(), ref.organizationId(), ref.purpose(), ref.domainType(),
				ref.domainId(), ref.objectKey(), ref.uploadKey(), ref.mimeType(), ref.sizeBytes(), ref.checksum(),
				ref.source(), status, ref.createdAt(), ref.expiresAt(), ref.deletedAt());
	}
}
