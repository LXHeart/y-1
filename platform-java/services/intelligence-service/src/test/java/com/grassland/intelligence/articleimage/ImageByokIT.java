 ...
 * 零积分、端点来自密钥）、组织策略禁回退拒绝、密钥轮换后任务模式 409 fail-closed。 任务书 #58 决策
 * G：平台路径=控制面行+凭据（无行/无凭据密钥均 fail-closed，静态 env 回落已删）。
 */
...
class ImageByokIT extends IntelligenceItSupport {
	...
	void clean() {
		...
		when(generation.generate(anyString(), anyString(), any(), any())).thenReturn(
				Mono.just(new GeneratedImage(null, Base64.getEncoder().encodeToString("png-bytes".getBytes()), "风格锚")));
		for (String table : new String[]{"ai_provider_key", "ai_provider_preference", "ai_org_byok_policy",
				"platform_model_concurrency_slot", "platform_model_config", "intelligence_outbox",
				"ai_credit_compensation", "ai_run", "ai_model_budget", "creation_context_snapshot"}) {
	...
	void personalByokKeyDrivesIndependentGeneration() {
		...
		client().post().uri("/api/article-generation/generate-image")
				...
				.bodyValue(Map.of("prompt", "一张门店封面", "size", "1024x1024")).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.success").isEqualTo(true).jsonPath("$.data.imageUrl")
				.value(value -> assertThat(String.valueOf(value))
		...
		assertThat(audit).containsEntry("provider", BYOK_PROVIDER).containsEntry("model", BYOK_MODEL)
				.containsEntry("status", "completed").containsEntry("cost", 0);
	...
	void orgByokKeyDrivesMerchantGeneration() {
		...
		client().post().uri("/api/article-generation/generate-image")
				...
				.bodyValue(Map.of("prompt", "组织键出图", "size", "1024x1024")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.success").isEqualTo(true);

		ImageGenerationClient.Endpoint endpoint = capturedEndpoint();
		assertThat(endpoint.apiKey()).isEqualTo("sk-org-image");
		Map<String, Object> audit = latestRun();
		assertThat(audit).containsEntry("provider", BYOK_PROVIDER).containsEntry("cost", 0).containsEntry("byok_org",
				ORG);
	}
	...
	void orgPolicyDenyRejectsImageGeneration() {
		...
		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", signWithOrg(ACCOUNT, ORG)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "应被拒绝", "size", "1024x1024")).exchange().expectStatus().isForbidden()
				.expectBody().jsonPath("$.success").isEqualTo(false);

		verify(generation, org.mockito.Mockito.never()).generate(anyString(), anyString(), any(), any());
	}
	...
	void controlPlanePlatformModelDrivesGeneration() {
		...
		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "治理台模型出图", "size", "1024x1024")).exchange().expectStatus().isOk()
		...
		Map<String, Object> audit = latestRun();
		assertThat(audit).containsEntry("provider", "cp-image").containsEntry("model", "cp-image-model")
	...
	void noPlatformRowFailsClosed() {
		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "无平台模型应拒绝", "size", "1024x1792")).exchange().expectStatus().isEqualTo(503)
				.expectBody().jsonPath("$.success").isEqualTo(false);
	...
	void platformRowWithoutCredentialKeyFailsClosed() {
		...
		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "无凭据密钥应拒绝", "size", "1024x1024")).exchange().expectStatus().isEqualTo(503)
				.expectBody().jsonPath("$.success").isEqualTo(false);
	...
	void taskModeByokFreezesAndFailsClosedOnRotation() throws Exception {
		...
		String updatedAt = db.sql("SELECT updated_at FROM ai_provider_key WHERE id=CAST(:id AS uuid)").bind("id", keyId)
				.map(row -> row.get("updated_at", OffsetDateTime.class)).one().block().toInstant().toString();
		String snapshotId = seedByokSnapshot(keyId, updatedAt);

		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "任务出图", "size", "1024x1024", "taskMode", true, "contextSnapshotId",
		...
		Map<String, Object> audit = latestRun();
		assertThat(audit).containsEntry("provider", BYOK_PROVIDER).containsEntry("cost", 0).containsEntry("snapshot",
				snapshotId);
		...
		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "轮换后应拒绝", "size", "1024x1024", "taskMode", true, "contextSnapshotId",
						snapshotId, "targetPlatform", "xiaohongshu"))
				.exchange().expectStatus().isEqualTo(409).expectBody().jsonPath("$.success").isEqualTo(false);
	}
	...
	void platformRunGatedByOrgBudget() {
		...
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_cents_per_run, enabled) "
				+ "VALUES (:org, 'image_generation', 'platform', 1, true)").bind("org", ORG).then().block();

		client().post().uri("/api/article-generation/generate-image")
				.header("X-Grassland-Identity", signWithOrg(ACCOUNT, ORG)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("prompt", "预算内应拒绝", "size", "1024x1024")).exchange().expectStatus().isEqualTo(402)
				.expectBody().jsonPath("$.success").isEqualTo(false);
	...
	private ImageGenerationClient.Endpoint capturedEndpoint() {
		ArgumentCaptor<ImageGenerationClient.Endpoint> captor = ArgumentCaptor
				.forClass(ImageGenerationClient.Endpoint.class);
		verify(generation).generate(anyString(), anyString(), captor.capture(), any());
	...
	private String seedByokSnapshot(String keyId, String updatedAt) {
		return db.sql("""
				...
				.bind("application", UUID.randomUUID().toString()).bind("keyId", keyId).bind("provider", BYOK_PROVIDER)
				.bind("model", BYOK_MODEL).bind("updatedAt", updatedAt).map(row -> row.get("id", String.class)).one()
				.block();
	}
}
