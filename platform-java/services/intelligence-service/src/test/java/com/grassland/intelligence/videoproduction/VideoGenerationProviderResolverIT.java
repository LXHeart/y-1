...
/**
 * 任务书 #64 卡2：capabilities 动态化——video_generation / video_tts 经控制面解析，
 * 无行 = slideshow / 无配音；凭据解密失败与缺价目 fail-closed。
 *
 * <p>价目侧刻意用兜底种子里的既有模型（qwen-plus 单秒 30 分；sandbox-video-v1=1、
...
class VideoGenerationProviderResolverIT extends IntelligenceItSupport {
    ...
    void noControlPlaneRowMeansSlideshowAndNoTts() {
        client().get().uri("/api/video-production/capabilities").exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.mode").isEqualTo("slideshow")
                .jsonPath("$.video.available").isEqualTo(false)
                .jsonPath("$.video.reason").isEqualTo("未配置视频生成模型")
                .jsonPath("$.tts.available").isEqualTo(false)
                .jsonPath("$.tts.reason").isEqualTo("配音模型未配置")
                // 旧契约兼容镜像（卡4 前端改造完成后移除）
                .jsonPath("$.available").isEqualTo(false)
                .jsonPath("$.reason").isEqualTo("未配置视频生成模型");
    }
    ...
    void pricedSeedanceRowMakesVideoModeAvailable() {
        ...
        client().get().uri("/api/video-production/capabilities").exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.mode").isEqualTo("video")
                .jsonPath("$.video.available").isEqualTo(true)
                .jsonPath("$.video.provider").isEqualTo("seedance")
                .jsonPath("$.video.model").isEqualTo("qwen-plus")
                .jsonPath("$.video.unitPriceCents").isEqualTo(30)
                .jsonPath("$.tts.available").isEqualTo(false);
    }
    ...
    void sandboxVideoRowIsPricedAtOneCentPerSecond() {
        ...
        client().get().uri("/api/video-production/capabilities").exchange()
                ...
                .expectBody()
                .jsonPath("$.mode").isEqualTo("video")
                .jsonPath("$.video.provider").isEqualTo("sandbox")
    ...
    void vendorModelWithoutCredentialKeyFailsClosed() {
        ...
        client().get().uri("/api/video-production/capabilities").exchange()
                ...
                .expectBody()
                .jsonPath("$.video.available").isEqualTo(false)
                .jsonPath("$.video.reason").isEqualTo("平台凭据缺失：该能力的凭据未配置密钥");
    }
    ...
    void sandboxTtsRowMakesVoiceoverAvailable() {
        ...
        client().get().uri("/api/video-production/capabilities").exchange()
                .expectStatus().isOk()
                .expectBody()
                .jsonPath("$.tts.available").isEqualTo(true)
                .jsonPath("$.tts.model").isEqualTo("sandbox-tts-v1")
    ...
    void resolverExposesAdapterForTaskCreation() {
        ...
        org.assertj.core.api.Assertions.assertThat(resolution.plan().resolution().provider())
                .isEqualTo("seedance");
        org.assertj.core.api.Assertions.assertThat(resolution.plan().resolution().baseUrl())
                .isEqualTo(QWEN.baseUrl());
    }

    /** 带（可解密的）密钥凭据 + capability 行；withKey=false 时凭据无密钥。 */
    private void seedVideoModel(String provider, String model, boolean withKey) {
        String encrypted = withKey
                ? encryptionProvider.getIfAvailable().encrypt("sk-it-video-key")
                : null;
