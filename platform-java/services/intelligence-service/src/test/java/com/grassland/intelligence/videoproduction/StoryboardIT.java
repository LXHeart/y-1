...
/**
 * 任务书 #64 卡3：结构化分镜 SSE——NDJSON 上游逐镜转发、锚定图映射校验、时长步进校验、 L1
 * 命中出安全帧、storyboard/shots 落库、lineage 落 video_storyboard。
 */
...
class StoryboardIT extends IntelligenceItSupport {
	...
	void cleanAndSeed() {
		...
		db.sql("DELETE FROM video_shot_take").then().then(db.sql("DELETE FROM video_shot_audio").then())
				...
				.then(db.sql("DELETE FROM ai_run").then()).block(java.time.Duration.ofSeconds(10));
		db.sql("DELETE FROM platform_model_concurrency_slot WHERE config_id IN (SELECT id FROM platform_model_config WHERE capability='text')")
				.then().then(db.sql("DELETE FROM platform_model_config WHERE capability='text'").then()).then(db.sql("""
						...
						""").bind("baseUrl", QWEN.baseUrl()).then()).block(java.time.Duration.ofSeconds(10));
		QWEN.resetAll();
	...
	void streamsShotsAndPersistsStoryboard() {
		String content = String.join("\n",
				List.of("{\"seq\":1,\"visual\":\"招牌特写\",\"narration\":\"老王面馆，现熬骨汤\",\"plannedSeconds\":5,"
		...
		String body = client().post().uri("/api/video-production/storyboard")
				...
				.bodyValue(requestBody(30, 2)).exchange().expectStatus().isOk().expectHeader()
		...
		String storyboardId = db
				.sql("SELECT id::text FROM video_storyboard "
		...
		List<String> shots = db
				.sql("SELECT seq || ':' || planned_seconds || ':' || anchor_image_index AS shot_row "
		...
		String lineage = db
				.sql("SELECT kind || ':' || result::text AS lineage_row FROM creation_generation "
		...
		QWEN.verify(1, postRequestedFor(urlEqualTo("/chat/completions")).withRequestBody(containing("短视频分镜导演"))
	...
	void lexiconHitSurfacesSafetyFrame() {
		...
		String body = client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(15, 1)).exchange().expectStatus().isOk().expectBody(String.class).returnResult()
				.getResponseBody();
	...
	void anchorImageIndexOutOfRangeRejected() {
		...
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(20, 2)).exchange().expectStatus().isBadRequest().expectBody().jsonPath("$.error")
				.isEqualTo("分镜锚定图序号超出范围");
	}
	...
	void durationMustStepByFive() {
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(17, 1)).exchange().expectStatus().isBadRequest();
	}
	...
	void bilibili180sDefaultsToLandscape() {
		...
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(180, 1, "bilibili", null)).exchange().expectStatus().isOk();
		QWEN.verify(1, postRequestedFor(urlEqualTo("/chat/completions")).withRequestBody(containing("目标总时长：180 秒"))
				.withRequestBody(containing("镜头数 3–30")));

		String resolution = db
				.sql("SELECT resolution FROM video_storyboard "
						+ "WHERE account_id=:account ORDER BY created_at DESC LIMIT 1")
	...
	void nonBilibiliDefaultsPortraitAndResolutionIsWhitelisted() {
		...
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(180, 1, "douyin", null)).exchange().expectStatus().isOk();
		String resolution = db
				.sql("SELECT resolution FROM video_storyboard "
						+ "WHERE account_id=:account ORDER BY created_at DESC LIMIT 1")
				.bind("account", ACCOUNT).map(row -> row.get("resolution", String.class)).one().block();
		...
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(30, 1, "douyin", "720x1280")).exchange().expectStatus().isBadRequest()
	...
	void durationAndShotCountUpperBounds() {
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(185, 1, "douyin", null)).exchange().expectStatus().isBadRequest().expectBody()
				.jsonPath("$.error").isEqualTo("成片时长须为 15-180 秒且按 5 秒步进");

		stubCompletion(shotLines(31));
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(180, 1, "douyin", null)).exchange().expectStatus().isBadRequest().expectBody()
				.jsonPath("$.error").isEqualTo("分镜镜头数须在 1-30 之间");
	}

	@Test
	@DisplayName("未登录 → 401 且不触发上游调用")
	void unauthenticatedRejected() {
		client().post().uri("/api/video-production/storyboard").contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(15, 1)).exchange().expectStatus().isUnauthorized();
	...
	void referenceStructureInjectedPerContract() {
		...
		body.put("referenceShotStructure",
				Map.of("shotStructure",
						List.of(Map.of("durationSeconds", 5, "purpose", "hook"),
								Map.of("durationSeconds", 4, "purpose", "point"),
								Map.of("durationSeconds", 6, "purpose", "cta")),
						"hookAtSeconds", 0));
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT_E1, "recommender")).contentType(MediaType.APPLICATION_JSON)
		...
		QWEN.verify(1,
				postRequestedFor(urlEqualTo("/chat/completions"))
						...
						.withRequestBody(containing("镜头时长序列 [5, 4, 6] 秒")).withRequestBody(containing("开场钩子位于第 0 秒")));
	}
	...
	void withoutReferenceStructureZeroChange() {
		...
		client().post().uri("/api/video-production/storyboard")
				.header("X-Grassland-Identity", sign(ACCOUNT_E1, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(requestBody(20, 1)).exchange().expectStatus().isOk();
		QWEN.verify(1, postRequestedFor(urlEqualTo("/chat/completions")).withRequestBody(containing("目标总时长：20 秒"))
				.withRequestBody(com.github.tomakehurst.wiremock.client.WireMock.not(containing("参考结构"))));
	}
	...
	void toleratesDeviatedUpstreamOutput() {
		...
		String body = client().post().uri("/api/video-production/storyboard")
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(requestBody(20, 1)).exchange().expectStatus().isOk()
	...
	void scriptModeWithoutStorePhotos() {
		...
		body.put("shopName", "");
		body.put("script",
				"开场直接说明问题：新手第一次组装电脑最容易忽略的三件事。" + "第一，电源功率要按整机功耗留余量；第二，内存插槽按说明书双通道站位；" + "第三，机箱风道前进后出。最后给出预算分配建议。");
		...
		QWEN.verify(1,
				postRequestedFor(urlEqualTo("/chat/completions")).withRequestBody(containing("已有脚本"))
						.withRequestBody(containing("新手第一次组装电脑")).withRequestBody(containing("B站适配"))
	...
	private static Map<String, Object> requestBody(int targetDurationSeconds, int imageCount, String targetPlatform,
			String resolution) {
	...
	private static String shotLines(int count) {
		...
		for (int seq = 1; seq <= count; seq++) {
			...
			lines.append("{\"seq\":").append(seq).append(",\"visual\":\"画面").append(seq)
