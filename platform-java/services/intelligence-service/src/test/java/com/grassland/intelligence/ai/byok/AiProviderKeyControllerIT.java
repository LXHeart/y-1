 ...
 * <p>测试完整的 HTTP 请求/响应流程，使用真实 testcontainers PostgreSQL 数据库。
 */
...
class AiProviderKeyControllerIT extends IntelligenceItSupport {
    ...
    @Test
    @DisplayName("POST /api/ai/keys - 创建 BYOK 密钥成功返回 201")
    ...
    @Test
    @DisplayName("POST /api/ai/keys - 创建个人 BYOK 密钥（organizationId 为 null）")
    void createPersonalKey_withoutOrgId() {
        client().post()
                ...
                .expectBody()
                .jsonPath("$.organizationId").isNull()
    ...
    void listKeys_returnsUserKeys() {
        // 先创建一个密钥
        UUID createdId = createTestKey(TEST_ORG_ID, "text", "openai-compatible");
    ...
    void deleteKey_softDeletes() {
        ...
        Integer enabled = db.sql("SELECT enabled FROM ai_provider_key WHERE id = CAST(:id AS uuid)")
                .bind("id", createdId.toString())
                .map(r -> r.get("enabled", Boolean.class))
                .one()
                .map(e -> e ? 1 : 0)
                .block();
    ...
    void createDuplicateKey_returns409() {
        createTestKey(TEST_ORG_ID, "text", "openai-compatible");
        ...
        client().post()
                ...
                .exchange()
                .expectStatus().isEqualTo(409);  // UNIQUE constraint violation
    }
     ...
     * 辅助方法：创建测试密钥并返回 ID
     */
    private UUID createTestKey(String orgId, String capability, String provider) {
        return client().post()
                .uri("/api/ai/keys")
                .header("X-Grassland-Identity", sign(TEST_ACCOUNT_ID, "recommender"))
                .contentType(MediaType.APPLICATION_JSON)
