...
class OutboxRepositoryIT extends IntelligenceItSupport {
    ...
    void claimBatch_claimsOnlyDueRows_andReclaimsOnlyExpiredLeases() {
        ...
        db.sql("UPDATE intelligence_outbox SET next_attempt_at = now() + interval '60 seconds' WHERE event_id = 'later'")
                .then().block();
        ...
        db.sql("UPDATE intelligence_outbox SET claimed_until = now() - interval '1 second' WHERE event_id = 'due'")
                .then().block();
    ...
    void conditionalTransitions_rejectStaleOwners_andTrackPendingAge() {
        ...
        db.sql("UPDATE intelligence_outbox SET next_attempt_at = now() - interval '1 second' WHERE event_id = 'old'")
                .then().block();
