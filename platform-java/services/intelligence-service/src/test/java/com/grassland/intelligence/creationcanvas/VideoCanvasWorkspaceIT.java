 ...
 *
 * <p>
 * 覆盖：旧 storyboard-only 深链唯一补关联且不新建制作任务、同键重放返回同一关联、 分镜已有关联不迁移、带 draftId 的版本 CAS
 * 与归属校验、存量多候选 409、 跨账号 404、关联草稿被删 409、最小草稿不携带 base64。
 */
...
class VideoCanvasWorkspaceIT extends IntelligenceItSupport {
	...
	@BeforeEach
	void clean() {
		db.sql("DELETE FROM video_storyboard_workspace").then().then(db.sql("DELETE FROM creation_draft").then())
	...
	void legacyDeepLinkCreatesSingleBindingAndMinimalDraft() {
		...
		String workspaceJson = db
				.sql("SELECT workspace_json::text AS w FROM creation_draft " + "WHERE id=CAST(:id AS uuid)")
				.bind("id", project.get("id")).map(row -> row.get("w", String.class)).one()
	...
	void legacyDeepLinkWithoutPlatformStillBinds() {
		...
		UUID storyboardId = UUID.fromString(db
				...
				.bind("account", ACCOUNT).bind("payload", payload).map(row -> row.get("id", String.class)).one()
				.block(Duration.ofSeconds(5)));
	...
	void reboundToDifferentDraftRejected() {
		...
		bindExpect(storyboardId, Map.of("operationId", "66666666-6666-4666-8666-666666666666", "draftId",
				otherDraft.toString(), "expectedDraftVersion", 1), 409);
	...
	void draftVersionCasAndCapabilityGuard() {
		...
		bindExpect(storyboardId,
				Map.of("operationId", "88888888-8888-4888-8888-888888888888", "draftId", draft.toString()), 400);
		bindExpect(storyboardId, Map.of("operationId", "99999999-9999-4999-8999-999999999999", "draftId",
				draft.toString(), "expectedDraftVersion", 2), 409);
		...
		bindExpect(freshStoryboard, Map.of("operationId", "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", "draftId",
				article.toString(), "expectedDraftVersion", 1), 400);
	}
	...
	void crossAccountAndDeletedDraftGuards() {
		...
		client().post().uri("/api/video-production/storyboards/{id}/workspace", storyboardId)
				.header("X-Grassland-Identity", sign(ACCOUNT_B, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("operationId", "dddddddd-dddd-4ddd-8ddd-dddddddddddd")).exchange().expectStatus()
				.isNotFound();

		// 关联草稿软删后再绑定 → 409（资源不可用，不迁移）
		db.sql("UPDATE creation_draft SET deleted_at=now() WHERE id=CAST(:id AS uuid)").bind("id", draftId).then()
				.block(Duration.ofSeconds(5));
		bindExpect(storyboardId, Map.of("operationId", "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee"), 409);
	}

	@Test
	void legacyTaskStoryboardBindsTheFrozenSourceAndRejectsForeignSnapshot() {
		UUID storyboardId = seedStoryboard();
		UUID snapshot = db
				.sql("INSERT INTO creation_context_snapshot(account_id, task_id, application_id, task_version, "
						+ "platform_id, content_form_id, task_snapshot, platform_rules_snapshot, material_snapshot, ai_config_snapshot) "
						+ "VALUES (:account, 'task-102', :app, 7, 'douyin', 'video', '{\"storeId\":\"store-102\"}'::jsonb, "
						+ "'{}'::jsonb, '{}'::jsonb, '{}'::jsonb) RETURNING id")
				.bind("account", ACCOUNT).bind("app", UUID.randomUUID().toString()).map(row -> row.get(0, UUID.class))
				.one().block();
		db.sql("UPDATE video_storyboard SET context_snapshot_id=:snapshot WHERE id=:sb").bind("snapshot", snapshot)
				.bind("sb", storyboardId).then().block();
		var result = bind(storyboardId, Map.of("operationId", UUID.randomUUID().toString()));
		...
		assertThat(project.get("taskVersion")).isEqualTo(7);
		assertThat(project.get("storeId")).isEqualTo("store-102");
		assertThat(project.get("platform")).isEqualTo("douyin");
		assertThat(project.get("contentForm")).isEqualTo("video");

		UUID foreign = seedStoryboard();
		UUID foreignSnapshot = db
				.sql("INSERT INTO creation_context_snapshot(account_id, task_id, application_id, task_version, "
						+ "platform_id, content_form_id, task_snapshot, platform_rules_snapshot, material_snapshot, ai_config_snapshot) "
						+ "SELECT :other, task_id, application_id, task_version, platform_id, content_form_id, task_snapshot, "
						+ "platform_rules_snapshot, material_snapshot, ai_config_snapshot FROM creation_context_snapshot WHERE id=:id RETURNING id")
				.bind("other", ACCOUNT_B).bind("id", snapshot).map(row -> row.get(0, UUID.class)).one().block();
		db.sql("UPDATE video_storyboard SET context_snapshot_id=:snapshot WHERE id=:sb")
				.bind("snapshot", foreignSnapshot).bind("sb", foreign).then().block();
		bindExpect(foreign, Map.of("operationId", UUID.randomUUID().toString()), 404);
		assertThat(countRows("creation_draft")).isEqualTo(1);
	}
	...
	private Map<String, Object> bindExpect(UUID storyboardId, Map<String, Object> body, int status) {
		...
		client().post().uri("/api/video-production/storyboards/{id}/workspace", storyboardId)
				...
				.bodyValue(body).exchange().expectStatus().isEqualTo(status).expectBody(Map.class)
				.consumeWith(result -> {
	...
	private long countRows(String table) {
		return db.sql("SELECT COUNT(*) AS n FROM " + table).map(row -> row.get("n", Long.class)).one()
				.block(Duration.ofSeconds(5));
	}
	...
	private UUID seedVideoDraft(UUID storyboardId, String title, int version) {
		Map<String, Object> workspace = Map.of("schemaVersion", 1, "capability", "video", "currentStep", "storyboard",
	...
	private UUID seedRawDraft(String account, Map<String, Object> workspace, String title, int version) {
		return UUID.fromString(db
				.sql("INSERT INTO creation_draft(id, owner_account_id, title, source_type, "
				...
				.bind("workspace", writeJson(workspace)).map(row -> row.get("id", String.class)).one()
				.block(Duration.ofSeconds(5)));
	}
