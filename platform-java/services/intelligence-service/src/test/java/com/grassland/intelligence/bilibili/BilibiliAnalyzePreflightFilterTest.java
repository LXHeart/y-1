...
/** {@link BilibiliAnalyzePreflightFilter}：复刻 legacy bilibili-analyze 的 20/60s 认证优先限流。 */
@ExtendWith(MockitoExtension.class)
class BilibiliAnalyzePreflightFilterTest {
    ...
    private BilibiliAnalyzePreflightFilter filter;

    @BeforeEach
    void setUp() {
        clock = new MutableClock();
        filter = new BilibiliAnalyzePreflightFilter(callers, clock);
    ...
    void unrelatedRouteBypassesFilter() {
        MockServerWebExchange exchange = MockServerWebExchange.from(
                MockServerHttpRequest.post("/api/bilibili/extract-video").build());
    ...
    private static MockServerWebExchange post() {
        return MockServerWebExchange.from(MockServerHttpRequest.post("/api/bilibili/analyze-video")
