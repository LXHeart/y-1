 ...
 * 显示名问题清单）、ai_run 留痕 capability=content_fix、免费分支积分零流水（CreditsClient 零交互）、 未配置模型
 * 503 显式错误、请求体校验 400。
 */
class ContentSafetyFixControllerIT extends IntelligenceItSupport {
	...
	private static final String STUB_OK = "{\"choices\": [{\"message\": {\"content\": \"修复后的正文：本店拿铁选用云南庄园豆，适合喜欢清爽口感的你，欢迎到店品尝。\"}}], \"usage\": {\"prompt_tokens\": 180, \"completion_tokens\": 60}}";

	private static Map<String, Object> fixBody() {
		return Map.of("text", "本店的拿铁是全上海最好喝的，想喝的加微信 latte888。", "findings",
				List.of(Map.of("category", "absolute_claims", "match", "全上海最好喝", "advice", "改为可验证的客观描述"),
						Map.of("category", "diversion", "match", "加微信 latte888", "advice", "删除联系方式")),
	...
	void fixStreamsResultAndLeavesFreeRun() {
		...
		QWEN.stubFor(post(urlEqualTo("/chat/completions")).willReturn(
				aResponse().withStatus(200).withHeader("Content-Type", "application/json").withBody(STUB_OK)));

		String account = UUID.randomUUID().toString();
		client().post().uri("/api/content-safety/fix").header(H, sign(account, "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(fixBody()).exchange().expectStatus().isOk()
		...
		assertThat(requestBody).contains("待修复正文").contains("结尾的 # 话题标签行原样保留").contains("广告法极限词").contains("全上海最好喝")
				.contains("输出修复后的完整正文");

		// ai_run 留痕（capability=content_fix）；feature=null 免费分支 → CreditsClient 零交互
		Long runs = db
				.sql("SELECT COUNT(*)::int AS c FROM ai_run" + " WHERE capability = 'content_fix' AND account_id = :a")
	...
	void missingModelReturnsExplicit503() {
		String account = UUID.randomUUID().toString();
		client().post().uri("/api/content-safety/fix").header(H, sign(account, "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(fixBody()).exchange().expectStatus().isEqualTo(503)
				.expectBody().jsonPath("$.success").isEqualTo(false).jsonPath("$.error")
				.isEqualTo("修复模型未配置,请在治理台为「内容修复」能力配置模型");
	...
	@Test
	@DisplayName("任务书 #78 卡 B：BYOK 主体修复跳过——200 skipped 帧（非 503）、无 ai_run、模型零调用")
	void byokSubjectGetsSkippedFrameInsteadOfFix() {
		seedContentFixModel();
		QWEN.stubFor(post(urlEqualTo("/chat/completions")).willReturn(
				aResponse().withStatus(200).withHeader("Content-Type", "application/json").withBody(STUB_OK)));
		String account = UUID.randomUUID().toString();
		...
		client().post().uri("/api/content-safety/fix").header(H, sign(account, "recommender"))
				...
				.expectHeader().contentTypeCompatibleWith(MediaType.TEXT_EVENT_STREAM).expectBody(String.class)
				.value(body -> {
					assertThat(body).contains("\"type\":\"skipped\"");
					assertThat(body).contains("own_model_source");
		...
		Long skippedRuns = db
				.sql("SELECT COUNT(*)::int AS c FROM ai_run" + " WHERE capability = 'content_fix' AND account_id = :a")
				.bind("a", account).map(r -> r.get("c", Integer.class)).one().block().longValue();
		assertThat(skippedRuns).as("skipped 不落修复 run").isZero();
	...
	void validationRejectsBadRequests() {
		client().post().uri("/api/content-safety/fix").contentType(MediaType.APPLICATION_JSON).bodyValue(fixBody())
				.exchange().expectStatus().isUnauthorized();

		client().post().uri("/api/content-safety/fix").header(H, sign(UUID.randomUUID().toString(), "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", "正文", "findings", List.of()))
		...
		client().post().uri("/api/content-safety/fix").header(H, sign(UUID.randomUUID().toString(), "recommender"))
				...
				.bodyValue(Map.of("text", new String(big), "findings",
						List.of(Map.of("category", "diversion", "match", "加微信"))))
				.exchange().expectStatus().isBadRequest();
	}
}
