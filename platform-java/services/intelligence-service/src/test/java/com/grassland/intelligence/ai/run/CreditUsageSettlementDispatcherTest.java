...
class CreditUsageSettlementDispatcherTest {
	...
	void successfulSettlementCompletesIntentAndEmitsUsageAdjusted() {
		...
		when(repository.claimBatch(eq(10), any(UUID.class), eq(Duration.ofMinutes(1)))).thenReturn(Flux.just(claim));
		when(credits.settleUsage(any(CreditCharge.class), eq(100L), eq("money-v1"))).thenReturn(Mono.just(settlement));
		when(repository.markCompleted(claim.id(), claim.claimToken(), settlement)).thenReturn(Mono.just(true));
	...
	void insufficientTopUpRemainsRetryableUntilAttemptLimit() {
		...
		when(repository.markFailed(eq(claim.id()), eq(claim.claimToken()), eq(Duration.ofSeconds(2)),
				eq("IntelligenceException:402"))).thenReturn(Mono.just(true));

		dispatcher(3).dispatchBatch().block();

		verify(repository).markFailed(claim.id(), claim.claimToken(), Duration.ofSeconds(2),
				"IntelligenceException:402");
	...
	void policyConflictIsTerminalAndDoesNotEmitEvent() {
		...
		when(repository.markTerminalFailed(claim.id(), claim.claimToken(), "IntelligenceException:409"))
				.thenReturn(Mono.just(true));

		dispatcher(10).dispatchBatch().block();

		verify(repository).markTerminalFailed(claim.id(), claim.claimToken(), "IntelligenceException:409");
	...
	void transientFailureBecomesTerminalAtAttemptLimit() {
		...
		when(repository.markTerminalFailed(claim.id(), claim.claimToken(), "IntelligenceException:502"))
				.thenReturn(Mono.just(true));

		dispatcher(3).dispatchBatch().block();

		verify(repository).markTerminalFailed(claim.id(), claim.claimToken(), "IntelligenceException:502");
		verify(repository, never()).markFailed(any(), any(), any(), any());
	}

	private CreditUsageSettlementDispatcher dispatcher(int maxAttempts) {
		return new CreditUsageSettlementDispatcher(repository, credits, outbox, transactions, true, 10, maxAttempts,
	...
	private static CreditUsageSettlementRepository.SettlementClaim claim(int attempt) {
		return new CreditUsageSettlementRepository.SettlementClaim(UUID.randomUUID(), UUID.randomUUID(),
				UUID.randomUUID(), "44444444-4444-4444-4444-444444444444", "ai_run_text", "money-v1", 100,
				UUID.randomUUID(), attempt);
	}

	private static CreditSettlement settlement(CreditUsageSettlementRepository.SettlementClaim claim, int adjustment) {
		return new CreditSettlement(claim.accountId(), CreditFeature.AI_RUN_TEXT, claim.consumeOperationId().toString(),
				CreditCharge.Source.PAID, claim.policyVersion(), 300, 3, claim.actualCents(), 1, adjustment, false);
	}
}
