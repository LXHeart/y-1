...
class MediaReferenceRepositoryIT extends IntelligenceItSupport {
	...
	void insertPersistsBothKeysAndFindsById() {
		...
		StepVerifier.create(repo.insert(ref)).assertNext(saved -> {
	...
	void finalizeStateMachineClaimsOnceAndCompletes() {
		...
		StepVerifier.create(repo.claimFinalize(ref.id())).assertNext(claimed -> {
		...
		StepVerifier.create(repo.completeFinalize(ref.id(), "image/png", 42L, "abc123")).assertNext(active -> {
	...
	void releaseFinalizeAllowsRetry() {
		...
		StepVerifier.create(repo.claimFinalize(ref.id()))
				.assertNext(claimed -> assertThat(claimed.status()).isEqualTo(MediaStatus.FINALIZING)).verifyComplete();
	}
	...
	void deleteStateMachineChecksOwnerAndRetainsAudit() {
		...
		StepVerifier.create(repo.claimDelete(ref.id(), ref.ownerAccountId()))
				.assertNext(claimed -> assertThat(claimed.status()).isEqualTo(MediaStatus.DELETING)).verifyComplete();
		StepVerifier.create(repo.completeDelete(ref.id())).expectNext(true).verifyComplete();

		StepVerifier.create(repo.findById(ref.id())).assertNext(deleted -> {
	...
	void releaseQuotaIsAtomicUploadOnlyAndIdempotent() {
		...
		MediaReference generatedRow = new MediaReference(generated.id(), generated.ownerAccountId(),
				generated.organizationId(), generated.purpose(), generated.domainType(), generated.domainId(),
				generated.objectKey(), generated.uploadKey(), generated.mimeType(), generated.sizeBytes(),
				generated.checksum(), "generated", MediaStatus.ACTIVE, generated.createdAt(), generated.expiresAt(),
				generated.deletedAt());
	...
	private Long counterObjectCount(String owner) {
		return db.sql(
				"SELECT COALESCE(object_count,0)::bigint AS c FROM media_owner_quota WHERE owner_account_id=:owner")
	...
	void insertIfQuotaAllowedSerializesConcurrentInsertsToOneWinner() {
		...
		java.util.List<MediaReference> pending = java.util.stream.IntStream.range(0, 6)
				.mapToObj(i -> newMedia(owner, MediaStatus.PENDING, 1L, null)).toList();

		StepVerifier.create(reactor.core.publisher.Flux.fromIterable(pending)
				.flatMap(
						ref -> repo.insertIfQuotaAllowed(ref, 1, 1_000_000L).map(ignored -> true).defaultIfEmpty(false))
				.collectList()).assertNext(results -> {
	...
	void usageCountsReservedBytesAndExcludesDeleting() {
		...
		StepVerifier.create(repo.usageByOwner(owner)).assertNext(usage -> {
	...
	/**
	 * #104 TC104-02-02：注销 manifest 持有中的 exact key（pending/failed/retained + 活动
	 * manifest）不进候选、不能 claim、物删前重验为 true；对象终态 deleted（或 manifest completed）后护栏解除，GC
	 * 恢复接管。
	 */
	@Test
	void erasureGuardedKeysAreExcludedFromCleanupUntilManifestResolves() {
		...
		StepVerifier.create(repo.findCleanupCandidates(Duration.ofHours(1)).map(MediaReference::id).collectList())
				.assertNext(ids -> assertThat(ids).doesNotContain(expired.id())).verifyComplete();
		...
		db.sql("UPDATE personal_data_erasure_object SET state='deleted', object_key=NULL"
				+ " WHERE manifest_id=CAST(:m AS uuid)").bind("m", manifestId.toString()).then().block();
		...
		StepVerifier.create(repo.findCleanupCandidates(Duration.ofHours(1)).map(MediaReference::id).collectList())
				.assertNext(ids -> assertThat(ids).contains(expired.id())).verifyComplete();
	}
	...
	void kybLeaseRenewalCannotShortenAndExpiredLeaseStopsBlockingDelete() {
		...
		KybMediaRetentionRepository.Retention first = kybRetentions
				.upsertLease(retained.id(), retained.organizationId(), referenceId, "attachment", Duration.ofHours(2))
				.block();
		KybMediaRetentionRepository.Retention renewed = kybRetentions
				.upsertLease(retained.id(), retained.organizationId(), referenceId, "attachment", Duration.ofMinutes(5))
				.block();
	...
	void sealedRetentionCannotBeShortenedOrReleasedBeforeItsDeadline() {
		...
		KybMediaRetentionRepository.Retention sealed = kybRetentions
				.seal(retained.id(), retained.organizationId(), requestId, "review_request", retainUntil).block();
		KybMediaRetentionRepository.Retention shorter = kybRetentions.seal(retained.id(), retained.organizationId(),
				requestId, "review_request", Instant.now().plus(Duration.ofDays(1))).block();
	...
	private void assertConcurrentRetentionWins(Function<MediaReference, Mono<MediaReference>> claim) throws Exception {
		...
		CompletableFuture<Void> retention = transactions
				.transactional(db.sql("SELECT id FROM media_reference WHERE id=CAST(:media AS uuid) FOR UPDATE")
						.bind("media", retained.id()).map(row -> row.get(0)).one()
						.doOnNext(ignored -> mediaLocked.countDown()).then(allowRetentionInsert.asMono())
		...
		CompletableFuture<MediaReference> deletion = claim.apply(retained).subscribeOn(Schedulers.boundedElastic())
				.toFuture();
		Thread.sleep(150);
	...
	private static MediaReference newKybMedia(MediaStatus status, Instant expiresAt) {
		...
		return new MediaReference(id, "acct-" + UUID.randomUUID(), organizationId, MediaPurpose.MERCHANT_KYB.db(),
				MediaPurpose.MERCHANT_KYB.db(), organizationId, "media/merchant_kyb/" + id, "media-pending/" + id,
				"image/png", 8L, null, "upload", status, null, expiresAt, null);
	}
	...
	private static MediaReference newMedia(String ownerAccountId, MediaStatus status, long sizeBytes,
			Instant expiresAt) {
		UUID id = UUID.randomUUID();
		return new MediaReference(id, ownerAccountId, null, MediaPurpose.USER_UPLOAD.db(), null, null,
				"media/user_upload/" + id, "media-pending/" + id, "image/png", sizeBytes, null, "upload", status, null,
				expiresAt, null);
	}
}
