...
/**
 * 内容安全端点与深检编排端到端（任务书 #34 B1/B2 / ADR-D16）： 手动端点鉴权与体积上限；未配置 content_safety →
 * deepCheck:false 降级且生成零影响； 配置后深检 run 落
 * ai_run（capability=content_safety）、用户积分零扣减（feature=null 免费分支）、 深检 findings
 * 折叠；模型坏 JSON 降级不炸。
 */
class ContentSafetyControllerIT extends IntelligenceItSupport {
	...
	void checkRequiresLoginAndDegradesWithoutModel() {
		// 未登录 → 401（fail-closed）
		client().post().uri("/api/content-safety/check").contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("text", "正常文案")).exchange().expectStatus().isUnauthorized();

		// 短文本仅 L1（未配置模型也如此）
		client().post().uri("/api/content-safety/check").header(H, sign(UUID.randomUUID().toString(), "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", "全城最好吃的甜品，加微信 sweet8888")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.safety.deepCheck").isEqualTo(false)
	...
	void oversizedTextRejected() {
		...
		client().post().uri("/api/content-safety/check").header(H, sign(UUID.randomUUID().toString(), "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", new String(big))).exchange()
	...
	void deepCheckRunsZeroCostAndMergesFindings() {
		...
		client().post().uri("/api/content-safety/check").header(H, sign(account, "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", longText("这家轻食店的全麦贝果全城销量第一")))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.safety.deepCheck").isEqualTo(true)
		...
		Long runs = db
				.sql("SELECT COUNT(*)::int AS c FROM ai_run"
						+ " WHERE capability = 'content_safety' AND account_id = :a")
				.bind("a", account).map(r -> r.get("c", Integer.class)).one().block().longValue();
	...
	void badModelOutputDegradesGracefully() {
		...
		client().post().uri("/api/content-safety/check").header(H, sign(UUID.randomUUID().toString(), "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", longText("正常长文本"))).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.safety.deepCheck").isEqualTo(false);
	}
	...
	void byokRejectsContentSafetyCapability() {
		// BYOK controller 挂 @Conditional(CryptoKekConfiguredCondition)——测试环境未配 KEK 时端点
		// 404，
	...
	// ---------- 任务书 #78 卡 B（D3）：BYOK 主体跳过 L2 深检 ----------

	private void seedOwnMode(String account) {
		// 密文占位即可：绕过判定只做路由解析（isByok），不解密不执行
		db.sql("""
				...
				VALUES (:a,'text','openai-compatible','https://byok.example.com','byok-model',
				    'ciphertext','v1','sk-***byok',true)
				""").bind("a", account).then().block();
		db.sql("INSERT INTO ai_provider_preference(account_id, capability, use_own_key) VALUES (:a,'*',true)")
				.bind("a", account).then().block();
	}

	private void stubDeepCheckModel() {
		seedContentSafetyModel();
		QWEN.stubFor(post(urlEqualTo("/chat/completions"))
				.willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")
						.withBody("{\"choices\":[{\"message\":{\"content\":\"{\\\"findings\\\":[]}\"}}],"
								+ "\"usage\":{\"prompt_tokens\":120,\"completion_tokens\":40}}")));
	}

	@Test
	@DisplayName("任务书 #78 卡 B：own 模式主体跳过 L2——deepCheckSkipped=true、L1 照常、深检零发生")
	void ownSourceSkipsDeepCheck() {
		stubDeepCheckModel();
		String account = UUID.randomUUID().toString();
		seedOwnMode(account);
		...
		client().post().uri("/api/content-safety/check").header(H, sign(account, "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", longText("这家轻食店的全麦贝果全城销量第一")))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.safety.deepCheck").isEqualTo(false)
				.jsonPath("$.data.safety.deepCheckSkipped").isEqualTo(true)
				// L1 词库 + 原创度底线保留：findings 仍产出
				.jsonPath("$.data.safety.findings").isNotEmpty();

		Long runs = db
				.sql("SELECT COUNT(*)::int AS c FROM ai_run"
						+ " WHERE capability = 'content_safety' AND account_id = :a")
				.bind("a", account).map(r -> r.get("c", Integer.class)).one().block().longValue();
		assertThat(runs).as("绕过后不落深检 run").isZero();
		QWEN.verify(0, postRequestedFor(urlEqualTo("/chat/completions")));
	}

	@Test
	@DisplayName("任务书 #78 卡 B：组织 BYOK 主体同样跳过 L2（凡自有凭据即绕过，含组织命中）")
	void orgByokSubjectSkipsDeepCheck() {
		stubDeepCheckModel();
		String account = UUID.randomUUID().toString();
		String org = UUID.randomUUID().toString();
		// 组织 text 密钥命中 → 商家活动身份 text 路由为 BYOK（个人总开关不参与组织段）
		db.sql("""
				...
				VALUES (:org,:a,'text','openai-compatible','https://byok.example.com','byok-model',
				    'ciphertext','v1','sk-***byok',true)
				""").bind("org", org).bind("a", account).then().block();

		client().post().uri("/api/content-safety/check").header(H, signWithRole(account, "merchant", org, "owner"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("text", longText("正常长文本"))).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.safety.deepCheck").isEqualTo(false)
				.jsonPath("$.data.safety.deepCheckSkipped").isEqualTo(true);
