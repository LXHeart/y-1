...
/**
 * 控制面 Run 执行闭环（GL-P3-AI-001）：平台 run（扣分/结算/事件）、平台 run 失败退款、 BYOK
 * run（解密→调用链，不扣分）、BYOK 回退未授权拒绝、预算超限拒绝、TaskContext 快照。
 *
 * <p>
 * CREDITS WireMock 托管 legacy 积分端点；QWEN WireMock 托管 provider（平台 + BYOK 共用）。 KEK
 * 注入使 BYOK 密钥创建 + 运行时解密可用（与 {@code AiProviderKeyControllerIT} 同值）。
 */
...
class AiRunControllerIT extends IntelligenceItSupport {
	...
	void platformRunSucceeds() {
		stubQwenOk();
		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", sign(ACCOUNT, "merchant"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"写一句问候","maxTokens":128,"allowFallback":true}
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.content").isEqualTo("hello")
				.jsonPath("$.status").isEqualTo("completed").jsonPath("$.inputTokens").isEqualTo(10)
				.jsonPath("$.outputTokens").isEqualTo(5).jsonPath("$.actualCents").isEqualTo(2) // qwen-plus:
																								// (10*3+999)/1000 +
																								// (5*6+999)/1000
				.jsonPath("$.taskContext.priceTableVersion").isEqualTo("v1")
				.jsonPath("$.taskContext.platformModelVersion").isEqualTo(1).jsonPath("$.taskContext.resolutionType")
				.isEqualTo("PLATFORM").jsonPath("$.runId").isNotEmpty();

		// ai_run 落 usage 计量
		Long completed = db.sql(
				"SELECT COUNT(*) AS n FROM ai_run WHERE status='completed' AND input_tokens=10 AND output_tokens=5")
	...
	void consumeResponseLossPersistsCompensationIntent() {
		...
		CREDITS.stubFor(post(urlEqualTo("/internal/credits/consume")).atPriority(1)
				.willReturn(aResponse().withFault(Fault.EMPTY_RESPONSE)));
		CREDITS.stubFor(post(urlEqualTo("/internal/credits/consume-compensations")).atPriority(1)
				.willReturn(aResponse().withStatus(500)));

		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", signWithOrg(ORG_ACCOUNT, ORG))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
		...
		Long pending = db
				.sql("SELECT COUNT(*) AS n FROM ai_credit_compensation "
	...
	void clientCancellationFinalizesRunAndReleasesBudget() {
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_daily, enabled) "
				+ "VALUES (:org, 'text', 'platform', 1000, true)").bind("org", ORG).then().block();
		doReturn(Mono.never()).when(textClient).complete(anyString(), anyString(), anyString(), anyString(),
				anyString(), anyInt(), eq(false));
		var request = MockServerHttpRequest.post("/api/ai/runs")
				.header("X-Grassland-Identity", signWithOrg(ORG_ACCOUNT, ORG)).build();
		var exchange = MockServerWebExchange.from(request);

		var subscription = runController.execute(new ExecuteRunRequest("text", "x", 16, true), exchange)
				.subscribe(ignored -> {
				}, ignored -> {
				});
		awaitRunStatus("running");
		verify(textClient, timeout(5000)).complete(anyString(), anyString(), anyString(), anyString(), anyString(),
				anyInt(), eq(false));
		...
		Long cancelledEvents = db.sql("SELECT COUNT(*) AS n FROM intelligence_outbox WHERE event_type='AiRunCancelled'")
				.map((row, meta) -> row.get("n", Long.class)).one().block();
		Long leasedSlots = db
				.sql("SELECT COUNT(*) AS n FROM platform_model_concurrency_slot WHERE lease_token IS NOT NULL")
				.map((row, meta) -> row.get("n", Long.class)).one().block();
		assertThat(reserved).isZero();
	...
	void cancellationDuringCreditConsumeFinalizesPreparation() {
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_daily, enabled) "
				+ "VALUES (:org, 'text', 'platform', 1000, true)").bind("org", ORG).then().block();
		doReturn(Mono.never()).when(creditsClient).consume(eq(ORG_ACCOUNT), eq(CreditFeature.AI_RUN_TEXT), anyString());
		var request = MockServerHttpRequest.post("/api/ai/runs")
				.header("X-Grassland-Identity", signWithOrg(ORG_ACCOUNT, ORG)).build();
		var exchange = MockServerWebExchange.from(request);
	...
	void byokRunDecryptsAndCalls() {
		doReturn(Mono.just(new TextCompletionResult("hello", 10, 5))).when(textClient).complete(eq("openai-compatible"),
				eq("https://api.example.com"), eq("sk-test-byok-secret"), eq("byok-model"), eq("x"), eq(32), eq(true));
		// 注册个人 BYOK 密钥；provider 调用通过 spy 验证解密后的 bearer 参数。
		client().post().uri("/api/ai/keys").header("X-Grassland-Identity", sign(BYOK_ACCOUNT, "merchant"))
		...
		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", sign(BYOK_ACCOUNT, "merchant"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":32}
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.content").isEqualTo("hello")
				.jsonPath("$.actualCents").isEqualTo(0) // BYOK 不收平台 AI 费（D-11）
				.jsonPath("$.taskContext.resolutionType").isEqualTo("BYOK").jsonPath("$.runId").isNotEmpty();
	...
	void orgByokRunUsesOrgKeyForMember() {
		doReturn(Mono.just(new TextCompletionResult("org-hello", 10, 5))).when(textClient).complete(
		...
		db.sql("""
				...
				""").bind("org", ORG).bind("owner", ORG_ACCOUNT)
				.bind("encrypted", encryption.encrypt("sk-org-run-secret")).then().block();

		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", signWithOrg(ORG_ACCOUNT, ORG))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":32}
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.content").isEqualTo("org-hello")
				.jsonPath("$.actualCents").isEqualTo(0).jsonPath("$.taskContext.resolutionType").isEqualTo("BYOK")
		...
		String recorded = db.sql("SELECT byok_organization_id FROM ai_run WHERE account_id = :account")
				.bind("account", ORG_ACCOUNT).map((r, m) -> r.get("byok_organization_id", String.class)).one().block();
		assertThat(recorded).isEqualTo(ORG);
		CREDITS.verify(0, postRequestedFor(urlEqualTo("/internal/credits/consume")));
	}

	/**
	 * 任务书 #47 D16：本用例原本正是 D16 要修的场景——组织只配了 image_generation 密钥， text 运行就被 DENIED，而
	 * org admin 完全不会预期「配了一个能力会让另一个能力对全组织不可用」。 故断言顺序反转：先验<b>无行默认允许</b>，再验<b>显式
	 * false</b> 才严格拒绝。 D-11 的「双闸」语义保留——严格模式下 allowFallback=true 也照样拒。
	 */
	@Test
	...
	void orgFallbackDefaultsToAllowAndExplicitFalseStillDenies() {
		...
		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", signWithOrg(ORG_ACCOUNT, ORG))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":16,"allowFallback":true}
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.taskContext.resolutionType")
				.isEqualTo("PLATFORM").jsonPath("$.taskContext.fallbackAuthorized").isEqualTo(true);
		...
		db.sql("INSERT INTO ai_org_byok_policy(organization_id, allow_platform_fallback, updated_by_account_id) "
				+ "VALUES (:org, false, :owner)").bind("org", ORG).bind("owner", ORG_ACCOUNT).then().block();

		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", signWithOrg(ORG_ACCOUNT, ORG))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":16,"allowFallback":true}
						""").exchange().expectStatus().isForbidden().expectBody().jsonPath("$.error")
				.isEqualTo("无 BYOK 且未授权回退平台模型");
	}
	...
	void synchronousProviderValidationFailureRefundsAndMarksRunFailed() {
		doThrow(new IllegalArgumentException("synchronous provider validation failure")).when(textClient)
	...
	void byokUsesZeroEstimatedCents() {
		doReturn(Mono.just(new TextCompletionResult("hello", 10, 5))).when(textClient).complete(anyString(),
				anyString(), anyString(), anyString(), anyString(), anyInt(), eq(true));
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_cents_per_run, enabled) "
				+ "VALUES (:org, 'text', 'platform', 0, true)").bind("org", ORG).then().block();
		...
		db.sql("""
				...
				""").bind("org", ORG).bind("owner", ORG_ACCOUNT).bind("encrypted", encryption.encrypt("sk-byok-secret"))
				.then().block();
		...
		Integer reserved = db.sql("SELECT budget_cents FROM ai_run WHERE account_id = :account")
				.bind("account", ORG_ACCOUNT).map((r, m) -> r.get("budget_cents", Integer.class)).one().block();
		assertThat(reserved).isZero();
		CREDITS.verify(0, postRequestedFor(urlEqualTo("/internal/credits/consume")));
	}
	...
	void platformRunUsesPlatformBudget() {
		...
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_per_run, enabled) "
				+ "VALUES (:org, 'text', 'platform', 1, true), " + "(:org, 'text', 'openai-compatible', 1000, true)")
	...
	void organizationMonthlyCentsBudgetBlocksRun() {
		...
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, "
				+ "max_cents_monthly, enabled) VALUES (:org, '*', '*', 0, true)").bind("org", ORG).then().block();
		...
		Long runs = db.sql("SELECT COUNT(*) AS n FROM ai_run").map((row, meta) -> row.get("n", Long.class)).one()
				.block();
		assertThat(runs).isZero();
		CREDITS.verify(0, postRequestedFor(urlEqualTo("/internal/credits/consume")));
		QWEN.verify(0, postRequestedFor(urlEqualTo("/chat/completions")));
	...
	void byokRunUsesResolvedProviderBudget() {
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_per_run, enabled) "
				+ "VALUES (:org, 'text', 'platform', 1000, true), " + "(:org, 'text', 'openai-compatible', 1, true)")
				.bind("org", ORG).then().block();
	...
	void unknownPlatformModelPriceFailsClosed() {
		db.sql("UPDATE platform_model_config SET model = 'unpriced-platform-model' WHERE enabled = true").then()
				.block();
	...
	@Test
	@DisplayName("任务书 #78 卡 B：个人 BYOK 超个人预算上限仍成功（预算闸豁免）；平台 run 照拒")
	void personalByokExemptFromPersonalBudget() {
		stubQwenOk();
		// own 模式 + 个人密钥（KEK 密文，执行层要解密明文）
		db.sql("INSERT INTO ai_provider_preference(account_id, capability, use_own_key) VALUES (:a,'*',true)")
				.bind("a", BYOK_ACCOUNT).then().block();
		db.sql("""
				INSERT INTO ai_provider_key(owner_account_id, capability, provider, base_url, model,
				    encrypted_key, key_version, masked_hint, enabled)
				VALUES (:a,'text','openai-compatible','https://byok.example.com','byok-model',
				    :encrypted,'v1','sk-***byok',true)
				""").bind("a", BYOK_ACCOUNT).bind("encrypted", encryption.encrypt("sk-personal-run")).then().block();
		// 个人作用域全局预算行（'*','*'，单次 token 上限 1）：豁免前 BYOK 也会被 exceeds_run_budget 拦
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_per_run, enabled) "
				+ "VALUES (:scope, '*', '*', 1, true)").bind("scope", "u:" + BYOK_ACCOUNT).then().block();

		doReturn(Mono.just(new TextCompletionResult("own-hello", 10, 5))).when(textClient).complete(
				eq("openai-compatible"), eq("https://byok.example.com"), eq("sk-personal-run"), eq("byok-model"),
				eq("x"), eq(32), eq(true));

		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", sign(BYOK_ACCOUNT, "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":32,"allowFallback":true}
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.status").isEqualTo("completed")
				.jsonPath("$.content").isEqualTo("own-hello");

		// ai_run 照建照结算（completed），预算零预留；不扣积分（BYOK）
		Integer reservedCents = db.sql("SELECT budget_cents FROM ai_run WHERE account_id = :a").bind("a", BYOK_ACCOUNT)
				.map((r, m) -> r.get("budget_cents", Integer.class)).one().block();
		assertThat(reservedCents).isEqualTo(0);
		Long completed = db.sql("SELECT COUNT(*) AS n FROM ai_run WHERE account_id = :a AND status='completed'")
				.bind("a", BYOK_ACCOUNT).map((r, m) -> r.get("n", Long.class)).one().block();
		assertThat(completed).isEqualTo(1);
		Long usedTokens = db.sql("SELECT current_daily_tokens FROM ai_model_budget WHERE organization_id = :scope")
				.bind("scope", "u:" + BYOK_ACCOUNT).map((r, m) -> r.get("current_daily_tokens", Long.class)).one()
				.block();
		assertThat(usedTokens).as("个人预算行用量不动（豁免）").isEqualTo(0);
		CREDITS.verify(0, postRequestedFor(urlEqualTo("/internal/credits/consume")));

		// 同一预算行对平台 run 仍然生效（控制组：证明行是活的，豁免只针对自有凭据）
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_per_run, enabled) "
				+ "VALUES (:scope, '*', '*', 1, true)").bind("scope", "u:" + ACCOUNT).then().block();
		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", sign(ACCOUNT, "recommender"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":128,"allowFallback":true}
						""").exchange().expectStatus().isEqualTo(402);
	}
	...
	void budgetReservationIsAtomic() {
		...
		var results = reactor.core.publisher.Flux.merge(budgetService.checkAndReserve(ORG, "text", "platform", 80, 0),
				budgetService.checkAndReserve(ORG, "text", "platform", 80, 0)).collectList().block();
	...
	void byokUsageCountsTowardOrganizationTokenBudget() {
		...
		var usage = db
				.sql("SELECT current_daily_tokens, current_daily_cents FROM ai_model_budget WHERE organization_id=:org")
				.bind("org", ORG).map((row, meta) -> java.util.List.of(row.get("current_daily_tokens", Long.class),
	...
	void completionStateFailureDoesNotPublishCompletedEvent() {
		stubQwenOk();
		doReturn(Mono.just(false)).when(runRepository).complete(any(), anyInt(), anyInt(), anyInt(), anyInt(),
				anyInt());
	...
	void reservationReleaseDoesNotPolluteNextDailyWindow() {
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_daily, enabled) "
				+ "VALUES (:org, 'text', 'platform', 1000, true)").bind("org", ORG).then().block();
		ModelBudgetService.BudgetCheckResult reservation = budgetService.checkAndReserve(ORG, "text", "platform", 20, 4)
				.block();
		db.sql("UPDATE ai_model_budget SET current_daily_tokens=7, current_daily_cents=3, "
				+ "last_reset_date=CURRENT_DATE + 1 WHERE organization_id=:org").bind("org", ORG).then().block();
	...
	void reservationReleaseDoesNotPolluteNextMonthlyWindow() {
		...
		var monthly = db.sql(
				"SELECT current_monthly_tokens, current_monthly_cents FROM ai_model_budget WHERE organization_id=:org")
				.bind("org", ORG).map((row, meta) -> java.util.List.of(row.get("current_monthly_tokens", Long.class),
	...
	void runInsertFailureDoesNotConsumeCreditsAndReleasesBudget() {
		db.sql("INSERT INTO ai_model_budget(organization_id, capability, provider, max_tokens_daily, enabled) "
				+ "VALUES (:org, 'text', 'platform', 1000, true)").bind("org", ORG).then().block();
		doReturn(Mono.error(new IllegalStateException("insert failed"))).when(runRepository).create(any(AiRun.class));
	...
	void getRunScoped() {
		...
		client().get().uri("/api/ai/runs/" + runId).header("X-Grassland-Identity", sign(ACCOUNT, "merchant")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.taskContext.priceTableVersion").isEqualTo("v1")
				.jsonPath("$.taskContext.fallbackAuthorized").isEqualTo(true).jsonPath("$.taskContext.capability")
				.isEqualTo("text");

		// 跨账号 → 404（资源按账号作用域）
		client().get().uri("/api/ai/runs/" + runId)
				.header("X-Grassland-Identity", sign("66666666-6666-6666-6666-666666666666", "merchant")).exchange()
	...
	void creationContextSnapshotIsAccountScopedAndReturned() {
		...
		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", sign(ACCOUNT, "merchant"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":32,"allowFallback":true,
						 "contextSnapshotId":"%s"}
						""".formatted(ownedSnapshot)).exchange().expectStatus().isOk().expectBody()
		...
		client().post().uri("/api/ai/runs").header("X-Grassland-Identity", sign(ACCOUNT, "merchant"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","prompt":"x","maxTokens":32,"allowFallback":true,
						 "contextSnapshotId":"%s"}
						""".formatted(foreignSnapshot)).exchange().expectStatus().isForbidden();
		Long runCount = db.sql("SELECT COUNT(*) AS n FROM ai_run").map(row -> row.get("n", Long.class)).one().block();
	...
	private String seedCreationContext(String accountId) {
		return db.sql("""
				...
				.bind("application", UUID.randomUUID().toString()).map(row -> row.get("id", String.class)).one()
				.block();
	}
	...
	private void stubEntitlement(String accountId) {
		CREDITS.stubFor(get(urlEqualTo("/internal/marketplace/reputation/" + accountId + "/ai-entitlement"))
				.willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")
	...
	private void awaitRunStatus(String status) {
		Mono.defer(() -> db.sql("SELECT status FROM ai_run ORDER BY started_at DESC LIMIT 1")
				.map((row, meta) -> row.get("status", String.class)).one()).filter(status::equals)
				.repeatWhenEmpty(repeats -> repeats.delayElements(Duration.ofMillis(25))).block(Duration.ofSeconds(5));
	}
