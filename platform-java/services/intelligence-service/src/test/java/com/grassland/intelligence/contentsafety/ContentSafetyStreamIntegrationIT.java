...
/**
 * 五条流内联安全检查帧端到端（任务书 #34 B3 / ADR-D16 D8）：每条流「生成 → 流尾 safety 帧」。 任务书指定 mock
 * 生成边界（AiCapabilityAdapter/CreditsClient 替身，真 controller + 真 L1 检查）； 深检默认未配置（无
 * platform_model_config）→ safety 帧 deepCheck:false，验证「生成零影响」基线。
 */
class ContentSafetyStreamIntegrationIT extends IntelligenceItSupport {
	...
	private String run(String uri, Map<String, Object> body) {
		return new String(client().post().uri(uri).header(H, sign(UUID.randomUUID().toString(), "recommender"))
	...
	void articleTitlesEmbedSafetyInData() {
		// titles 已迁执行环：桩环出口把 transform 应用到带 code fence 的模型输出
		...
		when(frozenText.executeIndependent(any(), any(), org.mockito.ArgumentMatchers.anyInt(), any(), any()))
				.thenAnswer(invocation -> {
					...
					java.util.function.Function<com.grassland.intelligence.ai.run.TextCompletionResult, Object> transform = (java.util.function.Function<com.grassland.intelligence.ai.run.TextCompletionResult, Object>) invocation
							.getArgument(4);
					return Mono.just(new com.grassland.intelligence.ai.run.FrozenTextExecutionService.Traced<>(
							transform.apply(new com.grassland.intelligence.ai.run.TextCompletionResult(
									"```json\n" + titlesPayload + "\n```", 10, 5)),
							null, "qwen", "qwen-plus", 1, false));
				});
		client().post().uri("/api/article-generation/titles")
				.header(H, sign(UUID.randomUUID().toString(), "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("topic", "甜品探店")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.titles[0].title").exists().jsonPath("$.data.safety.deepCheck").isEqualTo(false)
	...
	void comedyScriptStreamAppendsSafetyFrame() {
		// 已迁执行环：桩环出口返回含命中词的完整脚本（单帧 content）
		when(frozenText.executeIndependent(any(), any(), org.mockito.ArgumentMatchers.anyInt(), any(), any()))
				.thenReturn(Mono.just(new com.grassland.intelligence.ai.run.FrozenTextExecutionService.Traced<>(
						GENERATED, null, "qwen", "qwen-plus", 1, false)));
		String stream = run("/api/comedy-generation/generate-script", Map.of("topic", "探店", "duration", 60));
		assertThat(stream).contains("\"content\"");
		assertThat(stream).contains("\"type\":\"safety\"");
	...
	void videoScriptStreamAppendsSafetyFrame() {
		...
		when(frozenText.executeIndependent(any(), any(), org.mockito.ArgumentMatchers.anyInt(), any(), any()))
				.thenReturn(Mono.just(new com.grassland.intelligence.ai.run.FrozenTextExecutionService.Traced<>(
						GENERATED, null, "qwen", "qwen-plus", 1, false)));
		String stream = run("/api/video-production/generate-script", Map.of("videoStyle", "烟火纪实", "industryType", "餐饮",
				"shopName", "甜品店", "targetPlatform", "douyin", "images", java.util.List.of(
	...
	void momentsStreamAppendsSafetyFrame() {
		...
		when(frozenText.executeIndependent(any(), any(), org.mockito.ArgumentMatchers.anyInt(), any(), any()))
				.thenReturn(
						Mono.just(new com.grassland.intelligence.ai.run.FrozenTextExecutionService.Traced<>(
								new com.grassland.intelligence.moments.MomentsGenerationService.MomentsResult(GENERATED,
										java.util.List.of(), java.util.List.of()),
								null, "qwen", "qwen-plus", 1, false)));
		String stream = run("/api/moments-generation/generate", Map.of("topic", "甜品店开业", "style", "lifestyle"));
