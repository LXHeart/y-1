...
class AiExecutionServiceWorkerTest {
	...
	void pricedPlatformRunReservesConvertedUsageUnderFrozenPolicy() {
		...
		when(credits.reserveUsage(eq("acct-1"), eq(CreditFeature.AI_RUN_EMBEDDING), any(), eq(300L), eq("money-v1")))
				.thenAnswer(invocation -> Mono.just(new CreditCharge("acct-1", CreditFeature.AI_RUN_EMBEDDING,
						invocation.getArgument(2), CreditCharge.Source.PAID, 7L, true, "money-v1", 300, 3)));

		AiExecutionService.ExecutionResult result = execution
				.prepareExecution("acct-1", "org-1", "retrieval", CreditFeature.AI_RUN_EMBEDDING, 40, 0, true).block();
		...
		verify(credits).reserveUsage(eq("acct-1"), eq(CreditFeature.AI_RUN_EMBEDDING),
	...
	void pricedModelWithZeroEstimateStillCreatesUsageReservation() {
		...
		when(credits.reserveUsage(eq("acct-1"), eq(CreditFeature.AI_RUN_EMBEDDING), any(), eq(0L), eq("money-v1")))
				.thenAnswer(invocation -> Mono.just(new CreditCharge("acct-1", CreditFeature.AI_RUN_EMBEDDING,
						invocation.getArgument(2), CreditCharge.Source.PAID, 7L, true, "money-v1", 0, 0)));

		AiExecutionService.ExecutionResult result = execution
				.prepareExecution("acct-1", "org-1", "retrieval", CreditFeature.AI_RUN_EMBEDDING, 0, 0, true).block();

		assertThat(result).isNotNull();
		assertThat(result.allowed()).isTrue();
		assertThat(result.context().creditsCentsPolicyVersion()).isEqualTo("money-v1");
		assertThat(result.context().creditCompensationRequired()).isTrue();
		verify(credits).reserveUsage(eq("acct-1"), eq(CreditFeature.AI_RUN_EMBEDDING), any(), eq(0L), eq("money-v1"));
	}
	...
	void successfulPricedRunEnqueuesSettlementWithCompletionTransaction() {
		...
		ProviderResolution provider = ProviderResolution.platform(UUID.randomUUID(), "qwen", "https://example.invalid",
				"priced-model", 4, null);
		ModelBudgetService.BudgetCheckResult budget = ModelBudgetService.BudgetCheckResult.allowed(null, null, 40, 300);
		CreditCharge charge = new CreditCharge("acct-1", CreditFeature.AI_RUN_EMBEDDING, operationId.toString(),
				CreditCharge.Source.PAID, 7L, true, "money-v1", 300, 3);
		var context = new AiExecutionService.ExecutionContext(runId, "org-1", "acct-1", "retrieval", provider, budget,
				operationId, charge, CreditFeature.AI_RUN_EMBEDDING, true, null, "v1", 40, 0, "money-v1");
		...
		when(usageSettlements.enqueue(runId, operationId, "acct-1", "ai_run_embedding", "money-v1", 100))
				.thenReturn(Mono.empty());

		StepVerifier.create(execution.settleSuccessWithCost(context, 100, 20, 5, 0, 0)).expectNext(true)
				.verifyComplete();

		verify(usageSettlements).enqueue(runId, operationId, "acct-1", "ai_run_embedding", "money-v1", 100);
	...
	void userCancellationSettlesPricedReservationWithoutRefundingIt() {
		...
		when(outbox.append(any())).thenReturn(Mono.empty());
		when(usageSettlements.enqueue(runId, operationId, "acct-1", "ai_run_embedding", "money-v1", 300))
				.thenReturn(Mono.empty());

		StepVerifier.create(execution.handleCancellation(context)).expectNext(true).verifyComplete();

		verify(usageSettlements).enqueue(runId, operationId, "acct-1", "ai_run_embedding", "money-v1", 300);
	...
	void workerPreparationUsesExplicitOwnershipWithoutResolvingHttpCaller() {
		...
		StepVerifier.create(execution.prepareExecution("acct-1", "org-1", "retrieval", null, 40, 0, true))
				.assertNext(result -> assertThat(result.allowed()).isTrue()).verifyComplete();
	...
	void cancellationDuringCommittedPreparationHandoffCancelsRunAndReleasesBudget() throws Exception {
		...
		UUID runId = UUID.randomUUID();
		ModelBudgetService.BudgetCheckResult reservation = ModelBudgetService.BudgetCheckResult.allowed(budgetId,
				LocalDate.now(), 40, 100);
		ProviderResolution provider = ProviderResolution.platform(UUID.randomUUID(), "sandbox",
				"https://sandbox.invalid", "sandbox-embedding-v1", 4, null);
