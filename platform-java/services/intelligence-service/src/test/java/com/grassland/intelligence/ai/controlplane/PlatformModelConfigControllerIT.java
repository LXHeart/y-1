...
/**
 * 平台模型配置 admin CRUD（GL-P3-AI-001 model-control-plane）。requireAdmin 门闩 + 版本化 +
 * history。 controller 非 KEK 门控（无密钥），故本 IT 不注入 KEK。
 */
...
class PlatformModelConfigControllerIT extends IntelligenceItSupport {
	...
	void adminCreates() {
		client().post().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						...
						""").exchange().expectStatus().isCreated().expectBody().jsonPath("$.version").isEqualTo(1)
				.jsonPath("$.capability").isEqualTo("text").jsonPath("$.modelRole").isEqualTo("primary")
				.jsonPath("$.enabled").isEqualTo(true).jsonPath("$.healthStatus").isEqualTo("healthy");

		client().get().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN)).exchange()
	...
	void adminRevisesBumpsVersion() {
		createPrimary();
		client().put().uri("/api/admin/ai/models/text/primary").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"provider":"openai-completions","model":"qwen-max","baseUrl":"https://dashscope.aliyuncs.com"}
						""").exchange().expectStatus().isOk().expectBody().jsonPath("$.version").isEqualTo(2)
	...
	void adminDisables() {
		...
		client().get().uri("/api/admin/ai/models/text/primary").header("X-Grassland-Identity", signAdmin(ADMIN))
	 ...
	 * 即运营改不动、也看不出「该能力其实跑在沙箱假数据上」。 {@code PlatformProviderPolicy.validate} 本就支持
	 * sandbox，仅 bean validation 失同步。
	 */
	@Test
	...
	void adminManagesSandboxCapability() {
		client().post().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						...
						""").exchange().expectStatus().isCreated().expectBody().jsonPath("$.provider")
				.isEqualTo("sandbox");

		// 修订：S0 之前这一步是 400（正则拒 sandbox），运营无法改动 seeder 种下的行
		client().put().uri("/api/admin/ai/models/image_edit/primary").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
	...
	/**
	 * provider 受控值集从「厂商名」改为「协议方言名」：四个方言名 POST/PUT 都必须通过 bean validation。 与上面
	 * sandbox 那组同一个 bug 类——正则漏一个名字，运营就建不出、也改不动该行。 地址用 V56 内置受信的
	 * api.openai.com，把断言聚焦在 provider 名而非 origin 表。
	 */
	@Test
	@DisplayName("四个协议方言名均可创建与修订")
	void adminManagesAllDialectNames() {
		String[] dialects = {"openai-completions", "openai-responses", "anthropic-messages", "google-generative-ai"};
		for (String dialect : dialects) {
			client().post().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN))
					.contentType(MediaType.APPLICATION_JSON).bodyValue("""
							{"capability":"text","modelRole":"primary","provider":"%s",
							 "model":"m-1","baseUrl":"https://api.openai.com"}
							""".formatted(dialect)).exchange().expectStatus().isCreated().expectBody()
					.jsonPath("$.provider").isEqualTo(dialect);

			client().put().uri("/api/admin/ai/models/text/primary").header("X-Grassland-Identity", signAdmin(ADMIN))
					.contentType(MediaType.APPLICATION_JSON).bodyValue("""
							{"provider":"%s","model":"m-2","baseUrl":"https://api.openai.com"}
							""".formatted(dialect)).exchange().expectStatus().isOk().expectBody().jsonPath("$.provider")
					.isEqualTo(dialect).jsonPath("$.model").isEqualTo("m-2");
	...
	void adminGate() {
		...
		// 缺断言
		client().get().uri("/api/admin/ai/models").exchange().expectStatus().isUnauthorized();
	}
	...
	void createsFromCredentialId() {
		...
		client().post().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN))
				...
				.jsonPath("$.provider").isEqualTo("openai-compatible").jsonPath("$.baseUrl")
				.isEqualTo("https://api.openai.com/v1").jsonPath("$.model").isEqualTo("gpt-4");
		...
		client().post().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"text","modelRole":"primary","credentialId":"%s",
						 ...
						 "model":"qwen-plus"}
						""".formatted(credentialId)).exchange().expectStatus().isCreated().expectBody()
				.jsonPath("$.provider").isEqualTo("openai-completions").jsonPath("$.baseUrl")
	...
	@Test
	@DisplayName("capability 白名单：image_generation 与 video_generation 均入控制面 → 201；拼错 → 400")
	void gatesCapabilityWhitelist() {
		...
		// video_generation 任务书 #64 卡2 起入控制面（env 型配置退场）；厂商名 provider 同步合法
		client().post().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"capability":"video_generation","modelRole":"primary","provider":"minimax",
						 "model":"video-01","baseUrl":"https://dashscope.aliyuncs.com"}
						""").exchange().expectStatus().isCreated();
	...
	void revisesFromCredentialId() {
		...
		client().put().uri("/api/admin/ai/models/text/primary").header("X-Grassland-Identity", signAdmin(ADMIN))
				.contentType(MediaType.APPLICATION_JSON).bodyValue("""
						{"credentialId":"%s","model":"gpt-4"}
						""".formatted(credentialId)).exchange().expectStatus().isOk().expectBody().jsonPath("$.version")
				.isEqualTo(2).jsonPath("$.provider").isEqualTo("openai-compatible").jsonPath("$.baseUrl")
	...
	void listIncludeDisabled() {
		...
		// 默认：只有生效的 v2
		client().get().uri("/api/admin/ai/models").header("X-Grassland-Identity", signAdmin(ADMIN)).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.length()").isEqualTo(1).jsonPath("$[0].version")
				.isEqualTo(2);

		// includeDisabled：两行，生效的排在前
		client().get().uri("/api/admin/ai/models?includeDisabled=true").header("X-Grassland-Identity", signAdmin(ADMIN))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.length()").isEqualTo(2)
				.jsonPath("$[0].enabled").isEqualTo(true).jsonPath("$[0].version").isEqualTo(2).jsonPath("$[1].enabled")
				.isEqualTo(false).jsonPath("$[1].version").isEqualTo(1);
	}
	...
	void restoresDisabledRow() {
		...
		client().post().uri("/api/admin/ai/models/" + id + "/restore").header("X-Grassland-Identity", signAdmin(ADMIN))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.enabled").isEqualTo(true);
	...
	void restoreConflictsWithLiveRow() {
		...
		client().post().uri("/api/admin/ai/models/" + oldId + "/restore")
				.header("X-Grassland-Identity", signAdmin(ADMIN)).exchange().expectStatus().isEqualTo(409);
	}
	...
	void deletesOnlyDisabledRows() {
		...
		// 生效中不可删
		client().delete().uri("/api/admin/ai/models/" + id).header("X-Grassland-Identity", signAdmin(ADMIN)).exchange()
	...
	void singleSegmentIdDoesNotShadowTwoSegmentRoute() {
		...
		client().delete().uri("/api/admin/ai/models/not-a-uuid").header("X-Grassland-Identity", signAdmin(ADMIN))
				.exchange().expectStatus().isBadRequest();
	}
	...
	private String currentId(String capability, String modelRole) {
		return db
				...
				.bind("capability", capability).bind("modelRole", modelRole)
	...
	private Long historyCountOf(String changeType) {
		return db.sql("SELECT COUNT(*) AS n FROM platform_model_config_history WHERE change_type = :t")
				.bind("t", changeType).map((row, meta) -> row.get("n", Long.class)).one().block();
	}
	...
	private String credentialIdOf(String capability, String modelRole) {
		return db
				.sql("SELECT credential_id::text AS id FROM platform_model_config "
						+ "WHERE capability = :capability AND model_role = :modelRole AND enabled = true")
				.bind("capability", capability).bind("modelRole", modelRole)
				.map((row, meta) -> row.get("id", String.class)).one().block();
	}
