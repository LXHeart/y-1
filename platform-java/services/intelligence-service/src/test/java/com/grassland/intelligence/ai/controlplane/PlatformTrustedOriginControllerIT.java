 ...
 *
 * <p>覆盖：V56 种子可见、CRUD 契约（重复 409 / 乐观锁 409 / 删除 404）、
 * 校验（HTTPS、剥 path）、模型/凭据保存的 origin 未命中 422 引导闭环，以及
 * <b>写后生效</b>（origin 增删后不重启，下一次校验即按新表——缓存事件重拉与
 * {@link TrustedOriginService#refresh()} 是同一条重拉路径，测试里同步调它消除异步时序）。
 */
...
class PlatformTrustedOriginControllerIT extends IntelligenceItSupport {
    ...
    void clean() {
        ...
        db.sql("DELETE FROM platform_trusted_origin WHERE origin NOT IN (:a, :b)")
                .bind("a", SEED_QWEN).bind("b", SEED_OPENAI).then().block();
    ...
    void seedsAreListed() {
        client().get().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .exchange().expectStatus().isOk()
                .expectBody()
                .jsonPath("$.length()").isEqualTo(2)
                .jsonPath("$[?(@.origin == '" + SEED_QWEN + "')].label")
                .isEqualTo("内置默认·Qwen/DashScope");
    }

    @Test
    @DisplayName("新增端点：合法 201；重复 409；HTTP 非回环 400；带路径 400")
    void createValidatesOriginShape() {
        client().post().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"origin\":\"https://api.minimaxi.com\",\"label\":\"MiniMax 图像\"}")
                .exchange().expectStatus().isCreated()
                .expectBody().jsonPath("$.origin").isEqualTo("https://api.minimaxi.com:443");

        client().post().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"origin\":\"https://api.minimaxi.com\"}")
                .exchange().expectStatus().isEqualTo(409);

        client().post().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"origin\":\"http://insecure.example.com\"}")
                .exchange().expectStatus().isBadRequest();

        client().post().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"origin\":\"https://api.example.com/v1\"}")
                .exchange().expectStatus().isBadRequest();
    }
    ...
    void updateUsesOptimisticLockAndDisablingTakesEffectImmediately() {
        ...
        client().put().uri("/api/admin/ai/trusted-origins/" + id)
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void semanticDuplicateWithSeedIsRejected() {
        ...
        client().post().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"origin\":\"https://dashscope.aliyuncs.com\"}")
                .exchange().expectStatus().isEqualTo(409);
    ...
    void deletesEndpoint() {
        ...
        client().delete().uri("/api/admin/ai/trusted-origins/" + id)
                .header("X-Grassland-Identity", signAdmin(ADMIN))
    ...
    void modelSaveGuidesToAddOriginFirst() {
        saveCredentialFor("https://fresh.example/v1")
                .expectStatus().isEqualTo(422)
                .expectBody().jsonPath("$.error")
                .value(message -> assertThat(String.valueOf(message))
                        .contains("受信")
    ...
    private String createOrigin(String origin) {
        byte[] body = client().post().uri("/api/admin/ai/trusted-origins")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"origin\":\"" + origin + "\",\"label\":\"测试\"}")
                .exchange().expectStatus().isCreated()
    ...
    private org.springframework.test.web.reactive.server.WebTestClient.ResponseSpec saveCredentialFor(
            String baseUrl) {
        return client().post().uri("/api/admin/ai/credentials")
                .header("X-Grassland-Identity", signAdmin(ADMIN))
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue("{\"name\":\"cred-" + baseUrl.hashCode() + "\",\"provider\":\"openai-compatible\","
                        + "\"baseUrl\":\"" + baseUrl + "\"}")
                .exchange();
    }
}
