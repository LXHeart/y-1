...
public class EngagementDecisionService {
	...
	public EngagementDecisionService(TaskApplicationRepository apps, SubmissionRepository submissions,
			EngagementVerificationRepository verifications, RatingRepository ratings,
			...
			SettlementWorkflowStarter settlementWorkflows, OutboxRepository outbox, TransactionalOperator transactions,
			TaskRepository tasks, com.grassland.marketplace.milestone.EngagementMilestoneService milestoneService,
			...
			EngagementActionContract actionContract, ApplicationMutationGuard mutationGuard) {
		this.apps = apps;
	 ...
	 *
	 * <p>
	 * <b>必须先有待核验的交付物</b>：此前 confirm 是凭空点的——推荐官交了什么、商家在确认什么，
	...
	public Mono<ResponseEntity<Map<String, Object>>> confirm(Task task, TaskApplication app,
		...
		return app.contestRequestedAt() != null
				...
				: app.confirmedAt() != null
						...
						: requiredDeclaredMetric(app, body).flatMap(metric -> mutationGuard
								.withLockedApplication(app.taskId(), app.id(),
										(lockedTask, lockedApp) -> confirmWork(lockedTask.id(), lockedApp.id(),
												lockedTask, metric.orElse(null), lockedApp))
								...
								.onErrorResume(ConfirmationConflict.class,
										conflict -> apps.findById(app.id())
	...
	/**
	 * D-02：阶梯佣金任务的手动确认必须携带商家申报的指标达成值（Sandbox 指标事实来源）； 固定佣金契约（快照无
	 * ladder）无此要求。快照解析失败按损坏处理，阻断确认。
	 */
	private Mono<Optional<Long>> requiredDeclaredMetric(TaskApplication app, ConfirmEngagementRequest body) {
		return apps.findTaskContextSnapshot(app.id())
				...
				.flatMap(ladder -> {
					...
					if (declared == null) {
						return Mono.error(new MarketplaceException(400, "阶梯佣金任务确认时必须申报指标达成值（confirmedMetricValue）"));
					}
					if (declared < 0) {
				...
				.onErrorMap(IllegalArgumentException.class, error -> new MarketplaceException(409, error.getMessage()))
				.defaultIfEmpty(Optional.empty());
	}

	/**
	 * 手动确认领域写：submission accepted + application confirmed（含 D-02 申报指标值）+ published
	 * 里程碑互签 + MerchantConfirmed outbox，同一事务（任务书 #103 C103-02 经父行锁执行，锁内终态先判）。
	 */
	private Mono<TaskApplication> confirmWork(String taskId, String appId, Task task, Long confirmedMetricValue,
		...
		return submissions.findPending(appId).switchIfEmpty(Mono.error(new ConfirmationConflict("推荐官尚未提交履约凭证，无法确认")))
				.flatMap(pending -> verifications.findEffectiveStatus(pending.id())
						.map(status -> "failed".equalsIgnoreCase(status)).defaultIfEmpty(false).flatMap(blocked -> {
							// 可选闸门：仅 failed 阻断 confirm（absent/passed/inconclusive 照常）。
							// GL-P2-ADMIN-004：生效状态含人工改判 override（override 优先于自动结论）。
							if (Boolean.TRUE.equals(blocked)) {
								return Mono.<EngagementSubmission>error(
										new MarketplaceException(409, "履约核验未通过，请退回重交或重新核验"));
							}
				...
				.flatMap(confirmed -> milestoneService.confirmPendingPublished(appId, task.ownerAccountId())
	...
	private Mono<ResponseEntity<Map<String, Object>>> resumeConfirmedSettlement(Task task,
	...
	private Mono<ResponseEntity<Map<String, Object>>> startSettlementWorkflow(Task task, TaskApplication app) {
		return settlementWorkflows.start(task, app)
				.map(wid -> ResponseEntity.status(org.springframework.http.HttpStatus.ACCEPTED).body(Map.of("success",
						true, "data", Map.of("workflowId", wid, "applicationId", app.id(), "status", "settling"))));
	}

	/**
	 * 商家拒绝「系统核实通过」的履约（D-03 §2）→ 先取得本地 durable contest 门闩，再转客服终审。 门闩与 manual/auto
	 * confirm 更新同一 application 行；一旦提交，Timer 即使尚未读到 trust 案也不得 capture。
	 */
	public Mono<ResponseEntity<Map<String, Object>>> contest(Task task, TaskApplication app, String reason) {
		...
		if (app.merchantConfirmDeadlineAt() == null || !app.merchantConfirmDeadlineAt().isAfter(Instant.now())) {
			return fail(409, "商家确认窗口已到期，不能再发起拒绝");
		}
		return submissions.findPending(app.id()).switchIfEmpty(fail(409, "无待确认的履约凭证"))
				.flatMap(submission -> verifications.findBySubmission(submission.id())
						.filter(v -> "passed".equalsIgnoreCase(v.status())).switchIfEmpty(fail(409, "仅系统核实通过的履约可转客服拒绝"))
						// claim 自身单独提交后才允许任何远端动作；与 Timer 的 guarded update 决胜。
						.then(transactions.transactional(
								apps.claimContest(app.id(), task.id(), reason).switchIfEmpty(fail(409, "该履约状态已变"))))
	...
	/**
	 * 轮询商家确认窗口状态（D-03）：返回 {@code status} + {@code deadline} +
	 * {@code remainingSeconds}（估算展示）。
	 * <ul>
	 * <li>{@code confirmed_at} 已设 → {@code confirmed}（已确认，结算进行中/已完成，细节走 settlement
	 * 轮询）。</li>
	 * <li>未确认 + 有 deadline → {@code awaiting_confirmation} + 倒计时（真正到期由 Temporal
	 * Timer 驱动，估算不作判定）。</li>
	...
	public ResponseEntity<Map<String, Object>> confirmationOutcome(TaskApplication app) {
		...
		return ApplicationBodies.ok(Map.of("status", "awaiting_confirmation", "deadline",
				app.merchantConfirmDeadlineAt().toString(), "remainingSeconds", remainingSeconds));
	}
	 ...
	 * <li>有对账行：reconciled→settled（钱已确认到位）、blocked/pending/started→held（reason
	 * 区分阻断/进行中）；</li>
	 * <li>无对账行（无争议的正常结算）→ 回退 outbox 的 EngagementSettled/SettlementHeld，否则
	 * settling。</li>
	 * </ul>
	 ...
	 * settlementStatus / holdReason / allowedActions。状态推导复用
	 * {@link #settlementOutcome} （对账行优先，outbox 回退），在其上叠加契约字段。T+N 未到窗口是正常的「等待结算」态
	...
	public Mono<ResponseEntity<Map<String, Object>>> settlementContract(Task task, TaskApplication app,
		...
		return settlementOutcome(app).zipWith(tasks.promotionEnded(task.id())).flatMap(tuple -> {
			...
			contract.put("holdReason",
					"held".equals(settlementStatus)
							? String.valueOf(outcomeData.getOrDefault("reason", "blocked"))
							: null);
			contract.put("confirmedAt", app.confirmedAt() == null ? null : app.confirmedAt().toString());
			contract.put("settlementEligibleAt",
					app.confirmedAt() == null
							? null
							: app.confirmedAt()
									.plusSeconds(com.grassland.marketplace.workflow.saga.SettlementWindowPolicy
											.windowSeconds(app, settlementDaySeconds, settlementDisputeWindowSeconds))
									.toString());
			...
			return actionContract
					.read(task, app, viewerIsManager, settlementStatus, (String) contract.get("holdReason"), due)
					.map(next -> {
	...
	/**
	 * 允许动作（受限闭集，按当前状态推导）：商家侧 accept/reject/confirm/contest/end_promotion， 推荐官侧
	 * withdraw/reconsent/submit。不可用动作不出现（前端据此渲染按钮，C90-06）。
	 */
	private static java.util.List<String> allowedActions(Task task, TaskApplication app, boolean viewerIsManager,
			boolean promotionEnded) {
	...
	public Mono<ResponseEntity<Map<String, Object>>> settlementOutcome(TaskApplication app) {
		if (app.merchantRejectedAt() != null) {
			return reconciliations.findLatestForApplication(app.id()).map(this::reconciliationOutcome)
		...
		return reconciliations.findLatestForApplication(app.id()).map(this::reconciliationOutcome)
				.switchIfEmpty(outbox.latestSettlementStatus(app.id()).map(ApplicationBodies::ok)
		...
		return switch (rec.status()) {
			case "reconciled" -> {
				String reason = rec.finalDecision() == null || rec.finalDecision().isBlank()
						? "adjudication"
						: "adjudication:" + rec.finalDecision();
				yield ApplicationBodies.ok(Map.of("status", "settled", "reason", reason));
			}
			case "blocked" -> ApplicationBodies
					.ok(Map.of("status", "held", "reason", rec.reason() == null ? "blocked" : rec.reason()));
	...
	public Mono<EngagementRating> rate(TaskApplication app, Caller merchant, RateEngagementRequest body) {
		return ratings
				.create(app.id(), app.taskId(), app.recommenderAccountId(), merchant.accountId(), body.score(),
						body.comment())
				.switchIfEmpty(fail(409, "该履约已评价过"))
				.flatMap(rating -> outbox.append(ApplicationEvents.ratingEnvelope(app, rating)).thenReturn(rating));
	}
