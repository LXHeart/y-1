...
/**
 * 退出资金恢复看门狗（任务书 #103 C103-03，照 EngagementExitExpireDispatcher 范式）： 有界批量（默认
 * 50）扫描 pending/到点 retry_wait 的退出操作并推进；单条失败/未知不阻断整批 （service 内部已分类退避或
 * needs_review）。租约由数据库行守卫保证多副本不重复执行； 停机接管依赖租约过期（默认 60s）后下一个 tick 自然领取。
 */
@Component
@ConditionalOnProperty(name = "marketplace.engagement.exit-funds.worker-enabled", havingValue = "true", matchIfMissing = true)
