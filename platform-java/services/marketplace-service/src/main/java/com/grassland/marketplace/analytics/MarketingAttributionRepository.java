...
public class MarketingAttributionRepository {
	...
	public MarketingAttributionRepository(DatabaseClient db) {
		this.db = db;
	}
	...
	public Mono<Campaign> create(CampaignRequest request, String accountId) {
		...
		var spec = db.sql("""
				...
				RETURNING %s
				""".formatted(CAMPAIGN_COLUMNS)).bind("id", id).bind("provider", request.provider())
	...
	public Mono<Campaign> findActive(String provider, String externalCampaignId) {
		return db
				.sql("SELECT " + CAMPAIGN_COLUMNS + " FROM marketing_attribution_campaign"
	...
	public Flux<Alert> listAlerts(String organizationId, String storeId, boolean includeResolved) {
		...
		var spec = db.sql("SELECT " + ALERT_COLUMNS + " FROM marketing_attribution_alert"
				...
				// 任务书 #77 卡 B（D2）连带：不传 storeId = 组织全量（旧口径只看主体级告警，新告警全带门店会漏）。
				+ " AND (:store IS NULL OR store_id=CAST(:store AS uuid))" + statusClause
	...
	public Mono<Void> syncAlerts(String organizationId, String storeId, List<AlertCandidate> candidates) {
		...
		String[] ruleCodes = active.isEmpty()
				? new String[]{"__none__"}
				: active.stream().map(AlertCandidate::ruleCode).toArray(String[]::new);
		return Flux.fromIterable(active).concatMap(candidate -> {
			var spec = db.sql("""
			...
			return nullable(spec, "store", storeId).then();
				...
				""").bind("scopeKey", scopeKey).bind("rules", ruleCodes).then());
	}

	public Mono<Boolean> acknowledge(String id, String accountId) {
		return db
				.sql("UPDATE marketing_attribution_alert SET status='acknowledged', acknowledged_at=now(),"
	...
	private static Alert mapAlert(Readable row) {
		return new Alert(row.get("id", String.class), row.get("organization_id", String.class),
				...
				row.get("severity", String.class), row.get("status", String.class), row.get("message", String.class),
				row.get("observed_value", Double.class), row.get("threshold_value", Double.class),
				instant(row.get("last_observed_at", OffsetDateTime.class)),
	...
	private static DatabaseClient.GenericExecuteSpec nullable(DatabaseClient.GenericExecuteSpec spec, String name,
			String value) {
	...
	public enum WebhookClaim {
		CLAIMED, DUPLICATE, PAYLOAD_CONFLICT
	}
}
