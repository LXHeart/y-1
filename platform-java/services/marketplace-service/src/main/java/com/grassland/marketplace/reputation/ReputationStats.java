 ...
 *
 * <p><b>完成率公式</b>（业务审查 2026-09-07 C05 口径）：完成数 / 推荐官责任范围内<b>已到终态或已到期</b>
 * 的内容履约数。商家取消（商家违约）与进行中（未到期）都从分母排除；被拒和接单前撤销从未形成接单，
 * 也不进入分母。推荐官有责终结态（履约超时/解约）随履约期限规则（任务书后续批次）落地后自然进入分母。
 * 无责任接单时为 0。
 */
public record ReputationStats(int acceptedCount, int completedCount,
                              ...
                              int inProgressCount, int promotionAcceptedCount,
    ...
    public ReputationStats(int acceptedCount, int completedCount,
        ...
        this(acceptedCount, completedCount, merchantCancelledCount, rejectedCount, withdrawnCount,
                0, 0, ratingCount, averageScore, averageResponseSeconds, lastActiveAt);
    }
    ...
    /**
     * 完成率 = 完成数 / (累计已接单 - 商家取消 - 进行中)。
     * 进行中是「未到期的合作」，进入分母会把正常接单记成失败（C05：10 完成 + 5 进行中应保持 100%）。
     */
