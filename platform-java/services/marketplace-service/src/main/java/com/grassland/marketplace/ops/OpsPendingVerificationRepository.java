 ...
 *
 * <p>
 * 查询自动核验为 inconclusive、交付物仍 submitted 且尚未有人工作出 override 的记录。 人工改判写入 marketplace
 * 的 {@code verification_override}，不改自动核验真相表。
 */
@Repository
public class OpsPendingVerificationRepository {
	...
	/** 列出待人工复核核验，最近核验的在前（offset 分页，任务书 #94 D94-09）。 */
	public Flux<OpsPendingVerification> list(int limit, int offset) {
		return db.sql("""
				...
				.map((row, meta) -> new OpsPendingVerification(row.get("verification_id", String.class),
						...
						row.get("organization_id", String.class), row.get("task_owner_account_id", String.class),
						row.get("recommender_account_id", String.class), row.get("content_url", String.class),
						row.get("checks_json", String.class), row.get("last_checked_at", java.time.Instant.class),
						row.get("submitted_at", java.time.Instant.class)))
				.all();
	}

	/** 列表同口径计数 —— 分页信封的 {@code total}。 */
	...
	public reactor.core.publisher.Mono<VerificationOverrideContext> findOverrideContext(String submissionId) {
		return db.sql("""
				...
				.map((row, meta) -> new VerificationOverrideContext(row.get("submission_id", String.class),
						row.get("application_id", String.class), row.get("task_id", String.class),
						row.get("organization_id", String.class), row.get("task_owner_account_id", String.class),
	...
	public record VerificationOverrideContext(String submissionId, String applicationId, String taskId,
			String organizationId, String taskOwnerAccountId, String recommenderAccountId) {
	}
}
