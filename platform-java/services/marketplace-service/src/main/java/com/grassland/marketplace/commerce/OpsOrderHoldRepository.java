 ...
 *
 * <p>flagged=自动标记候选（不碰钱）；held=人工确认（结算挂起 + 处理期限）；released/dismissed=终态。
...
public class OpsOrderHoldRepository {
	...
	public Mono<HoldRow> insertFlagged(UUID orderId, String rule, String reason) {
		return db.sql("INSERT INTO ops_order_hold(id, order_id, rule, reason, status)"
				+ " VALUES (CAST(:id AS uuid), CAST(:order AS uuid), :rule, :reason, 'flagged')"
				+ " ON CONFLICT (order_id, rule) WHERE status IN ('flagged', 'held') DO NOTHING"
				+ " RETURNING " + COLS)
				.bind("id", UUID.randomUUID().toString()).bind("order", orderId.toString())
				.bind("rule", rule).bind("reason", reason)
				.map(OpsOrderHoldRepository::map).one();
	}
	...
	public Flux<HoldRow> listQueue(String status, int limit) {
		...
		return db.sql("SELECT " + COLS + " FROM ops_order_hold WHERE status IN ('" + safe + "')"
	...
	public Mono<Boolean> hasActiveHold(String orderId) {
		return db.sql("SELECT 1 AS ok FROM ops_order_hold WHERE order_id = CAST(:order AS uuid)"
				+ " AND status = 'held' LIMIT 1").bind("order", orderId).map(row -> true).one().defaultIfEmpty(false);
	}

	/** 人工确认：flagged → held（写原因期限与操作者）；0 行 → empty（调用方 409/404）。 */
	public Mono<HoldRow> confirm(UUID holdId, UUID operator, Instant deadline) {
		return db.sql("UPDATE ops_order_hold SET status = 'held', confirmed_by = CAST(:op AS uuid),"
	...
	public Mono<HoldRow> dismiss(UUID holdId, UUID operator) {
		return db.sql("UPDATE ops_order_hold SET status = 'dismissed', released_by = CAST(:op AS uuid),"
	...
	private static HoldRow map(Readable row) {
		return new HoldRow(row.get("id", UUID.class), row.get("order_id", UUID.class), row.get("rule", String.class),
				...
				row.get("released_by", UUID.class), instant(row, "released_at"), row.get("released_reason", String.class));
	}
