...
/**
 * finance escrow 出站 HTTP 客户端（草场 Epic 4 Slice 4F / HLD 5.4、11.1）。marketplace
 * Saga 调 finance 的 ReserveFunds / ReleaseFunds。WebClient 已在 spring-webflux
 * classpath（无新依赖）。
 *
 * <ul>
 * <li>{@code reserve(orgId, engagementRef, amountCents)} → POST
 * /api/finance/accounts/{orgId}/reservations， 仅完整匹配请求 scope 的
 * 2xx→Reserved、明确余额不足的 409→InsufficientFunds；其余→抛异常（Temporal 重试）。</li>
 * <li>{@code release(orgId, engagementRef)} → POST
 * /api/finance/reservations/{ref}/release（任务书 #90 D90-02 验证语义）：200 核对回包；409
 * 回读核对确为 released 才算幂等成功；404 无预留可退；其余抛异常。</li>
 * <li>{@code captureVerified(...)} → POST
 * /api/finance/reservations/{ref}/capture：成功/已处理/异常三态， 404 与 409
 * 均回读核对状态、金额、组织、收款人，不一致返回对账结论而非静默成功。</li>
 * </ul>
 *
 * <p>
 * 每请求由 {@link ServiceAssertionIssuer} 现签 {@code X-Grassland-Identity} 服务断言（带
 * org，principal=marketplace）。 org 用于 finance 的 org 级授权自查（release 的 org 校验在
 * finance 加载 reservation 后做，断言需带正确 org）。
 */
@Component
public class FinanceEscrowClient {
	...
	private static final ParameterizedTypeReference<Envelope<ReservationData>> RESERVATION_TYPE = new ParameterizedTypeReference<>() {
	};
	private static final ParameterizedTypeReference<ErrorEnvelope> ERROR_TYPE = new ParameterizedTypeReference<>() {
	};
	private static final ParameterizedTypeReference<Envelope<java.util.Map<String, Object>>> FACTS_TYPE = new ParameterizedTypeReference<>() {
	};
	...
	public Mono<ReserveResult> reserve(String orgId, String engagementRef, long amountCents, String payeeAccountId,
		...
		return webClient.post().uri("/api/finance/accounts/{orgId}/reservations", orgId)
				.header(headerName, issuer.issueForOrg(orgId, "grassland-finance"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(new ReserveRequestPayload(engagementRef, amountCents, payeeAccountId, commissionBonusBps))
				.exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					log.info("reserve HTTP {} org={} ref={}", code, orgId, engagementRef);
					return switch (code) {
						case 200, 201 -> verifiedReservation(resp, orgId, engagementRef, amountCents, payeeAccountId,
								commissionBonusBps);
	...
	private Mono<ReserveResult> verifiedReservation(ClientResponse response, String organizationId,
			String engagementRef, long amountCents, String payeeAccountId, int commissionBonusBps) {
		return response.bodyToMono(RESERVATION_TYPE)
				...
				.map(envelope -> verifyReservation(envelope, organizationId, engagementRef, amountCents, payeeAccountId,
						commissionBonusBps))
	...
	private ReserveResult verifyReservation(Envelope<ReservationData> envelope, String organizationId,
			String engagementRef, long amountCents, String payeeAccountId, int commissionBonusBps) {
		...
		boolean matches = Boolean.TRUE.equals(envelope.success()) && data != null
				...
				&& Objects.equals(expectedBonusCents, data.commissionBonusCents()) && "reserved".equals(data.status());
		if (!matches) {
	...
	private Mono<ReserveResult> reserveConflict(ClientResponse response, int code) {
		return response.bodyToMono(ERROR_TYPE)
				...
				.flatMap(envelope -> {
					...
					return Mono.error(
							new FinanceEscrowException("reserve failed: HTTP " + code + ": " + envelope.error()));
				})
				.onErrorMap(error -> error instanceof FinanceEscrowException
						? error
						: new FinanceEscrowException(
	...
	private Mono<ReserveResult> responseError(ClientResponse response, int code) {
		return response.bodyToMono(String.class).defaultIfEmpty("")
				.flatMap(body -> Mono.error(new FinanceEscrowException("reserve failed: HTTP " + code + ": " + body)));
	}
	...
	/**
	 * 释放预留（任务书 #90 D90-02 验证语义）：200 须回包核对组织/引用/终态；409 须回读核对确为 released 才按幂等成功（已
	 * captured/refunded 的预留绝不能当已释放——抛异常交上层对账）； 404 = 无预留可退（补偿/取消幂等 no-op，reserve
	 * 可能未发生过）；其余抛异常重试。
	 */
	public Mono<Void> release(String orgId, String engagementRef) {
		return webClient.post().uri("/api/finance/reservations/{ref}/release", engagementRef)
				.header(headerName, issuer.issueForOrg(orgId, "grassland-finance")).exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					...
					return resp.bodyToMono(String.class).defaultIfEmpty("").flatMap(b -> Mono
							.<Void>error(new FinanceEscrowException("release failed: HTTP " + code + ": " + b)));
				});
	}
	...
	private Mono<ReservationData> verifiedRelease(ClientResponse response, String organizationId,
		...
		return response.bodyToMono(RESERVATION_TYPE)
				...
				.map(envelope -> {
					...
					boolean matches = Boolean.TRUE.equals(envelope.success()) && data != null
							&& Objects.equals(organizationId, data.organizationId())
							&& Objects.equals(engagementRef, data.engagementRef()) && "released".equals(data.status());
					if (!matches) {
	...
	private Mono<Void> confirmReleasedAfterConflict(String organizationId, String engagementRef) {
		return fetchReservation(organizationId, engagementRef)
				.switchIfEmpty(Mono.error(
						new FinanceEscrowException("release failed: reservation vanished after 409: " + engagementRef)))
	 ...
	 * {@code expectedAmountCents}/{@code expectedPayeeAccountId} 为 accept
	 * 时冻结的赏金与收款推荐官—— 200 与 409 幂等回读都要核对组织、引用、金额、收款人，一致才算捕获成功；不一致返回
	 * {@link CaptureOutcome#reconciliationRequired(String)}，调用方<b>不得发
	 * settled</b>，转对账处置。
	 */
	public Mono<CaptureOutcome> captureVerified(String orgId, String engagementRef, long expectedAmountCents,
		...
		return webClient.post().uri("/api/finance/reservations/{ref}/capture", engagementRef)
				.header(headerName, issuer.issueForOrg(orgId, "grassland-finance"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(new CaptureRequestPayload(settlementAmountCents))
				.exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					log.info("capture HTTP {} org={} ref={}", code, orgId, engagementRef);
					if (code == 200) {
						return verifiedCapture(resp, orgId, engagementRef, expectedAmountCents, expectedPayeeAccountId);
					}
					if (code == 404) {
					...
					return resp.bodyToMono(String.class).defaultIfEmpty("").flatMap(b -> Mono.<CaptureOutcome>error(
							new FinanceEscrowException("capture failed: HTTP " + code + ": " + b)));
				});
	}

	private Mono<CaptureOutcome> verifiedCapture(ClientResponse response, String organizationId, String engagementRef,
	...
	private Mono<CaptureOutcome> classifiedCaptureConflict(String organizationId, String engagementRef,
			long expectedAmountCents, String expectedPayeeAccountId) {
		return fetchReservation(organizationId, engagementRef).<CaptureOutcome>map(data -> {
	...
	private static boolean captureScopeMatches(Envelope<ReservationData> envelope, ReservationData data,
			String organizationId, String engagementRef, long expectedAmountCents, String expectedPayeeAccountId) {
		boolean envelopeOk = envelope == null || Boolean.TRUE.equals(envelope.success());
		return envelopeOk && data != null && Objects.equals(organizationId, data.organizationId())
				&& Objects.equals(engagementRef, data.engagementRef())
	...
	Mono<ReservationData> fetchReservation(String organizationId, String engagementRef) {
		return webClient.get().uri("/api/finance/reservations/{ref}", engagementRef)
				.header(headerName, issuer.issueForOrg(organizationId, "grassland-finance")).exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					if (code == 200) {
						return resp.bodyToMono(RESERVATION_TYPE)
								.switchIfEmpty(Mono.error(
										new FinanceEscrowException("fetch reservation failed: empty success response")))
								.map(envelope -> {
								...
								.onErrorMap(error -> error instanceof FinanceEscrowException
										? error
										: new FinanceEscrowException(
												"fetch reservation failed: " + error.getMessage()));
					}
					if (code == 404) {
						return Mono.empty();
					}
					return resp.bodyToMono(String.class).defaultIfEmpty("").flatMap(b -> Mono.<ReservationData>error(
							new FinanceEscrowException("fetch reservation failed: HTTP " + code + ": " + b)));
				});
	}

	/**
	 * capture 结算校验结论（D90-02）：{@code captured=true} 才允许发 settled 事件
	 * （{@link #alreadyCaptured()} 是经回读核对的幂等重试，重发确定性 event_id 仍 exactly-once）； 否则
	 * {@code reconciliationReason} 供结算侧登记对账处置单。
	 */
	/**
	 * 退出资金权威事实回读（任务书 #103 C103-03 / §6.2）：GET
	 * /internal/engagements/{ref}/exit-facts。 只读；200 返回 bounty/deposit 已提交事实；404 =
	 * 本组织无该 engagement 的事实；其余异常交上层 走「结果未知先核实」路径（不按失败处理）。
	 */
	public Mono<Map<String, Object>> exitFacts(String orgId, String engagementRef) {
		return webClient.get().uri("/internal/engagements/{ref}/exit-facts?organizationId={org}", engagementRef, orgId)
				.header(headerName, issuer.issueForOrg(orgId, "grassland-finance")).exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					...
					return responseError(resp, code)
							.then(Mono.error(new FinanceEscrowException("exit-facts failed: HTTP " + code)));
				});
	}
	 ...
	 * 2xx→Reserved；409「钱包余额不足」→InsufficientFunds（镜像商家 reserve 语义，Saga 补偿回
	 * pending）；其余→抛异常。
	 */
	public Mono<ReserveResult> freebieReserve(String orgId, String engagementRef, long amountCents,
		...
		return webClient.post().uri("/internal/freebie/reserve")
				.header(headerName, issuer.issueForOrg(orgId, "grassland-finance"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(new FreebieReservePayload(engagementRef,
						recommenderAccountId, taskOwnerAccountId, orgId, amountCents))
				.exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					...
					return switch (code) {
						...
						case 409 -> resp.bodyToMono(ERROR_TYPE)
								.switchIfEmpty(Mono.error(
										new FinanceEscrowException("freebie reserve failed: HTTP 409: empty response")))
						...
						default ->
							resp.bodyToMono(String.class).defaultIfEmpty("").flatMap(body -> Mono.<ReserveResult>error(
	...
	private Mono<Void> postFreebieLifecycle(String orgId, String engagementRef, String action) {
		return webClient.post().uri("/internal/freebie/{ref}/" + action, engagementRef)
				.header(headerName, issuer.issueForOrg(orgId, "grassland-finance")).exchangeToMono(resp -> {
					int code = resp.statusCode().value();
					...
					return resp.bodyToMono(String.class).defaultIfEmpty("").flatMap(b -> Mono.<Void>error(
	...
	private record ReserveRequestPayload(String engagementRef, long amountCents, String payeeAccountId,
			int commissionBonusBps) {
	}

	private record FreebieReservePayload(String engagementRef, String recommenderAccountId, String taskOwnerAccountId,
	...
	private record ReservationData(String organizationId, String engagementRef, Long amountCents, String payeeAccountId,
			Integer commissionBonusBps, Long commissionBonusCents, String status) {
	}
}
