 ...
 * historyTable=marketplace_flyway_schema；与 identity 历史表隔离；共库多服务并发迁移，transactional lock 按既有口径关闭。
 */
@Configuration
...
public class MarketplaceDataSourceConfig {
    ...
    Flyway flyway(DataSource dataSource, Environment env) {
        return FlywayBootstrap.flyway(
                dataSource,
                "marketplace_flyway_schema",
                env.getProperty("marketplace.flyway.locations", "classpath:db/migration"),
                true);
    }
}
