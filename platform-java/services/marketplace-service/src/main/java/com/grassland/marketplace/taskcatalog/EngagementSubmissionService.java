...
/**
 * 履约交付物领域服务（PRD 九 + 7C 事务 + D-03）：提交/列表/商家退回（补证上限）、附件校验（IDOR 守卫）、 附件下载 URL
 * 中转、商家确认窗口 Saga 启动。可见性守卫（owner 或提交人）由控制器完成后传入。
 */
@Component
public class EngagementSubmissionService {
	...
	/**
	 * 缺口清偿之九：评论类互动的评论文本契约——actionType=comment 必填 ≤500 字；非 comment 任务 携带该字段一律
	 * 400（评论文本是互动任务专属证据，不与其它内容形式混用）。
	 */
	public static Mono<Void> requireCommentText(Task task, String commentText) {
		...
		boolean commentTask = TaskRequirements.isInteractionForm(task.contentForm()) && interaction != null
		...
		if (commentText != null && !commentText.isBlank()) {
			...
			if (commentText.trim().length() > CreateSubmissionRequest.MAX_COMMENT_TEXT) {
				return Mono.error(
						new MarketplaceException(400, "评论内容最多 " + CreateSubmissionRequest.MAX_COMMENT_TEXT + " 字"));
			}
		}
		return Mono.empty();
	}
	 ...
	 */
	public Mono<List<AttachmentInput>> validateAttachments(String orgId, String ownerAccountId, String applicationId,
			List<UUID> mediaIds) {
		if (mediaIds.isEmpty()) {
			return Mono.just(List.of());
		}
		return Flux.fromIterable(mediaIds)
				.concatMap(mediaId -> mediaClient.metadata(orgId, mediaId, "application", applicationId)
						.switchIfEmpty(fail(404, "附件不存在或已不可用")).filter(m -> ownerAccountId.equals(m.ownerAccountId()))
						.switchIfEmpty(fail(403, "不能挂接他人的附件"))
						.filter(m -> "application".equals(m.domainType()) && applicationId.equals(m.domainId()))
						.switchIfEmpty(fail(404, "附件不属于当前报名")).map(m -> new AttachmentInput(mediaId, m.mimeType(),
								m.sizeBytes(), m.domainType(), m.domainId(), m.checksum(), m.status(), 1)))
				.collectList();
	}
	 ...
	 */
	public Mono<EngagementSubmission> submit(TaskApplication app, String appId, Caller caller, String contentUrl,
	 ...
	 * <p>
	 * D-03 规则 4：补证（退回）次数上限 {@code marketplace.confirmation.supplement-cap}（默认 2）。超出
	 * → 409， 不再允许补证，submission 留 submitted，确认窗口照常跑到自动结算（规则 1）。
	 */
	...
	/**
	 * 列交付物（含历史）及其附件与核验记录；无交付物 → 空列表。 评论类交付物附带 commentFlagged（运营人工复核判违规 →
	 * true，之九遗留清偿）——商家据此 在 confirm/reject 时考虑平台内容违规标记（平台只标记，业务决策仍归商家）。
	 */
	public Mono<List<Map<String, Object>>> list(String appId) {
		return submissions.findByApplication(appId).collectList().flatMap(list -> {
			if (list.isEmpty()) {
				return Mono.just(List.<Map<String, Object>>of());
			}
			List<String> submissionIds = list.stream().map(EngagementSubmission::id).toList();
			Mono<List<EngagementSubmissionAttachment>> attsM = attachments.findBySubmissionIds(submissionIds)
					.collectList();
			Mono<List<EngagementVerification>> verifsM = verifications.findBySubmissions(submissionIds).collectList();
			Mono<java.util.Set<String>> violationsM = commentReviews.findViolations(submissionIds).collectList()
					.map(java.util.HashSet::new);
			return Mono.zip(attsM, verifsM, violationsM)
					.map(t -> list.stream().map(s -> ApplicationBodies.submissionWithRows(s,
							t.getT1().stream().filter(a -> a.submissionId().equals(s.id())).toList(),
							t.getT2().stream().filter(v -> v.submissionId().equals(s.id())).findFirst().orElse(null),
							t.getT3().contains(s.id()))).toList());
		});
	}

	/**
	 * 附件短时下载 URL（Slice 11 Stage 2）：证 media 确挂该 submission（JOIN 限定
	 * application，防跨履约越权）， 再经 intelligence 取 presigned URL。media 已删/不可用 → 404。
	 */
	public Mono<Map<String, Object>> downloadUrl(Task task, String appId, String submissionId, UUID mediaId) {
		return attachments.findOne(appId, submissionId, mediaId.toString()).switchIfEmpty(fail(404, "附件未挂接到该交付物"))
				.flatMap(att -> mediaClient.downloadUrl(task.organizationId(), mediaId, "application", appId)
						.switchIfEmpty(fail(404, "附件已不可用")).map(ApplicationBodies::download));
	}
	 ...
	 *
	 * <p>
	 * fire-and-forget；submitDeliverable 响应仍是 201（提交回执），窗口状态经
	 * {@code GET .../confirmation}
	 * 轮询。{@code workflowId = "confirm-" + appId}、双击去重（{@code WorkflowExecutionAlreadyStarted}
	 * → 复用）。
	 */
	public Mono<String> startConfirmation(Task task, TaskApplication app, EngagementSubmission submission) {
		return confirmationWorkflows
				.start(app.id(), submission.id(), task.organizationId(), confirmationWindowSeconds,
						confirmationReminderLeadSeconds)
	...
	private Mono<List<EngagementSubmissionAttachment>> attachAll(String submissionId, List<AttachmentInput> inputs) {
		...
		return attachments.attach(submissionId, inputs).switchIfEmpty(fail(409, "附件重复，请勿重复挂接相同附件"));
	}
