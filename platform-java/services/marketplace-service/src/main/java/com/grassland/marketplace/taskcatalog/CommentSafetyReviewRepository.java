...
/**
 * {@code comment_safety_review} 仓储（缺口清偿之九遗留清偿）：评论词库 advisory（low/medium）
 * 命中的人工复核队列。提交链路落 open 行（失败 advisory——不影响提交本身）；运营复核
 * confirmed/violation；violation 经 {@link #findViolations} 在交付物列表透出
 * commentFlagged。 设计对齐 verification_override：不改自动词库检查真相，人工结论独立成表。
 */
@Component
public class CommentSafetyReviewRepository {
	...
	public record ReviewRow(UUID submissionId, String commentText, String findingsJson, String lexiconVersion,
			String status, String reviewerAccountId, String reviewNote, Instant reviewedAt, Instant createdAt) {
	}

	/** 运营队列行：复核结论 + 任务/履约上下文（运营判定需要看任务与提交时间）。 */
	public record QueueRow(UUID submissionId, String commentText, String findingsJson, String status, Instant createdAt,
			String taskId, String taskTitle, String platform, String recommenderAccountId, Instant submittedAt,
			String submissionStatus, String reviewerAccountId, String reviewNote, Instant reviewedAt) {
	}

	/** 提交链路落 open 行；同 submission 幂等（DO NOTHING——重放不覆盖既有复核结论）。 */
	public Mono<Void> recordOpen(String submissionId, String commentText, String findingsJson, String lexiconVersion) {
		return db.sql("""
				INSERT INTO comment_safety_review (
				    submission_id, comment_text, findings, lexicon_version, status)
				VALUES (CAST(:submission AS uuid), :commentText, CAST(:findings AS jsonb),
				        :lexiconVersion, 'open')
				ON CONFLICT (submission_id) DO NOTHING
				""").bind("submission", submissionId).bind("commentText", commentText).bind("findings", findingsJson)
	...
	public Flux<QueueRow> listQueue(String status, int limit) {
		return db.sql("""
				SELECT r.submission_id::text, r.comment_text, r.findings::text AS findings_json,
	...
	public Mono<ReviewRow> find(String submissionId) {
		return db.sql("""
				SELECT submission_id::text, comment_text, findings::text AS findings_json,
				       lexicon_version, status, reviewer_account_id, review_note, reviewed_at, created_at
				FROM comment_safety_review WHERE submission_id = CAST(:submission AS uuid)
				""").bind("submission", submissionId).map(CommentSafetyReviewRepository::mapRow).one();
	}

	/**
	 * 人工复核：confirmed（无问题）/ violation（违规）。可重判（upsert 修正，对齐 verification_override
	 * 口径）；行由提交链路创建，无行 = 该评论从未命中 advisory → 404。
	 */
	public Mono<ReviewRow> decide(String submissionId, String status, String reviewerAccountId, String reviewNote) {
		var spec = db.sql("""
				UPDATE comment_safety_review SET
				    ...
				    review_note = :note,
				    reviewed_at = now()
				WHERE submission_id = CAST(:submission AS uuid)
				RETURNING submission_id::text, comment_text, findings::text AS findings_json,
				          lexicon_version, status, reviewer_account_id, review_note,
				          reviewed_at, created_at
				""").bind("submission", submissionId).bind("status", status).bind("reviewer", reviewerAccountId);
	...
	private static ReviewRow mapRow(Readable row) {
		return new ReviewRow(UUID.fromString(row.get("submission_id", String.class)),
	...
	private static QueueRow mapQueueRow(Readable row) {
		return new QueueRow(UUID.fromString(row.get("submission_id", String.class)),
				row.get("comment_text", String.class), row.get("findings_json", String.class),
