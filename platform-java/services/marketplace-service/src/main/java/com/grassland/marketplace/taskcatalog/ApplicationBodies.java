...
final class ApplicationBodies {
	...
	static Map<String, Object> ranked(TaskApplication app, ReputationSnapshot snapshot) {
		...
		body.put("reputationTitle", snapshot.policy().ruleFor(snapshot.evaluation().effectiveLevel()).title());
	...
	static ResponseEntity<Map<String, Object>> acceptanceResponse(AcceptanceCommand command, String status,
	 ...
	 * commentFlagged：该评论已被运营人工复核判违规（无复核行/未判/无问题均不带该字段）。
	 */
	static Map<String, Object> submissionWithRows(EngagementSubmission submission,
			List<EngagementSubmissionAttachment> rows, EngagementVerification verification, boolean commentFlagged) {
		Map<String, Object> m = toBody(submission);
	...
	static Map<String, Object> run(EngagementVerificationRun run) {
		...
		body.put("id", run.id());
		body.put("runNumber", run.runNumber());
		body.put("engineVersion", run.engineVersion());
		...
		body.put("checks", parseChecks(run.checksJson()));
		body.put("triggeredBy", run.triggeredBy());
	...
	/** 对象 → JSON 文本（comment_safety_review.findings 等落库快照用）。 */
