...
/**
 * Structured, versioned task contract consumed by PRD section 4.12 creation
 * snapshots.
 */
public record TaskRequirements(String productServiceInfo, List<String> mustInclude, List<String> forbiddenContent,
		Instant publishStartAt, Instant publishEndAt, List<String> metricRequirements,
		List<String> evidenceRequirements,
		/** Optional single-metric D-02 policy; null keeps the fixed-payout contract. */
		CommissionLadder commissionLadder, /** 任务书 #23 / ADR-D13：点赞互动配置块；仅 contentForm=interaction 任务非空（三入口交叉校验）。 */
	 ...
	 */
	public TaskRequirements(String productServiceInfo, List<String> mustInclude, List<String> forbiddenContent,
			Instant publishStartAt, Instant publishEndAt, List<String> metricRequirements,
			List<String> evidenceRequirements) {
		this(productServiceInfo, mustInclude, forbiddenContent, publishStartAt, publishEndAt, metricRequirements,
				evidenceRequirements, null, null);
	}

	public TaskRequirements(String productServiceInfo, List<String> mustInclude, List<String> forbiddenContent,
			Instant publishStartAt, Instant publishEndAt, List<String> metricRequirements,
			List<String> evidenceRequirements, CommissionLadder commissionLadder) {
		this(productServiceInfo, mustInclude, forbiddenContent, publishStartAt, publishEndAt, metricRequirements,
				evidenceRequirements, commissionLadder, null);
	}
	...
	/**
	 * 任务书 #23 R2
	 * 交叉校验：{@code contentForm=interaction ⇔ requirements.interaction 非空}。 挂
	 * create/update/revise 三入口（请求 record 构造后调用，IllegalArgumentException→400）。
	 *
	 * <p>
	 * 任务书 #97 D97-03（付费互动停供）：显式携带 {@code contentForm=interaction} 的创建/修订请求 一律
	 * 400——存量数据零迁移，展示/报名/提交/审稿/结算路径照旧至自然到期（不触本守卫）。
	 */
	 ...
	 * 不新写一套；{@code actionType} 受控 like|favorite|follow|comment（评论为缺口清偿之九，提交侧 L1
	 * 词库审核）。 v1 单目标单次动作。
	 */
