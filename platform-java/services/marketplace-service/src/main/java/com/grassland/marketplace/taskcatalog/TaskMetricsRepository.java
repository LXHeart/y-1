...
/** Read-side aggregation over task matching facts. No counters are duplicated here. */
@Component
public class TaskMetricsRepository {
    ...
    public Flux<TaskProgress> findProgressByTaskIds(Collection<String> taskIds) {
        List<String> ids = taskIds == null ? List.of() : taskIds.stream().filter(id -> id != null && !id.isBlank()).toList();
        if (ids.isEmpty()) {
            return Flux.empty();
        }
        return db.sql("""
                ...
                """)
                .bind("taskIds", ids)
                .map((row, metadata) -> new TaskProgress(
                        row.get("task_id", String.class), value(row.get("total", Integer.class)),
                        value(row.get("pending", Integer.class)), value(row.get("reserving", Integer.class)),
                        value(row.get("accepted", Integer.class)), value(row.get("rejected", Integer.class)),
                        value(row.get("withdrawn", Integer.class)), value(row.get("refunded", Integer.class)),
                        value(row.get("submitted", Integer.class)), value(row.get("confirmed", Integer.class)),
                        value(row.get("settled", Integer.class)), value(row.get("occupied", Integer.class)),
                        valueLong(row.get("reserved_bounty", Long.class)),
                        valueLong(row.get("confirmed_bounty", Long.class))))
                .all();
    }

    public Mono<MerchantDashboard> dashboard(
            String organizationId, String storeId, Instant from, Instant to) {
        String scope = "t.organization_id = CAST(:organizationId AS uuid)"
                + (storeId == null ? " AND t.store_id IS NULL" : " AND t.store_id = CAST(:storeId AS uuid)")
                + (from == null ? "" : " AND t.created_at >= :from")
                + (to == null ? "" : " AND t.created_at < :to");
        var spec = db.sql("""
        ...
        if (storeId != null) spec = spec.bind("storeId", storeId);
        if (from != null) spec = spec.bind("from", from.atOffset(ZoneOffset.UTC));
        if (to != null) spec = spec.bind("to", to.atOffset(ZoneOffset.UTC));
        return spec.map(row -> {
            int applications = value(row.get("applications", Integer.class));
            int accepted = value(row.get("accepted", Integer.class));
            return new MerchantDashboard(organizationId, storeId,
                    value(row.get("task_count", Integer.class)), value(row.get("published_count", Integer.class)),
                    applications, accepted, value(row.get("confirmed", Integer.class)),
                    value(row.get("settled", Integer.class)), valueLong(row.get("reserved_bounty", Long.class)),
                    valueLong(row.get("settled_bounty", Long.class)),
                    applications == 0 ? 0d : ((double) accepted / applications),
                    row.get("average_rating", Double.class), false, false, false,
                    "not_collected");
        }).one().defaultIfEmpty(new MerchantDashboard(
                organizationId, storeId, 0, 0, 0, 0, 0, 0, 0L, 0L, 0d,
                null, false, false, false, "not_collected"));
    }
