 ...
 * <li>{@link ApplicationAcceptanceService}：单条/批量/dispatcher 共享接受内核、预留结局（Slice
 * 4F / ADR-D12）；</li>
 ...
 *
 * <p>
 * 资源级自查：accept/reject/reservation/settlement 等校验
 * caller==task.owner（loadManageableTask）， withdraw 把 recommender 烧进 WHERE（HLD
 * 7.4）。阻塞的 WorkflowClient 调用在服务内包 {@code boundedElastic}。
 */
@RestController
public class ApplicationController {
	...
	public Mono<ResponseEntity<Map<String, Object>>> apply(@PathVariable String id,
			@RequestBody(required = false) ApplyRequest body, ServerHttpRequest request) {
		String note = body == null ? null : body.note();
		return callers.requireRecommender(request)
				.flatMap(rec -> tasks.findById(id).switchIfEmpty(fail(404, "任务不存在"))
	...
	public Mono<ResponseEntity<Map<String, Object>>> reservation(@PathVariable String id, @PathVariable String appId,
			ServerHttpRequest request) {
		return callers.resolve(request).flatMap(caller -> loadManageableTask(id, caller)
	...
	/**
	 * 推荐官提交履约交付物（PRD 九第一步）。须是**本人**的、**已接受**的报名； 已有一份待核验的交付物时 409（防重复提交刷屏，被退回后可重交）。
	 */
	@PostMapping("/api/tasks/{id}/applications/{appId}/submissions")
	public Mono<ResponseEntity<Map<String, Object>>> submitDeliverable(@PathVariable String id,
			@PathVariable String appId, @RequestBody CreateSubmissionRequest body, ServerHttpRequest request) {
		return callers.requireRecommender(request).flatMap(caller -> loadAcceptedApp(id, appId)
						...
						.flatMap(task -> EngagementSubmissionService
								...
								.flatMap(safety -> submissionService
										.validateAttachments(task.organizationId(), caller.accountId(), appId,
												body.mediaIds())
										.flatMap(atts -> submissionService
												.submit(app, appId, caller, body.contentUrl(), body.note(), atts,
												...
												.flatMap(created -> recordCommentReview(created.id(), safety,
														...
														.onErrorResume(failure -> {
															log.warn(
																	"comment safety review record failed submission={}",
																	created.id(), failure);
															return Mono.empty();
														}).then(submissionService.startConfirmation(task, app, created)
																// DB 提交已成功，Temporal 瞬时失败不把提交回成 5xx；dispatcher 扫未标记行补启。
																.onErrorResume(failure -> {
																	log.warn(
																			"confirmation workflow initial start failed submission={} app={}",
																			created.id(), app.id(), failure);
																	return Mono.empty();
																})
																.then(Mono
																		.defer(() -> verificationService.runAndRecord(
																				task, app, created, caller.accountId()))
																		.doOnError(failure -> log.warn(
																				"automatic verification failed submission={}",
																				created.id(), failure))
																		// Submission is already committed. Verification
																		// remains retryable
																		// through the explicit checks endpoint and must
																		// not turn it into 5xx.
																		.onErrorResume(failure -> Mono.empty()))
																.thenReturn(created)))
												.map(created -> ResponseEntity.status(HttpStatus.CREATED).body(Map.of(
														"success", true, "data",
														ApplicationBodies.submissionWithInputs(created, atts)))))))));
	}

	/** 列交付物（含历史）及其附件。商家（任务 owner）与本人推荐官可见。 */
	@GetMapping("/api/tasks/{id}/applications/{appId}/submissions")
	public Mono<ResponseEntity<Map<String, Object>>> listSubmissions(@PathVariable String id,
			@PathVariable String appId, ServerHttpRequest request) {
		return callers.resolve(request).flatMap(caller -> loadSubmissionScoped(id, appId, caller).flatMap(
				task -> submissionService.list(appId).map(rows -> ApplicationBodies.ok(Map.of("submissions", rows)))));
	}

	/**
	 * 取某附件的短时下载 URL（草场 Slice 11 Stage 2）。可见性与 listSubmissions 一致（owner 或提交人）， 挂接核验与
	 * presigned URL 中转在 {@link EngagementSubmissionService#downloadUrl}（JOIN 限定
	 * application， 防跨履约越权）。media 已删/不可用 → 404。
	 */
	@GetMapping("/api/tasks/{id}/applications/{appId}/submissions/{submissionId}/attachments/{mediaId}/download-url")
	public Mono<ResponseEntity<Map<String, Object>>> attachmentDownloadUrl(@PathVariable String id,
			@PathVariable String appId, @PathVariable String submissionId, @PathVariable UUID mediaId,
			ServerHttpRequest request) {
		return callers.resolve(request)
				.flatMap(caller -> loadSubmissionScoped(id, appId, caller).flatMap(task -> submissionService
						.downloadUrl(task, appId, submissionId, mediaId).map(dl -> ApplicationBodies.ok(dl))));
	}
	...
	public Mono<ResponseEntity<Map<String, Object>>> rejectDeliverable(@PathVariable String id,
			@PathVariable String appId, @PathVariable String submissionId,
			@RequestBody(required = false) ReviewSubmissionRequest body, ServerHttpRequest request) {
		String note = body == null ? null : body.note();
		return callers.requireUser(request)
				.flatMap(merchant -> loadManageableTask(id, merchant).flatMap(task -> loadAcceptedApp(id, appId)
	...
	public Mono<ResponseEntity<Map<String, Object>>> contest(@PathVariable String id, @PathVariable String appId,
			@RequestBody ContestEngagementRequest body, ServerHttpRequest request) {
		return callers.requireUser(request).flatMap(merchant -> loadManageableTask(id, merchant).flatMap(
				task -> loadAcceptedApp(id, appId).flatMap(app -> decisions.contest(task, app, body.reason()))));
	}
	...
	public Mono<ResponseEntity<Map<String, Object>>> confirm(@PathVariable String id, @PathVariable String appId,
			@RequestBody(required = false) ConfirmEngagementRequest body, ServerHttpRequest request) {
		return callers.requireUser(request).flatMap(merchant -> loadManageableTask(id, merchant)
				.flatMap(task -> loadAcceptedApp(id, appId).flatMap(app -> decisions.confirm(task, app, body))));
	}
	 ...
	 *
	 * <p>
	 * <b>必须已确认履约</b>（{@code confirmed_at} 非空）才能评——评分是对「已完成的活」的评价。 一次履约只评一份（DB
	 * UNIQUE → 409），商家不能反复改分刷高/压低某个推荐官。 时序上是「确认履约」后紧接着的独立端点而非 confirm 请求体：confirm
	 * 返回 202 且触发结算 Saga， 把评分塞进去会让「结算启动了但评分写失败」这种半成功状态无处安放。
	 */
	...
	public Mono<ResponseEntity<Map<String, Object>>> rate(@PathVariable String id, @PathVariable String appId,
			@RequestBody RateEngagementRequest body, ServerHttpRequest request) {
		return callers.requireUser(request)
				.flatMap(merchant -> loadManageableTask(id, merchant)
						.flatMap(task -> loadConfirmedApp(id, appId).flatMap(app -> decisions.rate(app, merchant, body)
						...
						.flatMap(app -> tasks.findById(id).switchIfEmpty(fail(404, "任务不存在")).flatMap(task -> {
							...
							return canView.flatMap(allowed -> {
								...
								return decisions.rating(appId)
										.map(rating -> ResponseEntity.ok(Map.<String, Object>of("success", true, "data",
												ApplicationBodies.toBody(rating))))
	...
	private Mono<TaskApplication> loadConfirmedApp(String taskId, String appId) {
		return loadAcceptedApp(taskId, appId).filter(app -> app.confirmedAt() != null)
	 ...
	 * awaiting_confirmation/not_entered）见
	 * {@link EngagementDecisionService#confirmationOutcome}。
	 */
	@GetMapping("/api/tasks/{id}/applications/{appId}/confirmation")
	public Mono<ResponseEntity<Map<String, Object>>> confirmation(@PathVariable String id, @PathVariable String appId,
			ServerHttpRequest request) {
		return callers.resolve(request)
				.flatMap(caller -> loadManageableTask(id, caller).flatMap(task -> apps.findById(appId)
						.switchIfEmpty(fail(404, "报名不存在")).filter(app -> app.taskId().equals(id))
						.switchIfEmpty(fail(404, "报名不存在")).map(decisions::confirmationOutcome)));
	}

	/** 加载报名并校验属该 task + accepted：不存在/越界→404，非 accepted→409。 */
	private Mono<TaskApplication> loadAcceptedApp(String taskId, String appId) {
		return apps.findById(appId).switchIfEmpty(fail(404, "报名不存在")).filter(app -> app.taskId().equals(taskId))
				.switchIfEmpty(fail(404, "报名不存在")).filter(app -> "accepted".equals(app.status()))
	...
	public Mono<ResponseEntity<Map<String, Object>>> reject(@PathVariable String id, @PathVariable String appId,
			ServerHttpRequest request) {
		return callers.requireUser(request)
				.flatMap(merchant -> loadManageableTask(id, merchant).flatMap(
						task -> loadPendingApp(id, appId).flatMap(app -> lifecycle.reject(task, app, merchant))))
				.map(app -> ResponseEntity.ok(Map.of("success", true, "data", ApplicationBodies.toBody(app))));
	}

	/**
	 * 任务书 #27：批量接受报名（1–50 条）。逐项独立处理，允许部分成功（D2）；批内按传入顺序占名额（D3）。 复用单条 accept 的共享内核
	 * {@link ApplicationAcceptanceService#claimAcceptance}（D6）；单项失败不中断后续项。
	 */
	@PostMapping("/api/tasks/{id}/applications/batch-accept")
	public Mono<ResponseEntity<Map<String, Object>>> batchAccept(@PathVariable String id,
			@RequestBody BatchOperationRequest body, ServerHttpRequest request) {
		String baseKey = acceptanceIdempotencyKey(request);
		return callers.requireUser(request)
				.flatMap(
						merchant -> loadManageableTask(id, merchant)
								.flatMap(task -> Flux.fromIterable(body.applicationIds())
										.concatMap(appId -> acceptance.batchItem(task, merchant, baseKey + ":" + appId,
												appId))
										.collectList())
								.map(results -> ResponseEntity.ok(Map.of("success", true, "data",
										Map.of("results", results.stream().map(BatchItemResult::toBody).toList())))));
	}

	/**
	 * 任务书 #27：批量拒绝报名（1–50 条）。逐项独立，允许部分成功。 复用单条 reject 的共享逻辑（D6）；仅 pending 可处理。
	 */
	@PostMapping("/api/tasks/{id}/applications/batch-reject")
	public Mono<ResponseEntity<Map<String, Object>>> batchReject(@PathVariable String id,
			@RequestBody BatchOperationRequest body, ServerHttpRequest request) {
		return callers.requireUser(request)
				.flatMap(merchant -> loadManageableTask(id, merchant)
						.flatMap(task -> Flux.fromIterable(body.applicationIds())
								.concatMap(appId -> lifecycle.batchRejectItem(task, merchant, appId)).collectList())
						.map(results -> ResponseEntity.ok(Map.of("success", true, "data",
								Map.of("results", results.stream().map(BatchItemResult::toBody).toList())))));
	}
	...
	/**
	 * Aggregated list statistics are separate from the filtered rows so pagination
	 * does not distort totals.
	 */
	@GetMapping("/api/tasks/{id}/applications/summary")
	public Mono<ResponseEntity<Map<String, Object>>> applicationSummary(@PathVariable String id,
			ServerHttpRequest request) {
		return callers.requireUser(request).flatMap(caller -> loadManageableTask(id, caller).flatMap(task -> lifecycle
				.taskProgress(id, task).map(stats -> ResponseEntity.ok(Map.of("success", true, "data", stats)))));
	}

	/** 组织级任务仅 owner；门店任务允许当前 MANAGER，且每次操作实时向 Identity 重验。 */
	private Mono<Task> loadManageableTask(String taskId, Caller caller) {
		return taskAuthorization.requireManager(taskId, caller).map(TaskResourceAuthorization.ManagedTask::task);
	}
	...
	private Mono<Task> loadSubmissionScoped(String taskId, String appId, Caller caller) {
		return apps.findById(appId).switchIfEmpty(fail(404, "报名不存在")).filter(app -> app.taskId().equals(taskId))
				.switchIfEmpty(fail(404, "报名不存在"))
				.flatMap(app -> tasks.findById(taskId).switchIfEmpty(fail(404, "任务不存在")).flatMap(task -> {
					if (caller.accountId().equals(app.recommenderAccountId())) {
						return Mono.just(task);
					}
					return taskAuthorization.requireManager(task, caller)
							.map(TaskResourceAuthorization.ManagedTask::task).onErrorMap(MarketplaceException.class,
	...
	public Mono<ResponseEntity<Map<String, Object>>> runVerificationChecks(@PathVariable String id,
			@PathVariable String appId, @PathVariable String submissionId, ServerHttpRequest request) {
		return callers.requireUser(request)
				.flatMap(merchant -> loadManageableTask(id, merchant).flatMap(task -> loadAcceptedApp(id, appId)
						.flatMap(app -> verificationService.check(task, app, appId, submissionId, merchant.accountId())
	...
	public Mono<ResponseEntity<Map<String, Object>>> taskContext(@PathVariable String id, @PathVariable String appId,
			ServerHttpRequest request) {
		return callers.requireUser(request).flatMap(caller -> loadAcceptedApp(id, appId).flatMap(app -> {
			Mono<Void> authorized = app.recommenderAccountId().equals(caller.accountId())
					? Mono.empty()
					: loadManageableTask(id, caller).then();
			return authorized.then(verificationService.taskContext(appId)).map(json -> ApplicationBodies.ok(json));
		}));
	}

	@GetMapping("/api/tasks/{id}/applications/{appId}/submissions/{submissionId}/verification/runs")
	public Mono<ResponseEntity<Map<String, Object>>> verificationRuns(@PathVariable String id,
			@PathVariable String appId, @PathVariable String submissionId, ServerHttpRequest request) {
		return callers.requireUser(request).flatMap(caller -> loadAcceptedApp(id, appId).flatMap(app -> {
			Mono<Void> authorized = app.recommenderAccountId().equals(caller.accountId())
					? Mono.empty()
