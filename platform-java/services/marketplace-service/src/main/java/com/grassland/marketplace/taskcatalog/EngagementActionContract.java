...
public class EngagementActionContract {
	...
	public Mono<Next> read(Task task, TaskApplication app, boolean manager, String settlementStatus, String holdReason,
	...
	Next derive(Task task, TaskApplication app, boolean manager, String settlementStatus, String holdReason,
		...
		if (exitPending != null) {
			...
			return responderSide
					? next("exit_pending_confirm", manager ? "待退出确认" : "待退出确认", "确认后按已确认里程碑部分结算并终态合作",
							exitPending.respondDeadlineAt(), benefitStatus)
					: next("exit_await_response", "待对方回应", "exit_request_open", exitPending.respondDeadlineAt(),
							benefitStatus);
		}
		...
		if ("pending".equals(app.status())) {
			return next(manager ? "selection" : "waiting", manager ? "待筛选" : "等待商家筛选", manager ? null : "等待商家处理报名",
					task.applicationDeadline(), benefitStatus);
		}
		if ("reconsent".equals(app.status())) {
			return next("reconsent", manager ? "等待确认新条款" : "确认新条款", manager ? "等待推荐官确认" : null,
					task.applicationDeadline(), benefitStatus);
		}
		...
		if (task.isCommercePromotion())
			return next("promotion", "推广套餐", null, null, benefitStatus);
		if (benefit != null && benefit.defaultClaimOpen()) {
			return next("benefit_default", manager ? "回应体验失约主张" : "等待失约回应", manager ? null : "失约回应期间交付计时暂停",
					benefit.defaultDeadlineAt(), benefitStatus);
		}
		...
		if (extensionPending) {
			return next("extension", manager ? "审批延期" : "等待延期审批", manager ? null : "延期获批前原交付期限仍有效",
					app.deliveryDeadlineAt(), benefitStatus);
		}
		EngagementSubmission pending = history.stream().filter(EngagementSubmission::isPending).findFirst()
				.orElse(null);
		if (pending != null) {
			if (pending.isDraft()) {
				...
				return next(overdue ? "manual_review" : "draft_review",
						overdue ? "审稿超时待复核" : manager ? "待审稿" : "等待商家审稿", manager && !overdue ? null : "草稿获批后才可发布", due,
						benefitStatus);
			}
			return next("acceptance", manager ? "待验收" : "等待商家验收", manager ? null : "已提交凭证，等待验收",
					app.merchantConfirmDeadlineAt(), benefitStatus);
		}
		...
		if (latest != null && "rejected".equals(latest.status())) {
			Instant due = latest.isDraft() && latest.reviewedAt() != null
					? latest.reviewedAt().plusSeconds(resubmitSeconds)
					: app.deliveryDeadlineAt();
			boolean expired = latest.isDraft() && (due == null || due.isBefore(now));
			return next(expired ? "exception" : "revision", expired ? "补交超时，处理争议或退出" : manager ? "等待修改" : "待修改",
		...
		if (benefit != null && (ExperienceBenefit.STATUS_BOOKED.equals(benefit.status())
			...
			return next("benefit", manager ? "待供样或接待确认" : "待体验权益兑现", null, benefit.bookingWindow(), benefitStatus);
		}
		if (benefit == null && task.freebieDepositCents() != null && task.freebieDepositCents() > 0) {
			return next("benefit", manager ? "等待体验预约" : "预约体验权益", manager ? "等待推荐官预约" : null, app.deliveryDeadlineAt(),
					"pending_booking");
		}
		...
		return next("delivery", manager ? "等待交付" : task.requiresReview() && !approvedDraft ? "待交草稿" : "待交付",
				manager ? "等待推荐官交付" : remedy ? "交付已逾期，请在补救期限内处理" : null, remedy ? app.remedyDeadlineAt() : due,
				benefitStatus);
	}
