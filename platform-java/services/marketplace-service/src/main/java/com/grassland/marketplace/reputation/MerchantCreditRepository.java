 ...
 *
 * <p>事实来源全部为 marketplace 自有表：task（发布/取消）、task_application（确认/验收超时）、
...
public class MerchantCreditRepository {
	...
	public record OrgCreditFacts(long publishedTasks, long cancelledTasks, long confirmations,
			long autoConfirmations, long benefits, long benefitDefaults, long resolvedDisputes, long disputeLosses,
			long cooperations) {
	}
	...
	public Mono<OrgCreditFacts> facts(String organizationId) {
		return db.sql("""
				...
				""").bind("org", organizationId).map((row, metadata) -> new OrgCreditFacts(
				row.get("published_tasks", Long.class), row.get("cancelled_tasks", Long.class),
				row.get("confirmations", Long.class), row.get("auto_confirmations", Long.class),
				row.get("benefits", Long.class), row.get("benefit_defaults", Long.class),
				row.get("resolved_disputes", Long.class), row.get("dispute_losses", Long.class),
				row.get("cooperations", Long.class))).one();
	}
}
