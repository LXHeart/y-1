 ...
 *
 * <p>双向申请-响应两段式（照 {@link EngagementExtensionRepository} / V55-V56 惯例）：
 * 任一方 {@code insertPending}（V59 部分唯一索引保证同一报名至多一条 pending，并发双开收敛为唯一
 * 冲突 → 调用方 409）；对方 {@code respond}（pending→confirmed/rejected 的 guarded UPDATE，
 * 0 行 = 已处理/不存在，条件 UPDATE 单边胜出）；发起方 {@code cancelByInitiator} 撤回 pending。
 * 超时失效由 dispatcher 扫 {@code expireOverdue}（pending + 响应窗已过 → expired，合作照常继续）。
 */
@Component
public class EngagementExitRequestRepository {

    /** 协商退出申请行。 */
    public record EngagementExitRequest(String id, String applicationId, String taskId,
            String initiatedByAccountId, String initiatedRole, String reason, String status,
            Instant respondDeadlineAt, String respondedByAccountId, Instant respondedAt, Instant createdAt) {
    ...
    public Mono<EngagementExitRequest> insertPending(String applicationId, String taskId, String initiatedBy,
        ...
        return db.sql("""
                ...
                RETURNING %s
                """.formatted(COLS))
                .bind("id", UUID.randomUUID().toString()).bind("app", applicationId)
                .bind("task", taskId).bind("by", initiatedBy).bind("role", initiatedRole)
                .bind("reason", reason).bind("deadline", respondDeadlineAt.atOffset(java.time.ZoneOffset.UTC))
    ...
    public Mono<EngagementExitRequest> findById(String id) {
        return db.sql("SELECT " + COLS + " FROM exit_request WHERE id = CAST(:id AS uuid)")
                .bind("id", id).map(EngagementExitRequestRepository::map).one();
    }

    /** 该报名当前开放申请（无则空）——互斥入口与动作契约共用。 */
    public Mono<EngagementExitRequest> findPendingByApplication(String applicationId) {
        return db.sql("SELECT " + COLS + " FROM exit_request"
    ...
    public Flux<EngagementExitRequest> listByApplication(String applicationId) {
        return db.sql("SELECT " + COLS + " FROM exit_request WHERE application_id = CAST(:app AS uuid)"
                + " ORDER BY created_at DESC")
                .bind("app", applicationId).map(EngagementExitRequestRepository::map).all();
    }

    /**
     * 对方响应：pending → confirmed/rejected（guarded，0 行 = 已处理/不存在/响应窗已过——
     * 超时失效与响应的竞态由 {@code respond_deadline_at > now()} 烧进 WHERE 单边胜出：dispatcher
     * 置 expired 后本 UPDATE 自然 0 行；反之响应先落定时 dispatcher 的 pending 扫描不再命中）。
     * responded_by 烧入响应方账号（终局审计）；confirmed 后的结算腿由领域服务在事务外幂等收敛。
     */
    public Mono<EngagementExitRequest> respond(String id, boolean confirmed, String respondedBy) {
        return db.sql("""
                ...
                RETURNING %s
                """.formatted(COLS))
                .bind("id", id).bind("by", respondedBy)
                .bind("status", confirmed ? "confirmed" : "rejected")
                .map(EngagementExitRequestRepository::map).one();
    }
    ...
    public Flux<EngagementExitRequest> expireOverdue(int limit) {
        return db.sql("UPDATE exit_request SET status = 'expired', responded_at = now()"
                ...
                + " AND respond_deadline_at <= now() ORDER BY respond_deadline_at LIMIT :limit)"
                + " RETURNING " + COLS)
                .bind("limit", Math.max(1, Math.min(limit, 200)))
                .map(EngagementExitRequestRepository::map).all();
    }
