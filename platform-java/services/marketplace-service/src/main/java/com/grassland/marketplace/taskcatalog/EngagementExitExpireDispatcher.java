...
/**
 * 协商退出申请超时失效看门狗（任务书 #97 C97-03 / D97-04，照 C96-01 dispatcher 范式）：
 * pending 且响应窗（respond_deadline_at）已过 → expired，合作按原履约继续（不触发任何资金动作）。
 * 与对方响应的竞态由 {@code respond} SQL 烧入的 {@code respond_deadline_at > now()} 单边胜出：
 * 置 expired 后响应自然 0 行；响应先落定则本扫描的 pending 过滤不再命中。事件外发确定性 eventId。
 */
@Component
@ConditionalOnProperty(name = "marketplace.engagement.exit-expire-enabled", havingValue = "true",
        matchIfMissing = true)
public class EngagementExitExpireDispatcher {
    ...
    public void dispatch() {
        exits.expireOverdue(batchSize)
                .concatMap(expired -> outbox.append(expiredEnvelope(expired)).thenReturn(expired))
                .collectList()
                .block()
    ...
    private static EventEnvelope expiredEnvelope(EngagementExitRequestRepository.EngagementExitRequest request) {
        ...
        String eventId = UUID.nameUUIDFromBytes(
                ("EngagementExitExpired:" + request.id()).getBytes(StandardCharsets.UTF_8)).toString();
        return new EventEnvelope(eventId, "EngagementExitExpired", "TaskApplication",
                request.applicationId(), 1, Instant.now(), null, payload);
    }
}
