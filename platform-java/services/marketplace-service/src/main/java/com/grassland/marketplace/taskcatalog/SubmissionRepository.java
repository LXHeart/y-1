 ...
 *
 * <p>
 * 状态迁移一律 guarded UPDATE（{@code WHERE status='submitted'}）→ 返回 empty 即「已被处理」，
 * 避免读-改-写竞态（商家点确认与点退回同时发生时只有一个赢家）。
 */
@Component
public class SubmissionRepository {

	private static final String SELECT_COLS = "id::text, application_id::text, recommender_account_id::text, content_url, note, status,"
	 ...
	 *
	 * <p>
	 * D-03 cancel 并发守卫：INSERT 前 JOIN task 并 {@code FOR SHARE OF t}。它与 cancel 的 task
	 * UPDATE （NO KEY UPDATE 锁）冲突，保证二者串行：提交先拿锁→cancel 等提交落库后看见 submission 不退款；
	 * cancel 先拿锁→提交醒来见 cancelled 后 0 行，不会发生「已退款又补交」。
	 */
	public Mono<EngagementSubmission> create(String applicationId, String recommenderAccountId, String contentUrl,
			String note, String platformHandle) {
		return create(applicationId, recommenderAccountId, contentUrl, note, platformHandle, null);
	}

	public Mono<EngagementSubmission> create(String applicationId, String recommenderAccountId, String contentUrl,
			String note, String platformHandle, String commentText) {
		var spec = db
				.sql("""
						WITH eligible AS (
						    SELECT a.id
				...
				.bind("id", UUID.randomUUID().toString()).bind("app", applicationId).bind("rec", recommenderAccountId)
				.bind("url", contentUrl);
		spec = bindNullable(spec, "note", note);
		spec = bindNullable(spec, "handle", platformHandle);
		spec = bindNullable(spec, "comment", commentText);
		return spec.map(SubmissionRepository::map).one().onErrorResume(DataIntegrityViolationException.class,
				e -> Mono.empty());
	}
	...
	public Mono<EngagementSubmission> findById(String submissionId) {
		return db.sql("SELECT " + SELECT_COLS + " FROM engagement_submission WHERE id = CAST(:id AS uuid)")
				.bind("id", submissionId).map(SubmissionRepository::map).one();
	}
	...
	public Flux<EngagementSubmission> findConfirmationDispatchable(int limit) {
		return db
				.sql("SELECT " + SELECT_COLS + " FROM engagement_submission"
						+ " WHERE status = 'submitted' AND confirmation_workflow_started_at IS NULL"
						+ " ORDER BY created_at LIMIT :limit")
				.bind("limit", Math.max(1, limit)).map(SubmissionRepository::map).all();
	}
	...
	public Mono<Boolean> markConfirmationWorkflowStarted(String submissionId) {
		return db.sql("""
				...
				""").bind("id", submissionId).fetch().rowsUpdated().map(updated -> updated > 0).defaultIfEmpty(false);
	}
	...
	public Flux<EngagementSubmission> findByApplication(String applicationId) {
		return db
				...
				.bind("app", applicationId).map(SubmissionRepository::map).all();
	}
	...
	public Mono<Integer> countRejectedByApplication(String applicationId) {
		return db
				...
				.bind("app", applicationId).map(r -> r.get("c", Integer.class)).one();
	}
	...
	public Mono<EngagementSubmission> createDraft(String applicationId, String recommenderAccountId, String note) {
		return db.sql("""
				...
				FROM eligible
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", UUID.randomUUID().toString()).bind("app", applicationId)
				.bind("rec", recommenderAccountId).bind("note", note == null || note.isBlank() ? null : note.trim())
	...
	public Mono<Boolean> hasApprovedDraft(String applicationId) {
		return db
				.sql("SELECT EXISTS(SELECT 1 FROM engagement_submission"
						+ " WHERE application_id = CAST(:app AS uuid) AND submission_kind = 'draft'"
	...
	public Mono<EngagementSubmission> review(String submissionId, SubmissionStatus target, String reviewNote) {
		var spec = db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", submissionId).bind("status", target.dbValue());
	...
	private static EngagementSubmission map(Readable row) {
		return new EngagementSubmission(row.get("id", String.class), row.get("application_id", String.class),
				row.get("recommender_account_id", String.class), row.get("content_url", String.class),
				row.get("note", String.class), row.get("status", String.class), row.get("review_note", String.class),
				...
				row.get("platform_handle", String.class), row.get("comment_text", String.class),
