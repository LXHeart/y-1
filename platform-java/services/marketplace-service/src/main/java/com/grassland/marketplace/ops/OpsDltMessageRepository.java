 ...
 *
 * <p>
 * {@code offset} 是 SQL 保留字，列名一律带引号。
 */
@Component
public class OpsDltMessageRepository {

	private static final String SELECT_COLS = "id::text, topic, partition, \"offset\", original_topic, message_key, payload,"
	...
	public Mono<OpsDltMessage> insertIfAbsent(String topic, int partition, long offset, String originalTopic,
		...
		var spec = db.sql("""
				...
				RETURNING %s
				""".formatted(SELECT_COLS)).bind("id", UUID.randomUUID().toString()).bind("topic", topic)
				.bind("partition", partition).bind("offset", offset).bind("originalTopic", originalTopic)
	...
	public Mono<OpsDltMessage> findById(String id) {
		return db.sql("SELECT " + SELECT_COLS + " FROM ops_dlt_message WHERE id = CAST(:id AS uuid)").bind("id", id)
				.map(OpsDltMessageRepository::map).one();
	}

	public Mono<OpsDltMessage> findByPosition(String topic, int partition, long offset) {
		return db
				.sql("SELECT " + SELECT_COLS + " FROM ops_dlt_message"
	...
	/** 列表。{@code status} 省略 → 仅 pending（待处置）；offset 分页（任务书 #94 D94-09）。 */
	public Flux<OpsDltMessage> list(String status, int limit, int offset) {
		String where = status == null || status.isBlank() ? "status = 'pending'" : "status = :status";
		var spec = db
				.sql("SELECT " + SELECT_COLS + " FROM ops_dlt_message WHERE " + where
	...
	/** 列表同口径计数（status 语义同 {@link #list}）—— 分页信封的 {@code total}。 */
	public Mono<Long> countList(String status) {
		String where = status == null || status.isBlank() ? "status = 'pending'" : "status = :status";
		var spec = db.sql("SELECT COUNT(*) AS n FROM ops_dlt_message WHERE " + where);
		if (status != null && !status.isBlank()) {
			spec = spec.bind("status", status);
		}
		return spec.map((row, meta) -> row.get("n", Long.class)).one().defaultIfEmpty(0L);
	}
	...
	private Mono<Long> count(String where) {
		String sql = "SELECT COUNT(*) AS n FROM ops_dlt_message" + (where == null ? "" : " WHERE " + where);
		return db.sql(sql).map((row, meta) -> row.get("n", Long.class)).one().defaultIfEmpty(0L);
	}
	 ...
	 *
	 * <p>
	 * <b>绝不删 {@code pending}</b>：pending 是待运营处置的死信，删了就丢证据。只清已被重投或弃置的历史行， 让
	 * ops_dlt_message 表不会无限膨胀（弃置只改状态不删行是 Stage 2 的审计约定，retention 清理归本条）。
	 *
	 * @return 删除行数
	 */
	public Mono<Long> deleteTerminalOlderThan(Instant cutoff) {
		return db
				.sql("DELETE FROM ops_dlt_message"
						+ " WHERE status IN ('replayed','discarded') AND created_at < :cutoff")
				.bind("cutoff", OffsetDateTime.ofInstant(cutoff, ZoneOffset.UTC)).fetch().rowsUpdated();
	}
	 ...
	 *
	 * <p>
	 * 弃置只改状态，<b>不删行也不删 Kafka 消息</b>：死信是审计对象，「谁在什么时候放弃了哪条消息」 必须留痕。Kafka 侧的 retention
	 * 清理归 GL-P3-PLATFORM-001。
	 */
	...
	private Mono<OpsDltMessage> transition(String id, String status, String tsColumn) {
		return db.sql("""
				...
				RETURNING %s
				""".formatted(tsColumn, SELECT_COLS)).bind("id", id).bind("status", status)
				.map(OpsDltMessageRepository::map).one();
	}

	private static OpsDltMessage map(Readable row) {
		return new OpsDltMessage(row.get("id", String.class), row.get("topic", String.class),
				intValue(row.get("partition", Integer.class)), longValue(row.get("offset", Long.class)),
				row.get("original_topic", String.class), row.get("message_key", String.class),
				row.get("payload", String.class), row.get("error_summary", String.class),
				row.get("status", String.class), toInstant(row.get("replayed_at", OffsetDateTime.class)),
