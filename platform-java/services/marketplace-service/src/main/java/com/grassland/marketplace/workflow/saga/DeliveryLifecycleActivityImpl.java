...
public class DeliveryLifecycleActivityImpl implements DeliveryLifecycleActivity {
	...
	public DeliveryOutcome terminateDeliveryTimeout(DeliveryDeadlineInput input) {
		...
		TaskApplication terminated = transactions.transactional(apps.markDeliveryTimedOut(app.id(), task.id())
				.flatMap(done -> counters.release(task.id()).filter(Boolean::booleanValue)
						...
						.then(outbox.append(terminatedEnvelope(task, done, taskOwnerId, breakdown))).thenReturn(done)))
				.block();
