 ...
 *
 * <p>
 * bean 覆盖集合与 FreebieEscrowFlowIT 同族（真 Saga + temporal test-server，仅 finance 出站
 * mock）。 失约派发器在基座关闭——成立直接驱动 {@link ExperienceBenefitService#establishDefault}
 * seam； 回应窗用 IT 配置 1 小时，截止回拨用 SQL 后门。
 */
...
class ExperienceBenefitDefaultIT extends MarketplaceItSupport {
	...
	private com.grassland.marketplace.taskcatalog.EngagementExitFundsService fundsService;

	@org.springframework.beans.factory.annotation.Autowired
	private com.grassland.marketplace.taskcatalog.EngagementExitOperationRepository exitOperations;
	...
	void defaultClaimDenyAndAutoEstablishPaths() {
		...
		Map<String, Object> resp = client().post()
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("action", "respond_default")).exchange()
	...
	/** C103-02 起退出资金腿经 operation+worker：驱动 funds seam 至完成（重放幂等）。 */
	private void advanceExitOperation(String appId) {
		var operation = exitOperations.findByApplication(appId).block();
	...
	void unconsumedCanExitNoFaultButConsumedCannot() {
		...
		// C103-02 起无责退出资金腿经 engagement_exit_operation+worker 异步执行：
		// 直接驱动 funds seam 至完成（与 EngagementExitRecoveryIT 同款），再断言押金全退腿。
		advanceExitOperation(appId);
		verify(financeClient).freebieRefund(org, appId);
		...
		client().post().uri("/api/tasks/" + task2 + "/applications/" + app2 + "/benefit")
				...
				.bodyValue(Map.of("action", "fulfill")).exchange().expectStatus().isOk().expectBody()
	...
	private String publishFreebieTask(String merchant, String org) {
		Map<String, Object> resp = client().post().uri("/api/tasks")
				...
				.bodyValue(Map.of("organizationId", org, "title", "体验任务", "platform", "xiaohongshu", "storeId",
						UUID.randomUUID().toString(), "applicationDeadline", Instant.now().plusSeconds(3600).toString(),
						"freebieDepositCents", DEPOSIT))
	...
	private String applyAcceptFreebie(String merchant, String org, String taskId, String rec) {
		Map<String, Object> applied = client().post().uri("/api/tasks/" + taskId + "/applications")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "想体验")).exchange().expectStatus().isCreated().expectBody(Map.class)
	...
	private void bookBenefit(String taskId, String appId, String rec) {
		client().post().uri("/api/tasks/" + taskId + "/applications/" + appId + "/benefit")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("action", "book", "items", List.of("到店体验"), "bookingWindow",
						Instant.now().plusSeconds(86400).toString()))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.status").isEqualTo("booked");
	}
	...
	private void claimDefault(String taskId, String appId, String rec) {
		client().post().uri("/api/tasks/" + taskId + "/applications/" + appId + "/benefit/default-claim")
				.header(H, sign(rec, "recommender")).exchange().expectStatus().isOk();
	}

	private void claimDefaultExpect(String taskId, String appId, String rec, int status) {
		client().post().uri("/api/tasks/" + taskId + "/applications/" + appId + "/benefit/default-claim")
				.header(H, sign(rec, "recommender")).exchange().expectStatus().isEqualTo(status);
	}

	private Map<String, Object> benefitView(String taskId, String appId, String caller, String org) {
		Map<String, Object> resp = client().get().uri("/api/tasks/" + taskId + "/applications/" + appId + "/benefit")
				.header(H,
						caller.equals(merchantOf(taskId))
								? sign(caller, "merchant", org, "finance_transaction")
	...
	private String merchantOf(String taskId) {
		return db.sql("SELECT owner_account_id::text AS o FROM task WHERE id = CAST(:id AS uuid)").bind("id", taskId)
				.map(r -> r.get("o", String.class)).one().block();
	}
	...
	private void backdateDefaultDeadline(String appId) {
		db.sql("UPDATE experience_benefit SET default_deadline_at = now() - interval '1 second'"
				+ " WHERE application_id = CAST(:id AS uuid)").bind("id", appId).then().block();
	}
