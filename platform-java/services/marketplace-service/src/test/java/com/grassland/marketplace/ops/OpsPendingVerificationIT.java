 ...
 *
 * <p>
 * 锁住的核心口径：{@code inconclusive} 不进处置队列（{@code /api/ops/cases}），只在这里出现； 商家一旦
 * confirm/reject（submission 离开 {@code submitted}）就退出视野。
 */
class OpsPendingVerificationIT extends MarketplaceItSupport {
	...
	void listsOnlyInconclusivePendingSubmissions() {
		...
		var rows = pending.list(50, 0).collectList().block();
		assertThat(rows).extracting(OpsPendingVerification::submissionId).contains(inconclusive.submissionId)
				.doesNotContain(passed.submissionId, failed.submissionId, decided.submissionId);

		OpsPendingVerification row = rows.stream().filter(r -> r.submissionId().equals(inconclusive.submissionId))
	...
	void endpointRequiresOpsRole() {
		...
		client().get().uri("/api/ops/pending-verifications").exchange().expectStatus().isUnauthorized();

		client().get().uri("/api/ops/pending-verifications").header("X-Grassland-Identity", sign(OPS, "merchant"))
				.exchange().expectStatus().isForbidden();

		client().get().uri("/api/ops/pending-verifications?limit=200")
				.header("X-Grassland-Identity", signWithRole(OPS, "customer_service")).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.items[?(@.submissionId=='" + s.submissionId + "')].taskTitle")
				.isEqualTo("端点用例");
	}

	@Test
	@DisplayName("分页信封：items/total/limit/offset 齐全，翻页可达、非法参数 400")
	void endpointPaginationEnvelope() {
		for (int i = 0; i < 3; i++) {
			Seed s = seed("submitted", "分页任务-" + i);
			verifications.upsert(s.submissionId, "inconclusive", "[]").block();
		}
		// 本类无逐例清表，total 以仓储同口径计数为准（自洽断言，不依赖用例执行顺序）
		long total = pending.count().block();

		client().get().uri("/api/ops/pending-verifications")
				.header("X-Grassland-Identity", signWithRole(OPS, "customer_service")).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.total").isEqualTo((int) total).jsonPath("$.data.limit").isEqualTo(50)
				.jsonPath("$.data.offset").isEqualTo(0);

		client().get().uri("/api/ops/pending-verifications?limit=2&offset=" + (total - 1))
				.header("X-Grassland-Identity", signWithRole(OPS, "customer_service")).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.items.length()").isEqualTo(1).jsonPath("$.data.total")
				.isEqualTo((int) total).jsonPath("$.data.limit").isEqualTo(2).jsonPath("$.data.offset")
				.isEqualTo((int) total - 1);

		client().get().uri("/api/ops/pending-verifications?limit=201")
				.header("X-Grassland-Identity", signWithRole(OPS, "customer_service")).exchange().expectStatus()
				.isBadRequest();
		client().get().uri("/api/ops/pending-verifications?offset=-1")
				.header("X-Grassland-Identity", signWithRole(OPS, "customer_service")).exchange().expectStatus()
				.isBadRequest();
	}
	...
	void inconclusiveDoesNotEnterCaseQueue() {
		...
		Long cases = db.sql("SELECT count(*) AS n FROM ops_case WHERE source_ref = :ref").bind("ref", s.submissionId)
	...
	private Seed seed(String submissionStatus, String title) {
		...
		db.sql("""
				...
				""").bind("t", task).bind("o", UUID.randomUUID().toString()).bind("g", UUID.randomUUID().toString())
				.bind("title", title).then().block();
		db.sql("""
		...
		db.sql("""
				...
				""").bind("s", sub).bind("a", app).bind("r", UUID.randomUUID().toString()).bind("st", submissionStatus)
				.then().block();
