...
/**
 * 核销资格纯规则单测（任务书 #103 C103-07 / TC103-07-01 资格表）。 R07 反例 →
 * 不变量：展示能力与实际核销资格同源——部分退款（未核销、净额&gt;0）同为 「返回原码」与「可核销」；全退/在途/过期同为「不给码」与「不可核销」。
 */
class OrderRedemptionPolicyTest {
	...
	private CommerceModels.Order order(String status, long priceCents, long refundedCents, Instant paidAt,
		...
		return new CommerceModels.Order(UUID.randomUUID().toString(), "consumer", "org", "store", null, "pkg", "ver", 1,
				...
				null, null, deadline, null, null, null, null, null, null, 0, now.minusSeconds(3600), paidAt, redeemedAt,
	...
	void blockedStatesCarryStableMachineReadableReasons() {
		assertThat(OrderRedemptionPolicy.evaluate(order("paid", 10_000, 0, null, null, now.plusSeconds(3600)), now)
				.blockedReason()).isEqualTo("not_paid");
		assertThat(OrderRedemptionPolicy.evaluate(order("redeemed", 10_000, 0, now, now, now.plusSeconds(3600)), now)
				.blockedReason()).isEqualTo("already_redeemed");
		...
		assertThat(OrderRedemptionPolicy.evaluate(order("splitting", 10_000, 0, now, null, now.plusSeconds(3600)), now)
				.blockedReason()).isEqualTo("fund_operation_in_progress");
		assertThat(OrderRedemptionPolicy
		...
		assertThat(OrderRedemptionPolicy.evaluate(order("paid", 10_000, 0, now, null, now), now).blockedReason())
				.isEqualTo("expired");
		assertThat(OrderRedemptionPolicy.evaluate(order("cancelled", 10_000, 0, now, null, now.plusSeconds(3600)), now)
				.blockedReason()).isEqualTo("not_redeemable");
	}
	...
	void oneCentRemainingNetAmountStaysEligible() {
		assertThat(OrderRedemptionPolicy
				.evaluate(order("partially_refunded", 10_000, 9_999, now, null, now.plusSeconds(3600)), now).allowed())
				.isTrue();
	}
}
