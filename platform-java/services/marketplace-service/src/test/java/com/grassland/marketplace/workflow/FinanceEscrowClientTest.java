...
class FinanceEscrowClientTest {
	...
	@Test
	void exitFactsRejectEmptyAndWrongScopeResponses() {
		String path = "/internal/engagements/" + REF + "/exit-facts?organizationId=" + ORG;
		for (String body : new String[]{"",
				"{\"success\":true,\"data\":{\"organizationId\":\"other\",\"engagementRef\":\"" + REF + "\"}}"}) {
			wireMock.resetAll();
			...
			StepVerifier.create(client.exitFacts(ORG, REF)).verifyError(FinanceEscrowException.class);
		}
	}
	...
	void matchingReservationEnvelopeReturnsReservedAndSendsServiceAssertion() {
		...
		StepVerifier.create(client.reserve(ORG, REF, 1_000, PAYEE, 1_000)).assertNext(result -> {
	...
	void reservationWithoutPayeeUsesStableNullableRequestPayload() {
		...
		StepVerifier.create(reactor.core.publisher.Mono.defer(() -> client.reserve(ORG, REF, 1_000, null)))
				.assertNext(result -> {
					assertThat(result.reserved()).isTrue();
					assertThat(result.amountCents()).isEqualTo(1_000);
				}).verifyComplete();
	...
	void successWithMismatchedFrozenScopeThrows() {
		String[] mismatchedResponses = {
				reservationEnvelope("33333333-3333-3333-3333-333333333333", REF, 1_000, PAYEE, 1_000, 100, "reserved"),
				...
				reservationEnvelope(ORG, REF, 1_000, "44444444-4444-4444-4444-444444444444", 1_000, 100, "reserved"),
	...
	void malformedOrUnsuccessfulSuccessResponseThrows() {
		String[] invalidResponses = {"", "not-json", "{\"success\":false,\"error\":\"unexpected\"}",
	...
	void scopeConflictIsAnExceptionRatherThanInsufficientFunds() {
		...
		StepVerifier.create(client.reserve(ORG, REF, 1_000, PAYEE, 1_000)).verifyError(FinanceEscrowException.class);
	}
	...
	void captureVerifiedSuccessVerifiesScope() {
		...
		StepVerifier.create(client.captureVerified(ORG, REF, 1_000, PAYEE, null)).assertNext(outcome -> {
	...
	void captureVerifiedScopeMismatchOnSuccessRequiresReconciliation() {
		...
		wireMock.stubFor(post(urlEqualTo("/api/finance/reservations/" + REF + "/capture")).willReturn(
				aResponse().withStatus(200).withHeader("Content-Type", "application/json").withBody(reservationEnvelope(
						ORG, REF, 1_000, "44444444-4444-4444-4444-444444444444", 0, 0, "captured"))));
	...
	void captureVerifiedConflictButReservationVanishedRequiresReconciliation() {
		...
		StepVerifier.create(client.captureVerified(ORG, REF, 1_000, PAYEE, null))
				.assertNext(outcome -> assertThat(outcome.reconciliationReason()).isEqualTo("reservation_missing"))
				.verifyComplete();
	}
	...
	private void stubGetReservation(int status, String body) {
		wireMock.stubFor(get(urlEqualTo("/api/finance/reservations/" + REF)).willReturn(
	...
	private void stubReservation(int status, String body) {
		wireMock.stubFor(post(urlEqualTo("/api/finance/accounts/" + ORG + "/reservations")).willReturn(
				aResponse().withStatus(status).withHeader("Content-Type", "application/json").withBody(body)));
	}

	private static String reservationEnvelope(String org, String ref, long amount, String payee, int bonusBps,
			long bonusCents, String status) {
		return "{\"success\":true,\"data\":{" + "\"organizationId\":\"" + org + "\"," + "\"engagementRef\":\"" + ref
				+ "\"," + "\"amountCents\":" + amount + "," + "\"payeeAccountId\":"
				+ (payee == null ? "null" : "\"" + payee + "\"") + "," + "\"commissionBonusBps\":" + bonusBps + ","
				+ "\"commissionBonusCents\":" + bonusCents + "," + "\"status\":\"" + status + "\"}}";
	}
}
