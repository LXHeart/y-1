 ...
 *
 * <p>
 * 真 DB + 真 temporal test-server；仅 finance 出站 mock（镜像 ApplicationControllerIT 的
 * bean 覆盖集合， 复用其 Spring 上下文缓存）。看门狗派发器在基座已关——提醒/终结直接驱动
...
class EngagementDeliveryLifecycleIT extends MarketplaceItSupport {
	...
	void acceptSnapshotsDeliveryContractThenReminderAndTimeoutTerminateChain() {
		...
		// R10: timeout is recommender-accountable, unlike merchant_cancel.
		var timeoutStats = reputationService.snapshot(rec).block().stats();
	...
	/**
	 * TC03-01: a capacity write failure rolls the terminal application and outbox
	 * back together.
	 */
	@Test
	void noFaultExitRollsBackWhenCapacityReleaseFails() {
		String merchant = UUID.randomUUID().toString();
		String org = UUID.randomUUID().toString();
		String rec = UUID.randomUUID().toString();
		String task = publishTask(merchant, org);
		...
		doReturn(Mono.just(false)).when(acceptanceCounters).release(task);
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/exit")
				...
				.bodyValue(Map.of("kind", "no_fault", "reason", "档期冲突")).exchange().expectStatus().is5xxServerError();
	...
	void noFaultExitLandsWithdrawnAndStaysOutOfCompletionDenominator() {
		...
		// 重复退出 / 已退出再确认 → 409
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/exit")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON).exchange().expectStatus()
				.isEqualTo(409);
	}
	...
	void legacyApplicationWithoutPolicyVersionIsExempt() {
		...
		db.sql("UPDATE task_application SET engagement_policy_version = NULL, delivery_deadline_at = NULL,"
				+ " remedy_deadline_at = NULL WHERE id = CAST(:id AS uuid)").bind("id", appId).then().block();
	...
	void timeoutTerminationAndMerchantCancelAreSingleWinner() {
		...
		// A：超时终结先落定
		backdateRemedy(appA);
		assertThat(deliveryActivity.terminateDeliveryTimeout(input(appA, task, org)).status()).isEqualTo("terminated");
		...
		client().post().uri("/api/tasks/" + task + "/cancel")
				...
				.bodyValue(Map.of("expectedVersion", 1)).exchange().expectStatus().isOk().expectBody()
		...
		assertThat(deliveryActivity.terminateDeliveryTimeout(input(appB, task, org)).status()).isEqualTo("aborted");
	...
	void extensionRequestApproveRejectLifecycle() {
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/extend")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("reason", "n/a")).exchange().expectStatus().isBadRequest();
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/extend")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("days", 0)).exchange().expectStatus().isBadRequest();

		// 正常申请 → pending；重复申请 → 409
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/extend")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("days", 3, "reason", "素材未就绪")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.status").isEqualTo("pending").jsonPath("$.data.days").isEqualTo(3);
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/extend")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("days", 5)).exchange().expectStatus().isEqualTo(409);
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/extend")
				.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "approve")).exchange().expectStatus().isOk().expectBody()
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/extend")
				.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "approve")).exchange().expectStatus().isEqualTo(409);
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appR2 + "/extend")
				...
				.bodyValue(Map.of("days", 2)).exchange().expectStatus().isOk();
		client().post().uri("/api/tasks/" + task + "/applications/" + appR2 + "/extend")
				.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("decision", "reject")).exchange().expectStatus().isOk().expectBody()
