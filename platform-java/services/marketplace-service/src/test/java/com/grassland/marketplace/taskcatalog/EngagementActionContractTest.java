...
class EngagementActionContractTest {
	...
	void openExitRequestSurfacesConfirmOrAwaitPerViewerSide() {
		var pending = new EngagementExitRequestRepository.EngagementExitRequest("exit-1", "app-1", "task-1",
				"merchant-1", "recommender", "档期冲突", "pending", now.plusSeconds(72 * 3600L), null, null,
				now.minusSeconds(60));
		...
		// 商家发起 → 推荐官视角待确认。
		var merchantInitiated = new EngagementExitRequestRepository.EngagementExitRequest("exit-2", "app-1", "task-1",
	...
	private EngagementSubmission submission(String status, boolean draft, Instant created, Instant reviewed) {
		return new EngagementSubmission("s", "a", "r", "", null, status, null, reviewed, created, null, null, null,
