 ...
 *
 * <p>
 * 「至多一次」由 finance 四层 operation_id 幂等保证（refund 表唯一键/预留守卫/账本部分唯一/ provider
 * operation 唯一）；marketplace 的职责是<b>确定性重试键</b>——同一订单重试恒用同一
 ...
 * <li>响应丢失重试：finance 二次调用仍收同一 operationId（幂等重放不双退的前提）； 重复 markRefunded 幂等（终态守卫
 * 0 行回退原行）。</li>
 * </ul>
 */
class ExpiryRefundReconciliationIT extends MarketplaceItSupport {
	...
	void expiredRedeemDeadlineAutoRefundCompletes() {
		...
		Long refundFactCount = db
				.sql("SELECT COUNT(*)::bigint FROM consumer_order_refund WHERE order_id = CAST(:id AS uuid)")
				.bind("id", order.get("id")).map(row -> row.get(0, Long.class)).one().block();
		assertThat(refundFactCount).isEqualTo(1L);
	}
	...
	void responseLostRetryReusesDeterministicOperationId() {
		...
		when(finance.refund(any(Order.class), any()))
				.thenAnswer(inv -> Mono.error(new IllegalStateException("response lost"))).thenReturn(Mono.empty());
		...
		Order retried = service.attemptRefund(claimed, "dispatcher_retry").block();
		assertThat(retried.status()).isEqualTo("refunded");
		ArgumentCaptor<String> reasonCaptor = ArgumentCaptor.forClass(String.class);
		verify(finance, times(2)).refund(refundCaptor.capture(), reasonCaptor.capture());
		assertThat(refundCaptor.getAllValues()).allMatch(sent -> operationId.equals(sent.refundOperationId()));
		assertThat(reasonCaptor.getAllValues()).containsExactly("automatic_expiry", "automatic_expiry");
	...
	private Map<String, Object> createPaidOrder(String packageId) {
		Map<String, Object> response = client().post().uri("/api/v2/orders")
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("packageId", packageId)).exchange()
