...
class RecommenderMatchingControllerIT extends MarketplaceItSupport {
	...
	void merchantGetsExplainableStableRankingFromMarketplaceFacts() {
		...
		Map<String, Object> response = client().get()
				.uri("/api/tasks/" + fixture.taskId() + "/recommendations?limit=100")
				.header("X-Grassland-Identity",
						sign(fixture.merchantId(), "merchant", fixture.organizationId(), "basic_publish"))
	...
	void onlyTaskManagerCanReadOrInvite() {
		...
		client().post().uri("/api/tasks/" + fixture.taskId() + "/recommendations/" + candidate + "/invite")
				.header("X-Grassland-Identity",
						sign(UUID.randomUUID().toString(), "merchant", fixture.organizationId(), "basic_publish"))
	...
	void invitationIsIdempotentFreezesScoreAndEmitsOneEvent() {
		...
		Integer invitations = db
				.sql("SELECT COUNT(*)::int c FROM task_recommender_invitation"
						+ " WHERE task_id=CAST(:task AS uuid) AND recommender_account_id=CAST(:candidate AS uuid)")
				.bind("task", fixture.taskId()).bind("candidate", candidate).map(row -> row.get("c", Integer.class))
				.one().block();
		Integer events = db
				.sql("SELECT COUNT(*)::int c FROM marketplace_outbox"
		...
		String version = db
				.sql("SELECT score_snapshot->>'scoringVersion' v FROM task_recommender_invitation"
						+ " WHERE id=CAST(:id AS uuid)")
				.bind("id", first.get("id")).map(row -> row.get("v", String.class)).one().block();
		String recipient = db
				.sql("SELECT payload->>'recommenderAccountId' v FROM marketplace_outbox"
						+ " WHERE event_type='TaskRecommenderInvited' AND payload->>'taskId'=:task")
	...
	void applyingThroughNormalEndpointClosesInvitationAndRemovesCandidate() {
		...
		client().post().uri("/api/tasks/" + fixture.taskId() + "/applications")
				.header("X-Grassland-Identity", sign(candidate, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "应邀报名")).exchange().expectStatus().isCreated();

		Instant appliedAt = db
				.sql("SELECT applied_at FROM task_recommender_invitation"
						+ " WHERE task_id=CAST(:task AS uuid) AND recommender_account_id=CAST(:candidate AS uuid)")
	...
	private static int indexOf(java.util.List<Map<String, Object>> items, String accountId) {
		for (int index = 0; index < items.size(); index++) {
			if (accountId.equals(items.get(index).get("accountId")))
				return index;
		}
		return -1;
	}
	...
	private Fixture fixture() {
		...
		body.put("storeId", UUID.randomUUID().toString()); // 任务书 #77 卡 B：三字段必填造数
		...
		Map<String, Object> response = client().post().uri("/api/tasks")
				.header("X-Grassland-Identity", sign(merchant, "merchant", organization, "basic_publish"))
		...
		db.sql("UPDATE task SET status='published', published_at=now() WHERE id=CAST(:id AS uuid)").bind("id", taskId)
				.then().block();
		// 任务匹配/邀请 ACL 走 owner 判定（组织级）——#77 卡 B 后造数带 storeId，SQL 剥离模拟
		stripStoreScope(taskId);
		return new Fixture(merchant, organization, taskId);
	}

	private String candidateWithHistory(String platform, boolean completed, Integer rating, Integer responseHours) {
		...
		db.sql("""
				...
				""").bind("id", oldTask).bind("owner", oldOwner).bind("org", oldOrg).bind("platform", platform).then()
				.block();
		var insert = db
				.sql("""
						...
						""".formatted(completed ? "now() - interval '8 days'" : "NULL",
