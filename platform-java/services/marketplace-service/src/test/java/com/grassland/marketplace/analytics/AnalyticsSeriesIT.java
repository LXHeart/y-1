...
class AnalyticsSeriesIT extends MarketplaceItSupport {
	...
	void dayGranularityBucketsOrdersAndEventsWithZeroFill() {
		...
		EntityExchangeResult<byte[]> result = client().get()
				.uri(uri -> uri.path("/api/analytics/series").queryParam("organizationId", org)
				...
				.expectStatus().isOk().expectBody().jsonPath("$.success").isEqualTo(true).jsonPath("$.data.granularity")
				.isEqualTo("day").jsonPath("$.data.buckets.length()").isEqualTo(4).jsonPath("$.data.buckets[0].bucket")
				.isEqualTo("2026-08-12").jsonPath("$.data.buckets[0].orders").isEqualTo(1)
				.jsonPath("$.data.buckets[0].paidOrders").isEqualTo(1).jsonPath("$.data.buckets[0].grossGmvCents")
				.isEqualTo(1000).jsonPath("$.data.buckets[0].netGmvCents").isEqualTo(1000)
				// C103-15 事实口径：未完成分账不算已实现收入（§6.6），已结收入只来自 settlement fact。
				.jsonPath("$.data.buckets[0].merchantRevenueCents").isEqualTo(0)
				.jsonPath("$.data.buckets[0].recommenderRevenueCents").isEqualTo(0).jsonPath("$.data.buckets[1].bucket")
				.isEqualTo("2026-08-13").jsonPath("$.data.buckets[1].exposures").isEqualTo(1)
				.jsonPath("$.data.buckets[1].orders").isEqualTo(0).jsonPath("$.data.buckets[2].orders").isEqualTo(0)
				.jsonPath("$.data.buckets[3].bucket").isEqualTo("2026-08-15").jsonPath("$.data.buckets[3].conversions")
				.isEqualTo(0).returnResult();
	...
	void weekGranularityAlignsToMonday() {
		...
		client().get()
				...
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.buckets.length()").isEqualTo(1)
				.jsonPath("$.data.buckets[0].bucket").isEqualTo("2026-08-10").jsonPath("$.data.buckets[0].orders")
				.isEqualTo(1).jsonPath("$.data.buckets[0].conversions").isEqualTo(1);
	}

	@Test
	void adminSeriesRequiresFinanceOrRiskRole() {
		String org = UUID.randomUUID().toString();
		String query = "/api/admin/analytics/series?organizationId=" + org + "&from=" + FROM + "&to=" + TO
				+ "&granularity=month";

		client().get().uri(query).header("X-Grassland-Identity", signWithRole(UUID.randomUUID().toString(), "user"))
				.exchange().expectStatus().isForbidden();

		client().get().uri(query).header("X-Grassland-Identity", signWithRole(UUID.randomUUID().toString(), "finance"))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.buckets.length()").isEqualTo(1);
	}
	...
	void validatesGranularityWindowAndScope() {
		...
		client().get()
				...
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isBadRequest().expectBody()
				.jsonPath("$.error").isEqualTo("granularity 仅支持 day/week/month");

		client().get()
				.uri(uri -> uri.path("/api/analytics/series").queryParam("organizationId", org)
						.queryParam("from", TO.toString()).queryParam("to", FROM.toString()).build())
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isBadRequest().expectBody()
				.jsonPath("$.error").isEqualTo("to 必须晚于 from");

		// 跨度桶数超限（day 粒度 > 400 天）
		client().get()
				.uri(uri -> uri.path("/api/analytics/series").queryParam("organizationId", org)
						.queryParam("from", "2020-01-01T00:00:00Z").queryParam("to", "2026-08-15T00:00:00Z").build())
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isBadRequest().expectBody()
				.jsonPath("$.error").isEqualTo("时间跨度的桶数超过上限（400）");
	...
	private String seedPaidOrder(String organizationId, Instant createdAt) {
		...
		db.sql("""
				INSERT INTO consumer_order(
				  ...
				  redeem_code_hash, redeem_deadline, payment_operation_id, created_at, paid_at)
				VALUES (CAST(:id AS uuid), CAST(:consumer AS uuid), CAST(:org AS uuid),
				        ...
				        :redeemHash, now() + interval '30 days', :paymentOperation, :createdAt, :createdAt)
	...
	private void seedAttributionEvent(String organizationId, String eventType, Instant occurredAt) {
		db.sql("""
				...
				""").bind("id", UUID.randomUUID().toString()).bind("key", "series-" + UUID.randomUUID())
