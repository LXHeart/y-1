 ...
 *
 * <p>
 * bean 覆盖集合与 ApplicationControllerIT 一致（复用上下文缓存）；审稿派发器在基座关闭—— 超时转人工直接构造
 * {@link DraftReviewDispatcher} 驱动 seam。
 */
...
class DraftReviewContractIT extends MarketplaceItSupport {
	...
	void draftSubmissionWithoutLinkAndReviewGate() {
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions")
				...
				.bodyValue(Map.of("contentUrl", "https://www.xiaohongshu.com/p/abc")).exchange().expectStatus()
				.isEqualTo(409);
		...
		Map<String, Object> created = client().post()
				...
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "脚本初稿：开箱场景")).exchange().expectStatus().isCreated().expectBody(Map.class)
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions/draft")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "v2")).exchange().expectStatus().isEqualTo(409);

		// 商家批准 = script 里程碑互签联锁过审（C96-02 端点复用）
		EngagementMilestone proposal = latestScriptProposal(appId);
		client().post()
				.uri("/api/tasks/" + task + "/applications/" + appId + "/milestones/" + proposal.id() + "/confirm")
	...
	void draftSubmissionRejectedWhenContractDoesNotRequireReview() {
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions/draft")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "不请自来")).exchange().expectStatus().isEqualTo(409);
	}
	...
	void draftRevisionCapSharedWithSupplementCap() {
		...
		for (int round = 1; round <= 2; round++) {
			...
			client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions/" + draftId + "/reject")
					.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
					.bodyValue(Map.of("note", "改一下开头")).exchange().expectStatus().isOk();
		}
		// 第三稿仍可交，但第三次退回被限次拦截（cap=2）→ 退改耗尽走争议/退出
		String thirdId = submitDraft(task, appId, rec, "第三稿");
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions/" + thirdId + "/reject")
				.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "再改")).exchange().expectStatus().isEqualTo(409);
	}
	...
	void draftResubmitDeadlineEnforced() {
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions/" + draftId + "/reject")
				.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "重写")).exchange().expectStatus().isOk();
		// 把被拒草稿的 reviewed_at 拨到补交窗之外
		db.sql("UPDATE engagement_submission SET reviewed_at = now() - interval '2 hours'"
				+ " WHERE id = CAST(:id AS uuid)").bind("id", draftId).then().block();

		client().post().uri("/api/tasks/" + task + "/applications/" + appId + "/submissions/draft")
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", "过期补交")).exchange().expectStatus().isEqualTo(409);
	}
	...
	void draftReviewTimeoutEscalatesToManualReview() {
		...
		DraftReviewDispatcher dispatcher = new DraftReviewDispatcher(submissionRepo, applicationRepo,
				taskRepository(db), new com.grassland.marketplace.event.OutboxRepository(db), opsCases, 32, 2L);
	...
	private String submitDraft(String taskId, String appId, String rec, String note) {
		Map<String, Object> created = client().post()
				...
				.header(H, sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("note", note)).exchange().expectStatus().isCreated().expectBody(Map.class)
				.returnResult().getResponseBody();
	...
	private EngagementMilestone latestScriptProposal(String appId) {
		EngagementMilestone proposal = milestoneRepo.findByApplication(appId)
				.filter(m -> EngagementMilestone.KIND_SCRIPT.equals(m.kind()) && !m.confirmed()).blockLast();
	...
	private void injectBounty(String appId, long bountyCents) {
		db.sql("UPDATE task_application SET bounty_cents = :b WHERE id = CAST(:id AS uuid)").bind("b", bountyCents)
	...
	private Map<String, Object> cancelTask(String merchant, String org, String taskId) {
		Map<String, Object> resp = client().post().uri("/api/tasks/" + taskId + "/cancel")
				.header(H, sign(merchant, "merchant", org, "basic_publish")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("expectedVersion", 1)).exchange().expectStatus().isOk().expectBody(Map.class)
				.returnResult().getResponseBody();
	...
	/** C103-13 后 DraftReviewDispatcher 需要 TaskRepository（M 侧收件字段）；从 db 直接构造。 */
	private static TaskRepository taskRepository(org.springframework.r2dbc.core.DatabaseClient db) {
