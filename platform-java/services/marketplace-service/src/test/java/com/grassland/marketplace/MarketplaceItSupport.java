 ...
 * <p>
 * 单例 testcontainers
 * postgres（全程不重启，端口稳定）；{@code marketplace.datasource.from-database-url=true} 触发
 * {@code MarketplaceDataSourceConfig} → Flyway V1 建
 * task/outbox；{@code identity-assertion} 注入 signer； 禁用 object-storage（免 MinIO）+
 * temporal test-server（内存，免 temporal 容器）。提供 {@link #sign} 签断言。
 */
...
public abstract class MarketplaceItSupport {
	...
	void authorizeIdentityScopeByDefault() {
		lenient().when(storeAuthorization.authorize(anyString(), anyString(), any(), anyString()))
				.thenAnswer(invocation -> {
					...
					return Mono.just(new IdentityStoreAuthorizationClient.Authorization(true, invocation.getArgument(0),
							invocation.getArgument(1), storeId, "manager", storeId == null ? "organization" : "store",
							"finance_transaction"));
				});
		// 任务书 #24：门店公开块批量拉默认回空（需要品牌数据的用例自行覆写 stub）；
		// 不 stub 时 Mockito 回 null Mono → enrichment 链路 NPE。
		lenient().when(storeAuthorization.publicProfiles(any())).thenReturn(Mono.just(java.util.List.of()));
	}
	...
	static void props(DynamicPropertyRegistry r) {
		...
		// 任务书 #103 C103-03：退出资金恢复 worker 同理默认关（专用 IT 直接驱动 service seam）。
	...
	public static String marketplaceMigrationDir() {
		try {
			java.net.URL resource = MarketplaceItSupport.class.getClassLoader()
					.getResource("db/migration/V1__init_task.sql");
			if (resource == null) {
	...
	protected WebTestClient client() {
		// responseTimeout 30s：默认 5s 在 CI 慢 runner 负载下偶发超时（AnalyticsControllerIT round
		// 32501501102；
		// 与 IdentityItSupport/FinanceItSupport/IntelligenceItSupport 同口径）。
	...
	protected String sign(String accountId, String activeIdentityType, String organizationId, String permissionTier) {
		...
		return userSigner("edge-bff", "grassland-marketplace").sign(new IdentityAssertion(accountId, activeIdentityType,
	 ...
	 *
	 * <p>
	 * {@code activeIdentityType} 留 null —— 运营操作与「选了哪个业务视角」正交， 这正是端点必须按 role
	 * 判定的原因（历史上按 activeIdentityType 建模职能会导致恒 403）。
	 */
	protected String signWithRole(String accountId, String role) {
		Instant now = Instant.now();
		return userSigner("edge-bff", "grassland-marketplace")
	...
	protected String signService(String principal) {
		Instant now = Instant.now();
		return serviceSigner(principal, "grassland-marketplace")
				.sign(new IdentityAssertion("service:" + principal, null, null, null, null, "service", "internal", null,
						"r", "t", "grassland-marketplace", now, now.plusSeconds(30), "service", principal));
	}
}
