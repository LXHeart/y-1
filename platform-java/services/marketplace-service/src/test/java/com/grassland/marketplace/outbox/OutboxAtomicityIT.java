...
class OutboxAtomicityIT extends MarketplaceItSupport {
	...
	void withdrawRollsBackWhenOutboxFails() {
		...
		client().post().uri("/api/tasks/" + taskId + "/applications/" + appId + "/withdraw")
				.header("X-Grassland-Identity", sign(recommender, "recommender")).exchange().expectStatus()
				.is5xxServerError();
	...
	private String taskStatus(String taskId) {
		return db.sql("SELECT status FROM task WHERE id = CAST(:t AS uuid)").bind("t", taskId)
	...
	private int taskVersion(String taskId) {
		Integer version = db.sql("SELECT version FROM task WHERE id = CAST(:t AS uuid)").bind("t", taskId)
				.map(row -> row.get("version", Integer.class)).one().block();
		return version == null ? -1 : version;
	}

	private boolean taskPublishedAtIsNull(String taskId) {
		Boolean publishedAtIsNull = db
				.sql("SELECT published_at IS NULL AS is_null FROM task WHERE id = CAST(:t AS uuid)").bind("t", taskId)
	...
	private int taskMinimumRecommenderLevel(String taskId) {
		Integer level = db.sql("SELECT min_recommender_level FROM task WHERE id = CAST(:t AS uuid)").bind("t", taskId)
				.map(row -> row.get("min_recommender_level", Integer.class)).one().block();
	...
	private WebTestClient.ResponseSpec publishFailing(String merchant, String org) {
		...
		return client().post().uri("/api/tasks")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(b).exchange();
	}
