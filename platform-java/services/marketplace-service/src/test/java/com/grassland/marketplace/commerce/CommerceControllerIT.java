...
/**
 * End-to-end marketplace commerce lifecycle against PostgreSQL, with finance
 * kept at its HTTP seam.
 */
class CommerceControllerIT extends MarketplaceItSupport {
	...
	void packageRevisionDoesNotChangePaidOrderSnapshotAndRedemptionIsSingleUse() {
		...
		// 任务书 #75 D4：无推广任务的套餐，recommender 参数不构成有效归因 → 自然流量（推荐官份额 0）。
		...
		assertThat(order.get("recommenderAmountCents")).isEqualTo(0);
		...
		client().put().uri("/api/v2/merchant/packages/" + offer.get("id"))
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(revisedBody).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.version").isEqualTo(2).jsonPath("$.data.priceCents").isEqualTo(2500);

		client().get().uri("/api/v2/orders/" + order.get("id")).header("X-Grassland-Identity", sign(consumer, null))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.packageVersion").isEqualTo(1)
				.jsonPath("$.data.priceCents").isEqualTo(1000);

		// 任务书 #75 D3：核销即刻 redeemed（不再内联分账），冷静期由 dispatcher 承接。
		...
		client().post().uri("/api/v2/merchant/redemptions")
				...
				.isOk().expectBody().jsonPath("$.data.status").isEqualTo("redeemed").jsonPath("$.data.splitEligibleAt")
				.exists();
		client().post().uri("/api/v2/merchant/redemptions")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "finance_transaction"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("code", code)).exchange().expectStatus()
		...
		client().post().uri("/api/v2/orders/" + order.get("id") + "/review")
				...
				.bodyValue(Map.of("rating", 5, "comment", "很好")).exchange().expectStatus().isCreated();
	}
	...
	void merchantOrderExportSupportsCsvAndRealXlsx() {
		...
		byte[] csv = client().get()
				.uri(uri -> uri.path("/api/v2/merchant/orders/export").queryParam("organizationId", org)
						.queryParam("format", "csv").build())
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isOk().expectHeader()
				.contentType("text/csv;charset=UTF-8").expectBody(byte[].class).returnResult().getResponseBody();
		...
		byte[] xlsx = client().get()
				.uri(uri -> uri.path("/api/v2/merchant/orders/export").queryParam("organizationId", org)
						.queryParam("format", "xlsx").build())
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isOk().expectHeader()
	...
	void lastInventoryCanOnlyBeSoldOnceAndRefundRestoresItOnce() {
		...
		client().post().uri("/api/v2/orders").header("X-Grassland-Identity", sign(UUID.randomUUID().toString(), null))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("packageId", offer.get("id"))).exchange()
				.expectStatus().isEqualTo(409);

		String firstConsumer = String.valueOf(first.get("consumerAccountId"));
		client().post().uri("/api/v2/orders/" + first.get("id") + "/refund")
				.header("X-Grassland-Identity", sign(firstConsumer, null)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("reason", "change_of_mind")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.status").isEqualTo("refunded");
		client().post().uri("/api/v2/orders/" + first.get("id") + "/refund")
				.header("X-Grassland-Identity", sign(firstConsumer, null)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("reason", "again")).exchange().expectStatus().isEqualTo(409);

		client().get().uri("/api/v2/packages/" + offer.get("id")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.remainingStock").isEqualTo(1);
	...
	void timeSlotInventoryOrdersReserveTheSlotAndRefundRestoresIt() {
		...
		java.util.List<Map<String, Object>> slots = (java.util.List<Map<String, Object>>) offer.get("inventorySlots");
		...
		Map<String, Object> booked = createOrderWithSlot(UUID.randomUUID().toString(), (String) offer.get("id"),
				morning, null);
		assertThat(booked.get("status")).isEqualTo("paid");

		client().get().uri("/api/v2/orders/" + booked.get("id"))
				.header("X-Grassland-Identity", sign(String.valueOf(booked.get("consumerAccountId")), null)).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.inventorySlotId").isEqualTo(morning)
				.jsonPath("$.data.slotStart").isEqualTo("2026-09-01T10:00:00Z").jsonPath("$.data.slotEnd")
				.isEqualTo("2026-09-01T11:00:00Z");

		client().post().uri("/api/v2/orders").header("X-Grassland-Identity", sign(UUID.randomUUID().toString(), null))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("packageId", offer.get("id"), "inventorySlotId", morning)).exchange().expectStatus()
				.isEqualTo(409);
		...
		client().post().uri("/api/v2/orders/" + booked.get("id") + "/refund")
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("reason", "schedule_change")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.status").isEqualTo("refunded");

		client().get().uri("/api/v2/packages/" + offer.get("id")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.inventorySlots[0].remainingStock").isEqualTo(1)
	...
	void afterSalesDisputeDetailIsVisibleToOwnerAndStoreStaffAndPartialResolutionRefunds() {
		...
		client().post().uri("/api/v2/orders/" + order.get("id") + "/after-sales-dispute")
				.header("X-Grassland-Identity", sign(consumer, null)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("reason", "到店后商家无法提供服务")).exchange().expectStatus().isCreated().expectBody()
				.jsonPath("$.data.status").isEqualTo("after_sales_disputed");
		...
		client().get().uri("/api/v2/orders/" + order.get("id") + "/after-sales-dispute")
				...
				.jsonPath("$.data.status").isEqualTo("open").jsonPath("$.data.reason").isEqualTo("到店后商家无法提供服务");
		...
		client().get().uri("/api/v2/orders/" + order.get("id") + "/after-sales-dispute")
				.header("X-Grassland-Identity", merchantAuth).exchange().expectStatus().isOk().expectBody()
	...
	void partialRefundsAccumulateAndBuyerRebindEndpointIsGone() {
		...
		client().post().uri("/api/v2/orders/" + order.get("id") + "/refund")
				.header("X-Grassland-Identity", sign(consumer, null)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("amountCents", 2000, "reason", "少上一道菜")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.status").isEqualTo("partially_refunded").jsonPath("$.data.refundedAmountCents")
				.isEqualTo(2000);

		// createOrder 停写 V37：归因分配视图为空。
	...
	/** 任务书 #103 C103-07（TC103-07-01/02/06）：部分退款后原码继续可用——展示与核销同源。 */
	@Test
	void partiallyRefundedOrderKeepsOriginalRedeemCodeAndStaysRedeemable() {
		String merchant = UUID.randomUUID().toString();
		String consumer = UUID.randomUUID().toString();
		String org = UUID.randomUUID().toString();
		Map<String, Object> offer = createAndPublish(merchant, org, 10000, 5);
		...
		// 实际核销用同一原码成功（资格与展示同源；库存/核销次数一次）。
		client().post().uri("/api/v2/merchant/redemptions").header("X-Grassland-Identity", merchantAuth)
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("code", originalCode)).exchange()
				.expectStatus().isOk();
	...
	void adminOrdersEnvelopePaginatesAndClamps() {
		...
		client().get().uri("/api/admin/commerce/orders?limit=1").header("X-Grassland-Identity", admin).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.items.length()").isEqualTo(1)
				.jsonPath("$.data.items[0].id").isEqualTo(secondOrder.get("id")).jsonPath("$.data.limit").isEqualTo(1)
				.jsonPath("$.data.offset").isEqualTo(0).jsonPath("$.data.total").isNumber();
		client().get().uri("/api/admin/commerce/orders?limit=1&offset=1").header("X-Grassland-Identity", admin)
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.items[0].id")
				.isEqualTo(firstOrder.get("id")).jsonPath("$.data.offset").isEqualTo(1);
		...
		assertThat((Integer) unfiltered.get("total")).isEqualTo((Integer) dataOf(adminOrders(admin, null)).get("total"))
		...
		client().get().uri("/api/admin/commerce/orders?limit=0").header("X-Grassland-Identity", admin).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.limit").isEqualTo(50);
		client().get().uri("/api/admin/commerce/orders?limit=201").header("X-Grassland-Identity", admin).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.limit").isEqualTo(200);
		client().get().uri("/api/admin/commerce/orders?offset=-5").header("X-Grassland-Identity", admin).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.offset").isEqualTo(0);
	}
	...
	void adminRedemptionsEnvelopeMixesStatusesOrderedByCreatedAt() {
		...
		db.sql("UPDATE consumer_order SET status = 'redeemed', redeemed_at = now()" + " WHERE id = CAST(:id AS uuid)")
				.bind("id", early.get("id")).then().block();
		db.sql("UPDATE consumer_order SET status = 'redeeming'" + " WHERE id = CAST(:id AS uuid)")
				.bind("id", late.get("id")).then().block();
		...
		assertThat(items).allSatisfy(item -> assertThat(item.get("status")).isIn("redeeming", "redeemed"));
		...
		// 分页生效 + total 跨页同口径。
		client().get().uri("/api/admin/commerce/redemptions?limit=1&offset=1").header("X-Grassland-Identity", admin)
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.items.length()").isEqualTo(1)
				.jsonPath("$.data.total").isEqualTo(total).jsonPath("$.data.offset").isEqualTo(1);

		// 钳制边界。
		client().get().uri("/api/admin/commerce/redemptions?limit=0").header("X-Grassland-Identity", admin).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.limit").isEqualTo(50);
		client().get().uri("/api/admin/commerce/redemptions?limit=999&offset=-1").header("X-Grassland-Identity", admin)
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.limit").isEqualTo(200)
	...
	private Map<String, Object> adminOrders(String adminHeader, String status) {
		...
		return client().get().uri(uri).header("X-Grassland-Identity", adminHeader).exchange().expectStatus().isOk()
