 ...
 *
 * <p>
 * 走完整真实链路：发布 → 报名 → 接受 → 提交交付物 → 确认履约 → 评分 → 读声誉。 非资金型任务（bounty 为空）以免 IT 依赖
 * finance；结算 workflow 的出站边界仍以替身隔离。
 */
class ReputationControllerIT extends MarketplaceItSupport {
	...
	/**
	 * 配置 finance mock：release/reserve/capture 全返回成功（幂等）——非资金型任务不走 finance，但 cancel
	 * 路径会调用 release。
	 */
	@BeforeEach
	...
	void rateAfterConfirmThenReputationReflectsIt() {
		...
		client().post().uri(ratingUri(task, app))
				...
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("score", 5, "comment", "内容质量很好")).exchange()
				.expectStatus().isCreated().expectBody().jsonPath("$.data.score").isEqualTo(5)
				.jsonPath("$.data.recommenderAccountId").isEqualTo(rec).jsonPath("$.data.ratedByAccountId")
				.isEqualTo(merchant);
		...
		client().get().uri("/api/reputation/" + rec)
				...
				.expectStatus().isOk().expectBody().jsonPath("$.data.accountId").isEqualTo(rec)
				.jsonPath("$.data.acceptedCount").isEqualTo(1).jsonPath("$.data.completedCount").isEqualTo(1)
				.jsonPath("$.data.merchantCancelledCount").isEqualTo(0).jsonPath("$.data.rejectedCount").isEqualTo(0)
				.jsonPath("$.data.withdrawnCount").isEqualTo(0).jsonPath("$.data.terminalCount").isEqualTo(1)
				.jsonPath("$.data.completionRate").isEqualTo(1.0).jsonPath("$.data.ratingCount").isEqualTo(1)
				.jsonPath("$.data.averageScore").isEqualTo(5.0).jsonPath("$.data.lastActiveAt").exists()
				...
				.jsonPath("$.data.level").isEqualTo("Lv1").jsonPath("$.data.levelTitle").isEqualTo("新手草友");
	}
	...
	void unknownRecommenderReturnsEmptyStats() {
		...
		client().get().uri("/api/reputation/" + UUID.randomUUID())
				.header("X-Grassland-Identity", sign(viewer, "merchant")).exchange().expectStatus().isOk().expectBody()
				.jsonPath("$.data.acceptedCount").isEqualTo(0).jsonPath("$.data.completedCount").isEqualTo(0)
				.jsonPath("$.data.merchantCancelledCount").isEqualTo(0).jsonPath("$.data.rejectedCount").isEqualTo(0)
				.jsonPath("$.data.withdrawnCount").isEqualTo(0).jsonPath("$.data.terminalCount").isEqualTo(0)
				.jsonPath("$.data.completionRate").isEqualTo(0.0).jsonPath("$.data.averageScore").doesNotExist() // 无评分
																													// →
																													// null（不是
																													// 0）
				.jsonPath("$.data.lastActiveAt").doesNotExist().jsonPath("$.data.inactiveDowngraded").isEqualTo(false)
	...
	void merchantDecisionDoesNotRefreshRecommenderActivity() {
		...
		client().get().uri("/api/reputation/" + rec)
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.lastActiveAt")
				.value(value -> assertThat(Instant.parse((String) value))
	...
	void rateBeforeConfirmConflict() {
		...
		client().post().uri(ratingUri(task, app))
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("score", 4)).exchange().expectStatus()
				.isEqualTo(409);
	}
	...
	void duplicateRatingConflict() {
		...
		client().post().uri(ratingUri(task, app))
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("score", 1)).exchange().expectStatus()
				.isEqualTo(409);
	}
	...
	void invalidScoreAndForeignMerchantRejected() {
		...
		client().post().uri(ratingUri(task, app))
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("score", 6)).exchange().expectStatus()
				.isBadRequest();

		client().post().uri(ratingUri(task, app))
				.header("X-Grassland-Identity", sign(UUID.randomUUID().toString(), "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("score", 5)).exchange().expectStatus()
				.isForbidden();
	}
	...
	void ratingVisibility() {
		...
		client().get().uri(ratingUri(task, app)).header("X-Grassland-Identity", sign(rec, "recommender")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data").doesNotExist();

		rate(merchant, org, task, app, 4);

		client().get().uri(ratingUri(task, app)).header("X-Grassland-Identity", sign(rec, "recommender")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.score").isEqualTo(4);
	...
	void merchantCancelEngagementCountsCorrectly() {
		...
		client().post().uri("/api/tasks/" + task + "/cancel")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("expectedVersion", taskVersion(task)))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.refundedCount").isEqualTo(1);

		// 验证声誉指标
		client().get().uri("/api/reputation/" + rec)
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.acceptedCount").isEqualTo(1) // 累计接单包含后续被商家取消的单
				.jsonPath("$.data.completedCount").isEqualTo(0) // 未完成
				.jsonPath("$.data.merchantCancelledCount").isEqualTo(1) // 商家取消计数
				.jsonPath("$.data.rejectedCount").isEqualTo(0).jsonPath("$.data.withdrawnCount").isEqualTo(0)
	...
	void mixedCompletedAndCancelledCalculatesCorrectly() {
		...
		client().post().uri("/api/tasks/" + task2 + "/cancel")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("expectedVersion", taskVersion(task2)))
				.exchange().expectStatus().isOk();

		// 验证声誉指标
		client().get().uri("/api/reputation/" + rec)
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.acceptedCount").isEqualTo(2)
				.jsonPath("$.data.completedCount").isEqualTo(1).jsonPath("$.data.merchantCancelledCount").isEqualTo(1)
	...
	void recommenderWithdrawsCountsCorrectly() {
		...
		client().get().uri("/api/reputation/" + rec)
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.acceptedCount").isEqualTo(0)
				.jsonPath("$.data.completedCount").isEqualTo(0).jsonPath("$.data.merchantCancelledCount").isEqualTo(0)
				.jsonPath("$.data.rejectedCount").isEqualTo(0).jsonPath("$.data.withdrawnCount").isEqualTo(1)
				.jsonPath("$.data.terminalCount").isEqualTo(1).jsonPath("$.data.completionRate").isEqualTo(0.0);
	}
	...
	void merchantRejectsApplicationCountsCorrectly() {
		...
		client().post().uri("/api/tasks/" + task + "/applications/" + app + "/reject")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("note", "不符合要求")).exchange().expectStatus()
				.isOk();
		...
		client().get().uri("/api/reputation/" + rec)
				...
				.expectStatus().isOk().expectBody().jsonPath("$.data.acceptedCount").isEqualTo(0)
				.jsonPath("$.data.completedCount").isEqualTo(0).jsonPath("$.data.merchantCancelledCount").isEqualTo(0)
				.jsonPath("$.data.rejectedCount").isEqualTo(1).jsonPath("$.data.withdrawnCount").isEqualTo(0)
				.jsonPath("$.data.terminalCount").isEqualTo(1).jsonPath("$.data.completionRate").isEqualTo(0.0);
	}
	...
	private void rate(String merchant, String org, String task, String app, int score) {
		client().post().uri(ratingUri(task, app))
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("score", score)).exchange().expectStatus()
				.isCreated();
	}
	...
	private void accept(String merchant, String org, String task, String app) {
		client().post().uri("/api/tasks/" + task + "/applications/" + app + "/accept")
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "basic_publish")).exchange()
				.expectStatus().isOk(); // 非资金型 → 4B 直连同步 200
	}
	...
	private void submit(String rec, String task, String app) {
		client().post().uri("/api/tasks/" + task + "/applications/" + app + "/submissions")
				...
				.bodyValue(Map.of("contentUrl", "https://example.com/post/" + app)).exchange().expectStatus()
				.isCreated();
	}
	...
	private String apply(String rec, String task) {
		Map<String, Object> resp = client().post().uri("/api/tasks/" + task + "/applications")
				.header("X-Grassland-Identity", sign(rec, "recommender")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of()).exchange().expectStatus().isCreated().expectBody(Map.class).returnResult()
				.getResponseBody();
	...
	private int taskVersion(String taskId) {
		return db.sql("SELECT version FROM task WHERE id = CAST(:id AS uuid)").bind("id", taskId)
				.map(row -> row.get("version", Integer.class)).one().block();
	}
}
