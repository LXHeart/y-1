 ...
 *
 * <p>
 * 登记走 {@link OpsDltRegistrar}（消费者的纯逻辑部分，不需要真 Kafka）；重投 mock KafkaTemplate。
 */
class OpsDltControllerIT extends MarketplaceItSupport {
	...
	void registerOpensCase() {
		...
		client().get().uri("/api/ops/dlt").header("X-Grassland-Identity", signWithRole(OPS_A, "customer_service"))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.items.length()").isEqualTo(1)
				.jsonPath("$.data.total").isEqualTo(1).jsonPath("$.data.limit").isEqualTo(50).jsonPath("$.data.offset")
				.isEqualTo(0).jsonPath("$.data.items[0].topic").isEqualTo(DLT_TOPIC).jsonPath("$.data.items[0].offset")
				.isEqualTo(10).jsonPath("$.data.items[0].status").isEqualTo("pending");
	}

	@Test
	@DisplayName("分页信封：默认 50 条、offset 翻页可达第 51+ 条、total 恒等于全量、非法参数 400")
	void queuePaginationEnvelope() {
		for (long offset = 0L; offset < 55L; offset++) {
			register(100L + offset);
		}

		client().get().uri("/api/ops/dlt").header("X-Grassland-Identity", signWithRole(OPS_A, "customer_service"))
				.exchange().expectStatus().isOk().expectBody().jsonPath("$.data.items.length()").isEqualTo(50)
				.jsonPath("$.data.total").isEqualTo(55);

		client().get().uri("/api/ops/dlt?limit=50&offset=50")
				.header("X-Grassland-Identity", signWithRole(OPS_A, "customer_service")).exchange().expectStatus()
				.isOk().expectBody().jsonPath("$.data.items.length()").isEqualTo(5).jsonPath("$.data.total")
				.isEqualTo(55).jsonPath("$.data.offset").isEqualTo(50);

		// status 筛选口径不变（默认 pending；replayed/discarded 各自可查）
		client().get().uri("/api/ops/dlt?status=discarded")
				.header("X-Grassland-Identity", signWithRole(OPS_A, "customer_service")).exchange().expectStatus()
				.isOk().expectBody().jsonPath("$.data.items.length()").isEqualTo(0).jsonPath("$.data.total")
				.isEqualTo(0);

		client().get().uri("/api/ops/dlt?limit=0")
				.header("X-Grassland-Identity", signWithRole(OPS_A, "customer_service")).exchange().expectStatus()
				.isBadRequest();
		client().get().uri("/api/ops/dlt?offset=-1")
				.header("X-Grassland-Identity", signWithRole(OPS_A, "customer_service")).exchange().expectStatus()
				.isBadRequest();
	}
	...
	void replayRequiresApproval() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				...
				.bodyValue("{\"replay\":true,\"operationId\":\"op-dlt-1\"}").exchange().expectStatus().isEqualTo(409);
	...
	void replaySendsToOriginalTopic() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				...
				.bodyValue("{\"replay\":true,\"operationId\":\"op-dlt-replay\"}").exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.action").isEqualTo("dlt_replay").jsonPath("$.data.status")
				.isEqualTo("succeeded");
	...
	void replayIsIdempotent() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(body).exchange().expectStatus().isOk().expectBody().jsonPath("$.data.status")
				.isEqualTo("succeeded");
	...
	void alreadyHandledIsRejected() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"replay\":true,\"operationId\":\"op-first\"}").exchange().expectStatus().isOk();

		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"replay\":true,\"operationId\":\"op-second\"}").exchange().expectStatus().isEqualTo(409);
	...
	void discardMarksOnly() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"replay\":false,\"operationId\":\"op-discard\"}").exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.action").isEqualTo("dlt_discard").jsonPath("$.data.status")
				.isEqualTo("succeeded").jsonPath("$.data.outcome").isEqualTo("discarded");
	...
	void replayFailureKeepsPending() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"replay\":true,\"operationId\":\"op-dlt-fail\"}").exchange().expectStatus().isOk()
	...
	void validation() {
		...
		client().post().uri("/api/ops/dlt/" + message.id() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"operationId\":\"op-x\"}").exchange().expectStatus().isEqualTo(400);

		client().post().uri("/api/ops/dlt/" + UUID.randomUUID() + "/actions")
				.header("X-Grassland-Identity", signWithRole(OPS_B, "admin")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue("{\"replay\":true,\"operationId\":\"op-y\"}").exchange().expectStatus().isNotFound();
	}

	private Long count(String table) {
		return db.sql("SELECT count(*) AS n FROM " + table).map(r -> r.get("n", Long.class)).one().block();
	}
}
