...
public class ConsumerPaymentRepository {
    ...
    public Mono<Payment> findPayment(String orderRef) {
        return db.sql("""
                SELECT id::text, order_ref, consumer_account_id::text, organization_id::text,
                       amount_cents, currency, channel, provider_ref, operation_id, status,
    ...
    public Mono<Void> markPaymentRefunded(String orderRef) {
        return db.sql("""
                UPDATE consumer_payment
                   SET status = 'refunded', refunded_at = now(), updated_at = now()
                 WHERE order_ref = :orderRef AND status = 'succeeded'
                """)
                .bind("orderRef", orderRef).then();
    }
    ...
    public record Payment(
            ...
            long amountCents, String currency, String channel, String providerRef,
