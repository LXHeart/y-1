 ...
 *
 * <ul>
 * <li>POST /api/finance/accounts/{orgId}/credit — sandbox 充值（requireMerchant +
 * org 自查；原子加余额）。</li>
 * <li>POST /api/finance/accounts/{orgId}/reservations — 预留（原子条件扣余额；余额不足
 * 409；engagementRef 幂等）。</li>
 * <li>POST /api/finance/reservations/{engagementRef}/release —
 * 释放（reserved→released + 还原余额）。</li>
 * </ul>
 *
 * <p>
...
public class EscrowController {
	...
	public EscrowController(FinanceCallerResolver callers, AccountRepository accounts,
			ReservationRepository reservations, OutboxRepository outbox, WalletRepository wallets,
			PlatformFeePolicy fees, EscrowLifecycleService lifecycle, TransactionalOperator transactions,
	...
	public Mono<ResponseEntity<Map<String, Object>>> credit(@PathVariable String orgId, @RequestBody CreditRequest body,
			ServerHttpRequest request) {
		long amount = body.amountCents();
		return callers.requireMerchant(request).filter(caller -> orgId.equals(caller.organizationId()))
				.switchIfEmpty(fail(403, "无权操作该组织账户")).flatMap(
						caller -> transactions
								.transactional(accounts.credit(orgId, amount).switchIfEmpty(fail(404, "账户不存在，请先开户"))
										.flatMap(acct -> ledger.postDeposit(orgId, amount)
												.then(outbox.append(accountEnvelope("AccountCredited", acct, amount,
														caller.accountId())).thenReturn(acct))))
	...
	public Mono<ResponseEntity<Map<String, Object>>> reserve(@PathVariable String orgId,
		...
		return callers.authorizeForOrg(request, orgId, FinanceCallerResolver.MARKETPLACE_SERVICE)
				...
				.switchIfEmpty(fail(409, "交易金额超出本组织单笔上限")).flatMap(caller -> {
					...
					return transactions
							.transactional(reserveWork(orgId, ref, amount, payeeAccountId, commissionBonusBps));
	...
	/**
	 * 预留领域写（扣余额 + 建预留）+ outbox append，绑进同一事务（Slice 7C）。幂等：engagementRef
	 * 既有→200；并发冲突→既有。
	 */
	private Mono<Reserved> reserveWork(String orgId, String ref, long amount, String payeeAccountId,
		...
		return reservations.findByEngagementRef(ref)
				.flatMap(r -> existingReservation(orgId, ref, amount, payeeAccountId, commissionBonusBps,
						commissionBonusCents, r))
				.switchIfEmpty(accounts.decrement(orgId, amount).switchIfEmpty(fail(409, "余额不足"))
						.flatMap(acct -> reservations
								.create(acct.id(), orgId, ref, amount, payeeAccountId, commissionBonusBps,
								...
								.switchIfEmpty(accounts.credit(orgId, amount)
										...
										.flatMap(r -> existingReservation(orgId, ref, amount, payeeAccountId,
												commissionBonusBps, commissionBonusCents, r))
										.switchIfEmpty(fail(409, "幂等预留冲突")))))
				.flatMap(
						res -> (res.created()
								? ledger.postReserve(orgId, ref, amount)
										.then(outbox.append(reservationEnvelope("FundsReserved", res.reservation()))
												.thenReturn(res))
								: Mono.just(res)));
	}
	...
	/**
	 * 冲正（D-06 争议处置，Slice 6C Phase D）：captured → refunded + 还原余额（判商家方胜诉、资金已 capture
	 * 时退还商家）。 仅 trust 服务可调（争议终局钱侧分派）。镜像 release，守卫态为 captured。
	 */
	@PostMapping("/api/finance/reservations/{engagementRef}/reverse")
	public Mono<ResponseEntity<Map<String, Object>>> reverse(@PathVariable String engagementRef,
			ServerHttpRequest request) {
		return callers.requireService(request, FinanceCallerResolver.TRUST_SERVICE).flatMap(caller -> reservations
				.findByEngagementRef(engagementRef).switchIfEmpty(fail(404, "预留不存在")).flatMap(r -> {
					if (!r.organizationId().equals(caller.organizationId())) {
						return fail(403, "无权操作该组织预留");
					}
	...
	public Mono<ResponseEntity<Map<String, Object>>> reconcile(@PathVariable String engagementRef,
			@RequestBody ReconciliationRequest body, ServerHttpRequest request) {
		return callers.requireServiceForOrg(request, body.organizationId(), FinanceCallerResolver.MARKETPLACE_SERVICE)
				.flatMap(caller -> lifecycle.reconcile(body.organizationId(), engagementRef, body.finalDecision()))
				.map(result -> {
	...
	/**
	 * {@code payeeAccountId} 为收款推荐官的用户账号（非 finance ledger account）， 供 identity
	 * 通知中心解析收件人（Slice 12 Stage 3）。
	 */
	private EventEnvelope reservationEnvelope(String eventType, FundsReservation r) {
		...
		return new EventEnvelope(UUID.randomUUID().toString(), eventType, "FundsReservation", r.id(), 1, Instant.now(),
				null, payload);
	}
	...
	private EventEnvelope accountEnvelope(String eventType, Account acct, long amountCents, String payeeAccountId) {
		...
		return new EventEnvelope(UUID.randomUUID().toString(), eventType, "Account", acct.id(), 1, Instant.now(), null,
				payload);
	}
	...
	private static Mono<Reserved> existingReservation(String orgId, String engagementRef, long amountCents,
			String payeeAccountId, int commissionBonusBps, long commissionBonusCents, FundsReservation reservation) {
		boolean sameScope = orgId.equals(reservation.organizationId())
				&& engagementRef.equals(reservation.engagementRef()) && amountCents == reservation.amountCents()
		...
		return sameScope ? Mono.just(new Reserved(reservation, false)) : fail(422, "engagementRef 预留范围冲突");
	}
