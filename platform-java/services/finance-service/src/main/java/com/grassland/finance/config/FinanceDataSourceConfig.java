 ...
 * historyTable=finance_flyway_schema；历史表 finance_flyway_schema。
 */
@Configuration
...
public class FinanceDataSourceConfig {
	...
	DataSource dataSource(Environment env) {
		return FlywayBootstrap.dataSource(env.getProperty("DATABASE_URL"), "finance-service");
	}

	/** initMethod=migrate：bean 初始化即迁移；baseline 兼容非空 legacy 库。 */
	@Bean(initMethod = "migrate")
	Flyway flyway(DataSource dataSource, Environment env) {
		return FlywayBootstrap.flyway(dataSource, "finance_flyway_schema", "classpath:db/migration", false);
	}
}
