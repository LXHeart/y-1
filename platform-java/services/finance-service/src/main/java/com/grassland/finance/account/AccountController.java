 ...
 *
 * <ul>
 * <li>POST /api/finance/accounts — 开户（requireMerchant；org 用
 * caller.organizationId，须非空； 幂等：已存在返回既有；首次创建写 outbox
 * {@code AccountProvisioned}；201 首次 / 200 既有）。</li>
 * <li>GET /api/finance/accounts/{orgId} — 查余额（requireMerchant +
 * caller.organizationId==orgId 资源自查； 不存在 404）。</li>
 * </ul>
 *
 * <p>
 * 身份靠 {@link FinanceCallerResolver}（BFF 断言）；org 归属用 caller.organizationId
 * 自查（HLD 7.4）。 本 slice 无充值路径，余额恒 0；错误统一由全局 {@code FinanceErrorHandler} 处理。
 */
@RestController
public class AccountController {
	...
	public Mono<ResponseEntity<Map<String, Object>>> provision(ServerHttpRequest request) {
		return callers.requireMerchant(request).filter(caller -> caller.organizationId() != null)
				.switchIfEmpty(fail(403, "无组织归属，无法开户"))
				.flatMap(caller -> transactions
						.transactional(provision(caller.organizationId()).flatMap(p -> (p.created()
	...
	public Mono<ResponseEntity<Map<String, Object>>> balance(@PathVariable String orgId, ServerHttpRequest request) {
		return callers.requireMerchant(request).flatMap(caller -> {
			if (!orgId.equals(caller.organizationId())) {
				return Mono.<Account>error(new FinanceException(403, "无权查看该组织账户"));
			}
			return accounts.findByOrganization(orgId).switchIfEmpty(Mono.error(new FinanceException(404, "账户不存在")));
	...
	private Mono<Provisioned> provision(String orgId) {
		return accounts.findByOrganization(orgId).<Provisioned>map(a -> new Provisioned(a, false))
				.switchIfEmpty(accounts.create(orgId).<Provisioned>map(a -> new Provisioned(a, true)).switchIfEmpty(
						accounts.findByOrganization(orgId).<Provisioned>map(a -> new Provisioned(a, false))));
	}
	...
	private EventEnvelope envelope(String eventType, Account account) {
		...
		return new EventEnvelope(UUID.randomUUID().toString(), eventType, "Account", account.id(), 1, Instant.now(),
				null, payload);
	}
