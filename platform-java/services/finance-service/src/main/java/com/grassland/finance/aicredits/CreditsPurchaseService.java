 ...
 * 订单落库（operationId 幂等）→ 账本过账（含外部腿记录，sandbox 即时）→ provider operation （对账事实）→
 * 积分入账（type='purchase'，键 {@code purchase:<orderId>}）→ markPaid → outbox。
 *
 * <p>
 * sandbox 通道下「支付」即账本 {@code EXTERNAL} 腿的记录（无独立外部调用）； 真实 PSP 接入时在 adapter
 * 层替换，本编排不变。生产 overlay 强制非 sandbox 且真实 adapter 缺失时启动
 * fail-fast——购买入口在生产天然关闭，符合「等真实支付再接」决策。
 */
@Service
public class CreditsPurchaseService {
	...
	public CreditsPurchaseService(CreditsPackageRepository packages, CreditsPurchaseOrderRepository orders,
			LedgerService ledger, PaymentProviderAdapter provider, ProviderOperationRepository providerOperations,
			CreditsService credits, OutboxRepository outbox, TransactionalOperator transactions) {
	...
	public Mono<PurchaseOutcome> purchase(String accountId, String packageId, String operationId) {
		String opId = operationId == null || operationId.isBlank() ? UUID.randomUUID().toString() : operationId;
		Mono<PurchaseOutcome> work = packages.findById(packageId)
				.switchIfEmpty(Mono.error(new FinanceException(404, "积分包不存在"))).map(pkg -> {
				...
				.flatMap(pkg -> orders
						.insert(accountId, pkg.id(), pkg.versionId(), pkg.priceCents(), pkg.creditsAmount(),
								provider.channel(), provider.channel() + ":ai-credit:" + UUID.randomUUID(), opId)
	...
	private Mono<PurchaseOutcome> settle(String accountId, PackageView pkg, PurchaseOrder order) {
		return ledger.postAiCreditPurchase(order.id(), order.priceCents())
				.then(providerOperations.register(order.provider(), opKey(order.operationId()), "payment", order.id(),
						order.priceCents(), "CNY", order.providerRef()))
				.then(credits.purchaseCredit(accountId, order.creditsAmount(),
						"购买积分包 " + pkg.name() + "（v" + pkg.version() + "）", "purchase:" + order.id()))
				.then(orders.markPaid(order.id())).then(outbox.append(event(order, accountId)))
				.then(credits.balance(accountId).map(
						account -> new PurchaseOutcome(order.id(), "paid", order.creditsAmount(), account.balance())));
	}
	...
	private Mono<PurchaseOutcome> replayExisting(String operationId) {
		return orders.findByOperationId(operationId)
				.map(existing -> new PurchaseOutcome(existing.id(), existing.status(), existing.creditsAmount(), null));
	}
	...
	private static EventEnvelope event(PurchaseOrder order, String accountId) {
		return new EventEnvelope(UUID.randomUUID().toString(), "AiCreditsPurchased", "CreditsPurchaseOrder", order.id(),
				1, Instant.now(), order.id(), Map.of("orderId", order.id(), "accountId", accountId, "packageId",
						order.packageId(), "priceCents", order.priceCents(), "creditsAmount", order.creditsAmount()));
	}
