...
public class ConsumerPaymentService {
	...
	public ConsumerPaymentService(ConsumerPaymentRepository payments, AccountRepository accounts,
			WalletRepository wallets, LedgerService ledger, PaymentProviderAdapter provider,
	...
	public Mono<ConsumerPaymentRepository.Payment> pay(PaymentCommand command) {
		...
		Mono<ConsumerPaymentRepository.Payment> work = payments
				.insertPayment(command.orderRef(), command.consumerAccountId(), command.organizationId(),
						command.amountCents(), provider.channel(), providerRef, command.operationId())
				.flatMap(payment -> ledger
						.postConsumerPayment(payment.organizationId(), payment.orderRef(), payment.amountCents())
						.then(providerOperations.register(payment.channel(), payment.operationId(), "payment",
								payment.orderRef(), payment.amountCents(), payment.currency(), payment.providerRef()))
						.then(outbox.append(event("ConsumerPaymentSucceeded", payment.orderRef(),
								Map.of("orderRef", payment.orderRef(), "consumerAccountId", payment.consumerAccountId(),
										"organizationId", payment.organizationId(), "amountCents",
										payment.amountCents(), "providerRef", payment.providerRef()))))
						.thenReturn(payment))
		...
		return payments.findPayment(orderRef).switchIfEmpty(Mono.error(new FinanceException(404, "支付不存在")))
				.flatMap(payment -> {
					...
					return payments.findSplit(orderRef).flatMap(split -> {
						...
						Mono<ConsumerPaymentRepository.Refund> work = payments
								...
								.flatMap(reserved -> payments
										...
										.flatMap(refund -> ledger
												...
												.then(providerOperations.register(reserved.channel(),
														refund.operationId(), "refund", orderRef, refund.amountCents(),
														reserved.currency(), refund.providerRef()))
	...
	private Mono<ConsumerPaymentRepository.Refund> refundAfterSplit(ConsumerPaymentRepository.Payment payment,
			ConsumerPaymentRepository.Split split, String orderRef, RefundCommand command) {
		String providerRef = provider.channel() + ":refund:" + command.operationId();
		Mono<ConsumerPaymentRepository.Refund> work = payments
				.reserveRefund(orderRef, command.amountCents(), command.operationId())
				.switchIfEmpty(Mono.error(new FinanceException(409, "退款金额超过可退余额或支付状态不可退款")))
				.flatMap(reserved -> payments.findSplitAllocations(orderRef).collectList().flatMap(rows -> {
					List<ConsumerPaymentRepository.SplitAllocation> source = rows.isEmpty()
							? (split.recommenderAmountCents() == 0
									? List.of()
									: List.of(new ConsumerPaymentRepository.SplitAllocation(
					...
					return walletsDebit.then(merchantDebit)
							.then(ledger.postConsumerSplitRefund(payment.organizationId(), orderRef, amount,
									refunds.stream()
											.map(r -> new LedgerService.ConsumerSplitAllocation(
													r.recommenderAccountId(), r.amountCents()))
											.toList(),
									merchantRefund, platformRefund, "consumer-refund:" + command.operationId()))
							.then(payments.insertRefund(orderRef, amount, command.reason(), command.operationId(),
									providerRef))
							.flatMap(refund -> providerOperations
									.register(payment.channel(), refund.operationId(), "refund", orderRef,
											refund.amountCents(), payment.currency(), refund.providerRef())
									.then(outbox.append(event("ConsumerPaymentRefunded", orderRef,
											Map.of("orderRef", orderRef, "organizationId", payment.organizationId(),
													"consumerAccountId", payment.consumerAccountId(), "amountCents",
													refund.amountCents(), "refundedAmountCents",
													reserved.refundedAmountCents(), "paymentStatus", reserved.status(),
				...
				.flatMap(payment -> {
					...
					List<ConsumerPaymentRepository.SplitAllocation> allocations = command.allocations() == null
							...
							: command.allocations().stream()
									.map(a -> new ConsumerPaymentRepository.SplitAllocation(a.recommenderAccountId(),
											a.amountCents()))
									.toList();
					Mono<ConsumerPaymentRepository.Split> work = payments
							.insertSplit(orderRef, command.recommenderAccountId(), command.recommenderAmountCents(),
	...
	private Mono<Void> applySplitProjections(ConsumerPaymentRepository.Payment payment,
			ConsumerPaymentRepository.Split split, List<ConsumerPaymentRepository.SplitAllocation> allocations) {
		...
		Mono<Void> recommender = allocations.isEmpty() && split.recommenderAmountCents() == 0
				...
				: allocations.isEmpty()
						? wallets.credit(split.recommenderAccountId(), split.recommenderAmountCents())
								.then(wallets.appendEntry(split.recommenderAccountId(),
										WalletEntryType.COMMERCE_COMMISSION, split.recommenderAmountCents(),
										split.platformFeeCents(), orderRef(payment), "消费订单核销佣金"))
								.then()
						: reactor.core.publisher.Flux.fromIterable(allocations)
								.flatMap(a -> wallets.credit(a.recommenderAccountId(), a.amountCents())
										.then(wallets.appendEntry(a.recommenderAccountId(),
												WalletEntryType.COMMERCE_COMMISSION, a.amountCents(), 0,
												orderRef(payment), "消费订单多推荐官佣金")))
								.then();
	...
	private static EventEnvelope splitEvent(ConsumerPaymentRepository.Payment payment,
			ConsumerPaymentRepository.Split split) {
	...
	private static EventEnvelope event(String type, String orderRef, Map<String, Object> payload) {
		return new EventEnvelope(UUID.randomUUID().toString(), type, "ConsumerPayment", orderRef, 1, Instant.now(),
				orderRef, payload);
	}

	private static void validatePayment(PaymentCommand command) {
		if (blank(command.orderRef()) || blank(command.consumerAccountId()) || blank(command.organizationId())
				|| command.amountCents() <= 0 || blank(command.operationId())) {
	...
	private static void requirePaymentMatch(ConsumerPaymentRepository.Payment existing, PaymentCommand command) {
	...
	public record PaymentCommand(String orderRef, String consumerAccountId, String organizationId, long amountCents,
			String operationId) {
	}
