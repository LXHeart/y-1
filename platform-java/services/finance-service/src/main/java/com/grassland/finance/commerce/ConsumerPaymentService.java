...
public class ConsumerPaymentService {
	...
	private Mono<ConsumerPaymentRepository.Refund> verifyRefundReplay(ConsumerPaymentRepository.Refund existing,
			String orderRef, RefundCommand command) {
		if (!existing.orderRef().equals(orderRef) || existing.amountCents() != command.amountCents()) {
			return Mono.error(new FinanceException(409, "退款幂等参数冲突"));
		}
	...
	public Mono<ConsumerPaymentRepository.Split> split(String orderRef, SplitCommand command) {
		...
		return transactions.transactional(payments.lockPayment(orderRef)
				.switchIfEmpty(Mono.error(new FinanceException(404, "支付不存在"))).flatMap(payment -> {
					...
					// 已在父行锁事务内（外层 transactional），内层不再重复包裹。
					return work;
				}));
	}
