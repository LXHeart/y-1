...
public class EscrowLifecycleService {
	...
	public EscrowLifecycleService(ReservationRepository reservations, AccountRepository accounts,
			WalletRepository wallets, OutboxRepository outbox, PlatformFeePolicy fees,
			TransactionalOperator transactions, LedgerService ledger, FreebieEscrowLifecycleService freebies) {
	...
	/**
	 * Capture the selected gross tier and return any reserved remainder to the
	 * merchant.
	 */
	...
	public Mono<ReconciliationResult> reconcile(String organizationId, String engagementRef, String finalDecision) {
		return reservations.findByEngagementRef(engagementRef).flatMap(reservation -> {
	...
	private Mono<ReconciliationResult> reconcileForMerchant(FundsReservation reservation) {
		return switch (reservation.status()) {
			case "reserved" -> release(reservation).map(updated -> repaired("released", updated));
			case "captured" -> reverse(reservation).map(updated -> repaired("refunded", updated))
					.onErrorResume(FinanceException.class, error -> error.status() == 409
							? Mono.just(new ReconciliationResult("blocked", "manual_clawback_required", reservation))
							: Mono.error(error));
			case "released", "refunded" ->
				Mono.just(new ReconciliationResult("verified", "already_refunded", reservation));
			default -> Mono.just(new ReconciliationResult("conflict", "unexpected_reservation_state", reservation));
		};
	}

	private Mono<ReconciliationResult> reconcileForRecommender(FundsReservation reservation) {
		return switch (reservation.status()) {
			case "reserved" -> capture(reservation).map(updated -> repaired("captured", updated));
			case "captured" -> Mono.just(new ReconciliationResult("verified", "already_captured", reservation));
			case "released", "refunded" ->
				Mono.just(new ReconciliationResult("conflict", "released_but_recommender_won", reservation));
			default -> Mono.just(new ReconciliationResult("conflict", "unexpected_reservation_state", reservation));
		};
	}

	private Mono<FundsReservation> releaseWork(FundsReservation reservation) {
		return reservations.release(reservation.id()).switchIfEmpty(Mono.error(new FinanceException(409, "该预留已处理")))
				.flatMap(released -> accounts.credit(reservation.organizationId(), reservation.amountCents())
						.then(ledger.postRelease(reservation.organizationId(), reservation.engagementRef(),
								reservation.amountCents()))
						.then(outbox.append(reservationEnvelope("FundsReleased", released))).thenReturn(released));
	}
	...
	private Mono<FundsReservation> captureWork(FundsReservation reservation, Long requestedSettlementAmountCents) {
		long settlementAmount = requestedSettlementAmountCents == null
				? reservation.amountCents()
				: requestedSettlementAmountCents;
		...
		long settlementBonus = reservation.payeeAccountId() == null
				? 0
				: CommissionBonusPolicy.calculateCents(settlementAmount, reservation.commissionBonusBps());
		Long payout = reservation.payeeAccountId() == null
				? null
				: Math.addExact(fees.payoutFor(settlementAmount), settlementBonus);
		return reservations.capture(reservation.id(), payout, settlementAmount, settlementBonus)
				.switchIfEmpty(Mono.error(new FinanceException(409, "该预留已处理"))).flatMap(this::splitToPayee)
				.flatMap(captured -> ledger
						.postCapture(captured.organizationId(), captured.engagementRef(), settledGross(captured),
								captured.payeeAccountId(), captured.payoutCents(),
								captured.effectiveSettlementCommissionBonusCents())
						.then(releaseRemainder(captured))
						.then(outbox.append(reservationEnvelope("FundsCaptured", captured))).thenReturn(captured));
	}

	private Mono<FundsReservation> reverseWork(FundsReservation reservation) {
		return clawbackFromPayee(reservation).then(reservations.reverse(reservation.id()))
				.switchIfEmpty(Mono.error(new FinanceException(409, "该预留不可冲正（须 captured）")))
				.flatMap(refunded -> accounts.credit(reservation.organizationId(), settledGross(reservation))
						.then(ledger.postReverse(reservation.organizationId(), reservation.engagementRef(),
						...
						.then(outbox.append(reservationEnvelope("FundsReversed", refunded))).thenReturn(refunded));
	}
	...
	private Mono<FundsReservation> splitToPayee(FundsReservation captured) {
		...
		return wallets.credit(captured.payeeAccountId(), payout)
				.then(wallets.appendEntry(captured.payeeAccountId(), WalletEntryType.TASK_PAYOUT, payout, fee,
						captured.effectiveSettlementCommissionBonusCents(), captured.engagementRef(), "任务结算入账"))
				.then(outbox.append(new EventEnvelope(UUID.randomUUID().toString(), "SplitCompleted",
						"FundsReservation", captured.id(), 1, Instant.now(), null,
						Map.of("engagementRef", String.valueOf(captured.engagementRef()), "payeeAccountId",
								captured.payeeAccountId(), "grossCents", settledGross(captured), "reservedGrossCents",
								captured.amountCents(), "basePayoutCents", basePayout, "payoutCents", payout,
								"platformFeeCents", fee, "commissionBonusBps", captured.commissionBonusBps(),
	...
	private Mono<Void> releaseRemainder(FundsReservation captured) {
		...
		return accounts.credit(captured.organizationId(), remainder)
				...
				.then(outbox.append(reservationEnvelope("FundsPartiallyReleased", captured))).then();
	}
	...
	private Mono<Void> clawbackFromPayee(FundsReservation reservation) {
		...
		return wallets.debit(reservation.payeeAccountId(), payout)
				.switchIfEmpty(Mono.error(new FinanceException(409, "推荐官余额不足以冲正（可能已提现），需人工处理")))
				.flatMap(wallet -> wallets.appendEntry(reservation.payeeAccountId(), WalletEntryType.CLAWBACK, -payout,
						0, reservation.effectiveSettlementCommissionBonusCents(), reservation.engagementRef(),
						"争议冲正扣回"))
				.then();
	}

	/**
	 * {@code payeeAccountId} 是收款推荐官的用户账号（非 finance ledger account）， 供 identity
	 * 通知中心解析钱包类通知收件人（Slice 12 Stage 3）；无分账对象时不携带。
	 */
	private EventEnvelope reservationEnvelope(String eventType, FundsReservation reservation) {
		...
		return new EventEnvelope(UUID.randomUUID().toString(), eventType, "FundsReservation", reservation.id(), 1,
	...
	public record ReconciliationResult(String outcome, String reason, FundsReservation reservation) {
	}
}
