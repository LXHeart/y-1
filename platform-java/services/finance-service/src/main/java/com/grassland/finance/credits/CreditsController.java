 ...
 *
 * <p>
 * <b>公共读（经 edge-bff，用户断言）</b>：{@code GET /api/credits/{balance,history}}。
 * accountId 取自 {@link FinanceCallerResolver}（edge-bff 签发的
 * {@code X-Grassland-Identity}），不接受路径/请求体传入， 故无越权维度。返回统一信封
 * {@code {success:true,data:…}}（任务书 #87 起 legacy 裸响应退役）： balance 为
 * {@code data:{balance,totalEarned,totalSpent}} 三字段对象、history 为
 * {@code data:{history:[…]}}， 与购买侧 {@code /api/credits/packages} 等同域端点单协议。
 *
 * <p>
 * <b>内部命令（容器直连、服务断言）</b>：{@code /internal/credits/**}。 每个端点按已验签的服务 principal
 * 授权：identity 可读取批量余额并人工调账， intelligence 可扣减、补偿及执行受限 AI 退款。通用共享密钥不授予任何权限。 响应保持
 * legacy 信封 {@code {success:true,
 * data:{…,deduplicated}}}，402→{@code {success:false,error}}（经全局 advice）。
 * {@code operation_id} 原样透传给 {@link CreditsService}（调用方自行派生
 * {@code refund:<consumeId>}）。
 */
@RestController
public class CreditsController {
	...
	private static final Set<String> AI_QUOTA_FEATURES = Set.of("video_analysis", "image_analysis",
			"article_generation", "comedy_generation", "video_production_script", "video_production_video",
			"ai_run_text", "ai_run_voice", "ai_run_embedding", "intelligence_smoke", "creation_assistant",
			"moments_generation", "video_studio_bgm", "card_series_plan");
	...
	public Mono<Map<String, Object>> balance(ServerHttpRequest request) {
		return callers.resolve(request).flatMap(caller -> credits.balance(caller.accountId()))
	...
	public Mono<Map<String, Object>> consume(ServerHttpRequest request, @RequestBody ConsumeRequest body) {
		return callers.requireService(request, FinanceCallerResolver.INTELLIGENCE_SERVICE)
				.then(credits.consume(body.accountId(), body.feature(), body.operationId(), body.aiQuotaMultiplierBps(),
						body.policyVersion()))
				.map(result -> success(mutationBody("consumed", result)));
	}

	@PostMapping("/internal/credits/refund")
	public Mono<Map<String, Object>> refund(ServerHttpRequest request, @RequestBody RefundRequest body) {
		...
		return callers.resolve(request)
				...
				.then(credits.refund(body.accountId(), amount, body.feature(), body.note(), body.operationId()))
				.map(result -> success(mutationBody("refunded", result)));
	}

	@PostMapping("/internal/credits/consume-compensations")
	public Mono<Map<String, Object>> compensateConsume(ServerHttpRequest request,
			@RequestBody ConsumeCompensationRequest body) {
		return callers
				.requireService(request, FinanceCallerResolver.INTELLIGENCE_SERVICE).then(credits
						.compensateConsume(body.accountId(), body.feature(), body.consumeOperationId(), body.note()))
				.map(CreditsController::compensationBody);
	}

	@PostMapping("/internal/credits/usage-reservations")
	public Mono<Map<String, Object>> reserveUsage(ServerHttpRequest request,
			@RequestBody UsageReservationRequest body) {
		return callers.requireService(request, FinanceCallerResolver.INTELLIGENCE_SERVICE)
				.then(credits.reserveUsage(body.accountId(), body.feature(), body.operationId(), body.estimatedCents(),
	...
	public Mono<Map<String, Object>> settleUsage(ServerHttpRequest request, @RequestBody UsageSettlementRequest body) {
		return callers.requireService(request, FinanceCallerResolver.INTELLIGENCE_SERVICE)
				.then(credits.settleUsage(body.accountId(), body.feature(), body.consumeOperationId(),
	...
	public Mono<Map<String, Object>> award(ServerHttpRequest request, @RequestBody AwardRequest body) {
	 ...
	 *
	 * <p>
	 * 仅接受 identity-service 签发、受众为 finance 的服务断言。 未建户的 accountId 不在 {@code accounts}
	 * 数组里（调用方按缺失 = 0 余额处理）。
	 */
	@PostMapping("/internal/credits/balances")
	public Mono<Map<String, Object>> internalBalances(ServerHttpRequest request, @RequestBody BalancesRequest body) {
		return callers.requireService(request, FinanceCallerResolver.IDENTITY_SERVICE)
				.thenMany(credits.balances(body.accountIds())).collectList().map(accounts -> success(
						Map.of("accounts", accounts.stream().map(CreditsController::balanceEntry).toList())));
	}

	/**
	 * Finance-authoritative, read-only consume fences for Intelligence
	 * reconciliation.
	 */
	@PostMapping("/internal/credits/consume-operations/query")
	public Mono<Map<String, Object>> consumeOperations(ServerHttpRequest request,
			@RequestBody ConsumeOperationsRequest body) {
		return callers.requireService(request, FinanceCallerResolver.INTELLIGENCE_SERVICE)
				.thenMany(credits.consumeOperations(body.operationIds())).map(CreditsController::consumeOperationItem)
				.collectList().map(items -> success(Map.of("operations", items)));
	}
	...
	private static Map<String, Object> consumeOperationItem(CreditsRepository.ConsumeOperation operation) {
		Map<String, Object> item = new LinkedHashMap<>();
		...
		item.put("consumeTransactionId",
				"quota".equals(operation.chargeSource())
						? operation.quotaConsumeTransactionId()
						: operation.consumeTransactionId());
		item.put("refundTransactionId",
				"quota".equals(operation.chargeSource())
	...
	public record ConsumeRequest(String accountId, String feature, String operationId, Integer aiQuotaMultiplierBps,
			Long policyVersion) {
	...
	/**
	 * Consume compensation derives the refund key server-side and never accepts an
	 * amount.
	 */
	public record ConsumeCompensationRequest(String accountId, String feature, String consumeOperationId, String note) {
	...
	public record UsageReservationRequest(String accountId, String feature, String operationId, Long estimatedCents,
			String creditsCentsPolicyVersion, Integer aiQuotaMultiplierBps, Long policyVersion) {
	...
	public record UsageSettlementRequest(String accountId, String feature, String consumeOperationId, Long actualCents,
	...
	private static void requireUsageScope(String accountId, String feature, String operationId,
		...
		requireMaxLength(operationId, MAX_OPERATION_ID_LENGTH, "operationId 过长");
		if (moneyPolicyVersion == null || moneyPolicyVersion.isBlank() || moneyPolicyVersion.length() > 64
	...
	private static boolean isAuthorizedIntelligenceRefund(FinanceCallerResolver.Caller caller, RefundRequest body,
			int amount) {
		return caller.isServicePrincipal(FinanceCallerResolver.INTELLIGENCE_SERVICE) && amount == 1
				&& AI_QUOTA_FEATURES.contains(body.feature()) && body.operationId() != null
				&& body.operationId().startsWith("refund:") && body.operationId().length() > "refund:".length();
	}
