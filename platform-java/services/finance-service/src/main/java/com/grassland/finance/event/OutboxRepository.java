...
public class OutboxRepository {
    ...
    public Flux<OutboxRow> claimBatch(int limit, UUID claimToken, Duration leaseDuration) {
        ...
        return db.sql("""
                WITH candidates AS (
                    SELECT candidate.id
                    FROM finance_outbox AS candidate
                    WHERE published_at IS NULL
                      AND NOT EXISTS (
                          SELECT 1 FROM finance_outbox AS earlier
                ...
                UPDATE finance_outbox AS target
    ...
    public Mono<Long> pendingCount() {
        return db.sql("SELECT COUNT(*)::bigint AS pending_count FROM finance_outbox WHERE published_at IS NULL")
