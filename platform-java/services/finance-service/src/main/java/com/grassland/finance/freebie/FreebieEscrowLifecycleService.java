...
/**
 * 霸王餐押金托管生命周期（ADR-D12，方向镜像 {@code EscrowLifecycleService}）：
 * 出资方是<b>推荐官</b>（不是商家 org）——reserve 扣推荐官钱包；达标 refund 全额退推荐官（fee=0，
 * §8.2「全额返还」）；未达标 compensate 入商家 org 账户。双录账本 + 钱包流水 + outbox 同事务，
 * journal 独立类型（FREEBIE_*），不复用商家出资语义。
 */
@Component
public class FreebieEscrowLifecycleService {
    ...
    public FreebieEscrowLifecycleService(
            FreebieEscrowRepository escrows,
    ...
    public Mono<Reserved> reserve(String engagementRef, String recommenderAccountId,
        ...
        return transactions.transactional(
                escrows.findByEngagementRef(engagementRef)
                        .flatMap(existing -> existingScopeMatches(
                                existing, engagementRef, recommenderAccountId, organizationId, amountCents))
                        .switchIfEmpty(Mono.defer(() -> reserveNew(
                                engagementRef, recommenderAccountId, taskOwnerAccountId,
                                organizationId, amountCents)))
                        .flatMap(res -> res.created()
                                ? wallets.appendEntry(
                                        recommenderAccountId, WalletEntryType.FREEBIE_RESERVE, -amountCents,
                                        0, engagementRef, "霸王餐押金预付托管")
                                        .then(ledger.postFreebieReserve(
                                                recommenderAccountId, engagementRef, amountCents))
                                        .then(outbox.append(envelope("FreebieReserved", res.escrow())))
                                        .thenReturn(res)
                                : Mono.just(res)));
    }

    /** 顺序镜像 reserveWork：先扣钱、再建行；并发唯一键落败时在本事务内 credit 撤销刚做的扣减， 流水/journal/outbox 只在 created 分支落——空结果提交不会留下「扣了钱没有托管行」的脏状态。 */
    private Mono<Reserved> reserveNew(String engagementRef, String recommenderAccountId,
                                      String taskOwnerAccountId, String organizationId, long amountCents) {
        return wallets.debit(recommenderAccountId, amountCents)
                .switchIfEmpty(Mono.error(new FinanceException(409, "钱包余额不足")))
                .then(escrows.create(engagementRef, recommenderAccountId, taskOwnerAccountId,
                        organizationId, amountCents))
    ...
    private static Mono<Reserved> existingScopeMatches(
            FreebieEscrow existing, String engagementRef, String recommenderAccountId,
        ...
        boolean sameScope = engagementRef.equals(existing.engagementRef())
                ...
                && organizationId.equals(existing.organizationId())
        ...
        return transactions.transactional(
                escrows.findByEngagementRef(engagementRef)
                        ...
                        .flatMap(escrow -> escrows.markRefunded(escrow.id())
                                ...
                                .flatMap(refunded -> wallets.credit(
                                                escrow.recommenderAccountId(), escrow.amountCents())
                                        .then(wallets.appendEntry(
                                                escrow.recommenderAccountId(), WalletEntryType.FREEBIE_REFUND,
                                                escrow.amountCents(), 0, engagementRef, "霸王餐押金达标返还"))
                                        .then(ledger.postFreebieRefund(
                                                escrow.recommenderAccountId(), engagementRef, escrow.amountCents()))
                                        .then(outbox.append(envelope("FreebieRefunded", refunded)))
                                        .thenReturn(refunded))));
    }
        ...
        return transactions.transactional(
                escrows.findByEngagementRef(engagementRef)
                        ...
                        .flatMap(escrow -> escrows.markCompensated(escrow.id())
                                .switchIfEmpty(Mono.error(new FinanceException(409, "该押金已处理")))
                                .flatMap(compensated -> accounts.credit(
                                                escrow.organizationId(), escrow.amountCents())
                                        .then(ledger.postFreebieCompensate(
                                                escrow.recommenderAccountId(), escrow.organizationId(),
                                                engagementRef, escrow.amountCents()))
                                        .then(outbox.append(envelope("FreebieCompensated", compensated)))
                                        .thenReturn(compensated))));
    }

    /**
     * 争议终局对账（D6 矩阵）：for_merchant → 补偿商家；for_recommender → 退还推荐官。
     * 结局语义镜像 {@code EscrowLifecycleService.reconcile}（verified/repaired/conflict/missing）。
     */
    ...
    private Mono<Outcome> reconcileForMerchant(FreebieEscrow escrow) {
        return switch (escrow.status()) {
            case FreebieEscrow.STATUS_RESERVED -> compensate(escrow.engagementRef())
                    .map(updated -> new Outcome("repaired", "compensated", updated));
            case FreebieEscrow.STATUS_COMPENSATED -> Mono.just(new Outcome(
                    "verified", "already_compensated", escrow));
            case FreebieEscrow.STATUS_REFUNDED -> Mono.just(new Outcome(
                    "conflict", "refunded_but_merchant_won", escrow));
    ...
    private Mono<Outcome> reconcileForRecommender(FreebieEscrow escrow) {
        return switch (escrow.status()) {
            case FreebieEscrow.STATUS_RESERVED -> refund(escrow.engagementRef())
                    .map(updated -> new Outcome("repaired", "refunded", updated));
            case FreebieEscrow.STATUS_REFUNDED -> Mono.just(new Outcome(
                    "verified", "already_refunded", escrow));
            case FreebieEscrow.STATUS_COMPENSATED -> Mono.just(new Outcome(
                    "conflict", "compensated_but_recommender_won", escrow));
            default -> Mono.just(new Outcome("conflict", "unexpected_escrow_state", escrow));
        };
    }
    ...
    private EventEnvelope envelope(String eventType, FreebieEscrow escrow) {
        ...
        return new EventEnvelope(UUID.randomUUID().toString(), eventType, "FreebieEscrow",
                escrow.id(), 1, Instant.now(), null, payload);
    }

    /** 对账结局（outcome/reason 语义对齐既有 ReconciliationResult，供 marketplace 对账 activity 消费）。 */
