...
class OutboxRepositoryIT extends FinanceItSupport {
	...
	void claimBatch_claimsOnlyDueRows_andReclaimsOnlyExpiredLeases() {
		...
		db.sql("UPDATE finance_outbox SET claimed_until = now() - interval '1 second' WHERE event_id = 'due'").then()
				.block();
	...
	void conditionalTransitions_rejectStaleOwners_andTrackPendingAge() {
		...
		db.sql("UPDATE finance_outbox SET next_attempt_at = now() - interval '1 second' WHERE event_id = 'old'").then()
				.block();
	...
	private static EventEnvelope event(String eventId) {
		return new EventEnvelope(eventId, "FundsChanged", "Account", UUID.randomUUID().toString(), 3, Instant.now(),
