...
/**
 * 退出资金权威事实查询 IT（任务书 #103 C103-03 / §6.2）：只读聚合 reservation/freebie 已提交事实；
 * 仅 marketplace principal 可读；不触发任何资金变动；组织不符按 404（不泄露存在性）。
 */
class EngagementFundsQueryIT extends FinanceItSupport {
	...
	void exitFactsReportCommittedReservationAndFreebieStates() {
		...
		// 赏金预留 500 + 押金托管 100（freebie 走 /internal/freebie/reserve，marketplace principal）。
		...
		client().post().uri("/internal/freebie/reserve")
				.header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("engagementRef", ref, "recommenderAccountId", rec, "taskOwnerAccountId", merchant,
						"organizationId", org, "amountCents", 100)).exchange().expectStatus().isCreated();

		client().get().uri("/internal/engagements/" + ref + "/exit-facts?organizationId=" + org)
				.header("X-Grassland-Identity", signService(org, "marketplace")).exchange()
				.expectStatus().isOk().expectBody()
				.jsonPath("$.data.bounty.exists").isEqualTo(true)
				.jsonPath("$.data.bounty.originalReservedCents").isEqualTo(500)
				.jsonPath("$.data.bounty.capturedCents").isEqualTo(0)
				.jsonPath("$.data.bounty.releasedCents").isEqualTo(0)
				.jsonPath("$.data.deposit.exists").isEqualTo(true)
				.jsonPath("$.data.deposit.amountCents").isEqualTo(100)
	...
	void exitFactsReflectCapturedAndRefundedLegs() {
		...
		client().post().uri("/api/finance/reservations/" + ref + "/capture")
				.header("X-Grassland-Identity", signService(org, "marketplace"))
				.contentType(MediaType.APPLICATION_JSON).bodyValue(Map.of("settlementAmountCents", 300)).exchange()
				.expectStatus().isOk();
		fundRecommenderWallet(org, rec, "fund-" + UUID.randomUUID(), 300);
		client().post().uri("/internal/freebie/reserve")
				.header("X-Grassland-Identity", signService(org, "marketplace"))
		...
		client().get().uri("/internal/engagements/" + ref + "/exit-facts?organizationId=" + org)
				.header("X-Grassland-Identity", signService(org, "marketplace")).exchange()
				.expectStatus().isOk().expectBody()
				.jsonPath("$.data.bounty.capturedCents").isEqualTo(300)
				.jsonPath("$.data.bounty.releasedCents").isEqualTo(0)
				.jsonPath("$.data.deposit.refundedCents").isEqualTo(100)
				.jsonPath("$.data.deposit.compensatedCents").isEqualTo(0);
	}
	...
	void exitFactsRequireMarketplacePrincipalAndHonorOrgScope() {
		...
		client().get().uri("/internal/engagements/" + ref + "/exit-facts?organizationId=" + org)
				.header("X-Grassland-Identity", signService(org, "trust")).exchange()
