 ...
 * <ul>
 * <li>不变式 Σ flows == 该 org 该月 ESCROW 腿净额 == netEscrowDeltaCents（对 posting
 * 逐行复核）；</li>
...
class MerchantBillControllerIT extends FinanceItSupport {
	...
	void monthlyBillDecomposesFlowsAndFee() {
		...
		seedJournal("CAPTURE", org, m, post("RESERVE", "DEBIT", 6000), post("WALLET", "CREDIT", 5400),
				post("FEE", "CREDIT", 600));
		// 释放 +1000（ESCROW 增）
		seedJournal("RELEASE", org, m, post("RESERVE", "DEBIT", 1000), post("ESCROW", "CREDIT", 1000));

		WebTestClient.ResponseSpec resp = client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-08").build())
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "finance_transaction")).exchange()
				.expectStatus().isOk();

		resp.expectBody().jsonPath("$.data.month").isEqualTo("2026-08")
				...
				.jsonPath("$.data.platformFeeCents").isEqualTo(600).jsonPath("$.data.netEscrowDeltaCents")
				.isEqualTo(5000);
	...
	void monthlyBillExportKeepsOrgScopeAndSupportsCsvAndXlsx() {
		String org = UUID.randomUUID().toString();
		String merchant = UUID.randomUUID().toString();
		seedJournal("DEPOSIT", org, beijing("2026-08-10T10:00:00"), post("EXTERNAL", "DEBIT", 500),
				post("ESCROW", "CREDIT", 500));
		String assertion = sign(merchant, "merchant", org, "finance_transaction");

		byte[] csv = client().get()
				.uri("/api/finance/organizations/" + org + "/monthly-bill/export?month=2026-08&format=csv")
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isOk().expectBody(byte[].class)
				.returnResult().getResponseBody();
		assertThat(new String(csv, StandardCharsets.UTF_8)).contains("DEPOSIT", "500");

		byte[] xlsx = client().get()
				.uri("/api/finance/organizations/" + org + "/monthly-bill/export?month=2026-08&format=xlsx")
				.header("X-Grassland-Identity", assertion).exchange().expectStatus().isOk().expectBody(byte[].class)
				.returnResult().getResponseBody();
		assertThat(xlsx).startsWith((byte) 'P', (byte) 'K');

		client().get().uri("/api/finance/organizations/" + org + "/monthly-bill/export?month=2026-08&format=csv")
				.header("X-Grassland-Identity",
						sign(merchant, "merchant", UUID.randomUUID().toString(), "finance_transaction"))
	...
	void reverseRefundsMerchantAndClawsBackFee() {
		...
		seedJournal("CAPTURE", org, m, post("RESERVE", "DEBIT", 6000), post("WALLET", "CREDIT", 5400),
				post("FEE", "CREDIT", 600));
		seedJournal("REVERSE", org, m, post("WALLET", "DEBIT", 5400), post("FEE", "DEBIT", 600),
				post("ESCROW", "CREDIT", 6000));

		client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-08").build())
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "finance_transaction")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.flows[?(@.type=='REVERSE')].amountCents")
				.isEqualTo(List.of(6000))
				// 平台费净额：+600（capture）-600（reverse 回冲）= 0
				.jsonPath("$.data.platformFeeCents").isEqualTo(0).jsonPath("$.data.netEscrowDeltaCents")
				.isEqualTo(6000);
	}
	...
	void monthBoundaryUsesBeijingTime() {
		...
		seedJournal("DEPOSIT", org, Instant.parse("2026-08-31T23:30:00Z"), post("EXTERNAL", "DEBIT", 500),
				post("ESCROW", "CREDIT", 500));
		// UTC 8-31 15:30 = 北京 8-31 23:30 → 归 8 月
		seedJournal("DEPOSIT", org, Instant.parse("2026-08-31T15:30:00Z"), post("EXTERNAL", "DEBIT", 700),
				post("ESCROW", "CREDIT", 700));

		client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-08").build())
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "finance_transaction")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.flows[?(@.type=='DEPOSIT')].amountCents")
				.isEqualTo(List.of(700));
		client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-09").build())
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "finance_transaction")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.flows[?(@.type=='DEPOSIT')].amountCents")
				.isEqualTo(List.of(500));
	}
	...
	void emptyMonthReturnsZero() {
		...
		client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-08").build())
				.header("X-Grassland-Identity", sign(merchant, "merchant", org, "finance_transaction")).exchange()
				.expectStatus().isOk().expectBody().jsonPath("$.data.flows.length()").isEqualTo(0)
				.jsonPath("$.data.platformFeeCents").isEqualTo(0).jsonPath("$.data.netEscrowDeltaCents").isEqualTo(0);
	}
	...
	void billIsOrgScoped() {
		...
		String merchant = UUID.randomUUID().toString();
		seedJournal("DEPOSIT", org, beijing("2026-08-10T10:00:00"), post("EXTERNAL", "DEBIT", 100),
				post("ESCROW", "CREDIT", 100));

		// 别的 org 的商家看本 org 账单 → 404
		client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-08").build())
				.header("X-Grassland-Identity", sign(merchant, "merchant", otherOrg, "finance_transaction")).exchange()
				.expectStatus().isNotFound();

		// 无断言 → 401
		client().get().uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
				.queryParam("month", "2026-08").build()).exchange().expectStatus().isUnauthorized();

		// 推荐官身份（非商家）→ 403
		client().get()
				.uri(uri -> uri.path("/api/finance/organizations/" + org + "/monthly-bill")
						.queryParam("month", "2026-08").build())
				.header("X-Grassland-Identity", sign(merchant, "recommender", org, null)).exchange().expectStatus()
				.isForbidden();
	...
	private void seedJournal(String journalType, String org, Instant createdAt, Post... posts) {
		...
		transactionalOperator.execute(status -> {
			Mono<Void> chain = db.sql("""
					...
					.bind("ts", OffsetDateTime.ofInstant(createdAt, ZoneOffset.UTC)).then();
			for (Post p : posts) {
	...
	private long escrowLegSum(String org, String month) {
		...
		Long v = db.sql("""
				...
				""").bind("org", org).bind("f", OffsetDateTime.ofInstant(start, ZoneOffset.UTC))
				.bind("t", OffsetDateTime.ofInstant(end, ZoneOffset.UTC)).map(r -> r.get("s", Long.class)).one()
				.block();
		return v == null ? 0L : v;
	}
}
