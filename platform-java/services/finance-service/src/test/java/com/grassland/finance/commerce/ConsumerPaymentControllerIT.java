...
class ConsumerPaymentControllerIT extends FinanceItSupport {
	...
	void paymentAndSplitAreIdempotentAndCreditThreeDestinations() {
		...
		Map<String, Object> payment = Map.of("orderRef", order, "consumerAccountId", consumer, "organizationId", org,
		...
		for (int i = 0; i < 2; i++) {
			client().post().uri("/internal/commerce/payments")
					...
					.contentType(MediaType.APPLICATION_JSON).bodyValue(payment).exchange().expectStatus().isCreated()
					.expectBody().jsonPath("$.data.status").isEqualTo("succeeded");
		}
		assertThat(ledger.sumBalance(LedgerAccount.Type.CONSUMER_ESCROW, order).block()).isEqualTo(10_000L);

		Map<String, Object> split = Map.of("organizationId", org, "totalAmountCents", 10_000, "recommenderAccountId",
				recommender, "recommenderAmountCents", 1_000, "merchantAmountCents", 8_500, "platformFeeCents", 500,
	...
	void unredeemedRefundIsIdempotentAndCannotBeCalledByUserPrincipal() {
		...
		Map<String, Object> refund = Map.of("organizationId", org, "amountCents", 6600, "operationId",
				"commerce-refund:" + order, "reason", "expiry");
		client().post().uri("/internal/commerce/payments/" + order + "/refund")
				.header("X-Grassland-Identity", sign(consumer, null, org, null)).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(refund).exchange().expectStatus().isForbidden();
		for (int i = 0; i < 2; i++) {
			client().post().uri("/internal/commerce/payments/" + order + "/refund")
					.header("X-Grassland-Identity", signService(org, "marketplace"))
					.contentType(MediaType.APPLICATION_JSON).bodyValue(refund).exchange().expectStatus().isOk()
					.expectBody().jsonPath("$.data.status").isEqualTo("succeeded");
		}
		assertThat(ledger.sumBalance(LedgerAccount.Type.CONSUMER_ESCROW, order).block()).isZero();
	...
	void multiRecommenderSplitCanBeReversedByAfterSalesRefund() {
		...
		client().post().uri("/internal/commerce/payments")
				...
				.bodyValue(Map.of("orderRef", order, "consumerAccountId", consumer, "organizationId", org,
						"amountCents", 10000, "operationId", "commerce-payment:" + order))
				.exchange().expectStatus().isCreated();
		Map<String, Object> split = Map.of("organizationId", org, "totalAmountCents", 10000, "recommenderAmountCents",
				2000, "merchantAmountCents", 7500, "platformFeeCents", 500, "operationId", "commerce-split:" + order,
	...
	@Test
	void singleRecommenderAfterSplitRefundWritesClawbackEntry() {
		String order = UUID.randomUUID().toString();
		String consumer = UUID.randomUUID().toString();
		String recommender = UUID.randomUUID().toString();
		String org = UUID.randomUUID().toString();
		...
		client().post().uri("/internal/commerce/payments/" + order + "/split")
				.header("X-Grassland-Identity", signService(org, "marketplace")).contentType(MediaType.APPLICATION_JSON)
				.bodyValue(Map.of("organizationId", org, "totalAmountCents", 10_000, "recommenderAccountId",
	...
	private long clawbackAmount(String accountId, String orderRef) {
		return db
				.sql("SELECT COALESCE(SUM(amount_cents), 0) AS c FROM wallet_ledger"
						+ " WHERE account_id = CAST(:account AS uuid) AND entry_type = 'clawback'"
						+ " AND engagement_ref = :orderRef")
				.bind("account", accountId).bind("orderRef", orderRef).map(row -> row.get("c", Long.class)).one()
				.block();
	}
	...
	private long providerOperationCount(String operationId, String operationType) {
		return db
				.sql("SELECT COUNT(*)::int AS c FROM finance_provider_operation"
