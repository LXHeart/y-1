...
class EdgeFailClosedIT {
	...
	private WebTestClient client() {
		// responseTimeout 30s：默认 5s 在 CI 慢 runner 负载下偶发超时（RoutingProxyContractTest 同款）。
	...
	void unknownApiReturns404WithoutAnUpstream() {
		client().get().uri("/api/not-migrated").exchange().expectStatus().isNotFound();
	}

	@Test
	void disabledRouteReturns404InsteadOfFallingBack() {
		client().get().uri("/api/auth/me").exchange().expectStatus().isNotFound();
	}
	...
	void guestTrialFlagOffFailsClosed() {
		client().post().uri("/api/guest-trial/article-titles").exchange().expectStatus().isNotFound();
		client().get().uri("/api/guest-trial/quota").exchange().expectStatus().isNotFound();
	}

	/** 任务书 #34：内容安全 flag 关闭 → /api/content-safety/* fail-closed 404。 */
	@Test
	void contentSafetyFlagOffFailsClosed() {
		client().post().uri("/api/content-safety/check").exchange().expectStatus().isNotFound();
	}
	...
	void speechFlagOffFailsClosed() {
		client().post().uri("/api/speech/transcriptions").exchange().expectStatus().isNotFound();
		client().get().uri("/api/speech/transcriptions/transcription-1").exchange().expectStatus().isNotFound();
	}

	/** 任务书 #43：图片编辑台 flag 关闭 → /api/image-studio/* fail-closed 404。 */
	@Test
	void imageStudioFlagOffFailsClosed() {
		client().post().uri("/api/image-studio/matting").exchange().expectStatus().isNotFound();
	}
