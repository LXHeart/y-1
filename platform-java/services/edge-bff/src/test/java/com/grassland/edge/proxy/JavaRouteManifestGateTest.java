...
/**
 * JBE-01/JBE-04 gate: every enabled production route is Java-owned and unknown
 * APIs fail closed.
 */
...
class JavaRouteManifestGateTest {
	...
	void everyEnabledManifestRouteTargetsAJavaUpstream() {
		...
		assertThat(properties.routes()).filteredOn(RouteProperties::enabled).allSatisfy(route -> {
	...
	private static Stream<Arguments> representativeJavaRoutes() {
		return Stream.of(route("GET", "/api/auth/me", "identity"), route("POST", "/api/auth/login", "identity"),
				...
				route("POST", "/api/creation-contexts", "intelligence"), route("GET", "/api/admin/users", "identity"),
				route("POST", "/api/admin/adjust-credits", "identity"),
				// 任务书 #71：治理台初始化商家账号。
				route("POST", "/api/admin/merchant-accounts", "identity"),
				route("GET", "/api/recommenders/account-1/profile", "identity"),
				route("GET", "/api/ops/cases", "marketplace"), route("GET", "/api/reputation/me", "marketplace"),
				route("GET", "/api/finance/wallets/me", "finance"), route("GET", "/api/credits/balance", "finance"),
				...
				route("POST", "/api/video-production/storyboard", "intelligence"),
				route("POST", "/api/video-production/tasks", "intelligence"),
				route("POST", "/api/video-production/tasks/task-1/compose", "intelligence"),
				route("GET", "/api/video-production/tasks", "intelligence"),
				route("GET", "/api/video-production/tasks/task-1", "intelligence"),
				route("GET", "/api/video-production/tasks/task-1/subtitle", "intelligence"),
				route("GET", "/api/admin/bgm-tracks", "intelligence"),
				route("POST", "/api/admin/bgm-tracks", "intelligence"),
				route("DELETE", "/api/admin/bgm-tracks/track-1", "intelligence"),
	...
	private static Stream<Arguments> failClosedBoundaries() {
		return Stream.of(Arguments.of("GET", "/api/not-migrated"), Arguments.of("GET", "/api/auth/login"),
