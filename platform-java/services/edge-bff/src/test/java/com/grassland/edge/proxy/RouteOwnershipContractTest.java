...
class RouteOwnershipContractTest {
	...
	static void properties(DynamicPropertyRegistry registry) {
		for (String name : new String[]{"identity", "marketplace", "finance", "trust", "intelligence"}) {
			registry.add("edge.upstreams." + name, () -> "http://localhost:" + upstream.port() + "/" + name);
		}
	...
	void routesToExpectedUpstream(HttpMethod method, String path, String expectedUpstream) {
		String body = WebTestClient.bindToServer().baseUrl("http://localhost:" + serverPort)
				.responseTimeout(java.time.Duration.ofSeconds(30)).build().method(method).uri(path).exchange()
