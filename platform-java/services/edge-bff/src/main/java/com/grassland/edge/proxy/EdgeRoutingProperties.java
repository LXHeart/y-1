	...
	public EdgeRoutingProperties {
		...
		if (!FAIL_CLOSED.equals(defaultUpstream) && !upstreams.containsKey(defaultUpstream)) {
			throw new IllegalArgumentException(
					"edge.default-upstream '" + defaultUpstream + "' is not defined in edge.upstreams");
		}
		routes = routes == null ? List.of() : routes;
		for (RouteProperties route : routes) {
			if (!upstreams.containsKey(route.upstream())) {
				throw new IllegalArgumentException(
						"route upstream '" + route.upstream() + "' is not defined in edge.upstreams");
			}
			if (route.exact() && route.path() != null && route.path().indexOf('{') >= 0) {
				validateTemplate(route.path());
			}
		}
	}

	/**
	 * Exact routes may use whole-segment {identifier} placeholders only (task-107
	 * K09.1).
	 */
	private static void validateTemplate(String path) {
		for (String segment : path.split("/", -1)) {
			boolean placeholder = segment.startsWith("{") && segment.endsWith("}");
			if (placeholder) {
				String name = segment.substring(1, segment.length() - 1);
				if (name.isEmpty() || !name.matches("[A-Za-z0-9_]+")) {
					throw new IllegalArgumentException(
							"route template segment '" + segment + "' must be a whole-segment {identifier}");
				}
			} else if (segment.indexOf('{') >= 0 || segment.indexOf('}') >= 0) {
				throw new IllegalArgumentException(
						"route template segment '" + segment + "' mixes static text with braces");
			}
		}
	}
}
