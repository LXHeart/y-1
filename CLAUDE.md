...
| 成员子账号 | `/api/organizations/{orgId}/accounts*`、`/api/organizations/{orgId}/member-review-required`（GET/PATCH）、`/api/auth/change-password` | 任务书 #48：主体直建子账号（建号+赋角色+挂门店一次完成，随机初始密码只在创建/重置响应出现一次，`account_flag.must_change_password` 首登强制改密；edge 对非豁免路径 428 硬闸）。POST `/accounts` ADMIN+ 直建任意角色，邮箱已存在需显式 `confirmBindExisting` 关联（不静默绑定）；POST `/stores/{storeId}/accounts` 店长 MANAGER+ 代建 staff，组织开关 `member_review_required` 开启时进 pending_review 待 `review` 过审（reject=rejected 终态）；suspend/restore 即时生效不设审核（D7），守卫：最后 active MANAGER/OWNER 保护/不可自操作。状态真相源=`app_users.status`（active/suspended/pending_review/rejected），成员表无状态列；登录对非 active 返回区分文案 403。前端 OrgTeamCard + FirstPasswordChangeView 路由闸 |
