...
| 成员子账号与默认门店 | `/api/organizations/{orgId}/accounts*`、`/api/organizations/{orgId}/member-review-required`（GET/PATCH）、`/api/organizations/{orgId}/account-prefix`（GET/PATCH）、`/api/auth/change-password`、`/api/me/bind-email*` | 任务书 #48+#49 单一附属模型：成员=主体自建账号，登录名=`主体前缀-登录名`（`organization.account_prefix` V43 + `account_username` 旁表 V44；建号入参 loginName，email 列存 `{账号名}@sub.grassland.invalid` 占位，MailOutboxEnqueuer 短路占位外发）。登录双标识（UserLookup.findByIdentifier 先旁表后邮箱，存量邮箱路径零变化）；login/me 响应带 username/hasEmail；成员登录后 `bind-email` 两步验码换绑（错码 4xx/被占 409）。POST `/accounts` ADMIN+ 直建任意角色（撞登录名 409 换名）；店长代建走组织开关审核；suspend/restore 即时可逆，DELETE `/accounts/{id}` 永久作废（关系解除+`status='deleted'`+deleted_at 留痕，restore/重删 409 终态预检先于关系守卫）；守卫：最后 active MANAGER/OWNER 保护/不可自操作。邀请流与挂靠端点（POST/DELETE memberships）已下线，存量非 owner 关系 V45 一次性清除（纯门店经理不受影响）。前端 OrgTeamCard（前缀设置/建号预览/删除强确认）+ EmailBindingCard + FirstPasswordChangeView 路由闸。任务书 #50：POST /api/organizations 同事务自动建默认门店（名=主体名，StoreCreated defaultStore=true）；单店/多店由门店数推导（前端 stores<=1 收敛，无模式字段）；V46 回填零店主体 |
