...
- **爆款文章创作**：主题 → 平台（微信公众号/知乎/小红书/抖音图集短文案）→ 标题 → 大纲 → 正文 → 按段落配图，SSE 流式输出，附只读平台规范提示。知乎为回答/文章双模式（任务书 #62）：platform 值仍是 `zhihu`，进创作流后选形态，回答模式六步（问题→开头→大纲→正文→配图→完成）围绕手输的「目标问题」生成首屏钩子；问题链接只做本地正则提取 questionId 存档，服务端对知乎零请求。商家任务可携带「目标问题」，accept 时冻进快照并锁定回答形态（快照优先于请求体）
...
| 成员子账号与默认门店 | `/api/organizations/{orgId}/accounts*`、`/api/organizations/{orgId}/stores/{storeId}/memberships*`（GET 列表 STAFF+；PUT 分配/调度与 DELETE 移除 ADMIN+）、`/api/organizations/{orgId}/account-prefix`（GET，任务书 #51 起只读）、`/api/auth/change-password`、`/api/me/bind-email*` | 任务书 #52 池模型：建号一律入主体池（organization_membership.role=member），门店身份是分配层（store_membership，至多挂一店；PUT=assign-or-move，同店=改角色）；一店一店长闸三处同拦（建号挂店/分配/调度→409）；分配/移除/调度仅 ADMIN+，店长只停用/恢复本店成员（StoreStaffCard 纯管理——建号与审核流已整体退役：店长代建/审核/开关端点删除，存量 pending_review V48 置 active，member_review_required 列保留）；D8①「最后店长不可停/删」已废除（店可无店长，主体代管兜底）；工作台路由按「组织角色+门店范围」：owner/admin 或纯池 member→主体台，挂店 member→门店台。登录名=`主体前缀-登录名`（V43+V44 旁表，占位邮箱 `{账号名}@sub.grassland.invalid`）；登录双标识（UserLookup 先旁表后邮箱）；`bind-email` 两步验码换绑；suspend/restore 即时可逆，DELETE `/accounts/{id}` 永久作废（关系解除+deleted 留痕）；守卫：OWNER 保护/不可自操作。组织成员列表带 username/accountStatus/storeId/storeRole/storeName（挂靠门店；未分配=池内待分配）。任务书 #50：注册同事务自动建默认门店，单店/多店由门店数推导；#51 单店收敛：无建号/分配入口，主体成员表操作列单店一律不渲染，隐式店长行仅单店。前端 OrgTeamCard（建号可选挂店/从池分配/调度/移除/删除强确认）+ StoreStaffCard + EmailBindingCard + FirstPasswordChangeView 路由闸 |
