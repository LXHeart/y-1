...
| 成员子账号与默认门店 | `/api/organizations/{orgId}/accounts*`、`/api/organizations/{orgId}/member-review-required`（GET/PATCH）、`/api/organizations/{orgId}/account-prefix`（GET，任务书 #51 起只读）、`/api/auth/change-password`、`/api/me/bind-email*` | 任务书 #48+#49 单一附属模型：成员=主体自建账号，登录名=`主体前缀-登录名`（`organization.account_prefix` V43 + `account_username` 旁表 V44；建号入参 loginName，email 列存 `{账号名}@sub.grassland.invalid` 占位，MailOutboxEnqueuer 短路占位外发）。登录双标识（UserLookup.findByIdentifier 先旁表后邮箱，存量邮箱路径零变化）；login/me 响应带 username/hasEmail；成员登录后 `bind-email` 两步验码换绑（错码 4xx/被占 409）。POST `/accounts` ADMIN+ 直建任意角色（撞登录名 409 换名）；店长代建走组织开关审核；suspend/restore 即时可逆，DELETE `/accounts/{id}` 永久作废（关系解除+`status='deleted'`+deleted_at 留痕，restore/重删 409 终态预检先于关系守卫）；守卫：最后 active MANAGER/OWNER 保护/不可自操作。邀请流与挂靠端点（POST/DELETE memberships）已下线，存量非 owner 关系 V45 一次性清除（纯门店经理不受影响）。前端 OrgTeamCard（前缀只读展示/建号预览/删除强确认）+ EmailBindingCard + FirstPasswordChangeView 路由闸。任务书 #50：POST /api/organizations 同事务自动建默认门店（名=主体名，StoreCreated defaultStore=true）；单店/多店由门店数推导（前端 stores<=1 收敛，无模式字段）；V46 回填零店主体。任务书 #51 单店收敛（纯前端，按门店数推导）：审核开关、建号入口（主体区+门店区）均只在多店呈现——单店只有主体账号一人，雇人须先开分店；主体成员表操作列单店一律不渲染（2026-08-28 严格字面拍板；owner 行永不给操作，服务端一律 403；多店期建的 role=member 回落单店后停用/删除由运营台处置）。2026-08-28 二轮收敛：建号入口按区分工——主体区只建组织成员（角色/门店下拉退役，全卡无 select），门店区内联「任命店长」+「添加店员」（D1 任命仅 ADMIN+ 不变，越权后端 403）；主体账号隐式店长行仅单店呈现（#51 第 4 条前半），多店门店成员区只显真实成员、无店长的店提示「主体账号代管」（orgSuperUserAsManager 权限保留——权限不是身份，不画店长行） |
...
| 成员账号前缀（运营） | `/api/admin/organizations`（GET `?q=`）、`/api/admin/organizations/{id}/account-prefix`（PATCH） | 任务书 #51：前缀自动生成、商家只读（商家侧 PATCH 已删），改名收归平台运营。搜索按名/前缀/id ILIKE（LIKE 元字符转义，上限 50）并回 `memberCount` = 改名影响面。改名**同事务重写三处**：`organization.account_prefix` + `account_username.username`（存完整账号名，`substring` 只换开头前缀段，不用 `replace()`——前缀字面出现在登录名里会误伤）+ 仅 `%@sub.grassland.invalid` 的占位邮箱；**已绑真实邮箱的成员邮箱不动**（本人资产）。成员范围 = 该 org 的 `organization_membership` ∪ 门店属于该 org 的 `store_membership`。**不动会话**（按 account id 认，成员不掉线，下次登录用新名；旧名立即失效且无自动通知，线下告知由运营负责）。发 outbox `OrganizationAccountPrefixChanged`；响应回 `{prefix, rewrittenAccounts, rewrittenPlaceholderEmails}`。edge flag `EDGE_ROUTE_ADMIN_ORGANIZATIONS_IDENTITY`；运营台入口 AdminView「账号前缀」页签 |
