...
核心功能模块：
- **AI 内容创作中心**：平台优先一级入口（九平台 × 内容形式 × 创作来源），解析为具体工作流并 handoff 到对应创作视图
...
- **视频制作**：上传素材图片 / 粘贴参考视频链接（内嵌抖音/B站参考提取）/ 从热点选主题 + 店铺信息 → AI 脚本生成（SSE 流式）→ 视频生成（Seedance API，待对接）
- **账号系统**：邮箱注册（图形验证码 + 邮箱验证码，注册时可选初始身份：商家/推荐官，暂存 localStorage）、登录、按用户隔离设置
- **积分系统**：功能使用扣积分，管理员可调整
...
- 积分域已 Java 原生化（GL-P3-AI-001 下属切片）：finance-service 承载 `/internal/credits/{consume,refund,award}` 写端 + `/api/credits/{balance,history}` 读端（edge 路由 `EDGE_ROUTE_CREDITS_FINANCE`）。intelligence 默认用 `FinanceCreditsClient`（`credits.client.impl=finance`）直连 finance 容器调 `/internal/credits/*`（不经 edge-bff，同 marketplace→finance 模式）；`credits.client.impl=legacy` 回退到旧 bridge。finance 侧 `/internal/credits/**` 由 `CreditsInternalAuthFilter` 用共享密钥 `X-Internal-Key`（`INTERNAL_API_KEY`，未配 fail-closed 503）+ rejectForwarded（带任一 `X-Forwarded-*`/`Forwarded`→404）把关，**不在 `/api` 树下**。legacy Express `credit.service.ts` 仍是薄代理（live 充值路径 douyin/bilibili fallback、video stub、注册赠送、admin 调整经它调 finance），旧 `/internal/credits` bridge + `/api/credits` 读端作为回滚保留（Phase B 待删）
- `requireCredit()` 返回 `CreditCharge` 句柄，上游失败时调 `charge.refund(note)` 写 `refund` 流水（GL-P0-BILL-002）。退款键是 `refund:<consumeId>`——**由调用方派生**（intelligence `FinanceCreditsClient` 与 legacy `createCharge` 都在调用前加 `refund:` 前缀），finance 按 `operation_id` **原样存储**，partial unique index 据此保证一次扣减至多一次退款。旧 `LegacyCreditsClient` 曾透传原始 consume id 与 consume 行撞键被 dedup 吞掉（退款不生效），`FinanceCreditsClient` 已修正。`refund()` 失败只记日志不抛，避免掩盖原始错误。用户主动 abort 不退款（内容已流出）
