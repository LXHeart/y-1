...
const tags = {
  ComponentWorkshop: {
    ...
    render: renderComponentWorkshop,
    assets: ["v0", "v1", "v2", "v3", "v4", "v5"],
    defaults: { z: 10 },
  },
  SemanticWorkshop: {
    name: "semantic-workshop",
    render: renderSemanticWorkshop,
    assets: ["creatify", "higgsfield"],
    defaults: { z: 10 },
  },
  DeliveryWorkshop: {
    name: "delivery-workshop",
    render: renderDeliveryWorkshop,
    assets: ["film", "clone", "water", "reference", "host", "thumb1", "thumb2", "thumb3"],
    defaults: { z: 10 },
  },
  CodeJourney: {
    name: "code-journey",
    render: renderCodeJourney,
    assets: ["codex", "remotion", "hyperframes", "ranking", "podcast", "interview", "target"],
    defaults: { z: 10 },
  },
  Outro: {
    name: "outro",
...
const assetType = (n) => (images.has(n) ? mediaTypes.blobArtifact : mediaTypes.synchronized);
const ins = (tag) => [...common, ...tags[tag].assets.map((n) => port(n, assetType(n)))];
      ...
      handler: ({ inputs: i }) =>
        out(
          ...
          tags[tag].render(
            ...
            inline(i.events),
            Object.fromEntries(
              tags[tag].assets.map((n) => [n, images.has(n) ? i[n].value : inline(i[n])]),
            ),
          ),
        ),
    })),
...
function decode(tag) {
  return ({ element, resolveReference }) => {
    ...
    for (const n of spec.assets) bindings[n] = ref(element, n, assetType(n));
