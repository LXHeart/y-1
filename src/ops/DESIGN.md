...
description: Grassland admin console design language — a clean operations interface anchored on white canvas with an indigo primary shared with the marketing site and Space Grotesk display typography. The system reads as friendly modern SaaS — generous whitespace, soft-rounded cards, data-dense tables, and quiet neutrals so status colors carry the meaning. Brand voltage comes from display headlines and in-card product UI fragments rather than from extra accent colors.
...
Type voice splits cleanly into two roles: **Space Grotesk** (the display face — used for h1, h2, h3, and hero headlines) and **Inter** (used for everything else — body, buttons, nav, captions). Space Grotesk uses weight 600 with negative letter-spacing (-0.5px to -2px depending on size) — it feels modern, slightly condensed, distinctly operational.

Component voltage comes from **product UI fragments shown directly inside cards** — data tables, workflow panels, automation diagrams, integration tiles. The system doesn't paint illustrations of the product; it shows the actual product chrome at small scale embedded in the marketing flow.
...
**Key Characteristics:**
- White canvas with indigo primary CTA (`{colors.primary}` — #533afd). Buttons are `{rounded.md}` (8px) with confident weight-600 labels. Standard friendly-SaaS button.
- `Space Grotesk` display typeface for headlines (open-source, self-hosted). Negative letter-spacing on display sizes — geometric, precise, slightly condensed.
...
- Product UI fragments embedded directly in cards — real data tables, workflow widgets, integration grids inside cards. Brand voltage from real product chrome at small scale.
...
### Brand & Accent
- **Primary** (`{colors.primary}` — #533afd): The dominant action color — indigo, shared with the marketing site for brand continuity. All primary CTAs; headlines stay `{colors.ink}`. Press state shifts to `{colors.primary-active}` (#4434d4).
- **Brand Accent** (`{colors.brand-accent}` — #3b82f6): Used sparely on inline links and on a small badge / "Customer story" highlight. Outside the indigo primary the system is near-monochrome — the blue appears rarely.
...
### Font Family
The system runs **Space Grotesk** for display + brand wordmark and **Inter** for everything else. Space Grotesk is an open-source geometric display typeface at weight 600 with negative letter-spacing, shared with the marketing site. Inter handles body, buttons, navigation, captions, and tabular code blocks. Both are self-hosted via Fontsource. The fallback stack walks `-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` for both families.
...
|---|---|---|---|---|---|
| `{typography.display-xl}` | 64px | 600 | 1.05 | -2px | Page h1 — Space Grotesk |
| `{typography.display-lg}` | 48px | 600 | 1.1 | -1.5px | Section heads — Space Grotesk |
| `{typography.display-md}` | 36px | 600 | 1.15 | -1px | Sub-section heads, card titles — Space Grotesk |
| `{typography.display-sm}` | 28px | 600 | 1.2 | -0.5px | CTA-band heads, pricing tier prices — Space Grotesk |
...
| `{typography.code}` | 14px | 400 | 1.5 | 0 | Code snippets, API examples — system mono |
| `{typography.button}` | 14px | 600 | 1.0 | 0 | Standard button labels |
| `{typography.nav-link}` | 14px | 500 | 1.4 | 0 | Top-nav menu items |

### Principles
Space Grotesk is the brand voice — every display headline uses it. Inter handles the supporting type. The boundary is strict: never put body copy in Space Grotesk, never put a display headline in Inter. Space Grotesk without negative letter-spacing reads as off-brand — the -0.5 to -2px tracking is part of the voice.
...
Space Grotesk and Inter are both open-source (OFL) and self-hosted via Fontsource — no substitution needed. If the bundle is somehow unavailable, **Inter** at weight 600 with -0.04em letter-spacing is a usable approximation that preserves the weight + tracking signature.
...
The system uses generous but not excessive whitespace — section padding sits at 96px (modern-SaaS standard), and card internal padding stays at 32px. The rhythm is calibrated for fast scanning: every band has a single h1 + h2 + supporting cards, never densely packed lists. The result reads as confident-not-shouting.
...
**`top-nav`** — White nav bar pinned to the top of every page. 64px tall, `{colors.canvas}` background. Carries the Grassland admin wordmark + logo at left, the primary horizontal module menu center, and a right-side cluster with "Sign in" text-link and `{component.button-primary}`. Menu items in `{typography.nav-link}` (Inter 14px / 500).

**`nav-pill-group`** — A small pill-radius wrapper around 2-3 sub-nav segments (e.g., the module switcher between console views). Background `{colors.surface-soft}` with internal padding 6px, rounded `{rounded.pill}`. Active segment renders as a white-canvas pill with a subtle drop shadow inside the wrapper. The pill-in-pill treatment is one of the system's signature interactive components.

### Buttons

**`button-primary`** — The signature primary CTA. Background `{colors.primary}` (#533afd), text `{colors.on-primary}`, type `{typography.button}` (Inter 14px / 600), padding 12px × 20px, height 40px, rounded `{rounded.md}` (8px). Active state `button-primary-active` shifts to `{colors.primary-active}` (#4434d4).
...
**`hero-app-mockup-card`** — A larger product-UI mockup card showing the actual admin product UI — data grid, detail panel, and a primary "Confirm" button inside. Background `{colors.canvas}`, 1px hairline border, rounded `{rounded.xl}` (16px), subtle drop shadow. Used as the hero's right-side artifact.
...
**`footer`** — Dark navy footer that closes every page. Background `{colors.surface-dark}` (#101010), text `{colors.on-dark-soft}`. 4-column link list at desktop covering Product / Solutions / Company / Resources. Vertical padding 64px. The Grassland admin wordmark sits at the top-left in `{colors.on-dark}`. The footer is the only dark surface on every page — the deliberate inversion visually closes the page.

## Do's and Don'ts

### Do
- Reserve `{colors.primary}` (#533afd) for primary CTAs. Headlines stay `{colors.ink}`. The primary button is always the brand indigo.
- Use Space Grotesk for every display headline. Pair with Inter body. Never blur the boundary.
...
### Don't
- Don't use accent colors (`{colors.brand-accent}`, badge pastels) on primary CTAs. Primary CTAs are always the indigo `{colors.primary}`.
...
- Don't repeat the same surface mode in two consecutive bands. The pacing alternates white → light-gray → white → product-mockup-card → white → dark-footer.
...
5. Display headlines stay Space Grotesk 600 with negative letter-spacing. Body stays Inter 400. The trinity does not blur.
...
- Button styles are documented from screenshot ground-truth + standard Space Grotesk / Inter baselines; note some CTAs render as styled `<a>` link elements rather than `<button>` tags.
- Space Grotesk and Inter are open-source (OFL) and self-hosted via Fontsource — no licensing constraints.
...
- The logged-in product surfaces beyond what is documented here are out of scope.
