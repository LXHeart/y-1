  ...
  <section class="admin-view">
    ...
    <div class="admin-tabs" role="tablist" aria-label="管理模块">
      ...
      <button type="button" role="tab" :aria-selected="activeSection === 'public-assets'"
        ...
        @click="activeSection = 'public-assets'; void loadPublicAssetReviews()">
        公共素材 <span v-if="publicAssetReviews.length" class="count-badge">{{ publicAssetReviews.length }}</span>
      </button>
    </div>
    ...
    <div v-else-if="activeSection === 'public-assets'" class="admin-panel public-assets-panel" role="tabpanel">
      <section class="public-generation" aria-labelledby="public-generation-title">
        <div class="panel-toolbar">
          <div>
            <h3 id="public-generation-title">AI 批量生产</h3>
            <p>逐张生成并提交到待审队列，单张失败不影响其他结果。</p>
          </div>
        </div>
        <form class="public-generation-form" data-testid="public-asset-generation-form" @submit.prevent="generatePublicAssets">
          <label>素材类型
            <select v-model="publicGeneration.kind">
              <option value="icon">图标</option>
              <option value="decoration">装饰元素</option>
              <option value="background">背景</option>
              <option value="mood">氛围图</option>
            </select>
          </label>
          <label>主题
            <input v-model="publicGeneration.theme" type="text" maxlength="100" placeholder="例如：夏日饮品" required>
          </label>
          <label>视觉风格
            <input v-model="publicGeneration.style" type="text" maxlength="100" placeholder="选填，例如：清爽扁平">
          </label>
          <label>数量
            <input v-model.number="publicGeneration.count" type="number" min="1" max="12" required>
          </label>
          <label>有效期
            <input v-model="publicGeneration.validUntil" type="datetime-local" required>
          </label>
          <button class="approve-btn generation-submit" type="submit" :disabled="publicGenerating">
            {{ publicGenerating ? '生成中...' : '开始生成' }}
          </button>
        </form>
        ...
        <div v-if="publicGenerationResult" class="generation-results" aria-live="polite">
          ...
          <ol>
            <li v-for="item in publicGenerationResult.items" :key="item.index"
              :class="item.ok ? 'generation-ok' : 'generation-failed'">
              <span>第 {{ item.index }} 张</span>
              <a v-if="item.ok && item.assetId" :href="`#public-asset-${item.assetId}`">{{ item.assetId }}</a>
              <span v-else>{{ item.errorReason || '生成失败' }}</span>
            </li>
          </ol>
        </div>
      </section>
      ...
      <section aria-labelledby="public-review-title">
        <div class="panel-toolbar">
          <div><h3 id="public-review-title">待审公共素材</h3><p>通过后进入公共素材库，驳回必须填写原因。</p></div>
          <button class="refresh-btn" type="button" :disabled="publicAssetsLoading" @click="loadPublicAssetReviews">刷新</button>
        </div>
        ...
        <div v-else-if="publicAssetReviews.length" class="public-review-grid">
          <article v-for="asset in publicAssetReviews" :id="`public-asset-${asset.id}`" :key="asset.id" class="public-review-item">
            <div class="public-asset-preview">
              <img v-if="publicAssetPreviewUrls[asset.id]" :src="publicAssetPreviewUrls[asset.id]" :alt="asset.title">
              <span v-else>暂无预览</span>
            </div>
            <div class="public-asset-body">
              <div class="public-asset-heading">
                <h4>{{ asset.title }}</h4>
                <span class="type-tag">{{ publicAssetCategoryLabel(asset.category) }}</span>
              </div>
              <p>{{ asset.tags.join(' · ') || '无标签' }}</p>
              <p class="td-time">有效至 {{ formatDateTime(asset.validUntil || null) }}</p>
              <label>审核备注
                <input v-model="publicAssetReviewNotes[asset.id]" class="field-input" type="text" maxlength="500"
                  placeholder="驳回时必填">
              </label>
              <div class="review-actions">
                <button class="approve-btn" type="button" :disabled="reviewingPublicAssetIds.has(asset.id)"
                  @click="reviewPublicAsset(asset, 'approve')">通过</button>
                <button class="reject-btn" type="button" :disabled="reviewingPublicAssetIds.has(asset.id)"
                  @click="reviewPublicAsset(asset, 'reject')">驳回</button>
              </div>
            </div>
          </article>
        </div>
...
import type {
  ...
  PublicAssetBatchGenerateResult,
  PublicAssetGenerationKind,
...
const publicGeneration = ref<{
  kind: PublicAssetGenerationKind
  theme: string
  style: string
  count: number
  validUntil: string
}>({ kind: 'icon', theme: '', style: '', count: 3, validUntil: defaultPublicAssetExpiry() })
...
async function loadPublicAssetReviews(): Promise<void> {
  ...
  if (!result) {
    publicAssetsError.value = grassland.error.value || '公共素材审核队列加载失败'
    publicAssetsLoading.value = false
    return
  }
  publicAssetReviews.value = [...result.items]
  const previews = await Promise.all(result.items.map(async (asset) => {
    const download = await grassland.getContentAssetDownloadUrl(asset.id)
    return [asset.id, download?.downloadUrl || ''] as const
  }))
  publicAssetPreviewUrls.value = Object.fromEntries(previews)
  publicAssetsLoading.value = false
}

async function generatePublicAssets(): Promise<void> {
  const input = publicGeneration.value
  const theme = input.theme.trim()
  const style = input.style.trim()
  const validUntil = new Date(input.validUntil)
  const now = Date.now()
  if (!theme || theme.length > 100) {
    publicGenerationError.value = '主题长度需为 1-100 字符'
    return
  }
  if (style.length > 100) {
    publicGenerationError.value = '视觉风格不能超过 100 字符'
    return
  }
  if (!Number.isInteger(input.count) || input.count < 1 || input.count > 12) {
    publicGenerationError.value = '生成数量需为 1-12'
    return
  }
  if (Number.isNaN(validUntil.getTime()) || validUntil.getTime() <= now
    || validUntil.getTime() > now + 90 * 24 * 60 * 60 * 1000) {
    publicGenerationError.value = '有效期必须在未来 90 天内'
    return
  }
  publicGenerating.value = true
  publicGenerationError.value = ''
  publicGenerationResult.value = null
  const result = await grassland.batchGeneratePublicAssets({
    kind: input.kind,
    theme,
  ...
  if (result) {
    publicGenerationResult.value = result
    await loadPublicAssetReviews()
  } else {
    publicGenerationError.value = grassland.error.value || '公共素材生成失败'
  }
...
async function reviewPublicAsset(asset: ContentAsset, decision: 'approve' | 'reject'): Promise<void> {
  ...
  if (decision === 'reject' && !note) {
    publicAssetsError.value = '驳回公共素材必须填写原因'
    return
  }
  reviewingPublicAssetIds.value = new Set([...reviewingPublicAssetIds.value, asset.id])
  publicAssetsError.value = ''
  const result = decision === 'approve'
    ? await grassland.approvePublicAsset(asset.id, asset.version, note || undefined)
    : await grassland.rejectPublicAsset(asset.id, asset.version, note)
  const nextReviewing = new Set(reviewingPublicAssetIds.value)
  nextReviewing.delete(asset.id)
  reviewingPublicAssetIds.value = nextReviewing
  if (!result) {
    publicAssetsError.value = grassland.error.value || '公共素材审核失败'
    return
  }
  publicAssetReviews.value = publicAssetReviews.value.filter((item) => item.id !== asset.id)
  delete publicAssetReviewNotes.value[asset.id]
  delete publicAssetPreviewUrls.value[asset.id]
}

function publicAssetCategoryLabel(category: ContentAssetCategory): string {
  return ({ scene: '场景', other: '图标/装饰' } as Partial<Record<ContentAssetCategory, string>>)[category]
    || category
}
...
.public-assets-panel,
.public-assets-panel > section,
.public-generation {
  display: grid;
  gap: 16px;
}

.public-assets-panel > section + section {
  padding-top: 20px;
  border-top: 1px solid var(--color-border);
}

.public-generation-form {
  display: grid;
...
.public-generation-form label,
.public-asset-body label {
  display: grid;
  gap: 5px;
  color: var(--color-text-muted);
  font-size: 0.78rem;
...
.public-generation-form input,
.public-generation-form select,
.public-asset-body input {
  width: 100%;
  min-height: 36px;
  padding: 6px 9px;
  border: 1px solid var(--color-border);
...
.generation-results li span:first-child {
  display: inline-block;
  min-width: 62px;
}
