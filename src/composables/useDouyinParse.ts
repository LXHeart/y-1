import { ref } from 'vue'
import type { ApiResponse, ExtractedDouyinVideoPayload } from '../types/douyin'
...
function normalizeExtractedDouyinVideoPayload(value: unknown): ExtractedDouyinVideoPayload | null {
  ...
  const fetchStage = value.fetchStage === 'page_json' || value.fetchStage === 'browser_json' || value.fetchStage === 'browser_network'
    ? value.fetchStage
    : null
  ...
  return {
    ...
    usedSession: typeof value.usedSession === 'boolean' ? value.usedSession : false,
    fetchStage,
  }
}
